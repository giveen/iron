...
| rope (Llama-3 banded) | ✗ | ✗ | ✓ | `kernels/rope/rope_llama.rs` → `mt_rope_llama<T>` (decode) + `mt_rope_llama_many<T>` (batched prefill). Decode-form, generic dtype, optional Llama-3 frequency-band scaling. |
