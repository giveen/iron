...
|---|---|---|---|---|
| HIP / ROCm | `wh-iron-codegen::hip::HipGenerator` (CudaGenerator + textual transform) | `wh-iron-runtime::HipDevice` (hand-rolled `amdhip64` + `hiprtc` FFI) | **4 / 4** on RX 9070 XT (gfx1201) | wave32 RDNA 4; wave64 CDNA gated by `TargetProfile::hip_wave64`, not yet exercised |
| Vulkan / SPIR-V | `wh-iron-codegen::spirv::GlslGenerator` (fresh elementwise walker → GLSL 460 compute) | `wh-iron-runtime::VulkanDevice` (hand-rolled `vulkan-1` + `shaderc_combined` FFI) | **4 / 4** on RX 9070 XT | Phase-1 Elementwise only; Reduction / Grid3D / Coop are Phase 2+ |

Build: `cargo test -p wh-iron-runtime --features hip,vulkan`.
...
3. `wh-iron-codegen/src/hip/mod.rs` — `HipGenerator` composes
...
4. `wh-iron-codegen/src/spirv/mod.rs` — `GlslGenerator` (fresh walker for
...
5. `wh-iron-runtime/src/device/hip/{mod,ffi}.rs` — `HipDevice`.
6. `wh-iron-runtime/src/device/vulkan/{mod,ffi}.rs` — `VulkanDevice`.
...
4. **gfx target detection.** Windows `hipDeviceGetName` returns the
   ...
   reads `IRON_HIP_GFX` if set; default `gfx1201` matches the user's
...
4. **GLSL.std.450 is missing `erf` / `erfinv` / `expm1` / `log10`.**
   The preamble synthesises them via `iron_erf`/`iron_erfinv`/`iron_expm1`/
   `iron_log10`. `iron_erfinv` is a coarse Winitzki approximation —
...
Override `gfx` (e.g. for RDNA 3 / MI300): `$env:IRON_HIP_GFX = "gfx1100"`.
...
- **Both:** wire `--target hip` / `--target vulkan` into `iron build` and
  `iron bench`. Add device-spec rows for the roofline view.
