...
│                                                         │
│  ┌──────────────────────────────────────────────────┐  │
│  │  [auto-generated by iron — never authored]       │  │
│  │  Thin harness: parses iron protocol commands,    │  │
...
The `__iron_runner` binary writes newline-delimited JSON to stdout. The CLI reads this stream and renders it. This is the only contract between them.
...
2. Generates a harness entry-point on the fly (in `$CARGO_TARGET_DIR/iron/`) — exactly like how `cargo test` generates a test harness without you writing a `fn main`.
...
```rust
// auto-generated by iron — do not edit, do not check in
fn main() {
...
2. Generate runner harness source into `$CARGO_TARGET_DIR/iron/__runner.rs` if absent or stale.
