...
vectorize fusion (4× float4)"` in `iron_qmv`; `"8 contiguous device
loads → 2× vec4 after vectorize pass"` in `iron_qmm_mma`).
...
1. **New `UnaryOpKind` variants** in `wh-iron-core/src/ir.rs`:
   ```rust
   FastExp,
   FastLog,
   FastSin,
...
2. **New DSL keywords** in `wh-iron-macros/src/body_parser.rs`:
   ```rust
...
4. **Kernel edits**: Replace `sin`/`cos`/`exp`/`log` with `fast_sin` /
   ...
   - `crates/wh-iron-std/src/iron/mel_spectrogram.rs`
   - `crates/wh-iron-std/src/mlx/softmax.rs`
   - `crates/wh-iron-std/src/mlx/logsumexp.rs`
   - `crates/wh-iron-std/src/iron/vocoder.rs`
