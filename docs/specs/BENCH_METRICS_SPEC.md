...
`iron bench` today reports **GB/s** (each kernel's `bytes_moved ÷ time`) as its headline metric. That is a *bandwidth-efficiency* number, and it is **not sufficient to optimize kernels or to compare precisions**:
...
- **Additive only** — do not remove or change the existing GB/s / ref-vs-Iron / correctness columns or the JSON fields already consumed by `baselines/*.json` diffing.
...
- **`BenchStats`** (`crates/wh-iron-std/src/stats.rs`) already captures per-kernel timing: `min_us`, `mean_us`, `median_us`, `p95_us`, `p99_us`, `stddev_us`, `cv_pct`. **Latency is measured but not surfaced** in the default table or JSON.
- **`OpResult`** (`crates/wh-iron-std/src/bench_types.rs`) carries `ref_perf`/`iron_perf` (GB/s), `equiv`, and optional `iron_timing`/`ref_timing` (`BenchStats`).
- **`run_kernel_bench`** (`crates/wh-iron-std/src/run_kernel.rs`) computes `gbps` from `bench_gbps(...)` and **discards the `BenchStats`** (`let (gbps, _stats) = ...`). `bytes_moved` comes from `BenchSetup::compute_bytes_moved()` (sum of buffer sizes unless overridden via `.bytes_moved()`).
- **`-vv` profile** (`compute_profiles` in `crates/wh-iron-cli/src/cmd/bench.rs`) already shows `occ%`, `regs`, and a coarse `bottleneck` label (e.g. "register-limited"), plus `p95/p99/cv%`.
- **JSON** (`iron bench --json`) emits only `{op, shape, metric, ref, mt}` — consumed by baseline diffing; must stay backward-compatible.
...
- `run_kernel_bench` must **stop discarding** the `BenchStats` and thread it into the `OpResult` (the `iron_timing`/`ref_timing` slots already exist on `result_sub_timed`).
- Display: add a `Iron(µs)` column (and `Ref(µs)` when a reference is present). This is the metric that makes cross-precision "fastest" readable.
...
| `OpResult` | `+ iron_us`/`ref_us` (from `BenchStats.min_us`), `+ gflops`, `+ pct_peak_bw`, `+ pct_peak_flops`, `+ arith_intensity`, `+ bottleneck` |
| Device specs | new `DeviceSpecs` lookup by device name |
| Table | default: add `Iron(µs)`, `GFLOP/s`, `%BW`/`%FLOP`, `bottleneck`; keep GB/s + `ok`. `-v`/`-vv` keep the existing profile detail |
| JSON | **add** `latency_us`, `gflops`, `pct_peak_bw`, `pct_peak_flops`, `arith_intensity`, `bottleneck`; **keep** `ref`/`iron` (GB/s) for baseline-diff compatibility |
...
3. **Widen f32-only tests** — several `#[test_kernel(dtypes=[f32])]` are on *generic `<T>`* kernels (`fft`, `softmax`, `logsumexp`, …) whose kernels already support f16/bf16; widen the tests. Genuinely-typed exceptions (`iron_fp4_quant_dequant` f32, `iron_random_hash` u32) stay as-is.
...
| fused RMSNorm+GEMV | `iron/rms_norm_block_scaled_qgemv.rs` |
| fused gated-RMSNorm+GEMV | `iron/gated_rms_norm_block_scaled_qgemv.rs` |
| batched-4 qgemv / qmm | `iron/batched_4_block_scaled_{qgemv,qmm}.rs` |
| batched-Q/K/V qgemv / qmm | `iron/batched_qkv_block_scaled_{qgemv,qmm}.rs` |
| flash SDPA (block-scaled KV, d64/96/128/256/512) | `iron/flash_block_scaled_sdpa.rs` |
| embedding gather | `iron/dequant_gather_block_scaled.rs` |
| qmm via MPP (tensor engine) | `mlx/block_scaled_qmm_mpp.rs` |
| qmm via NAX | `mlx/block_scaled_qmm_nax.rs` |
| MoE gather-qmm via MPP (bm8/bm16/bm64) | `iron/moe_mpp{,_bm8,_bm64}_block_scaled.rs` |
| expert-indexed GEMV | `iron/dequant_gemv_expert_indexed_block_scaled.rs` |
| patch embedding (linear projection) | `iron/patch_embed_block_scaled.rs` |
| patch embedding (simdgroup-MMA) | `iron/patch_embed_mma_block_scaled.rs` |
| conv2d / conv3d (direct) | `iron/{conv2d,conv3d}_block_scaled.rs` |
| conv2d / conv3d (im2col simdgroup-MMA) | `iron/{conv2d,conv3d}_mma_block_scaled.rs` |
| depthwise conv2d | `iron/depthwise_conv2d_block_scaled.rs` |
| audio conv1d (STT) / fishspeech conv1d (TTS) | `iron/{audio_conv1d,fishspeech_conv1d}_block_scaled.rs` |
...
> must enumerate the registry via `wh_iron_std::all_tests()`, not
> `wh-iron::harness::registry::all_tests()` — the latter leaves wh-iron-std's
...
> (1) the original `fp_quantized_mma::iron_fp4_qmm_mma` hand-rolled the E2M1 magnitude
...
> **also named `iron_fp4_qmm_mma`**, colliding with the original in the MSL/pipeline
> cache → order-dependent wrong pipelines (the source of the ~0.46 f32 anomaly and the
> apparent `iron_sdpa_multi_d256_causal` failure, which was a *victim* of the shared-
> state contamination, not itself buggy); renamed to `iron_fp4_float_qmm_mma`.
