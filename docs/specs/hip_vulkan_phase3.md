...
off). The last gain-sensitive kernel (`test_ffai_gated_delta_prep_chunk_no_gqa`,
...
sums through `ffai_subgroup_add`, a linear-order shuffle+accumulate loop
...
| `StrideReduce` dtype-aware loads | +19 | bf16 reductions need `ffai_bf16_to_f32(arr[i])`, not raw `float(arr[i])` |
...
| `Op::DeclareLocal`/`SetLocal` type-aware (LocalTypes map) | +12 | `let mut rem = p` for `uint p` was declaring `float ffai_loc_rem`; the strided_copy_nd unravel loop made every thread read src[0] |
| `ffai_atan2` helper (bypass AMD driver atan2 quirk) | +3 | Driver's GLSL `atan(y, x)` returns the wrong quadrant on the corpus inputs (off by exactly π) |
...
| Markstein correctly-rounded `ffai_fdiv` | +1 | AMD's driver substitutes `a / b` → `a * (1/b)` (1 ULP cost); `precise` qualifier doesn't block it. One Newton-Raphson step on top of hardware reciprocal → correctly-rounded f32 quotient (relies on `V_FMAC_F32` being correctly rounded, which AMDGPU guarantees) |
| NR-refined `ffai_rsqrt` + `Recip → ffai_fdiv(1.0, x)` | 0 | AMD's `V_RSQ_F32` is ~1.4 ULP; one Newton-Raphson step (`r * fma(-0.5*x*r, r, 1.5)`) tightens to ≤1 ULP. Same Markstein treatment for `Recip` since `1/x` is just division. Bit-exact `state_out` on the gated-delta recurrence |
| Linear-order `ffai_subgroup_add` | +1 | AMD's hardware `subgroupAdd` butterfly tree rounds differently than the CPU oracle's `iter().sum()` left-to-right linear sum. Replaced f32 subgroup-sum emission with a `subgroupShuffle(v, i)` accumulator over `i = 0..32` so the GPU result matches Rust's `iter().sum()` bit-exactly. Last KNOWN_HARD eliminated → PASS=4164 = 100% |
...
- Full fp16 / bf16 / i8 / u8 / i16 / u16 / i64 / u64 dtype coverage with
  `ffai_bf16_to_f32` / `ffai_f32_to_bf16` helpers and `scalar` block layout.
...
- `__ml_<name>` → `ffai_loc_<name>` GLSL identifier rewrite (`__` is
  reserved in GLSL).
- 60+ GLSL keyword / built-in collisions handled by `safe_glsl_ident`
  (`shared`, `length`, `input`, `output`, …).
- `ffai_atan2` helper to bypass AMD driver `atan(y, x)` quirk.
