...
**Scope:** Add an AMD GPU backend so Iron Kernels's `#[kernel]` DSL / IR lowers to
...
`HIPIFY` precisely to mechanically convert CUDA → HIP, and the constructs Iron Kernels
...
Iron Kernels kernels assume a **32-lane simdgroup** (Metal) ≙ 32-lane warp (NVIDIA).
...
microscaling, so Iron Kernels's quant matrix already spans Apple GPU (today) +
NVIDIA + AMD hardware.
