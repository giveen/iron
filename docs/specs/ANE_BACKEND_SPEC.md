...
**Scope:** Evaluate and design a path for Iron Kernels to target the **Apple Neural
...
This is a **category mismatch with Iron Kernels's model.** Iron Kernels lowers a Rust
...
backend" for Iron Kernels can only mean:

> Lower the subset of Iron Kernels ops that have **ANE-supported equivalents**
...
low power; the win is **not** running Iron Kernels's bespoke kernels there.
...
- The ANE delivers the **highest perf/Watt** on Apple silicon for the standard
  ...
  heavy layers on ANE while custom/unsupported ops run on the Iron Kernels GPU path.
- Apple's recent **blockwise / grouped weight quantization** in Core ML
  ...
  *kernels* do not.
- It future-proofs Iron Kernels as a multi-target toolchain (GPU now; ANE for the
  supported subgraph).
...
### 3.1 Pipeline
1. **IR → MIL.** Map Iron Kernels ops with MIL equivalents (`linear`/`matmul`,
...
### 3.3 What Iron Kernels contributes vs delegates
- **Contributes:** the IR→MIL lowering pass, the op-eligibility analysis
  (what can go to ANE vs stays on the Iron Kernels GPU backend), and the
...
**Working precedent:** `ane-infer` (a Rust project, like Iron Kernels) drives exactly
...
2. **Route A MVP.** IR→MIL emitter for a small supported subgraph (e.g. an FFN:
   ...
   `.mlpackage`; verify ANE residency + numerical match vs the Iron Kernels GPU path.
...
- **Category mismatch (the big one).** The ANE can't run Iron Kernels's defining
...
replacement for — Iron Kernels's GPU kernel generation.
...
- thebasedcapital, *ane-infer* — https://github.com/thebasedcapital/ane-infer —
  ...
  closest existing analog to a Iron Kernels ANE backend.
