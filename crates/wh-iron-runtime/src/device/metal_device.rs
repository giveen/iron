...
impl MetalDevice {
    ...
    pub fn create() -> Result<Option<Self>, IronError> {
        let device = MTLCreateSystemDefaultDevice().ok_or(IronError::DeviceCreation)?;

        let queue = device.newCommandQueue().ok_or(IronError::QueueCreation)?;
    ...
    pub fn create() -> Result<Option<Self>, IronError> { Ok(None) }
    ...
    pub fn command_buffer(
        &self,
    ) -> Result<Retained<ProtocolObject<dyn objc2_metal::MTLCommandBuffer>>, IronError> {
        self.queue.commandBuffer().ok_or(IronError::NoDevice)
    }
    ...
    pub fn acquire_shared(&self, bytes: Option<&[u8]>, len: usize) -> Result<BufRc, IronError> {
    ...
    /// chained passes.
    pub fn acquire_private(&self, len: usize) -> Result<BufRc, IronError> {
