...
use ffi::*;
use wh_iron_codegen::{CodegenBackend, GlslGenerator, spirv::GlslBindingPlan};
use wh_iron_core::{dtype::DType, ir::Kernel};
...
fn vk_check(res: VkResult, what: &str) -> Result<(), IronError> {
    if res == VK_SUCCESS {
        return Ok(());
    }
    Err(IronError::Dispatch(format!("{what}: VkResult={res}")))
}
...
/// live in a `'static Arc` inside the iron-vulkan resident-tensor cache. The
...
// VulkanDevice (hence VkDevice) outlives them, which the iron-vulkan cache
...
impl VulkanDevice {
    ...
    pub fn create() -> Result<Option<Self>, IronError> {
        unsafe {
            // Instance.
            let app_name = CString::new("wh-iron").unwrap();
            ...
            // wh-iron kernels' `subgroupAdd` etc. reduce within a
            ...
            // memory-model op). Gated on IRON_VK_COOPMAT; harmless otherwise.
            let coopmat = std::env::var("IRON_VK_COOPMAT").map(|v| v == "1").unwrap_or(false);
            if coopmat {
            ...
            // Gated VK_KHR_cooperative_matrix path (`IRON_VK_COOPMAT=1`). The
    ...
    fn find_memory_type(&self, mem_type_bits: u32, flags: u32) -> Result<u32, IronError> {
    ...
    pub fn alloc_storage(&self, size: u64) -> Result<VulkanBuffer<'_>, IronError> {
    ...
    pub fn upload(&self, data: &[u8]) -> Result<VulkanBuffer<'_>, IronError> {
    ...
    pub fn download(&self, buf: &VulkanBuffer, out: &mut [u8]) -> Result<(), IronError> {
        if out.is_empty() {
            return Ok(());
        }
        unsafe {
    ...
    pub fn compile(
        ...
        kernel_name: &str,
    ) -> Result<VulkanPipeline, IronError> {
        ...
        unsafe {
            ...
            // wh-iron kernels' Apple-simdgroup assumption). On AMD
    ...
    pub fn run_kernel(
        ...
        let plan = cg.binding_plan(kernel).map_err(IronError::Codegen)?;
        let glsl = cg.generate(kernel).map_err(IronError::Codegen)?;
        ...
        for p in &kernel.params {
            ...
            if matches!(p.kind, wh_iron_core::ir::ParamKind::Strided) {
    ...
    ) -> Result<VkDescriptorSet, IronError> {
    ...
    pub fn bench_kernel(
        ...
        if timestamp_period <= 0.0 {
            return Err(IronError::DeviceCapability(
        ...
        for p in &kernel.params {
            let Some(bytes) = buffers.get(&p.name) else {
                free_all(&dev_bufs);
                return Err(IronError::Dispatch(format!("missing buffer for param '{}'", p.name)));
            };
        ...
        let dispatch_once = |timed: bool| -> Result<f64, IronError> {
            unsafe {
                let cb_ai = VkCommandBufferAllocateInfo {
                    sType: VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
                    pNext: ptr::null(),
        ...
        let run = || -> Result<Vec<f64>, IronError> {
            for _ in 0..warmup {
                dispatch_once(false)?;
            }
    ...
    // dominant cost. This seam lets a higher layer (iron-vulkan) upload
    ...
    pub fn alloc_raw(&self, len: usize) -> Result<VulkanRawBuffer, IronError> {
    ...
    pub fn htod_raw(&self, buf: &VulkanRawBuffer, data: &[u8]) -> Result<(), IronError> {
        if data.is_empty() {
            return Ok(());
        }
        unsafe {
    ...
    pub fn alloc_raw_device_local(&self, data: &[u8]) -> Result<VulkanRawBuffer, IronError> {
        let _exec = self.exec_lock.lock();
        let size = (data.len().max(4)) as u64;
        let make_buffer =
            |usage: u32, props: u32| -> Result<(VkBuffer_, VkDeviceMemory), IronError> {
                let bci = VkBufferCreateInfo {
                    sType: VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
                    pNext: ptr::null(),
                    flags: 0,
    ...
    pub fn dtoh_raw(&self, buf: &VulkanRawBuffer, out: &mut [u8]) -> Result<(), IronError> {
        if out.is_empty() {
            return Ok(());
        }
        unsafe {
    ...
    /// iron-vulkan resident-tensor cache owns them across the batch).
    pub fn run_pipeline_batch(&self, items: &[BatchDispatch]) -> Result<(), IronError> {
        let _exec = self.exec_lock.lock();
        if items.is_empty() {
            return Ok(());
        }
...
pub fn compile_glsl_to_spv(glsl_src: &str, file_name: &str) -> Result<Vec<u8>, IronError> {
    let csrc = CString::new(glsl_src).map_err(|e| IronError::Compilation(e.to_string()))?;
    let cfile = CString::new(file_name).map_err(|e| IronError::Compilation(e.to_string()))?;
    let centry = CString::new("main").unwrap();
    unsafe {
        let compiler = shaderc_compiler_initialize();
        if compiler.is_null() {
            return Err(IronError::Compilation("shaderc_compiler_initialize failed".into()));
        }
        ...
        // reaps the faster shader. Override with IRON_SHADERC_OPT:
        //   0 = zero-opt (readable disasm / correctness fallback),
        //   1 = size, 2 = performance (default).
        let opt_level: ::std::os::raw::c_int = ::std::env::var("IRON_SHADERC_OPT")
            .ok()
