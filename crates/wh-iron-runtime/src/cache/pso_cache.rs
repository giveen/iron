...
pub(crate) fn pso_cache_key(kernel: &Kernel, fn_consts: &BTreeMap<String, u32>) -> u64 {
    ...
    // kernels (iron_qmm, dequant_gemv_int*, …) take a packed `Tensor<u32>`
...
impl PsoCache {
    ...
    ) -> Result<Pso, IronError>
    where
        F: FnOnce() -> Result<String, IronError>,
    {
        use std::ptr::NonNull;
        ...
        let lib = dev
            ...
            .map_err(|e| IronError::MslCompilation(format!("{e:?}")))?;

        let fun = if fn_consts.is_empty() {
            lib.newFunctionWithName(&NSString::from_str(kernel_name))
                .ok_or_else(|| IronError::FunctionNotFound { name: kernel_name.to_string() })?
        } else {
            ...
            lib.newFunctionWithName_constantValues_error(&NSString::from_str(kernel_name), &fcv)
                .map_err(|e| IronError::FunctionNotFound {
        ...
        let pso = dev
            ...
            .map_err(|e| IronError::PipelineCreation {
...
mod tests {
    ...
    fn pso_cache_key_folds_all_param_dtypes() {
        ...
        // iron_qmm / dequant_gemv monomorphizations. They must NOT collide.
        ...
        let mut k_bf16 = Kernel::new("iron_qmm");
        k_bf16.params = vec![
            tensor_param("w", DType::U32, &[4], false, ParamKind::Tensor),
