...
use wh_iron_codegen::msl::MslGenerator;
...
impl MslCache {
    ...
    #[cfg(target_os = "macos")]
    pub(crate) fn get_or_generate(&self, kernel: &Kernel, key: u64) -> Result<String, IronError> {
...
mod perf {
    ...
    //!
    //! ```text
    //! cargo test -p wh-iron-runtime --release perf_cache_lock_throughput \
    //!     -- --ignored --nocapture
    //! ```
    //!
...
mod perf_clone {
    ...
    fn realistic_kernel() -> Kernel {
        let mut k = Kernel::new("iron_aura_flash_sdpa_bench");
        k.params = vec![
            Param {
                name: "q".into(),
                dtype: DType::F16,
                shape: Shape::new([1024, 128].into_iter().map(wh_iron_core::shape::Dim::Known)),
                is_output: false,
    ...
    fn synth_msl(size: usize) -> String {
        let pattern = "// wh-iron auto-generated MSL — placeholder body. ";
    ...
    fn perf_eager_clone_vs_deferred_msl() {
        ...
        for _ in 0..ITERS {
            ...
            // before reaching it.
            let _provider = || -> Result<String, IronError> {
