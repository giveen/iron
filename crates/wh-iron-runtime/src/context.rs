...
use crate::{device::gpu_family::GpuFamily, error::IronError};
...
impl DispatchResult {
    ...
    pub fn output_f32(&self, name: &str) -> Result<Vec<f32>, IronError> {
        let bytes = self
            .output(name)
            .ok_or_else(|| IronError::Dispatch(format!("output '{name}' not found")))?;
    ...
    pub fn output_u32(&self, name: &str) -> Result<Vec<u32>, IronError> {
        let bytes = self
            .output(name)
            .ok_or_else(|| IronError::Dispatch(format!("output '{name}' not found")))?;
...
impl Context {
    ...
    /// constants.
    pub fn dispatch(&self, kernel: &Kernel) -> Result<DispatchResult, IronError> {
    ...
    ) -> Result<DispatchResult, IronError> {
    ...
    pub fn upload_resident(&self, bytes: &[u8]) -> Result<ResidentBuffer, IronError> {
        #[cfg(target_os = "macos")]
        {
            let dev = self.device.as_ref().ok_or(IronError::NoDevice)?;
    ...
    ) -> Result<Vec<DispatchResult>, IronError> {
        if specs.is_empty() {
            return Ok(Vec::new());
        }
...
#[cfg(test)]
mod tests {
    use wh_iron_core::{DType, Dim, Param, Shape, ir::ParamKind};
    ...
    //   cargo test --release -p wh-iron-runtime perf_resolve_strided_metadata \
    //     -- --ignored --nocapture
    //
    ...
    fn pso_cache_key_folds_all_param_dtypes() {
        ...
        // Quantized kernels (iron_qmm, dequant_gemv_int*) take a packed
        ...
        let consts = BTreeMap::new();
        let mut k_f32 = Kernel::new("iron_qmm");
        k_f32.params = vec![
        ...
        let mut k_f16 = Kernel::new("iron_qmm");
        k_f16.params = vec![
            tensor_param("w", DType::U32, &[4], false, ParamKind::Tensor),
    ...
    /// Representative MSL source size for a real wh-iron kernel:
    ...
    // Perf microbench — run with:
    //   cargo test --release -p wh-iron-runtime perf_dispatch_chain_pso_key \
    //     -- --ignored --nocapture
    //
