...
/// [`wh_iron_codegen::kernel_uses_n_simd`] — `true` iff the kernel derives
/// `n_simd = tpg / 32` and strides a reduction by it. (Mode alone is too coarse:
/// the per-thread `iron_hadamard_m*` matvecs are Reduction-mode but dispatch
/// safely at TPG < 32 because they never compute `n_simd`.)
///
/// Rejects, with a [`IronError::Dispatch`] describing the violation:
...
mod tests {
    ...
    fn rejects_n_simd_kernel_below_one_simdgroup() {
        ...
        for t in [1usize, 4, 16, 31] {
            let err = validate_dispatch_geometry(&kernel(), [4, 1, 1], [t, 1, 1], MAX_TPG, true)
                .unwrap_err();
            assert!(matches!(err, IronError::Dispatch(_)), "tpg {t} must be rejected");
        }
    }
    ...
    fn non_n_simd_kernel_allows_small_threadgroups() {
        // The per-thread `iron_hadamard_m*` case: Reduction-mode but never derives
