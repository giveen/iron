...
impl<'a> ChainDispatch<'a> {
    ...
    pub fn execute(&self) -> Result<Vec<DispatchResult>, IronError> {
    ...
    fn build_binding_plans(&self) -> Result<Vec<Vec<ParamBufferPlan>>, IronError> {
    ...
    fn compile_pso_pipeline(
        &self,
    ) -> Result<Vec<Retained<ProtocolObject<dyn MTLComputePipelineState>>>, IronError> {
    ...
    fn encode_passes(
    ...
    ) -> Result<Vec<Vec<BufRc>>, IronError> {
        ...
        for (i, spec) in self.specs.iter().enumerate() {
            ...
            crate::dispatch::validate::validate_dispatch_geometry(
                ...
                wh_iron_codegen::kernel_uses_n_simd(spec.kernel),
            )?;
            ...
            unsafe {
                enc.setBytes_length_atIndex(
                    NonNull::new(staged.as_ptr() as *mut _)
                        .ok_or_else(|| IronError::Buffer("setBytes null".into()))?,
                    elem,
