...
/// which fits every kernel currently in `wh-iron-std` (the tallest is
...
fn static_buffer_len(param: &Param) -> Result<Option<usize>, IronError> {
    let Some(num_elements) = param.shape.num_elements() else {
        return Ok(None);
    };
    num_elements
        .checked_mul(param.dtype.size_bytes())
        .ok_or_else(|| IronError::Buffer(format!("buffer '{}' size overflows usize", param.name)))
        .map(Some)
}
...
pub(crate) fn build_param_buffer_plans(
    kernel: &Kernel,
    buffers: &BTreeMap<String, Vec<u8>>,
) -> Result<Vec<ParamBufferPlan>, IronError> {
...
/// [`INLINE_RANK`].
fn known_shape_dims(param: &Param) -> Result<Option<DimVec>, IronError> {
...
fn row_major_strides(name: &str, dims: &[u32]) -> Result<DimVec, IronError> {
    ...
    for (idx, &dim) in dims.iter().enumerate().rev() {
        ...
        stride = stride.checked_mul(dim).ok_or_else(|| {
            IronError::Buffer(format!("row-major strides for '{}' overflowed u32", name))
        })?;
    }
    Ok(strides)
}
...
pub(crate) fn resolve_strided_metadata<'a>(
...
) -> Result<StridedMetadata<'a>, IronError> {
    ...
    let defaults = known_shape_dims(param)?
        .map(|dims| {
            ...
            Ok::<(Vec<u8>, Vec<u8>), IronError>((encode_u32s(&dims), encode_u32s(&strides)))
        })
        .transpose()?;
...
mod tests {
    ...
    fn tensor_param(
        ...
        Param {
            name: name.into(),
            dtype,
            shape: Shape::new(dims.iter().copied().map(wh_iron_core::shape::Dim::Known)),
            is_output,
            kind,
        }
    }
    ...
    fn perf_build_param_buffer_plans_modal_kernel() {
        // Modal Iron kernel: 8 params, 2 strided. Bench reflects what
