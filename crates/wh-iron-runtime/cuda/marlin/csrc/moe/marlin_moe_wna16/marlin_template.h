...
template <typename scalar_t,  // compute dtype, half or nv_float16
          const marlin_types::ScalarTypeId b_type_id,  // weight MarlinScalarType id
...
template <int count, marlin_types::ScalarTypeId type_id>
...
template <marlin_types::ScalarTypeId type_id>
...
template <const marlin_types::ScalarTypeId a_type_id,  // A ScalarType id
          const marlin_types::ScalarTypeId b_type_id,  // B ScalarType id
          const marlin_types::ScalarTypeId c_type_id,  // C ScalarType id
          const marlin_types::ScalarTypeId s_type_id,  // B_SCALE ScalarType id
          const int threads,          // number of threads in a threadblock
          const int thread_m_blocks,  // number of 16x16 blocks in the m
                                      // dimension (batchsize) of the
                                      // threadblock
...
__global__ void Marlin(
  ...
  if constexpr (a_type_id == marlin_types::kFE4M3fn.id()) return;
  #endif

  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
  // Turing TensorCore only supports fp16 and int8
  if constexpr (a_type_id != marlin_types::kFloat16.id() && a_type_id != marlin_types::kS8.id())
    return;
  #endif
  ...
  static constexpr auto num_bits =
      marlin_types::ScalarType::from_id(b_type_id).size_bits();
  // Disable use_fp16_accum for NVFP4 and cases when group_size == -1 &&
  // num_bits == 4
  constexpr bool use_fp16_accum =
      a_type_id == marlin_types::kFloat16.id() &&
      (!(b_type_id == marlin_types::kFE2M1f.id() && s_type_id == marlin_types::kFE4M3fn.id()) &&
  ...
  static constexpr auto a_type = marlin_types::ScalarType::from_id(a_type_id);
  static constexpr auto b_type = marlin_types::ScalarType::from_id(b_type_id);
  static constexpr auto c_type = marlin_types::ScalarType::from_id(c_type_id);
  static constexpr auto s_type = marlin_types::ScalarType::from_id(s_type_id);
  if constexpr (b_type == marlin_types::kFE2M1f) {
    static_assert(s_type == marlin_types::kFE4M3fn && group_blocks == 1 ||
                  s_type == marlin_types::kFE8M0fnu && group_blocks == 2);
  } else if constexpr (b_type == marlin_types::kFE4M3fn && s_type == marlin_types::kFE8M0fnu) {
    static_assert(group_blocks == 2);
  } else if constexpr (std::is_same<scalar_t, nv_bfloat16>::value) {
    static_assert(s_type == marlin_types::kBFloat16);
  } else if constexpr (std::is_same<scalar_t, half>::value) {
    static_assert(s_type == marlin_types::kFloat16);
  }
  ...
  constexpr bool has_zp = b_type == marlin_types::kU4 || b_type == marlin_types::kU8;
  constexpr bool is_int_type = b_type == marlin_types::kU4 || b_type == marlin_types::kU8 ||
                               b_type == marlin_types::kS4 || b_type == marlin_types::kS8 ||
                               b_type == marlin_types::kU4B8 || b_type == marlin_types::kU8B128;
  constexpr bool is_8bit_scale = s_type.size_bits() == 8;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      is_a_8bit || (b_type == marlin_types::kFE4M3fn && !(s_type == marlin_types::kFE8M0fnu)) ||
      b_type == marlin_types::kFE2M1f && s_type == marlin_types::kFE4M3fn ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
      has_zp && !is_zp_float && !(b_type == marlin_types::kU8);
  ...
  // part1: DP
  // part2: two-tile SK
  // see https://github.com/marlin_types-project/marlin_types/pull/24722 for more details
  ...
  #pragma unroll
    ...
    if (threadIdx.x < moe_block_size) {
      ...
      if (mul_topk_weights) {
        ...
        if constexpr (b_type == marlin_types::kFE2M1f && s_type == marlin_types::kFE4M3fn) {
  ...
  auto matmul = [&](int k, int pipe) {
    ...
    if constexpr (s_type == marlin_types::kFE4M3fn || s_type == marlin_types::kFE8M0fnu) {
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (b_type_id == marlin_types::kFE2M1f.id()) {
  ...
  #pragma unroll
      ...
      if constexpr (group_blocks != -1) {
        if (group_blocks == 2 || k == 1) {
          if constexpr (a_type == marlin_types::kS8) {
            int2 s_vals[2];
            s_vals[0] = {
  ...
  #pragma unroll
            ...
            float2 s_vals[2];
            if constexpr (s_type_id != marlin_types::kFE8M0fnu.id()) {
  ...
  #pragma unroll
      ...
      // F-85 round-7: sh_red (the cross-warp reduction scratch) is
      // pointer-aliased onto sh_b (the weight double-buffer -- see
      // `int4* sh_red = sh_new;` above). cp_async_wait<0>() above only
      // drains THIS thread's outstanding cp.async queue; it is not a
      // cross-thread barrier, so the plain (non-async) shared-memory
      // reads of sh_b done by fetch_to_registers() in the tail of the
      // k-loop (the k == b_sh_wr_iters - 1 iteration, issued after the
      // last wait_for_stage()) are not guaranteed complete on all
      // threads before thread_block_reduce() starts writing sh_red.
      // Confirmed via compute-sanitizer --tool racecheck: WAR hazard
      // between the sh_b read in fetch_to_registers (this file, the
      // `frag_b_quant[...] = *reinterpret_cast<I4*>(&sh_b_stage[...])`
      // line) and the sh_red write in thread_block_reduce's first
      // pass (`sh_red[red_sh_wr] = ...`), reproducing as large
      // (>10x) relative error on dense-random GEMM inputs despite the
      // dequant/repack/mma format all verifying exact independently.
      __syncthreads();
      thread_block_reduce();
