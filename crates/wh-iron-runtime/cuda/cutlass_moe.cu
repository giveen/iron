// CUTLASS grouped MoE GEMM, extern "C" entry for the wh-iron-runtime FFI.
    ...
    int n_groups, int N, int K, void* stream_v)
{
    ...
    size_t gp = n_groups;
    int rc = 0;
#define IRON_CUDA_CK(call) do { if ((call) != cudaSuccess) { rc = 3; goto cleanup; } } while (0)
    IRON_CUDA_CK(cudaMallocAsync((void**)&ps_d, gp*sizeof(cutlass::gemm::GemmCoord), stream));
    IRON_CUDA_CK(cudaMallocAsync((void**)&pA_d, gp*sizeof(ElementA*), stream));
    IRON_CUDA_CK(cudaMallocAsync((void**)&pB_d, gp*sizeof(ElementB*), stream));
    IRON_CUDA_CK(cudaMallocAsync((void**)&pC_d, gp*sizeof(ElementOutput*), stream));
    IRON_CUDA_CK(cudaMallocAsync((void**)&lda_d, gp*sizeof(StrideI), stream));
    IRON_CUDA_CK(cudaMallocAsync((void**)&ldb_d, gp*sizeof(StrideI), stream));
    IRON_CUDA_CK(cudaMallocAsync((void**)&ldc_d, gp*sizeof(StrideI), stream));
    IRON_CUDA_CK(cudaMemcpyAsync(ps_d, ps.data(), gp*sizeof(cutlass::gemm::GemmCoord), cudaMemcpyHostToDevice, stream));
    IRON_CUDA_CK(cudaMemcpyAsync(pA_d, pA.data(), gp*sizeof(ElementA*), cudaMemcpyHostToDevice, stream));
    IRON_CUDA_CK(cudaMemcpyAsync(pB_d, pB.data(), gp*sizeof(ElementB*), cudaMemcpyHostToDevice, stream));
    IRON_CUDA_CK(cudaMemcpyAsync(pC_d, pC.data(), gp*sizeof(ElementOutput*), cudaMemcpyHostToDevice, stream));
    IRON_CUDA_CK(cudaMemcpyAsync(lda_d, lda.data(), gp*sizeof(StrideI), cudaMemcpyHostToDevice, stream));
    IRON_CUDA_CK(cudaMemcpyAsync(ldb_d, ldb.data(), gp*sizeof(StrideI), cudaMemcpyHostToDevice, stream));
    IRON_CUDA_CK(cudaMemcpyAsync(ldc_d, ldc.data(), gp*sizeof(StrideI), cudaMemcpyHostToDevice, stream));
    ...
    // goto that jumps over one while it is still in scope at the label.
    {
        ...
        if (ws) IRON_CUDA_CK(cudaMallocAsync((void**)&work, ws, stream));
