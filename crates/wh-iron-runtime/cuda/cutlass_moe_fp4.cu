...
// wh-iron-runtime FFI (sm_120a/sm_121a only; AOT-built when CUTLASS_DIR set).
//
...
#define IRON_CUDA_CK(call) do { if ((call) != cudaSuccess) { rc = 3; goto cleanup; } } while (0)
    IRON_CUDA_CK(cudaMallocAsync((void**)&blob, blob_bytes, stream));
    IRON_CUDA_CK(cudaMemcpyAsync(blob, staging.data(), blob_bytes, cudaMemcpyHostToDevice, stream));
...
#define IRON_CK(c) do{ if((c)!=cudaSuccess){rc=3;goto cleanup;} }while(0)
    IRON_CK(cudaMallocAsync((void**)&blob,blob_bytes,stream));
    IRON_CK(cudaMemcpyAsync(blob,stg.data(),blob_bytes,cudaMemcpyHostToDevice,stream));
    {
        ...
        if (ws) IRON_CK(cudaMallocAsync((void**)&work,ws,stream));
...
__global__ void iron_fp4_fill_group_args(
    ...
    const void* off_dev, void* stream_v)
{
    ...
    iron_fp4_fill_group_args<<<blocks, threads, 0, stream>>>(
...
__global__ void iron_fp4_fill_group_args_fused(
    const unsigned* __restrict__ off,   // [n_groups+1] device row offsets
    ...
    const void* off_dev, void* stream_v)
{
    ...
    int threads = 128;
    int blocks = (h->n_groups + threads - 1) / threads;
    iron_fp4_fill_group_args_fused<<<blocks, threads, 0, stream>>>(
        (const unsigned*)off_dev,
        (const uint8_t*)A, (const uint8_t*)SFA, (uint8_t*)D, (uint8_t*)SFD,
        (int*)(h->blob + h->off_ps),
        (const void**)(h->blob + h->off_pA),
        ...
        h->n_groups, h->N, h->K);
    if (cudaGetLastError() != cudaSuccess) return 3;
    // DIAGNOSTIC (IRON_FUSE_REINIT=1): re-initialize after the fill kernel so the
    ...
    static int reinit = -1;
    if (reinit < 0) { const char* e = getenv("IRON_FUSE_REINIT"); reinit = (e && e[0]=='1') ? 1 : 0; }
    if (reinit) {
