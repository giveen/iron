...
//!
//! Used by the `ffaik build --emit` flow in `ffai-kernels-cli`. Kept in
//! `ffai-kernels-codegen` so other tooling (custom build scripts, IDE
...
pub struct Manifest {
    ...
    pub version: u32,
    /// `ffaik` version that produced this manifest.
    pub ffai_kernels_version: String,
...
pub fn build_manifest(kernels: &[Kernel]) -> Manifest {
    Manifest {
        version: 1,
        ffai_kernels_version: env!("CARGO_PKG_VERSION").to_string(),
...
/// Render `FFAIKernels.swift` — one static function per kernel,
...
/// hand-written on the Swift side (lives in `FFAIKernelsSwift`).
pub fn render_swift_wrappers(kernels: &[Kernel]) -> String {
    let mut out = String::new();
    out.push_str(
        "// AUTOGENERATED by `ffaik build --emit swift`. DO NOT EDIT.\n\
         //\n\
...
mod tests {
    ...
    fn emits_threadgroups_wrapper_with_bindings_and_dispatch() {
        ...
        k.constexprs.push(ffai_kernels_core::ir::ConstExprDecl {
            name: ffai_kernels_core::constexpr::ConstExpr::new("k_in"),
            dtype: DType::U32,
            value: None,
        });
        let swift = render_swift_wrappers(&[k]);
