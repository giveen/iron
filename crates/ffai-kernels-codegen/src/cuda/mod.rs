...
/// Shared preamble for every emitted CUDA kernel: headers, the `FFAI_INF`
...
const CUDA_PREAMBLE: &str = r#"// Generated by FFAI Kernels (CUDA backend). DO NOT EDIT.
#include <cuda_fp16.h>
#include <cuda_bf16.h>
...
#define FFAI_INF (__int_as_float(0x7f800000))
#ifndef INFINITY
#define INFINITY FFAI_INF
#endif

// E2M1 (fp4): codebook {0,.5,1,1.5,2,3,4,6}, sign in bit 3.
__device__ __forceinline__ float ffai_decode_e2m1(unsigned int code) {
...
__device__ __forceinline__ float ffai_decode_e4m3(unsigned int bits) {
...
__device__ __forceinline__ float ffai_decode_e5m2(unsigned int bits) {
...
__device__ __forceinline__ float ffai_decode_int8(unsigned int bits) {
...
// divides (`auto v = ffai_fdiv(int, int)` for SSA index math, `ffai_fdiv(uint,
...
__device__ __forceinline__ auto ffai_fdiv(A a, B b) -> decltype(a / b) { return a / b; }
template <>
__device__ __forceinline__ float ffai_fdiv<float, float>(float a, float b) {
    float r = 1.0f / b;
    float q = a * r;
...
// Newton-Raphson step tightens it to ≤1 ULP. Same flag as `ffai_fdiv`.
__device__ __forceinline__ float ffai_rsqrt(float x) {
    float r = rsqrtf(x);
...
__device__ __forceinline__ float ffai_expf(float x) { return exp2f(x * 1.4426950408889634f); }
__device__ __forceinline__ float ffai_logf(float x) { return log2f(x) * 0.6931471805599453f; }

// Neural activations (ported from msl/preamble.rs).
__device__ __forceinline__ float ffai_silu(float x) { return x / (1.0f + expf(-x)); }
__device__ __forceinline__ float ffai_relu(float x) { return fmaxf(0.0f, x); }
__device__ __forceinline__ float ffai_sigmoid(float x) { return 1.0f / (1.0f + expf(-x)); }
__device__ __forceinline__ float ffai_gelu(float x) {
...
impl CudaGenerator {
    ...
    fn emit_op(
        ...
        match op {
            ...
            Op::Load { src, indices, .. } => {
                ...
                if indices.is_empty() {
                    ...
                    if let Some(p) = ro_param {
                        ...
                        // doesn't thrash L2. Gated by FFAI_LDCS_WEIGHTS (default off);
                        // activations/scales (T/f16) stay __ldg (reused across lanes).
                        let ldcs =
                            p.dtype == DType::U32 && std::env::var("FFAI_LDCS_WEIGHTS").is_ok();
                        if ldcs {
            ...
            Op::BinOp { op: bop, lhs, rhs } => {
                ...
                // overloads of `ffai_fdiv` pass through to `/`, so the
                ...
                if matches!(bop, BinOpKind::Div) && self.profile.precise_simd_sum {
                    writeln!(out, "{pad}auto {v} = ffai_fdiv({l}, {r});").ok();
                } else {
            ...
            Op::Activation { kind, value } => {
                ...
                let rv = self.vname(Some(*value), block, ov);
                let expr = match kind {
                    ActKind::Silu => format!("ffai_silu((float)({rv}))"),
                    ActKind::Gelu => format!("ffai_gelu((float)({rv}))"),
                    ActKind::Relu => format!("ffai_relu((float)({rv}))"),
                    ActKind::Sigmoid => format!("ffai_sigmoid((float)({rv}))"),
    ...
    fn cuda_unary(&self, op: UnaryOpKind, arg: &str) -> String {
        ...
        if self.profile.precise_simd_sum {
            match op {
                Recip => return format!("ffai_fdiv(1.0f, (float)({arg}))"),
                Rsqrt => return format!("ffai_rsqrt((float)({arg}))"),
                Exp => return format!("ffai_expf((float)({arg}))"),
                Log => return format!("ffai_logf((float)({arg}))"),
                _ => {},
            }
        }
        match op {
...
fn fmt_f32_lit(v: f64) -> String {
    if !v.is_finite() {
        if v.is_nan() {
            return "(FFAI_INF - FFAI_INF)".to_string();
        }
        return if v > 0.0 { "FFAI_INF".to_string() } else { "(-FFAI_INF)".to_string() };
    }
...
fn reduce_init(kind: ReduceKind) -> &'static str {
    match kind {
        ...
        ReduceKind::Max => "(-FFAI_INF)",
        ReduceKind::Min => "FFAI_INF",
...
mod tests {
    ...
    fn row_reduce_ir() -> Kernel {
        ...
        k.constexprs.push(ffai_kernels_core::ir::ConstExprDecl {
            name: ffai_kernels_core::constexpr::ConstExpr::new("n"),
            dtype: DType::U32,
            value: None,
        });
    ...
    fn float_literals_are_valid_cpp() {
        ...
        assert_eq!(fmt_f32_lit(f64::INFINITY), "FFAI_INF");
        assert_eq!(fmt_f32_lit(f64::NEG_INFINITY), "(-FFAI_INF)");
    }
