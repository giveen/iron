...
//!
//! Bench dispatch in `ffai-kernels-std/src/run_spec.rs` sets `expected_tpg` from
...
use ffai_kernels_core::ir::{Kernel, ReduceKind};
...
impl super::MslGenerator {
    ...
    pub(super) fn emit_reduce(
        ...
        let use_threadgroup = (kernel.mode == ffai_kernels_core::ir::KernelMode::Reduction
            || kernel.mode == ffai_kernels_core::ir::KernelMode::Tile2D)
            && axis == 0;
        if use_threadgroup {
