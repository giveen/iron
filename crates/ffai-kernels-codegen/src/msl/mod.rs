...
//!
//! Walks the FFAI Kernels IR and emits valid MSL source text.
...
fn kernel_has_reduce_mean(kernel: &Kernel) -> bool {
    let check = |ops: &[Op]| {
        ops.iter()
            .any(|op| matches!(op, Op::Reduce { op: ffai_kernels_core::ir::ReduceKind::Mean, .. }))
    };
    check(&kernel.body.ops) || kernel.blocks.values().any(|b| check(&b.ops))
}
...
pub(super) fn kernel_reduce_uses_n_simd(kernel: &Kernel, config: &MslConfig) -> bool {
    let tg_modes = matches!(
        kernel.mode,
        ffai_kernels_core::ir::KernelMode::Reduction | ffai_kernels_core::ir::KernelMode::Tile2D
    );
    if !tg_modes {
        return false;
    }
...
impl MslGenerator {
    ...
    fn emit_msl(&self, k: &Kernel) -> Result<String> {
        ...
        wl!(out, "// Generated by FFAI Kernels");
...
///
/// **Why `ffaik build` callers should pass this:** the bench harness sets
/// `expected_tpg` from `ShapeSpec.tpg`. If `ffaik build --emit all`
/// dropped the TPG and produced different MSL than `ffaik bench` measured,
...
mod tests {
    ...
    #[test]
    fn unary_op_emit() {
        use ffai_kernels_core::ir::UnaryOpKind;
    ...
    fn activation_silu_emits_helper() {
        use ffai_kernels_core::ir::ActKind;
    ...
    fn multi_dim_load_stride() {
        use ffai_kernels_core::{constexpr::ConstExpr, shape::Dim};
        ...
        k.constexprs.push(ffai_kernels_core::ir::ConstExprDecl {
            name: m_ce,
            dtype: DType::U32,
            value: None,
        });
    ...
    fn fusion_cast_exp_binop_into_single_expression() {
        use ffai_kernels_core::ir::{BinOpKind, UnaryOpKind};
    ...
    fn atomic_threadgroup_emits_cast_form() {
        use ffai_kernels_core::ir::{AtomicKind, AtomicScope};
