...
//! T. Two forms share the banded-inv_freq math:
//!   - `mt_rope_llama` — per-token decode (single `position` constexpr)
//!   - `mt_rope_llama_many` — batched prefill (per-row `positions` tensor, one
//!     dispatch over T rows)
//!
...
/// Batched Llama-style RoPE — applies position-dependent rotation across T rows
/// in ONE dispatch. Identical math / banding to `mt_rope_llama`, but the scalar
/// `position` constexpr is replaced by a per-row `positions: Tensor<u32>` of
/// length T and a new outer grid axis selects the row. For prefill the caller
/// used to T-loop the single-token kernel once per token (T dispatches × N
/// layers); this collapses that to ONE dispatch per layer per buffer.
///
...
/// the host wraps two `_many` dispatches on a shared command encoder instead.
#[kernel]
pub fn mt_rope_llama_many<T>(
    qk: Tensor<T>,
    ...
    // from the positions vector — the only data-dependent piece vs the single
    // kernel.
    let position = load(positions[r]);
    // Banded inv_freq calculation — bit-identical to `mt_rope_llama`.
    let i_f = i.cast::<f32>();
    let half_f = half_dim.cast::<f32>();
    let inv_freq_base = exp2(-i_f * log2(theta_base) / half_f);
    let two_pi = 6.283185307179586f32;
...
/// New-syntax correctness + bench for `mt_rope_llama` (per-token decode RoPE
...
pub mod kernel_tests {
    ...
    use super::{mt_rope_llama, mt_rope_llama_many};
    ...
    /// Batched per-row Llama RoPE (`mt_rope_llama_many`). Grid3D `[T, n_heads,
    /// half_dim]`, tpg `[1,1,1]`. Reuses the banded inv_freq above; each row
    /// uses its own position drawn from the `positions` vector.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-2, 5e-2])]
    fn test_rope_llama_many(dt: DType) -> TestSetup {
        ...
        TestSetup::new(mt_rope_llama_many::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("qk", pack_f32(&qk_f, dt), dt))
...
/// New-syntax benchmark for `mt_rope_llama` (decode RoPE, head_dim 128).
pub mod kernel_benches {
    ...
    /// Prefill batched RoPE (`mt_rope_llama_many`) — T=512 rows in one dispatch.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_rope_llama_many(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(mt_rope_llama_many::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
