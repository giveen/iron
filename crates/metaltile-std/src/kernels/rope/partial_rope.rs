...
//! nope dims.
//!
//! Two forms share this math:
//!   - `mt_partial_rope` — per-token decode (single `position` constexpr)
//!   - `mt_partial_rope_rows` — batched prefill (N tokens in one dispatch, grid
//!     z = token, position = `base_position + token`)
...
/// BATCHED DSv4 partial RoPE — the per-token `mt_partial_rope` applied to N
/// tokens in ONE dispatch (grid z = token). Prefill ropes token `t` at absolute
/// position `base_position + t`; the old prefill looped the single-token kernel
/// N times (3N tiny dispatches/layer for q + kv + inverse — the #1 warm-prefill
/// cost). Identical math to `mt_partial_rope` (adjacent GPT-J pairs, YaRN).
///
/// `qk`/`out` are `[n_tokens, n_heads, head_dim]`. Grid3D `[n_heads, half_rot,
/// n_tokens]`, one thread per `(head, rot-pair, token)`.
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn mt_partial_rope_rows<T>(
    qk: Tensor<T>,
    out: Tensor<T>,
    #[constexpr] head_dim: u32,
    #[constexpr] n_nope: u32,
...
pub mod kernel_benches {
    ...
    use super::{mt_partial_rope, mt_partial_rope_rows};
    ...
    /// Batched prefill partial RoPE (`mt_partial_rope_rows`) — 256 tokens in one
    /// dispatch (DSv4 production shape).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_rope_rows(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(mt_partial_rope_rows::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
