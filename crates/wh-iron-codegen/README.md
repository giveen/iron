...
Multi-backend GPU code generator for Iron Kernels kernels. Takes algorithm IR from
`wh-iron-core`, applies backend-independent optimization passes, then lowers
...
This crate is the middle of the Iron Kernels compiler stack: it receives `Kernel`
...
for both programmatic use and the `iron inspect` / `iron build` CLI flows.

## Position in the pipeline

```
wh-iron-core (IR) ──► wh-iron-codegen (this crate) ──► wh-iron-runtime
                                │
...
Generated MSL is handed to `wh-iron-runtime` for PSO compilation
and dispatch, or serialized to disk by `wh-iron-cli`.
...
```rust,ignore
use wh_iron_codegen::msl::{MslGenerator, MslConfig};
...
```rust,ignore
use wh_iron_codegen::passes::PipelineBuilder;
...
|---|---|
| `wh-iron-core` | Reads `Kernel`, `Op`, `DType`, `Shape` from IR |
...
No platform gating — `wh-iron-codegen` is a pure-Rust compiler that
...
- [`wh-iron-core` README](../wh-iron-core/README.md) — the IR types this crate lowers
- [`wh-iron-runtime` README](../wh-iron-runtime/README.md) — the runtime that consumes generated MSL
- [Crate docs on docs.rs](https://docs.rs/wh-iron-codegen)
