...
//! `.metal` files.  Emitting `wh-iron-std`'s full kernel set produced
//! >5000 such warnings.
//!
//! ## Algorithm
//!
...
use wh_iron_core::ir::{BlockId, Kernel};
...
pub fn eliminate_dead_values(kernel: &mut Kernel) -> Result<()> {
    let has_anchor = |b: &wh_iron_core::ir::Block| b.ops.iter().any(|op| op.has_side_effects());
...
fn dve_block(block: &mut wh_iron_core::ir::Block, used: &FxHashSet<u32>) -> bool {
    let n = block.ops.len();
    if n == 0 {
        return false;
    }
...
#[cfg(test)]
mod tests {
    use wh_iron_core::ir::{Block, BlockId, IndexExpr, Op, ValueId, VarId};
    ...
    fn removes_orphan_binop_left_by_vectorize() {
        ...
        k.body.push_op(
            Op::BinOp {
                op: wh_iron_core::ir::BinOpKind::Add,
                lhs: ValueId::new(0),
                rhs: ValueId::new(2),
            },
            ValueId::new(3),
    ...
    fn iterates_to_fixpoint_on_dead_chain() {
        ...
        k.body.push_op(
            Op::BinOp {
                op: wh_iron_core::ir::BinOpKind::Mul,
                lhs: ValueId::new(2),
                rhs: ValueId::new(0),
            },
            ValueId::new(3),
        ...
        k.body.push_op(
            Op::BinOp {
                op: wh_iron_core::ir::BinOpKind::Sub,
                lhs: ValueId::new(3),
                rhs: ValueId::new(1),
            },
            ValueId::new(4),
