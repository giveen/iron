...
use wh_iron_core::ir::{BinOpKind, Block, BlockId, Kernel, Op, UnaryOpKind, ValueId};
...
#[cfg(test)]
mod tests {
    use wh_iron_core::ir::{BinOpKind, VarId};
    ...
    fn fold_in_block(
        ...
        ops.push(wh_iron_core::ir::Op::Store {
            dst: "out".into(),
            indices: vec![wh_iron_core::ir::IndexExpr::Value(used_vid)],
            value: used_vid,
            mask: None,
        });
        results.push(None);
    ...
    fn dce_removes_unused_const() {
        let ops = vec![Op::Const { value: 42 }, Op::Const { value: 99 }, Op::UnaryOp {
            op: wh_iron_core::ir::UnaryOpKind::Neg,
