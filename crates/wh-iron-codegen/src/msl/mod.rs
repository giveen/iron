...
//!
//! Walks the Iron Kernels IR and emits valid MSL source text.
...
/// `simd_*(value)` / `__iron_simd_product(value)` call with no `lsize`
/// reference.
fn kernel_has_reduce_mean(kernel: &Kernel) -> bool {
    let check = |ops: &[Op]| {
        ops.iter().any(|op| matches!(op, Op::Reduce { op: wh_iron_core::ir::ReduceKind::Mean, .. }))
    };
    check(&kernel.body.ops) || kernel.blocks.values().any(|b| check(&b.ops))
}
...
pub(super) fn kernel_reduce_uses_n_simd(kernel: &Kernel, config: &MslConfig) -> bool {
    let tg_modes = matches!(
        kernel.mode,
        wh_iron_core::ir::KernelMode::Reduction | wh_iron_core::ir::KernelMode::Tile2D
    );
    if !tg_modes {
        return false;
    }
...
/// directly (e.g. cross-simdgroup reductions like `iron_sdpa_decode`).
///
...
/// `iron_hadamard_m*` matvecs dispatched at TPG = M < 32 — returns `false` and is
...
impl MslGenerator {
    ...
    fn emit_msl(&self, k: &Kernel) -> Result<String> {
        ...
        wl!(out, "// Generated by Iron Kernels");
...
///
/// **Why `iron build` callers should pass this:** the bench harness sets
/// `expected_tpg` from `ShapeSpec.tpg`. If `iron build --emit all`
/// dropped the TPG and produced different MSL than `iron bench` measured,
...
mod tests {
    ...
    #[test]
    fn unary_op_emit() {
        use wh_iron_core::ir::UnaryOpKind;
    ...
    fn activation_silu_emits_helper() {
        ...
        sink(&mut k, ValueId::new(1));
        let msl = MslGenerator::default().generate(&k).unwrap();
        assert!(msl.contains("iron_silu"), "silu helper function name");
        assert!(msl.contains("inline T iron_silu"), "silu helper definition");
    }
    ...
    fn fused_activation_emits_helper() {
        ...
        // `iron_silu` helper preamble is still emitted — otherwise the MSL
        ...
        let msl = MslGenerator::default().generate(&k).unwrap();
        assert!(
            msl.contains("inline T iron_silu"),
    ...
    fn fusion_cast_exp_binop_into_single_expression() {
        use wh_iron_core::ir::{BinOpKind, UnaryOpKind};
    ...
    /// cheap path (`iron_rms_norm_small` etc.).
    #[test]
    fn reduction_preamble_omits_lsize_for_simd_only_reduce_sum() {
        use wh_iron_core::ir::ReduceKind;
    ...
    /// matvecs like `iron_hadamard_m*` that dispatch safely at TPG < 32.
    #[test]
    fn kernel_uses_n_simd_flags_only_freeze_prone_kernels() {
        ...
        // (c) Reduction-mode but a plain per-thread loop (iron_hadamard_m* shape):
    ...
    fn atomic_threadgroup_emits_cast_form() {
        use wh_iron_core::ir::{AtomicKind, AtomicScope};
