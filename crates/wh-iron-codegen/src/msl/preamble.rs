...
impl super::MslGenerator {
    ...
    pub(super) fn emit_activation_helpers(&self, feat: &KernelFeatures, out: &mut String) {
        if feat.needs_silu {
            ...
            wl!(out, "inline T iron_silu(T x) {{ return x / (T(1) + exp(-x)); }}");
        }
        if feat.needs_gelu {
            wl!(out);
        ...
        if feat.needs_relu {
            wl!(out);
            wl!(out, "template<typename T>");
            wl!(out, "inline T iron_relu(T x) {{ return max(T(0), x); }}");
        }
        if feat.needs_sigmoid {
            wl!(out);
            wl!(out, "template<typename T>");
            wl!(out, "inline T iron_sigmoid(T x) {{ return T(1) / (T(1) + exp(-x)); }}");
        }
        if feat.needs_erf {
            wl!(out);
            // Polynomial approximation matching MLX erf.h (max error < 1 ulp)
            wl!(out, "inline float iron_erf_impl(float a) {{");
            ...
            wl!(out, "}}");
            wl!(out, "template<typename T>");
            wl!(out, "inline T iron_erf_impl(T x) {{ return T(iron_erf_impl(float(x))); }}");
        }
        if feat.needs_erfinv {
            wl!(out);
            // Inverse error function, ported from MLX erf.h (max error < 2.4 ulp)
            wl!(out, "inline float iron_erfinv_impl(float a) {{");
            ...
            wl!(out, "}}");
            wl!(out, "template<typename T>");
            wl!(out, "inline T iron_erfinv_impl(T x) {{ return T(iron_erfinv_impl(float(x))); }}");
        }
        if feat.needs_expm1 {
            wl!(out);
            ...
            wl!(out, "inline float iron_expm1_impl(float x) {{");
        ...
        if feat.needs_simd_product {
            ...
            wl!(out, "inline float __iron_simd_product(float v) {{");
