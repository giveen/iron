...
/// declaring `float iron_loc_rem` turned `rem / extent` into float
...
impl GlslGenerator {
    ...
    /// active (set via `IRON_VK_COOPMAT=1`, see `TargetProfile::vulkan`).
    ...
    fn emit_preamble(&self, out: &mut String) {
        ...
        writeln!(out, "// Generated by Iron Kernels (Vulkan/SPIR-V backend). DO NOT EDIT.").ok();
        ...
        // promotes bf16 through f32 (see `iron_bf16_to_f32` helpers).
        writeln!(out, "#extension GL_EXT_bfloat16 : enable").ok();
        // Real cooperative-matrix path (gated by MmaStrategy::VkCooperativeMatrix
        // via `IRON_VK_COOPMAT=1`). Emits coopMatLoad/coopMatMulAdd/coopMatStore
        ...
        writeln!(out, "float iron_bf16_to_f32(uint16_t b) {{").ok();
        writeln!(out, "    return uintBitsToFloat(uint(b) << 16);").ok();
        writeln!(out, "}}").ok();
        writeln!(out, "uint16_t iron_f32_to_bf16(float f) {{").ok();
        ...
        writeln!(out, "float iron_decode_e2m1(uint code) {{").ok();
        ...
        writeln!(out, "}}").ok();
        writeln!(out, "float iron_decode_e4m3(uint bits) {{").ok();
        ...
        writeln!(out, "}}").ok();
        writeln!(out, "float iron_decode_e5m2(uint bits) {{").ok();
        ...
        writeln!(out, "    return (((bits >> 7u) & 1u) != 0u) ? -mag : mag;").ok();
        writeln!(out, "}}").ok();
        writeln!(out, "float iron_decode_int8(uint bits) {{").ok();
        ...
        // them. iron_erfinv is a coarse Winitzki approximation; tune later.
        writeln!(out, "float iron_expm1(float x) {{ return exp(x) - 1.0; }}").ok();
        writeln!(out, "float iron_log10(float x) {{ return log(x) / log(10.0); }}").ok();
        writeln!(out, "float iron_erf(float x) {{").ok();
        ...
        writeln!(out, "}}").ok();
        writeln!(out, "float iron_erfinv(float x) {{").ok();
        ...
        writeln!(out, "float iron_fdiv(float a, float b) {{").ok();
        ...
        writeln!(out, "float iron_rsqrt(float x) {{").ok();
        ...
        // `iron_subgroup_add` linearly broadcasts and accumulates so the
        // result matches Rust's `iter().sum()` bit-exactly.
        let lw = self.profile.lane_width;
        writeln!(out, "float iron_subgroup_add(float v) {{").ok();
        ...
        writeln!(out, "}}").ok();
        writeln!(out, "float iron_atan2(float y, float x) {{").ok();
        ...
        writeln!(out, "}}").ok();
        writeln!(out, "float iron_silu(float x) {{ return x / (1.0 + exp(-x)); }}").ok();
        writeln!(out, "float iron_relu(float x) {{ return max(0.0, x); }}").ok();
        writeln!(out, "float iron_sigmoid(float x) {{ return 1.0 / (1.0 + exp(-x)); }}").ok();
        writeln!(out, "float iron_gelu(float x) {{").ok();
        ...
        for blk in std::iter::once(&kernel.body).chain(kernel.blocks.values()) {
            for op in &blk.ops {
                if let Op::CoopTileSetup { name, m, n, k, exec_scope, .. } = op {
                    use wh_iron_core::ir::CoopTileScope;
            ...
            for blk in std::iter::once(&kernel.body).chain(kernel.blocks.values()) {
                for op in &blk.ops {
                    if let Op::CoopTileSetup { name, m, n, exec_scope, .. } = op
                        && matches!(exec_scope, wh_iron_core::ir::CoopTileScope::SimdGroup)
                    {
                        let cnm = ct_ident(&coop_c_name(name));
                        if seen_c.insert(cnm.clone()) {
        ...
        match op {
            ...
            Op::Load { src, indices, .. } => {
                ...
                if indices.is_empty() {
                    ...
                    if let Some(b) = match src.as_str() {
                        ...
                        writeln!(out, "{pad}{lt} {v} = iron_loc_{local_name};").ok();
                        if let Some(id) = vid {
                            types.insert(id, lt);
                        }
                    } else {
                    ...
                    match dtype {
                        Some(DType::BF16) => {
                            writeln!(out, "{pad}float {v} = iron_bf16_to_f32({arr}[uint({idx})]);")
                                .ok();
                            if let Some(id) = vid {
                                types.insert(id, "float");
                            }
            ...
            Op::Store { dst, indices, value, .. } => {
                ...
                let store_expr = match p.map(|p| p.dtype) {
                    Some(DType::BF16) => format!("iron_f32_to_bf16({val})"),
                    Some(dt) => {
            ...
            Op::BinOp { op: bop, lhs, rhs } => {
                ...
                // `test_iron_logits_repetition_penalty` misses bit-exact tol=0
                // by 1 ULP on 2 elements out of 8192.
                if matches!(bop, BinOpKind::Div) && ty == "float" {
                    writeln!(out, "{pad}{ty} {v} = iron_fdiv({l}, {r});").ok();
            ...
            Op::SetLocal { name, value } => {
                ...
                if rt == lt {
                    writeln!(out, "{pad}iron_loc_{name} = {rv};").ok();
                } else {
                    writeln!(out, "{pad}iron_loc_{name} = {lt}({rv});").ok();
                }
            },
            Op::Activation { kind, value } => {
                ...
                let rv = self.vname(Some(*value), block, ov);
                let expr = match kind {
                    ActKind::Silu => format!("iron_silu({rv})"),
                    ActKind::Gelu => format!("iron_gelu({rv})"),
                    ActKind::Relu => format!("iron_relu({rv})"),
                    ActKind::Sigmoid => format!("iron_sigmoid({rv})"),
            ...
            Op::StrideReduce {
                ...
                // iron_bf16_to_f32 (the systematic `all_reduce_*[bf16]`
                ...
                let load_src = |ix: &str| -> String {
                    if matches!(src_dtype, Some(DType::BF16)) {
                        format!("iron_bf16_to_f32({src_arr}[{ix}])")
                    } else {
                ...
                let base_elem = match secondary_src {
                    Some(sec) => {
                        ...
                        let load_sec = if matches!(sec_dtype, Some(DType::BF16)) {
                            format!("iron_bf16_to_f32({sec_arr}[uint(_i) - uint({bv})])")
                        } else {
            ...
            Op::SimdReduce { value, op: rk } => {
                let v = self.vname(vid, block, ov);
                let rv = self.vname(Some(*value), block, ov);
                // Sum/Mean go through `iron_subgroup_add` (linear-order
                ...
                // intrinsic.
                let expr = match rk {
                    ReduceKind::Sum | ReduceKind::Mean => format!("iron_subgroup_add({rv})"),
            ...
            Op::Atomic { op: ak, dst, index, value, .. } => {
                ...
                let arr = safe_glsl_ident(dst);
                let f = match ak {
                    wh_iron_core::ir::AtomicKind::Add => "atomicAdd",
                    wh_iron_core::ir::AtomicKind::Max => "atomicMax",
                    wh_iron_core::ir::AtomicKind::Min => "atomicMin",
                    wh_iron_core::ir::AtomicKind::And => "atomicAnd",
                    wh_iron_core::ir::AtomicKind::Or => "atomicOr",
                    wh_iron_core::ir::AtomicKind::Xor => "atomicXor",
                };
    ...
    fn coop_cfg(
        ...
        use wh_iron_core::ir::{CoopTileAccMode, CoopTileScope};
        for blk in std::iter::once(&kernel.body).chain(kernel.blocks.values()) {
            for op in &blk.ops {
    ...
    fn glsl_unary(&self, op: UnaryOpKind, arg: &str) -> String {
        ...
        match op {
            ...
            Recip => format!("iron_fdiv(1.0, {arg})"),
            // Rsqrt → iron_rsqrt with one Newton-Raphson refinement step
            // on top of `inversesqrt`. Improves AMD's 1.4-ULP hardware
            // estimate to ≤1 ULP, which the gated-delta recurrence needs.
            Rsqrt => format!("iron_rsqrt({arg})"),
...
fn glsl_binop(op: BinOpKind, l: &str, r: &str) -> String {
    ...
    match op {
        ...
        Min => format!("min({l}, {r})"),
        Pow => format!("pow({l}, {r})"),
        // Use the explicit `iron_atan2` helper from the preamble — AMD's
        // GLSL 2-arg `atan(y, x)` returns the wrong quadrant for some
        // inputs (off by π).
        ATan2 => format!("iron_atan2(float({l}), float({r}))"),
...
/// Map a wh-iron param / DSL identifier to a GLSL-safe one.
///
/// Two classes of fix:
/// - Common wh-iron param names (`out`, `in`, `inout`, `length`,
...
///   implementation). The wh-iron macro emits mutable-local reads as
///   `Op::Load { src: "__ml_<name>" }`; rewrite the prefix to `iron_loc_`
...
pub fn safe_glsl_ident(name: &str) -> String {
    ...
    if let Some(rest) = name.strip_prefix("__ml_") {
        return format!("iron_loc_{rest}");
    }
    // The set of GLSL reserved words a wh-iron kernel param is at all
    ...
    const RESERVED: &[&str] = &[
        ...
        // conv* / depthwise_conv2d / aura_value_int4 / iron_gemm kernels.
        "input",
        "output",
        "texture",
        "image",
