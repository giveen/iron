...
//! Quick perf-only timing for `ffai_softmax_categorical_sample` at vocab=152K.
//!
...
use ffai_kernels_std::kernels::sampling::categorical_sample::ffai_softmax_categorical_sample;
...
fn perf_softmax_categorical_sample_vocab_152k() {
    ...
    let mut kernel = ffai_softmax_categorical_sample::kernel_ir_for(DType::F32);
    ...
    println!(
        "ffai_softmax_categorical_sample vocab={n}: median={:.1}µs min={:.1}µs",
