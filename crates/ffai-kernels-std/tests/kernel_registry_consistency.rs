...
//! Structural consistency tests for the ffai-kernels-std kernel registry.
//!
...
use ffai_kernels::codegen::{MslGenerator, msl::MslConfig};
// Import the registries via `ffai_kernels_std` (not `ffai_kernels_core`) so the std
...
fn kernel_annotations_have_matching_inventory_submit() {
    ...
    // Find the ffai-kernels-std source tree relative to this test file. The
    // file lives at `crates/ffai-kernels-std/tests/kernel_registry_consistency.rs`;
    // `CARGO_MANIFEST_DIR` points at `crates/ffai-kernels-std`.
