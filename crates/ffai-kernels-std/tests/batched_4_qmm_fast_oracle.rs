...
//! Tight per-branch CPU oracle for `ffai_batched_4_qmm_fast` (the M>1
//! sibling of `ffai_batched_4_qgemv_fast`) at production-realistic
//! UNEQUAL output dims.
//!
...
//! buffer, see `run_kernel_test` in `ffai-kernels/src/runner/harness.rs`).
...
//!
//! Run:
//!   cargo test --release -p ffai-kernels-std --test batched_4_qmm_fast_oracle -- --nocapture
...
use ffai_kernels_std::kernels::gemm::batched_4_qmm::ffai_batched_4_qmm_fast;
...
fn run_unequal_dims_case(dt: Dt, tol: f32) {
    ...
    let mut kernel = ffai_batched_4_qmm_fast::kernel_ir_for(dtype);
    kernel.mode = KernelMode::Reduction;
    let result = ctx
        .dispatch_with_grid(&kernel, &buffers, &BTreeMap::new(), [n_tgs, m, 4], [64, 1, 1])
        .expect("dispatch ffai_batched_4_qmm_fast");
