...
//! (F-85 small-M follow-up): `ffai_moe_tile_plan_expert_counts` (phase 1,
//! shared) chained into `ffai_moe_build_tile_plan_parallel` /
...
use ffai_kernels_std::kernels::moe::{
    ...
    moe_tile_plan_builder_bm32_own_parallel::ffai_moe_build_tile_plan_bm32_own_parallel,
    moe_tile_plan_builder_bm32_parallel::ffai_moe_build_tile_plan_bm32_parallel,
    moe_tile_plan_builder_parallel::ffai_moe_build_tile_plan_parallel,
    moe_tile_plan_expert_counts::ffai_moe_tile_plan_expert_counts,
};
...
fn run_phase1(ctx: &Context, sorted_experts: &[u32], n_experts: usize) -> (Vec<u32>, Vec<u32>) {
    ...
    let mut k = ffai_moe_tile_plan_expert_counts::kernel_ir();
...
fn run_original(
    ...
    let mut k = match variant {
        Variant::Bm16 => ffai_moe_build_tile_plan::kernel_ir(),
        Variant::Bm32 => ffai_moe_build_tile_plan_bm32::kernel_ir(),
        Variant::Bm32Own => ffai_moe_build_tile_plan_bm32_own::kernel_ir(),
    };
    k.mode = KernelMode::Reduction;
    let r = ctx
...
fn run_parallel(
    ...
    let mut k = match variant {
        Variant::Bm16 => ffai_moe_build_tile_plan_parallel::kernel_ir(),
        Variant::Bm32 => ffai_moe_build_tile_plan_bm32_parallel::kernel_ir(),
        Variant::Bm32Own => ffai_moe_build_tile_plan_bm32_own_parallel::kernel_ir(),
    };
    k.mode = KernelMode::Reduction;
    let r = ctx
        .dispatch_with_grid(&k, &buffers, &BTreeMap::new(), [1, 1, 1], [n_experts, 1, 1])
