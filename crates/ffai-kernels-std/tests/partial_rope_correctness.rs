...
//! Batched partial RoPE (ffai_partial_rope) must equal the per-token
...
use ffai_kernels_std::kernels::rope::partial_rope::ffai_partial_rope;
...
fn partial_rope_matches_per_token_oracle() {
    ...
    let mut k = ffai_partial_rope::kernel_ir_for(Dt::F32.to_dtype());
