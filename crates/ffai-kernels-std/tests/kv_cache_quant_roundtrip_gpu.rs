...
//! GPU correctness for `ffai_quantize_kv_int4/int8` + `ffai_bulk_dequant_kv_int4/int8`
...
//! `ffai_quantize_kv_*` and `ffai_bulk_dequant_kv_*` are emitted from `macro_rules!`
...
//!   2. Dispatch `ffai_quantize_kv_*` → cache buffers at `position`
//!   3. Dispatch `ffai_bulk_dequant_kv_*` reading the whole [0..n_positions)
...
fn roundtrip_int4(shape: &Shape, dt: Dt, source: &[f32]) -> Vec<f32> {
    ...
    let mut qkernel = ffai_quantize_kv_int4::kernel_ir_for(dtype);
    ...
    let q_out = ctx
        ...
        .expect("ffai_quantize_kv_int4 dispatch");
    ...
    let mut dkernel = ffai_bulk_dequant_kv_int4::kernel_ir_for(dtype);
    ...
    let d_out = ctx
        ...
        .expect("ffai_bulk_dequant_kv_int4 dispatch");
...
fn roundtrip_int8(shape: &Shape, dt: Dt, source: &[f32]) -> Vec<f32> {
    ...
    let mut qkernel = ffai_quantize_kv_int8::kernel_ir_for(dtype);
    qkernel.mode = KernelMode::Grid3D;
    let (grid, tpg) = quantize_dispatch_grid(shape, bits);
    let q_out = ctx
        .dispatch_with_grid(&qkernel, &buffers, &BTreeMap::new(), grid, tpg)
        .expect("ffai_quantize_kv_int8 dispatch");
    ...
    let mut dkernel = ffai_bulk_dequant_kv_int8::kernel_ir_for(dtype);
    dkernel.mode = KernelMode::Grid3D;
    let (dgrid, dtpg) = dequant_dispatch_grid(shape);
    let d_out = ctx
        .dispatch_with_grid(&dkernel, &dbuf, &BTreeMap::new(), dgrid, dtpg)
        .expect("ffai_bulk_dequant_kv_int8 dispatch");
...
//
// `ffai_quantize_kv_*` writes only to its `position` slot — verify by
