...
/// New-syntax correctness for `ffai_rope` (Grid3D, single threadgroup with
...
pub mod kernel_tests {
    ...
    fn test_ffai_rope(dt: DType) -> TestSetup {
        ...
        let grid_x = head_dim / 2;
        TestSetup::new(ffai_rope::kernel_ir_for(dt))
...
/// New-syntax benchmark for `ffai_rope` (vs MLX `metal/rope.metal`). Multi-TG:
...
pub mod kernel_benches {
    ...
    //   `traditional  [[function_constant(2)]]` → false (rotate-half, like FFAI)
    ...
    //   in[[0]]            = `inp`  (shared by name with the FFAI input)
    ...
    // `pos.y·strides[1] + (pz·4+i)·strides[0] + pos.x` == FFAI's
    // `py·seq_stride + head·h_stride + px` exactly.
    //
    // Grid: MLX `dispatch_threads(grid_dims=(D/2, T, B·ceil(H/4)), group_dims)`,
    // so `threads_per_grid = (grid_x, seq_len, n_heads/4)`. The FFAI runner
    ...
    // so FFAI and the reference rotate identical data; tol floor 0.01 is the
    // legacy rope tolerance (FFAI folds in f32, MLX uses `fast::cos/sin`).
    ...
    fn bench_rope(dt: DType) -> BenchSetup {
        ...
        let strides: Vec<u8> = [(seq_len * head_dim) as i64, head_dim as i64, 1i64]
            ...
            .collect();
        BenchSetup::new(ffai_rope::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            ...
            .with_reference(
                ...
                // in[[0]] shared by name with the FFAI `inp` (placeholder bytes
                // overwritten by the runner with FFAI's exact input).
