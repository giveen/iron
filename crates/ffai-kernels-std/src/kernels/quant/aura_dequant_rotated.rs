...
///
/// Produces kernels: `ffai_aura_dequant_rotated_int2`, `_int3`, `_int4`,
/// `_int6`, `_int8`.
///
...
#[kernel(variants(BITS = [2, 3, 4, 6, 8], suffix = "int{BITS}"))]
pub fn ffai_aura_dequant_rotated<T>(
    packed: Tensor<u32>,
    norms: Tensor<T>,
    codebook: Tensor<T>,
    mut out: Tensor<T>,
...
/// Correctness tests for the `ffai_aura_dequant_rotated_int{2,3,4,6,8}` family.
...
pub mod kernel_tests {
    ...
    fn test_aura_dequant_rotated(dt: DType) -> TestSetup {
        dequant_setup(ffai_aura_dequant_rotated_intBITS::kernel_ir_for(dt), BITS, 128, 2, 3, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_aura_dequant_rotated(dt: DType) -> BenchSetup {
        setup(
            BenchSetup::new(ffai_aura_dequant_rotated_intBITS::kernel_ir_for(dt)),
            128,
            BITS,
            8,
            64,
