...
//! ## Macro structure
//!
//! `ffai_aura_encode_kernel!` wraps a single `#[kernel] pub fn …`
...
///
/// Produces kernels: `ffai_aura_encode_int2`, `ffai_aura_encode_int3`, `ffai_aura_encode_int4`,
/// `ffai_aura_encode_int6`, `ffai_aura_encode_int8`.
///
...
pub fn ffai_aura_encode<T>(
    ...
    // indices.  Caught by the ffai_aura_encode GPU correctness
    // test.  See FFAI post-mortem 2026-05-19 — a ffai-kernels
...
/// `tests/ffai_aura_encode_gpu_correctness.rs` (since removed) A/B test. Here we
...
pub mod kernel_tests {
    ...
    use super::ffai_aura_encode_int4;
    ...
    fn setup(dim: usize, rows: usize, dt: DType, rotation: &[f32], input: &[f32]) -> TestSetup {
        ...
        TestSetup::new(ffai_aura_encode_int4::kernel_ir_for(dt))
    ...
    // `ffai_aura_encode_int4_srht_rotation_f32` A/B test).
...
pub mod kernel_benches {
    ...
    fn bench_aura_encode(dt: DType) -> BenchSetup {
        setup(BenchSetup::new(ffai_aura_encode_intBITS::kernel_ir_for(dt)), 128, BITS, 256, dt)
    }
}
