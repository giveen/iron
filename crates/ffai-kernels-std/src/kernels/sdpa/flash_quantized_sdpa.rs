...
//! affine-quantized K/V cache. Port of `ffai_flash_quantized_sdpa.h`
//! (spec 041 phase 1.1/1.2). The affine-quant counterpart of
//! `ffai_aura_flash_sdpa`: K and V are dequantized inline per thread from
...
//! `ffai_aura_flash_sdpa` (token-parallelism is a perf follow-up).
//!
//! ## Mask variants
//!
...
//!
//! - **Bool mask** (`ffai_flash_quantized_sdpa_bool_mask_b{4,8}_d{64,128,256}`):
...
//!
//! - **Float mask** (`ffai_flash_quantized_sdpa_float_mask_b{4,8}_d{64,128,256}`):
...
//! `mask_t` convention used in `ffai_aura_flash_sdpa`.
//!
//! ## DISPATCH INVARIANTS
//!
...
///
/// Produces 10 kernels: `ffai_flash_quantized_sdpa_b4_d64`, `_b4_d96`, `_b4_d128`,
...
pub fn ffai_flash_quantized_sdpa<T>(
    queries: Tensor<T>,
    k_packed: Tensor<u32>,
    k_scales: Tensor<T>,
    k_biases: Tensor<T>,
...
///
/// Produces 6 kernels: `ffai_flash_quantized_sdpa_bool_mask_b4_d64`,
...
pub fn ffai_flash_quantized_sdpa_bool_mask<T>(
    queries: Tensor<T>,
    k_packed: Tensor<u32>,
    k_scales: Tensor<T>,
    k_biases: Tensor<T>,
...
///
/// Produces 6 kernels: `ffai_flash_quantized_sdpa_float_mask_b4_d64`,
/// `_b4_d128`, `_b4_d256`, `_b8_d64`, `_b8_d128`, `_b8_d256`.
#[kernel(variants(
    BITS = [4, 4, 4, 8, 8, 8],
    DIM  = [64, 128, 256, 64, 128, 256],
...
pub fn ffai_flash_quantized_sdpa_float_mask<T>(
    queries: Tensor<T>,
    k_packed: Tensor<u32>,
    k_scales: Tensor<T>,
    k_biases: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::{
        ffai_flash_quantized_sdpa_b4_d96,
        ffai_flash_quantized_sdpa_b4_d128,
        ffai_flash_quantized_sdpa_b4_d512,
        ffai_flash_quantized_sdpa_b8_d128,
        ffai_flash_quantized_sdpa_bool_mask_b4_d128,
        ffai_flash_quantized_sdpa_bool_mask_b8_d128,
        ffai_flash_quantized_sdpa_float_mask_b4_d128,
        ffai_flash_quantized_sdpa_float_mask_b8_d128,
    };
    ...
    fn test_ffai_flash_quantized_sdpa_b4_d128(dt: DType) -> TestSetup {
        base_setup(ffai_flash_quantized_sdpa_b4_d128::kernel_ir_for(dt), 128, 4, 64, false, 0, dt)
    }
    // Attention sinks.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_ffai_flash_quantized_sdpa_b4_d128_sinks(dt: DType) -> TestSetup {
        base_setup(ffai_flash_quantized_sdpa_b4_d128::kernel_ir_for(dt), 128, 4, 64, true, 0, dt)
    }
    // Sliding window (4 of 8 tokens).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_ffai_flash_quantized_sdpa_b4_d128_window(dt: DType) -> TestSetup {
        base_setup(ffai_flash_quantized_sdpa_b4_d128::kernel_ir_for(dt), 128, 4, 64, false, 4, dt)
    }
    // 8-bit quant (pack_factor 4).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_ffai_flash_quantized_sdpa_b8_d128(dt: DType) -> TestSetup {
        base_setup(ffai_flash_quantized_sdpa_b8_d128::kernel_ir_for(dt), 128, 8, 64, false, 0, dt)
    }
    // head_dim 96 (dims_per_lane 3) and 512 (dims_per_lane 16).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_ffai_flash_quantized_sdpa_b4_d96(dt: DType) -> TestSetup {
        base_setup(ffai_flash_quantized_sdpa_b4_d96::kernel_ir_for(dt), 96, 4, 32, false, 0, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_ffai_flash_quantized_sdpa_b4_d512(dt: DType) -> TestSetup {
        base_setup(ffai_flash_quantized_sdpa_b4_d512::kernel_ir_for(dt), 512, 4, 64, false, 0, dt)
    }
    ...
    fn test_ffai_flash_quantized_sdpa_bool_mask_b4_d128(dt: DType) -> TestSetup {
        mask_setup(
            ffai_flash_quantized_sdpa_bool_mask_b4_d128::kernel_ir_for(dt),
            128,
            4,
            64,
            false,
    ...
    fn test_ffai_flash_quantized_sdpa_bool_mask_b8_d128(dt: DType) -> TestSetup {
        mask_setup(
            ffai_flash_quantized_sdpa_bool_mask_b8_d128::kernel_ir_for(dt),
            128,
            8,
            64,
            false,
    ...
    fn test_ffai_flash_quantized_sdpa_float_mask_b4_d128(dt: DType) -> TestSetup {
        mask_setup(
            ffai_flash_quantized_sdpa_float_mask_b4_d128::kernel_ir_for(dt),
            128,
            4,
            64,
            true,
    ...
    fn test_ffai_flash_quantized_sdpa_float_mask_b8_d128(dt: DType) -> TestSetup {
        mask_setup(
            ffai_flash_quantized_sdpa_float_mask_b8_d128::kernel_ir_for(dt),
            128,
            8,
            64,
            true,
...
pub mod kernel_benches {
    ...
    fn base(ir: ffai_kernels::core::ir::Kernel, dim: usize, bits: usize, dt: DType) -> BenchSetup {
    ...
    fn bench_base(dt: DType) -> BenchSetup {
        base(ffai_flash_quantized_sdpa_bBITS_dDIM::kernel_ir_for(dt), DIM, BITS, dt)
    }
    ...
    fn bench_bool_mask(dt: DType) -> BenchSetup {
        masked(
            ffai_flash_quantized_sdpa_bool_mask_bBITS_dDIM::kernel_ir_for(dt),
            DIM,
            BITS,
            "mask_bool",
            DType::U32,
    ...
    fn bench_float_mask(dt: DType) -> BenchSetup {
        masked(
            ffai_flash_quantized_sdpa_float_mask_bBITS_dDIM::kernel_ir_for(dt),
            DIM,
            BITS,
            "mask_float",
            dt,
