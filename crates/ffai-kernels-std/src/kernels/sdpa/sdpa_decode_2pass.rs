...
//!   (MLX uses tid.z; ffai-kernels's DSL doesn't expose tgid_z, so we
...
pub mod kernel_benches {
    ...
    fn pass1(ir: ffai_kernels::core::ir::Kernel, head_dim: usize, dt: DType) -> BenchSetup {
        let gqa_factor = N_Q_HEADS / N_KV_HEADS;
        let kv_stride = N_KV;
        let scale = 1.0f32 / (head_dim as f32).sqrt();
    ...
    fn pass2(ir: ffai_kernels::core::ir::Kernel, head_dim: usize, dt: DType) -> BenchSetup {
        let partial_len = N_Q_HEADS * BLOCKS * head_dim;
        let ml_len = N_Q_HEADS * BLOCKS;
