...
//!
//! `ffai_sdpa_prefill_mma_d256`'s own doc comment recorded this as a
//! deferred follow-up: BK=16 (matching the d128 MMA sibling,
//! `ffai_sdpa_prefill_mma`) halves the number of K-block loop iterations
...
//! ## Geometry
//!
//! Same outer contract as `ffai_sdpa_prefill_mma_d256`: TPG = 128 (4
...
//! `ffai_sdpa_prefill_mma` (the d128 MMA sibling) established: separate
...
#[kernel]
pub fn ffai_sdpa_prefill_mma_d256_bk16<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
    ...
    // ffai_sdpa_prefill_mma / ffai_sdpa_prefill_mma_d256) ──
    let qid = lane / 4u32;
...
/// Bench at the same append shape as `ffai_sdpa_prefill_mma_d256`'s own
...
pub mod kernel_benches {
    ...
    use super::ffai_sdpa_prefill_mma_d256_bk16;
    ...
    fn bench_sdpa_prefill_mma_d256_bk16(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(ffai_sdpa_prefill_mma_d256_bk16::kernel_ir_for(dt))
...
/// `ffaik test`-harness CPU-oracle coverage, following
/// `ffai_sdpa_prefill_mma_d256`'s own `kernel_tests` convention: realistic
...
pub mod kernel_tests {
    ...
    fn mma_d256_bk16_setup(
        ...
        TestSetup::new(ffai_sdpa_prefill_mma_d256_bk16::kernel_ir_for(dt))
            .mode(KernelMode::SimdGroup2D)
    ...
    fn test_ffai_sdpa_prefill_mma_d256_bk16_causal(dt: DType) -> TestSetup {
        mma_d256_bk16_setup(dt, true, 32, 8, 300, 37)
    }

    // Full (bidirectional): every row attends `[0, base_kv + n_query)`.
    #[test_kernel(dtypes = [f16, bf16], tol = [1e-2, 5e-2])]
    fn test_ffai_sdpa_prefill_mma_d256_bk16_full(dt: DType) -> TestSetup {
    ...
    #[test_kernel(dtypes = [f16, bf16], tol = [1e-2, 5e-2])]
    fn test_ffai_sdpa_prefill_mma_d256_bk16_odd_base_kv(dt: DType) -> TestSetup {
