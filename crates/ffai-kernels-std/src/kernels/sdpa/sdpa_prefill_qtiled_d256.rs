...
//!
//! The current d=256 prefill path (`ffai_sdpa_multi_d256`, wrapped by
...
//! structure (the d=128 `ffai_sdpa_prefill` scalar-flash kernel does the
...
#[kernel]
pub fn ffai_sdpa_prefill_qtiled_d256<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
/// 32/8. Emits every dtype FFAI dispatches so `ffaik build --emit` picks
/// the kernel up for all three.
pub mod kernel_benches {
    use ffai_kernels::{bench, test::*};

    use super::ffai_sdpa_prefill_qtiled_d256;
    ...
    fn bench_sdpa_prefill_qtiled_d256(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(ffai_sdpa_prefill_qtiled_d256::kernel_ir_for(dt))
...
/// `ffaik test`-harness CPU-oracle coverage (the `#[test_kernel]`
...
/// path is exercised by the same harness `ffaik test` runs in CI.
pub mod kernel_tests {
    ...
    fn qtiled_d256_setup(dt: DType, causal: bool) -> TestSetup {
        ...
        TestSetup::new(ffai_sdpa_prefill_qtiled_d256::kernel_ir_for(dt))
            .mode(KernelMode::SimdGroup2D)
    ...
    fn test_ffai_sdpa_prefill_qtiled_d256_causal(dt: DType) -> TestSetup {
        qtiled_d256_setup(dt, true)
    }

    // Full (bidirectional): every row attends `[0, base_kv + n_query)`.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 3e-3, 1.5e-2])]
    fn test_ffai_sdpa_prefill_qtiled_d256_full(dt: DType) -> TestSetup {
