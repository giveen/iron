...
pub mod kernel_tests {
    ...
    fn test_mxfp4_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxfp4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxfp4, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_nvfp4_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_nvfp4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Nvfp4, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_fp4_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_fp4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Fp4, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxfp8_e4m3_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxfp8_e4m3_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxfp8E4, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxfp8_e5m2_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxfp8_e5m2_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxfp8E5, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_fp8_e5m2_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_fp8_e5m2_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Fp8E5m2, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_nvfp8_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_nvfp8_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Nvfp8, 32, 64, 512, dt)
    }
    // fp8_e4m3 reuses the nvfp8 kernel (8-bit E4M3 + f32 scale, block 32).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_fp8_e4m3_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_nvfp8_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Fp8E4m3, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_int8_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int8_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int8, 32, 64, 512, dt)
    }
    ...
    fn test_int2_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int2_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int2, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_int3_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int3_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int3, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_int4_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int4, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_int5_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int5_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int5, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_int6_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int6_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int6, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxint2_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxint2_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint2, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxint3_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxint3_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint3, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxint4_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxint4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint4, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxint5_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxint5_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint5, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxint6_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxint6_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint6, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 5e-2, 2e-1])]
    fn test_mxint8_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_mxint8_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint8, 32, 64, 512, dt)
    }
    ...
    fn test_nvfp8_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_nvfp8_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Nvfp8F16, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(
            ffai_nvfp8_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Fp8E4m3F16,
            32,
            64,
            512,
    ...
    fn test_fp4_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_fp4_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Fp4F16, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(
            ffai_fp8_e5m2_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Fp8E5m2F16,
            32,
            64,
            512,
    ...
    fn test_int2_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int2_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int2F16, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int3_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int3F16, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int4_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int4F16, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int5_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int5F16, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int6_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int6F16, 32, 64, 512, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int8_f16_qmm_mma_mpp(dt: DType) -> TestSetup {
        mpp_setup(ffai_int8_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int8F16, 32, 64, 512, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_mxfp4(dt: DType) -> BenchSetup {
        mpp_bench(ffai_mxfp4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxfp4, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp4(dt: DType) -> BenchSetup {
        mpp_bench(ffai_nvfp4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Nvfp4, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp4(dt: DType) -> BenchSetup {
        mpp_bench(ffai_fp4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Fp4, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e4m3(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_mxfp8_e4m3_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Mxfp8E4,
            128,
            4096,
            4096,
    ...
    fn bench_mxfp8_e5m2(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_mxfp8_e5m2_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Mxfp8E5,
            128,
            4096,
            4096,
    ...
    fn bench_fp8_e5m2(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_fp8_e5m2_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Fp8E5m2,
            128,
            4096,
            4096,
    ...
    fn bench_nvfp8(dt: DType) -> BenchSetup {
        mpp_bench(ffai_nvfp8_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Nvfp8, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8(dt: DType) -> BenchSetup {
        mpp_bench(ffai_int8_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int8, 128, 4096, 4096, dt)
    }
    // Symmetric sub-byte ints (FP32 group scale) + MXINT (E8M0 block scale) +
    // MXINT8 (8-bit, E8M0). k=4096 is a multiple of 32 → word-aligned per width.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2(dt: DType) -> BenchSetup {
        mpp_bench(ffai_int2_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int2, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3(dt: DType) -> BenchSetup {
        mpp_bench(ffai_int3_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int3, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4(dt: DType) -> BenchSetup {
        mpp_bench(ffai_int4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int4, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5(dt: DType) -> BenchSetup {
        mpp_bench(ffai_int5_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int5, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6(dt: DType) -> BenchSetup {
        mpp_bench(ffai_int6_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Int6, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint2(dt: DType) -> BenchSetup {
        mpp_bench(ffai_mxint2_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint2, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint3(dt: DType) -> BenchSetup {
        mpp_bench(ffai_mxint3_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint3, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint4(dt: DType) -> BenchSetup {
        mpp_bench(ffai_mxint4_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint4, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint5(dt: DType) -> BenchSetup {
        mpp_bench(ffai_mxint5_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint5, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint6(dt: DType) -> BenchSetup {
        mpp_bench(ffai_mxint6_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint6, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint8(dt: DType) -> BenchSetup {
        mpp_bench(ffai_mxint8_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Mxint8, 128, 4096, 4096, dt)
    }
    // FP16-scale twins — same element packing as their FP32 twins, scale → f16.
    // fp8_e4m3_f16 reuses the nvfp8_f16 kernel (8-bit E4M3 + f16 scale).
    #[bench(dtypes = [f32, f16, bf16])]
    ...
    fn bench_fp4_f16(dt: DType) -> BenchSetup {
        mpp_bench(ffai_fp4_f16_qmm_mma_mpp::kernel_ir_for(dt), QFormat::Fp4F16, 128, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    ...
    fn bench_int2_f16(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_int2_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Int2F16,
            128,
            4096,
            4096,
    ...
    fn bench_int3_f16(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_int3_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Int3F16,
            128,
            4096,
            4096,
    ...
    fn bench_int4_f16(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_int4_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Int4F16,
            128,
            4096,
            4096,
    ...
    fn bench_int5_f16(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_int5_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Int5F16,
            128,
            4096,
            4096,
    ...
    fn bench_int6_f16(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_int6_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Int6F16,
            128,
            4096,
            4096,
    ...
    fn bench_int8_f16(dt: DType) -> BenchSetup {
        mpp_bench(
            ffai_int8_f16_qmm_mma_mpp::kernel_ir_for(dt),
            QFormat::Int8F16,
            128,
            4096,
            4096,
