...
pub mod kernel_tests {
    ...
    fn test_vocoder_istft(dt: DType) -> TestSetup {
        ...
        TestSetup::new(ffai_vocoder_istft::kernel_ir_for(dt))
...
/// New-syntax benchmark for `ffai_vocoder_istft` — a Kokoro-class iSTFTNet tail
...
pub mod kernel_benches {
    ...
    fn bench_vocoder_istft(dt: DType) -> BenchSetup {
        ...
        let n_freq = n_fft / 2 + 1;
        let out_len = (n_frames - 1) * hop + n_fft;
        BenchSetup::new(ffai_vocoder_istft::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
