...
//! follow-up). `ffai_moe_build_tile_plan_bm32_parallel`.
//!
//! BM=32 sibling of `ffai_moe_build_tile_plan_parallel`
//! (`moe_tile_plan_builder_parallel.rs`) - same split, same phase 1
//! (`ffai_moe_tile_plan_expert_counts`), only the tile-height constant
//! differs (16 -> 32), matching `ffai_moe_build_tile_plan_bm32`'s
//! relationship to `ffai_moe_build_tile_plan`. This is the coop-core
//! gather GEMM's default production tile-plan builder
//! (`ffai_moe_gather_qmm_coop`, wired via `useCoopTilePlan` in
//! `MoELayer.swift`) - the hot path this whole split exists to speed up.
//!
//! Output format is IDENTICAL to `ffai_moe_build_tile_plan_bm32`,
...
//! (`ffai_moe_gather_qmm_coop`'s indirect dispatch) read these exactly
//! as before.
//!
//! ## Dispatch invariants
//!
...
#[kernel]
pub fn ffai_moe_build_tile_plan_bm32_parallel(
    ...
    // `ffai_moe_build_tile_plan_bm32`'s: `tile_offset` at the LAST lane is
...
mod tests {
    ...
    fn kernel_ir_constructs_without_inline_msl() {
        let k = ffai_moe_build_tile_plan_bm32_parallel::kernel_ir();
        assert_eq!(k.name, "ffai_moe_build_tile_plan_bm32_parallel");
        let all_ops =
...
pub mod kernel_benches {
    ...
    use super::ffai_moe_build_tile_plan_bm32_parallel;
    ...
    fn bench_moe_build_tile_plan_bm32_parallel(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(ffai_moe_build_tile_plan_bm32_parallel::kernel_ir())
