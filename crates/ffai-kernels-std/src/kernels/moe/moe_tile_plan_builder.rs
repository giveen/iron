...
//! Device-side tile-plan builder for `ffai_moe_gather_qmm_mma_int4_bm16_mpp_tileplan`
//! (`moe_mpp_tileplan.rs`). `ffai_moe_build_tile_plan`.
//!
...
//! implies. The token-order device routing plan (`ffai_moe_sort_plan` in
...
#[kernel]
pub fn ffai_moe_build_tile_plan(
...
mod tests {
    ...
    fn kernel_ir_constructs_without_inline_msl() {
        let k = ffai_moe_build_tile_plan::kernel_ir();
        assert_eq!(k.name, "ffai_moe_build_tile_plan");
        let all_ops =
...
/// a formality (mirrors `ffai_moe_sort_plan`'s bench in `moe_permute.rs`,
/// the same u32-only-kernel shape).
pub mod kernel_benches {
    use ffai_kernels::{bench, test::*};

    use super::ffai_moe_build_tile_plan;
    ...
    fn bench_moe_build_tile_plan(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(ffai_moe_build_tile_plan::kernel_ir())
