...
//! small-M follow-up). `ffai_moe_build_tile_plan_bm32_own_parallel`.
//!
//! "Own" sibling of `ffai_moe_build_tile_plan_bm32_parallel`
//! (`moe_tile_plan_builder_bm32_parallel.rs`) - shares the same phase 1
//! (`ffai_moe_tile_plan_expert_counts`); the only difference from that
//! file is the tile-count formula, mirroring
//! `ffai_moe_build_tile_plan_bm32_own`'s relationship to
//! `ffai_moe_build_tile_plan_bm32`: an expert whose remainder
//! (`count % 32`) is `1..=16` rows does NOT get a tile for that
//! remainder here (those rows are pairing candidates for the separate
//! `ffai_moe_build_tile_plan_bm32_paired` dispatch - see
...
//! (`FFAI_MOE_PAIRED`), which is not yet wired into `MoELayer.swift` -
...
//! `ffai_moe_build_tile_plan_bm32_parallel`, rather than needing a
...
//!   `ffai_moe_tile_plan_expert_counts`'s outputs for the SAME
//!   `sorted_experts` array and `n_experts` - must be the SAME array
//!   `ffai_moe_build_tile_plan_bm32_paired` reads in the same forward,
...
/// module docs.
#[kernel]
pub fn ffai_moe_build_tile_plan_bm32_own_parallel(
...
mod tests {
    ...
    fn kernel_ir_constructs_without_inline_msl() {
        let k = ffai_moe_build_tile_plan_bm32_own_parallel::kernel_ir();
        assert_eq!(k.name, "ffai_moe_build_tile_plan_bm32_own_parallel");
        let all_ops =
...
pub mod kernel_benches {
    ...
    use super::ffai_moe_build_tile_plan_bm32_own_parallel;
    ...
    fn bench_moe_build_tile_plan_bm32_own_parallel(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(ffai_moe_build_tile_plan_bm32_own_parallel::kernel_ir())
