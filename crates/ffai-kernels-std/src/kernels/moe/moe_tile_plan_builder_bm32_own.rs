...
//! `ffai_moe_gather_qmm_coop` (UNCHANGED, existing kernel) leg.
//! `ffai_moe_build_tile_plan_bm32_own`.
//!
//! Near-identical to `moe_tile_plan_builder_bm32.rs`'s
//! `ffai_moe_build_tile_plan_bm32` - same one-threadgroup/one-lane-per-
...
//! handled by the separate `ffai_moe_build_tile_plan_bm32_paired` /
//! `ffai_moe_gather_qmm_coop_paired` dispatch instead (see
...
//! `ffai_moe_build_tile_plan_bm32` unmodified) because that kernel emits a
...
//! entirely opt-in (`FFAI_MOE_PAIRED`), so this new kernel changes nothing
...
#[kernel]
pub fn ffai_moe_build_tile_plan_bm32_own(
...
mod tests {
    ...
    fn kernel_ir_constructs_without_inline_msl() {
        let k = ffai_moe_build_tile_plan_bm32_own::kernel_ir();
        assert_eq!(k.name, "ffai_moe_build_tile_plan_bm32_own");
        let all_ops =
...
pub mod kernel_benches {
    ...
    use super::ffai_moe_build_tile_plan_bm32_own;
    ...
    fn bench_moe_build_tile_plan_bm32_own(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(ffai_moe_build_tile_plan_bm32_own::kernel_ir())
