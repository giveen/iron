...
//! tile plan. `ffai_moe_gather_qmm_mma_int4_bm16_mpp_tileplan`.
//!
//! F-85 low-context prefill occupancy fix. The production default
//! (`ffai_moe_gather_qmm_mma_int4_bm16_mpp` in `moe_mpp.rs`) ties tile
...
pub fn ffai_moe_gather_qmm_mma_int4_bm16_mpp_tileplan<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
mod tests {
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = ffai_moe_gather_qmm_mma_int4_bm16_mpp_tileplan::kernel_ir_for(dt);
            assert_eq!(k.name, "ffai_moe_gather_qmm_mma_int4_bm16_mpp_tileplan");
            let all_ops =
...
/// invisible to `ffaik build` (kernel discovery for MSL/metallib/Swift
...
/// `ffaik build --emit swift` actually generate the `Ops`-facing wrapper.
pub mod kernel_benches {
    use ffai_kernels::{bench, test::*};

    use super::{build_tile_plan, ffai_moe_gather_qmm_mma_int4_bm16_mpp_tileplan};
