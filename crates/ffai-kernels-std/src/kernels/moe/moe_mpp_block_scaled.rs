...
//!
//! Algorithmically **byte-identical** to `ffai_moe_gather_qmm_mma_int4_bm16_mpp`:
...
//! |-------------------------------------|---------|--------|-------------|
//! | `ffai_mxfp4_moe_gather_qmm_bm16_mpp`  | E2M1    | u32    | E8M0 (u8)   |
//! | `ffai_nvfp4_moe_gather_qmm_bm16_mpp`  | E2M1    | u32    | E4M3 (u8) × global |
//! | `ffai_fp4_moe_gather_qmm_bm16_mpp`    | E2M1    | u32    | f32         |
//! | `ffai_mxfp8_e4m3_moe_gather_qmm_bm16_mpp` | E4M3 | u8   | E8M0 (u8)   |
//! | `ffai_mxfp8_e5m2_moe_gather_qmm_bm16_mpp` | E5M2 | u8   | E8M0 (u8)   |
//! | `ffai_fp8_e5m2_moe_gather_qmm_bm16_mpp`   | E5M2 | u8   | f32         |
//! | `ffai_nvfp8_moe_gather_qmm_bm16_mpp`  | E4M3    | u8     | f32         |
//! | `ffai_int8_moe_gather_qmm_bm16_mpp`   | int8    | u8     | f32         |
//!
...
//! |----------------------------------------------|---------|--------|-----------|
//! | `ffai_int{2,3,4,5,6}_moe_gather_qmm_bm16_mpp`  | intN    | u32    | f32       |
//! | `ffai_mxint{2,3,4,5,6}_moe_gather_qmm_bm16_mpp`| intN    | u32    | E8M0 (u8) |
//! | `ffai_mxint8_moe_gather_qmm_bm16_mpp`          | int8    | u8     | E8M0 (u8) |
//!
...
/// MPP MoE gather BGEMM (BM=16, cooperative-tensor path), folded over the
/// 28-format axis (§7). The expert sub-run walk, X-staging, matmul2d, and
/// write-back are format-independent; only the per-format W-dequant into the
/// `ws` tile folds onto `(BITS, WDEC, SKIND)` (buffer types `(WT, ST)`, legend
/// in `gemm/block_scaled_matmul`). Nibble (WDEC 0) and sub-byte bit-stream
/// (WDEC 1) stage 2 packs/lane; byte floats (WDEC 2-4) stage one BN-row/lane.
/// Decodes through `kernels/primitives.rs`. Produces
/// `ffai_<FMT>_moe_gather_qmm_bm16_mpp`.
#[kernel(variants(
    ...
    suffix = "{FMT}_moe_gather_qmm_bm16_mpp",
))]
                ...
                if WDEC == 0u32 {
                    ...
                    for _pi in range(0u32, 2u32, 1u32) {
                        ...
                        for _j in range(0u32, 8u32, 1u32) {
                            ...
                            threadgroup_store("ws", dst + _j, ffai_decode_e2m1(nib) * scale);
                        }
                    }
                } else if WDEC == 1u32 {
...
pub mod kernel_tests {
    ...
    fn test_mxfp4_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxfp4_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxfp4,
            SHAPE,
            dt,
        )
    ...
    fn test_nvfp4_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_nvfp4_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Nvfp4,
            SHAPE,
            dt,
        )
    ...
    fn test_fp4_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_fp4_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Fp4,
            SHAPE,
            dt,
        )
    ...
    fn test_mxfp8_e4m3_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxfp8_e4m3_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxfp8E4,
            SHAPE,
            dt,
        )
    ...
    fn test_mxfp8_e5m2_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxfp8_e5m2_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxfp8E5,
            SHAPE,
            dt,
        )
    ...
    fn test_fp8_e5m2_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_fp8_e5m2_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Fp8E5m2,
            SHAPE,
            dt,
        )
    ...
    fn test_nvfp8_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_nvfp8_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Nvfp8,
            SHAPE,
            dt,
        )
    ...
    fn test_int8_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int8_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int8,
            SHAPE,
            dt,
        )
    ...
    fn test_int2_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int2_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int2,
            SHAPE,
            dt,
        )
    ...
    fn test_int3_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int3_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int3,
            SHAPE,
            dt,
        )
    ...
    fn test_int4_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int4_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int4,
            SHAPE,
            dt,
        )
    ...
    fn test_int5_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int5_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int5,
            SHAPE,
            dt,
        )
    ...
    fn test_int6_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int6_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int6,
            SHAPE,
            dt,
        )
    ...
    fn test_mxint2_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxint2_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxint2,
            SHAPE,
            dt,
        )
    ...
    fn test_mxint3_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxint3_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxint3,
            SHAPE,
            dt,
        )
    ...
    fn test_mxint4_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxint4_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxint4,
            SHAPE,
            dt,
        )
    ...
    fn test_mxint5_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxint5_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxint5,
            SHAPE,
            dt,
        )
    ...
    fn test_mxint6_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxint6_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxint6,
            SHAPE,
            dt,
        )
    ...
    fn test_mxint8_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_mxint8_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Mxint8,
            SHAPE,
            dt,
        )
    ...
    fn test_nvfp8_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_nvfp8_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Nvfp8F16,
            SHAPE,
            dt,
        )
    ...
    fn test_fp4_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_fp4_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Fp4F16,
            SHAPE,
            dt,
        )
    ...
    fn test_fp8_e5m2_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_fp8_e5m2_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Fp8E5m2F16,
            SHAPE,
            dt,
        )
    ...
    fn test_int2_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int2_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int2F16,
            SHAPE,
            dt,
        )
    ...
    fn test_int3_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int3_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int3F16,
            SHAPE,
            dt,
        )
    ...
    fn test_int4_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int4_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int4F16,
            SHAPE,
            dt,
        )
    ...
    fn test_int5_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int5_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int5F16,
            SHAPE,
            dt,
        )
    ...
    fn test_int6_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int6_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int6F16,
            SHAPE,
            dt,
        )
    ...
    fn test_int8_f16_moe_gather_qmm_bm16_mpp(dt: DType) -> TestSetup {
        block_indexed_setup(
            ffai_int8_f16_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt),
            QFormat::Int8F16,
            SHAPE,
            dt,
        )
...
pub mod kernel_benches {
    ...
    fn bench_fp4(dt: DType) -> BenchSetup {
        block_bench(ffai_fp4_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt), QFormat::Fp4, SHAPE, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    ...
    fn bench_int8(dt: DType) -> BenchSetup {
        block_bench(ffai_int8_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt), QFormat::Int8, SHAPE, dt)
    }
    // Symmetric sub-byte ints (FP32 group scale) + MXINT (E8M0 block scale) +
    // MXINT8 (8-bit, E8M0). k_in=4096 is a multiple of 32 → word-aligned per width.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2(dt: DType) -> BenchSetup {
        block_bench(ffai_int2_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt), QFormat::Int2, SHAPE, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3(dt: DType) -> BenchSetup {
        block_bench(ffai_int3_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt), QFormat::Int3, SHAPE, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4(dt: DType) -> BenchSetup {
        block_bench(ffai_int4_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt), QFormat::Int4, SHAPE, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5(dt: DType) -> BenchSetup {
        block_bench(ffai_int5_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt), QFormat::Int5, SHAPE, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6(dt: DType) -> BenchSetup {
        block_bench(ffai_int6_moe_gather_qmm_bm16_mpp::kernel_ir_for(dt), QFormat::Int6, SHAPE, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
