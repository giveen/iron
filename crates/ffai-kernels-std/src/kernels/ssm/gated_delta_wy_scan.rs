...
//! Chunked-WY GDN prefill — **scan** kernel (pass 2 of 2) — `ffai_gdn_wy_scan`.
//!
//! Consumes the chunk-parallel intermediates
//! [`ffai_gdn_wy_plan`](super::gated_delta_wy_plan) produced and threads the
...
//!
//! ## Layout (must agree with `ffai_gdn_wy_plan`'s doc block)
//!
//!   - `q_eff`   : `[N, NC, C, Dk]`
//!   - `y_local` : `[N, NC, C, Dv]`
//!   - `p_s`     : `[N, NC, Dk, Dk]`
...
//!     this hardware (`ffaik device` reports 32 KiB on the M5 Max this was
...
mod tests {
    ...
    fn dump() {
        ...
        let mut k = ffai_gdn_wy_scan::kernel_ir_for(DType::F32);
...
/// New-syntax correctness for the sequential scan kernel (`ffai_gdn_wy_scan`).
...
pub mod kernel_tests {
    ...
    fn setup(nc: usize, c: usize, hv: usize, dk: usize, dv: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(ffai_gdn_wy_scan::kernel_ir_for(dt))
    ...
    fn test_ffai_gdn_wy_scan_single_tile(dt: DType) -> TestSetup { setup(2, 16, 2, 32, 32, dt) }

    // Multi-tile (Dv=128 -> 4 tiles), C=64, 3 chunks — closer to production
    // geometry (Dk=Dv=128 handled at reduced NC for test speed).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_ffai_gdn_wy_scan_multi_tile(dt: DType) -> TestSetup { setup(3, 64, 2, 128, 128, dt) }
}
...
pub mod kernel_benches {
    ...
    fn bench_gdn_wy_scan_t2048(dt: DType) -> BenchSetup {
        ...
        let n_total = hv;
        let nc = t / c;
        BenchSetup::new(ffai_gdn_wy_scan::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
