...
//! Fused GDN gate/beta — `ffai_gdn_gate_beta`.
//!
...
//! numerically-stable `ffai_softplus` kernel (`max(x,0) + log1p(exp(-|x|))`)
...
/// New-syntax correctness for `ffai_gdn_gate_beta`. CPU oracle mirrors
...
pub mod kernel_tests {
    ...
    fn setup(rows: usize, hv: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(ffai_gdn_gate_beta::kernel_ir_for(dt))
...
/// New-syntax benchmark for `ffai_gdn_gate_beta` at a Qwen3.6-A3B-class
...
pub mod kernel_benches {
    ...
    fn bench_gdn_gate_beta(dt: DType) -> BenchSetup {
        ...
        let n = t * hv;
        BenchSetup::new(ffai_gdn_gate_beta::kernel_ir_for(dt))
