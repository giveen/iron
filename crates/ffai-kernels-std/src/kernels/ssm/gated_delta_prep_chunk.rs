...
/// Shape-specialized sibling of [`ffai_gated_delta_prep_chunk`].
///
...
/// `ffai-kernels-codegen/src/msl/mod.rs`, the `Constexpr params` match arm).
...
/// -p ffai-kernels-std --lib --release -- \
...
/// `ffai_gated_delta_step_record`'s `d192_128_4_4` / `d64_32_2_2` shape
...
/// `ffai_gated_delta_prep_chunk`; this does not touch grid-level
...
///     `test_ffai_gated_delta_prep_chunk_gqa` shape) so correctness has a
...
pub fn ffai_gated_delta_prep_chunk_fast<T>(
    conv_out: Tensor<T>, // [B, T, 2·Hk·Dk + Hv·Dv]  (v slab; q/k slabs unused here)
    a_log: Tensor<T>,    // [Hv]
    dt_bias: Tensor<T>,  // [Hv]
    a_raw: Tensor<T>,    // [B, T, Hv]
    ...
    // 4 SGs per TG pack 4 Dv slots (reference Metal GDN geometry),
    // identical dispatch geometry to `ffai_gated_delta_prep_chunk`.
    let sg = simd_group_id();
    let dk_idx = simd_lane;
    let dv_idx = tgid_x * 4u32 + sg;
    let n = tgid_y;
...
mod tests {
    ...
    /// `cargo test -p ffai-kernels-std --lib --release -- kernels::ssm::gated_delta_prep_chunk::tests::dump_fast_prod --nocapture`
    #[test]
    fn dump_fast_prod() {
        use ffai_kernels::codegen::msl::MslGenerator;
        let mut k = ffai_gated_delta_prep_chunk_fast_d128_128_32_16::kernel_ir_for(DType::F32);
        k.mode = KernelMode::Reduction;
        let msl = MslGenerator::default().generate(&k).expect("codegen");
...
pub mod kernel_tests {
    ...
    use super::{
        ffai_gated_delta_prep_chunk,
        ffai_gated_delta_prep_chunk_fast_d64_8_4_2,
        ffai_gated_delta_prep_chunk_fast_d128_128_32_16,
    };
    ...
    /// shape-specialized `ffai_gated_delta_prep_chunk_fast_*` variants:
    ...
    // and fixture parameters to `test_ffai_gated_delta_prep_chunk_gqa` above,
    // so the two share the same expected numerics and the tol values carry
    // over directly.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_ffai_gated_delta_prep_chunk_fast_d64_8_4_2(dt: DType) -> TestSetup {
        setup_fast(
            ffai_gated_delta_prep_chunk_fast_d64_8_4_2::kernel_ir_for(dt),
            1,
            4,
            4,
            2,
    ...
    // `test_ffai_gated_delta_prep_chunk_no_gqa` widens its own: Dk=128 is a
    ...
    fn test_ffai_gated_delta_prep_chunk_fast_d128_128_32_16(dt: DType) -> TestSetup {
        setup_fast(
            ffai_gated_delta_prep_chunk_fast_d128_128_32_16::kernel_ir_for(dt),
            1,
            3,
            32,
            16,
...
pub mod kernel_benches {
    ...
    use super::{ffai_gated_delta_prep_chunk, ffai_gated_delta_prep_chunk_fast_d128_128_32_16};
    ...
    // Shape-specialized fast kernel (`ffai_gated_delta_prep_chunk_fast_d128_128_32_16`)
    ...
    fn bench_gated_delta_prep_chunk_fast_t1024(dt: DType) -> BenchSetup {
        ...
        let n_total = b * hv;
        let conv_w = 2 * hk * dk + hv * dv;
        BenchSetup::new(ffai_gated_delta_prep_chunk_fast_d128_128_32_16::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("conv_out", b * t * conv_w, dt))
            .buffer(BenchBuffer::random("a_log", hv, dt))
            .buffer(BenchBuffer::random("dt_bias", hv, dt))
