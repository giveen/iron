...
FFAI Kernels GPU kernel compiler. This is the foundation crate — every other
crate in the workspace depends on it.

Defines the SSA-form intermediate representation that `#[kernel]`
functions parse into, that `ffai-kernels-codegen` optimizes and lowers,
and that `ffai-kernels-runtime` reads for dispatch metadata. Also provides
the `inventory`-based kernel registry and Apple GPU family probes.

## Position in the pipeline

```
ffai-kernels-macros ──► ffai-kernels-core (this crate) ──► ffai-kernels-codegen
...
`ffai-kernels-core` defines the shared types that flow through the entire
compiler pipeline. It depends on `ffai-kernels-macros` for the derive macros
...
```rust,ignore
use ffai_kernels_core::ir::{Kernel, Block, Op, Param, ParamKind, BinOpKind};
use ffai_kernels_core::dtype::DType;
use ffai_kernels_core::shape::Shape;
...
> `ffai_kernels_core::ir::ParamKind` or import from the `ir` module directly.
...
| `protocol` | The `runner ↔ CLI` wire types (`ProtocolMessage`, `ProfileInfo`, bench/test/build results) for the `__ffai_runner` subprocess, plus `GpuFamily` Apple-GPU generation detection (M1–M5) |
...
| `ProtocolMessage` | JSON-line wire format for the `__ffai_runner` subprocess (bench/test/build/inspect results, `ProfileInfo`) | `src/protocol.rs` | ✅ |
...
| Crate | Role |
|---|---|
| `ffai-kernels-macros` | Derive macros (`OpFlags`, `ValueRefs`, `VariantName`) for IR types |
...
- **New IR op variant:** `src/ir.rs` — add to the `Op` enum. Add a
  `Display` arm. Update `ffai-kernels-codegen` passes that exhaustively match
  `Op` (start with `type_check` and `msl::emit_block`).

- **New shape constructors:** `src/shape.rs` — add free functions or methods
  on `Shape`. If you need a macro, add it to `ffai-kernels-macros`.

- **New error variant:** `src/error.rs` — add to `Error` enum.

- **Tests to update:** `src/ir.rs` tests, pass tests in `ffai-kernels-codegen`,
  `ffai-kernels-macros` tests.

## Related documentation

- [Root README](../../README.md) — project overview and architecture
- [CONTRIBUTING](../../CONTRIBUTING.md) — dev setup, PR process, CI
- [`ffai-kernels-codegen` README](../ffai-kernels-codegen/README.md) — the optimization passes that operate on this IR
- [`ffai-kernels-macros` README](../ffai-kernels-macros/README.md) — how `#[kernel]` produces this IR
- [Crate docs on docs.rs](https://docs.rs/ffai-kernels-core)
