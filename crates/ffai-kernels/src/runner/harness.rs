...
impl RunnerHarness {
    ...
    fn run_test(args: &RunnerArgs) -> bool {
        ...
        let ctx = match ffai_kernels_runtime::Context::new() {
            Ok(c) => c,
            Err(e) => {
                emit_stdout(&ProtocolMessage::ProtocolError {
                    name: "Context".into(),
    ...
    fn run_inspect(args: &RunnerArgs) -> bool {
        use ffai_kernels_core::protocol::InspectKind;
...
fn run_one_test_with_setup(
    ctx: &ffai_kernels_runtime::Context,
    setup: &TestSetup,
    tol: f64,
    name: &str,
    dt: DType,
...
/// (`crates/ffai-kernels-std/src/kernels/gemm/gemv_quantized.rs` → `"gemm"`).
...
mod name_filter_tests {
    ...
    fn kernel_family_from_registration_file() {
        ...
        assert_eq!(
            kernel_family("crates/ffai-kernels-std/src/kernels/gemm/gemv_quantized.rs"),
            "gemm"
        );
        assert_eq!(kernel_family("crates/ffai-kernels-std/src/kernels/ops/copy.rs"), "ops");
        assert_eq!(
            kernel_family("crates/ffai-kernels-std/src/kernels/sdpa/steel_attn/mod.rs"),
            "sdpa"
        );
        // Top-level kernels files and non-src paths have no family.
        assert_eq!(kernel_family("crates/ffai-kernels-std/src/kernels/primitives.rs"), "");
        assert_eq!(kernel_family("crates/ffai-kernels-std/src/utils.rs"), "");
