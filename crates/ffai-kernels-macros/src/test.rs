...
fn expand_one(
    ...
    let tol_impl: TokenStream2 = match test_attr.tol.clone() {
        Some(Tolerance::Scalar(tol)) => quote! {
            fn tolerance(&self, _dt: ::ffai_kernels::core::DType) -> f64 { #tol }
        },
        Some(Tolerance::Array(vals)) => {
            ...
            quote! {
                fn tolerance(&self, dt: ::ffai_kernels::core::DType) -> f64 {
                    match dt {
                        #(#arms,)*
    ...
    quote! {
        ...
        impl ::ffai_kernels::harness::test::KernelTest for #impl_name {
            ...
            ) -> ::ffai_kernels::harness::test::TestSetup {
                #fn_name(dt)
            }
        ...
        ::ffai_kernels::core::inventory::submit! {
            ::ffai_kernels::harness::test::KernelTestEntry::new(&#static_name, file!())
        }
    }
}
