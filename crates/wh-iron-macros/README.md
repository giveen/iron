...
User Rust code (fn with #[kernel])
            │
      wh-iron-macros (this crate)
            │
      wh-iron-core IR (Kernel, Block, Op, …)
            │
      wh-iron-codegen (opt passes → MSL)
            │
      wh-iron-runtime (GPU dispatch)
```
...
pub mod my_kernel {
    ...
    impl<'a> LaunchBuilder<'a> {
        ...
        pub fn dispatch(self) -> Result<DispatchResult, IronError> { … }
    }
...
fn bench_iron_scale(dt: DType) -> BenchSetup { /* BenchSetup::new(...).buffer(...).bytes_moved(...) */ }

#[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-6, 1e-3, 1e-3])]
fn test_iron_scale(dt: DType) -> TestSetup { /* TestSetup::new(...).input(...).expect(...) */ }
```
...
|---|---|
| `wh-iron-core` | Emits IR type constructors (`Kernel`, `Block`, `Op`, `DType`, `Shape`, `ConstExpr`) in the generated token stream |
...
- [`wh-iron-core` README](../wh-iron-core/README.md) — the IR types emitted by these macros
- [`wh-iron-codegen` README](../wh-iron-codegen/README.md) — the passes that consume the generated IR
- [`wh-iron-std` README](../wh-iron-std/README.md) — the `BenchSpec` type that `#[bench]` submits to
- [Crate docs on docs.rs](https://docs.rs/wh-iron-macros)
