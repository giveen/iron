...
/// Register a kernel benchmark with the `iron bench` runner.
///
/// ```ignore
/// #[bench(name = "unary/exp", dtypes = [f32, f16, bf16])]
/// fn bench_iron_exp(s: &BenchSetup) -> BenchBuffer { … }
/// ```
#[proc_macro_attribute]
...
/// Register a kernel correctness test with the `iron test` runner.
///
/// ```ignore
/// #[test_kernel(dtypes = [f32, f16, bf16], tol = 1e-4)]
/// fn test_iron_exp(s: &TestSetup) -> TestBuffer { … }
/// ```
#[proc_macro_attribute]
