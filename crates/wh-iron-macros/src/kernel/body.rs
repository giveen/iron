...
impl DslBodyParser {
            ...
            Stmt::Macro(mac) => {
                ...
                self.push_error(syn::Error::new_spanned(
                    ...
                    "macro_rules! invocations inside #[kernel] bodies are not \
                     expanded by the wh-iron proc-macro and would silently \
    ...
    fn parse_expr(&mut self, expr: &Expr) -> u32 {
        match expr {
            ...
            //   `let elem = { let raw = ...; iron_decode_e4m3(raw) }`
    ...
    fn parse_call(&mut self, call: &ExprCall) -> u32 {
        ...
        match name {
            ...
            _ => {
                ...
                // registered kernel naming convention (iron_* or iron_* prefix).
                ...
                if !path.starts_with("iron_") && !path.starts_with("iron_") {
                    return self.push_error_value(syn::Error::new_spanned(
                        &call.func,
                        format!(
...
mod tests {
    ...
    #[test]
    fn iron_prefixed_calls_emit_kernel_call() {
        // Names starting with `iron_` are treated as cross-kernel calls.
        ...
        assert!(tokens.contains("\"iron_silu\""), "{tokens}");
    }

    #[test]
    fn non_prefixed_unknown_calls_emit_compile_error() {
        // Names that don't start with `iron_` or `iron_` and don't match
