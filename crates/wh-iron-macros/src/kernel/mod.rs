    ...
    fn generate_kernel_module(
        ...
        quote! {
            #vis mod #fn_name {
                ...
                use wh_iron::core::shape::{Shape, Dim};
                use wh_iron::core::dtype::DType;
                use wh_iron::core::constexpr::ConstExpr;
                ...
                pub fn launch(ctx: &wh_iron::Context) -> LaunchBuilder<'_> {
                    LaunchBuilder::new(ctx)
                }

                const _: () = {
                    fn __build_for_inline(
                        dtypes: &[wh_iron::core::dtype::DType],
                    ) -> wh_iron::core::ir::Kernel {
                        #[allow(unused_variables)]
                        let _t = dtypes
                            .first()
                            .copied()
                            .unwrap_or(wh_iron::core::dtype::DType::F32);
                        kernel_ir_for(#(#arg_var_idents),*)
                    }

                    wh_iron::core::inventory::submit! {
                        wh_iron::harness::registry::KernelEntry::new(#fn_name_str, __build_for_inline)
                    }
                };
            }
        }
