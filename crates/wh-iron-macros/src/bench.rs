...
pub(crate) fn dtype_token(ident: &str) -> TokenStream2 {
    match ident {
        "f32" => quote! { ::wh_iron::core::DType::F32 },
        "f16" => quote! { ::wh_iron::core::DType::F16 },
        "bf16" => quote! { ::wh_iron::core::DType::BF16 },
        "i32" => quote! { ::wh_iron::core::DType::I32 },
        "u32" => quote! { ::wh_iron::core::DType::U32 },
        "i8" => quote! { ::wh_iron::core::DType::I8 },
        "u8" => quote! { ::wh_iron::core::DType::U8 },
        other => {
...
fn expand_one(bench_attr: &BenchAttr, input_fn: ItemFn) -> TokenStream2 {
    ...
    let bytes_impl: TokenStream2 = match &bench_attr.bytes {
        Some(expr) => quote! {
            fn bytes_moved(
                &self,
                setup: &::wh_iron::harness::bench::BenchSetup,
            ) -> u64 {
                (#expr)(setup)
            }
        },
    ...
    let flops_impl: TokenStream2 = match &bench_attr.flops {
        Some(expr) => quote! {
            fn flops(
                &self,
                setup: &::wh_iron::core::bench::BenchSetup,
    ...
    let metal_ref_impl: TokenStream2 = match &bench_attr.metal_ref {
        Some(expr) => quote! {
            fn reference_kernel(
                &self,
            ) -> ::std::option::Option<::wh_iron::harness::bench::RefKernel> {
    ...
    quote! {
        ...
        impl ::wh_iron::harness::bench::KernelBench for #impl_name {
            ...
            ) -> ::wh_iron::harness::bench::BenchSetup {
                #fn_name(dt)
            }
        ...
        ::wh_iron::core::inventory::submit! {
            ::wh_iron::harness::bench::KernelBenchEntry::new(&#static_name, file!())
        }
    }
}
