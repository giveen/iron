...
//! # Writing kernels
//!
//! - Import `wh_iron::prelude::*` anywhere you define `#[kernel]` functions.
...
//! - Use `<kernel>::build_kernel_ir()` or `wh_iron::codegen::msl::MslGenerator` to inspect the
...
pub use wh_iron_codegen as codegen;
/// Error returned by `wh_iron::codegen` helpers.
pub use wh_iron_codegen::error::Error as CodegenError;
/// Core IR, dtype, shape, and constexpr definitions.
pub use wh_iron_core as core;
/// Proc macros and helper macros used by kernel definitions.
pub use wh_iron_macros::{bench, constexpr, kernel, scalar, shape, strided, test_kernel, tile};
/// Vulkan device runtime entry points (feature `vulkan`).
#[cfg(feature = "vulkan")]
pub use wh_iron_runtime::{
    BatchDispatch,
    VulkanBuffer,
    VulkanDevice,
    VulkanPipeline,
...
pub use wh_iron_runtime::{Context, DispatchResult, DispatchSpec, IronError, ResidentBuffer};
/// CUDA / NVIDIA device runtime entry points (feature `cuda`).
#[cfg(feature = "cuda")]
pub use wh_iron_runtime::{CudaDevice, CudaFunction, CudaModule, DeviceBuffer};
/// HIP / ROCm device runtime entry points (feature `hip`).
#[cfg(feature = "hip")]
pub use wh_iron_runtime::{HipBuffer, HipDevice, HipKernel, HipModuleHandle};
