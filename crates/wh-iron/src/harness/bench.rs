...
impl ConstValue {
    ...
    pub fn as_u32(&self) -> wh_iron_core::Result<u32> {
        match *self {
            ConstValue::U32(v) => Ok(v),
            ConstValue::I32(v) => u32::try_from(v).map_err(|_| {
                wh_iron_core::Error::Internal(format!("ConstValue {v} out of u32 range"))
            }),
            ConstValue::Usize(v) => u32::try_from(v).map_err(|_| {
                wh_iron_core::Error::Internal(format!("ConstValue {v} out of u32 range"))
            }),
            _ => Err(wh_iron_core::Error::Internal(format!(
...
/// to read its kernel IR — as the codegen-consistency tests and `iron build` do
...
pub struct RefKernel {
    ...
    pub grid: Grid,
    /// Maximum absolute error tolerance for the Iron-vs-reference equivalence check.
    pub tol: f32,
...
impl BenchSetup {
    ...
    pub fn build(self) -> wh_iron_core::Result<BenchSetup> {
