...
//! Non-Metal backend dispatch for `__iron_runner` — `--backend cuda|hip|vulkan`.
//!
    ...
    match backend {
        "cuda" | "hip" | "vulkan" => format!(
            "runner was built without the `{backend}` feature — rebuild the \
             __iron_runner binary with `--features {backend}`"
        ),
...
mod enabled {
    ...
    enum BackendDevice {
        #[cfg(feature = "cuda")]
        Cuda(Box<wh_iron_runtime::CudaDevice>),
        #[cfg(feature = "hip")]
        Hip(Box<wh_iron_runtime::HipDevice>),
        #[cfg(feature = "vulkan")]
        Vulkan(Box<wh_iron_runtime::VulkanDevice>),
    }
        ...
        fn create(backend: &str) -> Result<Option<Self>, String> {
            match backend {
                "cuda" => {
                    #[cfg(feature = "cuda")]
                    {
                        wh_iron_runtime::CudaDevice::create()
                ...
                "hip" => {
                    #[cfg(feature = "hip")]
                    {
                        wh_iron_runtime::HipDevice::create()
                ...
                "vulkan" => {
                    #[cfg(feature = "vulkan")]
                    {
                        wh_iron_runtime::VulkanDevice::create()
    ...
    fn is_unsupported(e: &IronError) -> bool {
        use wh_iron_codegen::error::Error as CodegenError;
        matches!(
            e,
            IronError::Codegen(CodegenError::UnsupportedOp(_)) | IronError::DeviceCapability(_)
        )
    }
    ...
    /// `iron test --backend <name>` — run the `#[test_kernel]` corpus on a
    ...
    /// `iron bench --backend <name>` — event-timed kernel throughput on a
    /// non-Metal device. No reference A/B (the MSL reference kernels are
    /// Metal-only); correctness stays `iron test`'s job.
        ...
        for (bench, dt, family) in work {
            ...
            match dev.bench_kernel(kernel, &buffers, grid.grid, grid.tpg, warmup, iters) {
                Ok(samples) if !samples.is_empty() => {
                    ...
                    let iron_gbps = if stats.min_us > 0.0 {
