...
/// Canonical exit codes for the `iron` binary.
///
...
pub enum IronExitCode {
    ...
    BuildFailure = 2,
    /// `iron diff` detected a performance regression beyond the threshold.
    Regression = 3,
    /// `iron.toml` parsing or configuration error.
    ConfigError = 10,
}
...
impl CliError {
    ...
    pub fn exit_code(&self) -> i32 {
        match self {
            CliError::TestFailure => IronExitCode::TestFailure as i32,
            CliError::BuildFailure | CliError::MetalCompile(_) => IronExitCode::BuildFailure as i32,
            CliError::Regression => IronExitCode::Regression as i32,
            _ => 1,
        }
    }
}
