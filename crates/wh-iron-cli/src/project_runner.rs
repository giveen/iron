...
//! `ProjectRunner` — spawns `__iron_runner` as a subprocess and streams its
//! `ProtocolMessage` JSON-lines output to stdout.
//!
//! ```text
//!  iron CLI  ──spawn──►  __iron_runner  (wh-iron-std linked → inventory populated)
//!             JSON Lines ◄── stdout
//! ```
//!
//! `__iron_runner` is a hidden binary scaffolded by `iron init` into the
//! project's `bin/` directory and installed there via
//! `cargo install --path . --root .`.

use wh_iron_core::protocol::ProtocolMessage;

use crate::harness::Harness;

/// Describes one invocation of `__iron_runner`.
...
impl<'a> ProjectRunner<'a> {
    ...
    /// Spawn `__iron_runner` as a subprocess with stdout/stderr inherited,
    ...
    pub fn run(&self, inv: &RunnerInvocation) -> bool {
        ...
        match cmd.status() {
            Ok(s) => s.success(),
            Err(e) => {
                eprintln!("[iron] failed to spawn '{binary}': {e}");
                false
            },
        }
    }

    /// Spawn `__iron_runner`, capture its stdout, parse each
    ...
    /// Resolve the `__iron_runner` binary path.
    ///
    /// Precedence:
    /// 1. `iron.toml [runner] binary` config override (default: `bin/__iron_runner`).
    /// 2. `./bin/__iron_runner` relative to CWD — the standard install location
    ///    populated by `cargo install --path . --root .` in a iron project.
    /// 3. Sibling of the current `iron` executable — works when both binaries
    ///    are installed to the same prefix (e.g. `~/.cargo/bin/`).
    /// 4. Bare `"__iron_runner"` resolved via `$PATH` as a last resort.
    fn runner_binary(&self) -> String {
        let configured = self.harness.runner_binary();
        if configured != "__iron_runner" {
            return configured.to_string();
        }
        let cwd = std::env::current_dir().unwrap_or_default();
        let local = cwd.join("bin").join("__iron_runner");
        if local.exists() {
        ...
        {
            let sibling = dir.join("__iron_runner");
            if sibling.exists() {
                return sibling.to_string_lossy().into_owned();
            }
        }
        "__iron_runner".to_string()
    }
}
