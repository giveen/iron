...
//! `Harness` — owns the `IronConfig` and global CLI flags; passed to every
//! subcommand handler so they share a single consistent view of configuration.

use crate::{GlobalArgs, config::IronConfig};

/// Top-level harness for the `iron` CLI.  Constructed once in `main` and
...
impl Harness {
    ...
    pub fn new(global: GlobalArgs) -> Self {
        ...
        let config = crate::config::ConfigLoader::load_with_profile(profile).unwrap_or_else(|e| {
            tracing::warn!("iron.toml config error: {e}; using defaults");
            IronConfig::default()
        });
        Self { config, global }
    }

    /// Path to the `__iron_runner` binary (sub-table overrides flat field).
    ...
    /// Effective verbosity level: global `-v` count wins over `iron.toml verbose`.
