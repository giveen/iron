...
//! `iron inspect` — Print IR and/or MSL for kernels.
//!
//! Spawns `__iron_runner inspect` as a subprocess. All kernel discovery and
//! MSL/IR generation happens in the runner process where `wh-iron-std` is
//! linked.
//!
//! Usage:
//!   iron inspect                           # list all registered kernels
//!   iron inspect <kernel>                  # print final MSL (default)
//!   iron inspect <kernel> --ir             # print raw IR
//!   iron inspect <kernel> --stats          # print per-pass op-count table
//!   iron inspect -o /tmp/out               # write .metal file
//!   iron inspect --all -o /tmp/out         # dump every kernel to disk

use wh_iron_core::protocol::{InspectKind, ProtocolMessage};
...
/// `IronCommand` wrapper for `iron inspect`.
pub struct InspectCommand<'a>(pub &'a InspectArgs);

impl<'a> super::IronCommand for InspectCommand<'a> {
    ...
    if args.pass.is_some() {
        eprintln!(
            "{} --pass requires the kernel registry to be linked in-process. \
             Use `iron inspect --ir` to view the final IR, or file a feature request.",
        ...
        ProtocolMessage::Start { total, .. } =>
            if list_only {
                if !printed_list_header {
                    println!(
                        "{}",
                        paint_stdout("iron inspect", Style::new().fg(Color::Cyan).bold())
                    );
                    println!();
