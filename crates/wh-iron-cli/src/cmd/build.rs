...
//! `iron build` — Compile all registered kernels.
//!
//! Spawns `__iron_runner build` as a subprocess. All kernel discovery,
//! MSL generation, xcrun compile-checks, and artifact emission happen in
//! the runner process where `wh-iron-std` is linked.
//!
...
//!                    (implies msl)
//!   --emit swift     Write `<dir>/Generated/IronKernels.swift`
...
use wh_iron_core::protocol::{ArtifactKind, BuildResult, ProtocolMessage};
...
/// `IronCommand` wrapper for `iron build`.
pub struct BuildCommand<'a>(pub &'a BuildArgs);

impl<'a> super::IronCommand for BuildCommand<'a> {
...
pub fn run(args: &BuildArgs, harness: &crate::harness::Harness) -> Result<(), CliError> {
    ...
    // CLI --sdk overrides iron.toml sdk.
