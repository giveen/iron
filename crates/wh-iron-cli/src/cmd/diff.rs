...
//! `iron diff` — Compare bench results against a saved baseline.
//!
//! Usage:
//!   iron diff .iron-snapshots/m4max.json                          # run bench then diff
//!   iron diff .iron-snapshots/m4max.json run.json                 # diff two files
//!   iron diff .iron-snapshots/m4max.json -f softmax
//!   iron diff .iron-snapshots/m4max.json --threshold 3
...
pub fn run(args: &DiffArgs) -> Result<(), CliError> {
    ...
    let current = if let Some(path) = current_path {
        ...
        let temp_file =
            std::env::temp_dir().join(format!(".iron-diff-tmp-{}.json", std::process::id()));
        let mut child = Command::new(std::env::current_exe().map_err(CliError::Io)?)
            ...
            .spawn()
            .map_err(|e| CliError::Subprocess(format!("failed to spawn iron bench: {e}")))?;

        let status = child
            .wait()
            .map_err(|e| CliError::Subprocess(format!("iron bench did not start: {e}")))?;
        if !status.success() {
            eprintln!(
                "{} {}",
...
/// the table to speak for itself. `regressions` drives the `iron diff`
/// exit code.
...
/// w.r.t. the filesystem — callers (both `iron diff` and `iron bench`)
...
/// Build a map from (op, shape) -> (ref_perf, iron_perf).
fn build_result_map(results: &[Value]) -> HashMap<RowKey, (f64, f64)> {
    ...
    for item in results {
        ...
        let iron_perf = item.get("iron").and_then(|v| v.as_f64()).unwrap_or(0.0);
        map.insert(RowKey { op, shape }, (ref_perf, iron_perf));
    }
    map
}
...
mod tests {
    ...
    fn rows(pairs: &[(&str, &str, f64, f64)]) -> Vec<Value> {
        pairs
            .iter()
            .map(|(op, shape, r, m)| {
                json!({ "op": op, "shape": shape, "metric": "GB/s", "ref": r, "iron": m })
            })
            .collect()
    }
