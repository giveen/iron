...
//! `thewafflehaus/iron` repository, downloads the pre-built binary, and
...
const REPO_SLUG: &str = "thewafflehaus/iron";
const REPO_URL: &str = "https://github.com/thewafflehaus/iron.git";
const ASSET_NAME: &str = "iron-aarch64-apple-darwin.tar.gz";
...
fn download_release_binary(tag: &str, dest: &PathBuf) -> Result<(), crate::CliError> {
    ...
    let tar_path = std::env::temp_dir().join("iron-update.tar.gz");
    let extract_dir = std::env::temp_dir().join("iron-update-extract");
    ...
    install_binary(&extract_dir.join("iron"), dest)?;
...
fn update_from_source(src: SourceRef, check_only: bool) -> Result<(), crate::CliError> {
    ...
    let dest = install_path()?;
    let tmp_dir = std::env::temp_dir().join("iron-update-src");
    ...
    let status = Command::new("cargo")
        .current_dir(&tmp_dir)
        .args(["build", "--release", "-p", "wh-iron-cli"])
        .status()
    ...
    install_binary(&tmp_dir.join("target/release/iron"), &dest)?;
    eprintln!("done");
