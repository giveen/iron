...
use wh_iron_core::protocol::{BenchResult as ProtoBenchResult, ProtocolMessage};
...
pub fn run(args: &BenchArgs, harness: &crate::harness::Harness) -> Result<(), crate::CliError> {
    ...
    // Spawn __iron_runner bench and stream protocol results. Name/group
    ...
    let runner_ok = ProjectRunner::new(harness).run_streaming(&inv, |msg| match msg {
        ProtocolMessage::Start { device, total, .. } => {
            ...
            println!(
                "{} {}{}",
                paint_stdout("iron bench", Style::new().fg(Color::Cyan).bold()),
    ...
    if !runner_ok && all.is_empty() {
        return Err(crate::CliError::Other("__iron_runner failed".into()));
    }
    ...
    let impl_count = all.iter().filter(|r| r.iron_gbps > 0.0).count();
    ...
    let avg_pct: Option<f64> = {
        let valid: Vec<f64> = all.iter().filter_map(|r| r.iron_pct).collect();
    ...
    if let Some(p) = avg_pct {
        parts.push(format!("avg {}", paint_stdout(format!("{p:.0}% Iron"), pct_style(p)),));
    }
    if checked_count > 0 {
...
fn try_auto_diff(
    ...
    let heading = format!("iron bench · diff vs {reference} @ {short_sha} ({baseline_path})");
...
/// diff/baseline system: `op`/`subop`/`shape`/`metric`/`ref`/`iron`.
///
...
fn result_to_value(r: &ProtoBenchResult) -> Value {
    ...
    obj.insert("iron".into(), Value::from(r.iron_gbps));
    Value::Object(obj)
}
...
fn save_json(device: &str, results: &[ProtoBenchResult], path: &str) {
    ...
    for (i, r) in results.iter().enumerate() {
        ...
        out.push_str(&format!(
            "  {}{}\n",
            format_result_row(op, subop, &r.shape, "GB/s", r.ref_gbps, Some(r.iron_gbps)),
            comma
        ));
    }
    out.push_str("]}");
...
fn summarize(results: &[ProtoBenchResult]) -> Summary {
    Summary {
        total: results.len(),
        implemented: results.iter().filter(|r| r.iron_gbps > 0.0).count(),
...
/// that only read `op`/`shape`/`metric`/`ref`/`iron` are unaffected.
fn format_result_row(
    ...
    metric: &str,
    ref_perf: Option<f64>,
    iron_perf: Option<f64>,
) -> String {
    match subop {
        Some(s) => format!(
            "{{\"op\":{:?},\"subop\":{:?},\"shape\":{:?},\"metric\":{:?},\"ref\":{},\"iron\":{}}}",
            op,
            s,
            shape,
            metric,
...
impl<'a> super::IronCommand for BenchCommand<'a> {
...
mod tests {
    ...
    fn make_result(name: &str, dtype: &str, iron_gbps: f64, correct: bool) -> ProtoBenchResult {
        ProtoBenchResult {
            name: name.into(),
            group: String::new(),
            dtype: dtype.into(),
    ...
    fn summary_counts_per_category() {
        ...
        assert_eq!(s.total, 3);
        assert_eq!(s.implemented, 2); // exp + log (iron_gbps > 0)
    ...
    fn row_without_subop_matches_legacy_schema() {
        ...
        assert_eq!(
            row,
            r#"{"op":"rms_norm","shape":"B=1024 N=4096 f32","metric":"GB/s","ref":323.900,"iron":325.600}"#,
        );
    ...
    fn row_with_subop_emits_disambiguated_field() {
        ...
        assert_eq!(
            row,
            r#"{"op":"unary","subop":"sin","shape":"N=64M f32","metric":"GB/s","ref":544.800,"iron":114.500}"#,
        );
    }
    ...
    fn row_handles_missing_perf_values() {
        ...
        assert!(row.contains(r#""iron":null"#));
