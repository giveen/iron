...
//! `iron test` — run `#[test_kernel]` correctness setups against a CPU oracle.
//!
//! ## Output format (forge-style)
//!
//! ```text
//! Ran 3 tests for iron_add
//! [PASS] iron_add [f32]   (err=0.00e0)
//! [PASS] iron_add [f16]   (err=2.38e-7)
//! [PASS] iron_add [bf16]  (err=1.56e-3)
//! Suite result: ok. 3 passed; 0 failed
//!
//! Ran 2 test suites in 57.46ms: 3 tests passed, 1 failed (4 total tests)
//! ```
//!
//! This command spawns `__iron_runner test` as a subprocess and streams
//! `ProtocolMessage` JSON lines. All inventory lookup happens in the runner
//! process where `wh-iron-std` is linked.

use std::time::Instant;

use wh_iron_core::protocol::{ProtocolMessage, TestResult as ProtoTestResult};
...
/// `IronCommand` wrapper for `iron test`.
pub struct TestCommand<'a>(pub &'a TestArgs);

impl<'a> super::IronCommand for TestCommand<'a> {
...
pub fn run(args: &TestArgs, harness: &crate::harness::Harness) -> Result<(), crate::CliError> {
    ...
    let runner_ok = ProjectRunner::new(harness).run_streaming(&inv, |msg| match msg {
        ProtocolMessage::Start { total, .. } => {
            println!(
                "{} {}",
                paint_stdout("iron test", Style::new().fg(Color::Cyan).bold()),
