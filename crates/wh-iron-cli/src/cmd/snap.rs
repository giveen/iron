...
//! `iron snap` — Save current bench results as a regression baseline.
//!
//! Usage:
//!   iron snap                                                     # run bench then save
//!   iron snap -o .iron-snapshots/$(git rev-parse --short HEAD).json
//!   iron snap --from results/run.json                             # promote existing JSON
//!   iron snap --from results/run.json --note "after fusion fix"

use std::process::Command;

use serde::Serialize;
use serde_json::Value;
use wh_iron::runner::GpuFamily;
...
pub fn run(args: &SnapArgs) -> Result<(), CliError> {
    ...
    let out_path: String = match out_path {
        ...
        None => {
            ...
            format!(".iron-snapshots/{}.json", date)
        },
    };
    ...
    let results_json: Value = if let Some(from) = from_path {
        ...
        let temp_file =
            std::env::temp_dir().join(format!(".iron-snap-tmp-{}.json", std::process::id()));
