...
//! `iron device` — Show GPU device info and supported feature flags.

use wh_iron::runner::{GpuFamily, GpuRunner};
...
pub fn run(args: &DeviceArgs, harness: &crate::harness::Harness) -> Result<(), crate::CliError> {
    ...
    eprintln!("{}", paint_stdout("iron device", Style::new().fg(Color::Cyan).bold()),);
    eprintln!();
    eprintln!(
        "  {}  {}",
