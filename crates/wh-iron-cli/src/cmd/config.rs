...
//! `iron config` — display the effective merged configuration.
//!
//! Prints the fully-resolved `IronConfig` as TOML (default) or JSON
//! (`--json` / global `--json`), showing the result of merging:
//!   defaults → extends base → iron.toml → `[profiles.<name>]` → env vars
...
pub fn run(args: &ConfigArgs, harness: &Harness) -> Result<(), CliError> {
    ...
    // Show where iron.toml was found (helpful for debugging parent-dir walk).
    if let Some(path) = crate::config::ConfigLoader::find_iron_toml() {
        if !harness.is_quiet() && !json_out {
            eprintln!(
                "\n{}  {}",
                paint_stderr("iron.toml found at", Style::new().fg(Color::BrightBlack)),
        ...
        eprintln!(
            "{}",
            paint_stderr(
                "note: no iron.toml found; showing built-in defaults",
