...
//! `IronConfig` — layered configuration for the `iron` CLI.
//!
//! # Load order (later layers override earlier ones)
//!
//! 1. Built-in defaults
//! 2. `extends` base file (resolved relative to the iron.toml that references it)
//! 3. `iron.toml` found by walking up from CWD (optional, ignored if absent)
//! 4. Profile-specific overrides from `[profiles.<name>]` section
//! 5. `IRON_*` environment variables
//! 6. CLI flags (applied at the harness level after extraction)
//!
//! # Profile selection
//!
//! Select a profile with `IRON_PROFILE=ci` or `--profile ci`.
//!
//! ```toml
//! [profiles.ci]
//! verbose = 0
...
/// Bench-specific configuration (`[bench]` table in iron.toml).
...
/// Build-specific configuration (`[build]` table in iron.toml).
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
/// Runner subprocess configuration (`[runner]` table in iron.toml).
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
/// Top-level configuration for the `iron` CLI.
///
...
impl Default for IronConfig {
    fn default() -> Self {
        Self {
            runner_binary: "__iron_runner".to_string(),
            project_path: None,
            verbose: 0,
            runs: 3,
            warmup_runs: 1,
...
impl IronConfig {
    ...
    /// Serialize the effective config as pretty-printed TOML (`iron config`).
...
/// Loads [`IronConfig`] from the layered sources described in the module doc.
pub struct ConfigLoader;

impl ConfigLoader {
    /// Walk parent directories from CWD looking for `iron.toml`.
    ///
    /// Returns the first path found, or `None` if the file doesn't exist
    /// anywhere in the ancestor chain (stops at filesystem root).
    pub fn find_iron_toml() -> Option<PathBuf> {
        let cwd = std::env::current_dir().ok()?;
        let mut dir: &Path = cwd.as_path();
        loop {
            let candidate = dir.join("iron.toml");
            if candidate.exists() {
    ...
    /// mismatch in an env var, etc.).  A missing `iron.toml` is silently
    /// ignored.
    pub fn load() -> Result<IronConfig, Box<figment::Error>> { Self::load_with_profile(None) }
    ...
    ///   2. `IRON_PROFILE` environment variable
    ///   3. No profile (flat config only)
    pub fn load_with_profile(profile: Option<&str>) -> Result<IronConfig, Box<figment::Error>> {
        let toml_path = Self::find_iron_toml().unwrap_or_else(|| PathBuf::from("iron.toml"));

        // Determine effective profile (arg > env > none).
        let env_profile = std::env::var("IRON_PROFILE").ok().filter(|s| !s.is_empty());
        let selected = profile.or(env_profile.as_deref());

        // Resolve `extends = "..."` base file.
        let extends_base = Self::resolve_extends(&toml_path);

        let mut figment = Figment::from(Serialized::defaults(IronConfig::default()));
        ...
        // Layer 4: IRON_* env vars.
        figment = figment.merge(Env::prefixed("IRON_"));

        let mut config: IronConfig = figment.extract().map_err(Box::new)?;
    ...
    /// Look for `extends = "path"` at the top level of a iron.toml and
    ...
    /// Extract a `[profiles.<name>]` section from a iron.toml and return it
...
fn interpolate_config(cfg: &mut IronConfig) {
...
fn collect_warnings(toml_path: &Path) -> Vec<String> {
    ...
    for key in table.keys() {
        if !KNOWN_KEYS.contains(&key.as_str()) {
            warnings.push(format!("unknown config key `{key}` in iron.toml"));
        }
    }
    ...
    if has_bench && (table.contains_key("runs") || table.contains_key("warmup_runs")) {
        warnings.push(
            "iron.toml: top-level `runs`/`warmup_runs` are ignored when `[bench]` is present; \
    ...
    if has_runner && table.contains_key("runner_binary") {
        warnings.push(
            "iron.toml: top-level `runner_binary` is ignored when `[runner]` is present; \
...
mod tests {
    ...
    fn defaults_are_sensible() {
        let cfg = IronConfig::default();
        assert_eq!(cfg.runner_binary, "__iron_runner");
        ...
        assert_eq!(cfg.effective_runner_binary(), "__iron_runner");
        assert_eq!(cfg.effective_sdk(), "macosx");
    }

    #[test]
    fn sub_table_bench_overrides_flat() {
        let mut cfg = IronConfig::default();
    ...
    fn sub_table_runner_overrides_flat() {
        ...
        assert_eq!(cfg.runner_binary, "__iron_runner"); // flat unchanged
    }

    #[test]
    fn interpolate_str_replaces_env_var() {
        // SAFETY: single-threaded test, no concurrent reads.
        unsafe { std::env::set_var("_IRON_TEST_INTERP", "hello") };
        assert_eq!(interpolate_str("${_IRON_TEST_INTERP}/world"), "hello/world");
        unsafe { std::env::remove_var("_IRON_TEST_INTERP") };
    }

    #[test]
    fn interpolate_str_uses_default_when_unset() {
        unsafe { std::env::remove_var("_IRON_DEFINITELY_MISSING") };
        assert_eq!(interpolate_str("${_IRON_DEFINITELY_MISSING:-fallback}"), "fallback");
    }

    #[test]
    fn interpolate_str_empty_when_unset_no_default() {
        unsafe { std::env::remove_var("_IRON_DEFINITELY_MISSING") };
        assert_eq!(interpolate_str("before_${_IRON_DEFINITELY_MISSING}_after"), "before__after");
    }
    ...
    fn collect_warnings_empty_for_known_keys() {
        // Build a valid iron.toml string in a temp dir and verify no warnings.
        let dir = std::env::temp_dir().join("iron_config_test_known");
        let _ = std::fs::create_dir_all(&dir);
        let path = dir.join("iron.toml");
    ...
    fn collect_warnings_flags_unknown_key() {
        let dir = std::env::temp_dir().join("iron_config_test_unknown");
        let _ = std::fs::create_dir_all(&dir);
        let path = dir.join("iron.toml");
