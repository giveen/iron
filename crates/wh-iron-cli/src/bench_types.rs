...
//! `wh-iron-std` — the kernel stdlib should know nothing about how the CLI
//! renders results.
//!
...
/// Result of a numerical equivalence check between the reference and Iron kernels.
...
pub fn check_equiv_with(
    ...
    let n = ref_vals.len().min(iron_vals.len());
    let mut max_err = 0.0f32;
    let mut dot = 0.0f64;
    let mut ref_norm_sq = 0.0f64;
    let mut iron_norm_sq = 0.0f64;
    for (&r, &m) in ref_vals[..n].iter().zip(&iron_vals[..n]) {
        let err = (r - m).abs();
        if err > max_err {
            max_err = err;
        }
    ...
    let cosine_sim = match (ref_norm_sq > 0.0, iron_norm_sq > 0.0) {
        (false, false) => 1.0,
        (false, true) | (true, false) => 0.0,
        (true, true) => {
            let denom = ref_norm_sq.sqrt() * iron_norm_sq.sqrt();
            (dot / denom) as f32
        },
    }
    .clamp(-1.0, 1.0);
    let same_len = ref_vals.len() == iron_vals.len();
    EquivResult {
        n_checked: n,
        max_abs_err: max_err,
        cosine_sim,
...
pub fn check_equiv(ref_vals: &[f32], iron_vals: &[f32], max_abs_err: f32) -> EquivResult {
    check_equiv_with(ref_vals, iron_vals, EquivTolerance::new(max_abs_err, DEFAULT_MIN_COSINE_SIM))
}
...
pub struct OpResultExtras {
    pub iron_timing: Option<BenchStats>,
...
impl OpBench {
    ...
    pub fn result(
        ...
        self.result_sub(None::<&str>, shape, ref_perf, iron_perf, equiv)
    }
    ...
    pub fn result_sub(
        ...
        self.result_sub_timed(subop, shape, ref_perf, iron_perf, equiv, None, None)
    }
    ...
    pub fn result_sub_timed(
        ...
        iron_perf: Option<f64>,
        equiv: Option<EquivResult>,
        iron_timing: Option<BenchStats>,
        ref_timing: Option<BenchStats>,
    ) -> OpResult {
        self.result_with_extras(subop, shape, ref_perf, iron_perf, equiv, OpResultExtras {
            iron_timing,
            ref_timing,
            metrics: None,
        })
    ...
    pub fn result_with_extras(
        ...
        if iron_perf.is_some() && equiv.is_none() {
        ...
        let result = OpResult {
            ...
            iron_perf,
            equiv,
            iron_timing: extras.iron_timing,
    ...
    pub fn implemented(
        ...
        self.result(shape, ref_perf, Some(iron_perf), Some(equiv))
    }
...
impl OpResult {
    ...
    pub fn iron_perf(&self) -> Option<f64> { self.iron_perf }
    pub fn equiv(&self) -> Option<&EquivResult> { self.equiv.as_ref() }

    pub fn iron_us(&self) -> Option<f64> {
        self.iron_timing.as_ref().filter(|t| t.is_valid()).map(|t| t.min_us)
    }
    ...
    pub fn pct(&self) -> Option<f64> {
        match (self.ref_perf, self.iron_perf) {
    ...
    pub fn correctness_status(&self) -> CorrectnessStatus {
        match (&self.equiv, self.iron_perf) {
