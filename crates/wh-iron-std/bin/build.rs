...
/// carries the custom Metal kernels Iron ports from (turbo / aura, gated-delta,
/// ssm, ssm_replay, etc.). Stock upstream is missing those, so `iron bench`
...
fn main() {
    ...
    // crates/wh-iron-cli → crates/ → repo root
...
/// if absent or stale.
///
/// Multiple Cargo build scripts (wh-iron-std, wh-iron-cli) share the same
...
/// nothing. When stale/missing (first `iron bench` after `cargo clean`, or
