...
//! else in the codebase (see `iron_mma_probe_a_identity_b_identity`):
//!
//! ```text
//!   qid = lane / 4
...
//! `cargo run -p wh-iron-cli -- inspect iron_simdgroup_load_probe`):
//!
//! ```text
...
#[kernel]
pub fn iron_simdgroup_load_probe<T>(src: Tensor<T>, mut dst: Tensor<T>) {
    let lane = simd_lane;
...
pub mod kernel_tests {
    ...
    use super::iron_simdgroup_load_probe;
    ...
    fn test_simdgroup_load_probe(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_simdgroup_load_probe::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_simdgroup_load_probe(dt: DType) -> BenchSetup {
        BenchSetup::new(iron_simdgroup_load_probe::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
