...
//!
//! Run:
//!   cargo test --release -p wh-iron-std --test mma_layout_probe -- --nocapture

use wh_iron::kernel;

#[kernel]
pub fn iron_mma_probe_a_identity_b_gemm(out: Tensor<f32>) {
    let lane = simd_lane;
    let qid = lane / 4u32;
...
#[kernel]
pub fn iron_mma_probe_a_identity_b_identity(out: Tensor<f32>) {
    let lane = simd_lane;
    let qid = lane / 4u32;
    let fm = (qid & 4u32) + ((lane / 2u32) % 4u32);
    let fn0 = (qid & 2u32) * 2u32 + (lane % 2u32) * 2u32;
