...
/// New-syntax correctness for `iron_rope` (Grid3D, single threadgroup with
...
pub mod kernel_tests {
    ...
    fn test_iron_rope(dt: DType) -> TestSetup {
        ...
        let grid_x = head_dim / 2;
        TestSetup::new(iron_rope::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_rope` (vs MLX `metal/rope.metal`). Multi-TG:
...
pub mod kernel_benches {
    ...
    //   `traditional  [[function_constant(2)]]` → false (rotate-half, like Iron)
    ...
    //   in[[0]]            = `inp`  (shared by name with the Iron input)
    ...
    // `pos.y·strides[1] + (pz·4+i)·strides[0] + pos.x` == Iron's
    // `py·seq_stride + head·h_stride + px` exactly.
    //
    // Grid: MLX `dispatch_threads(grid_dims=(D/2, T, B·ceil(H/4)), group_dims)`,
    // so `threads_per_grid = (grid_x, seq_len, n_heads/4)`. The Iron runner
    ...
    // so Iron and the reference rotate identical data; tol floor 0.01 is the
    // legacy rope tolerance (Iron folds in f32, MLX uses `fast::cos/sin`).
    ...
    fn bench_rope(dt: DType) -> BenchSetup {
        ...
        let strides: Vec<u8> = [(seq_len * head_dim) as i64, head_dim as i64, 1i64]
            ...
            .collect();
        BenchSetup::new(iron_rope::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            ...
            .with_reference(
                ...
                // in[[0]] shared by name with the Iron `inp` (placeholder bytes
                // overwritten by the runner with Iron's exact input).
