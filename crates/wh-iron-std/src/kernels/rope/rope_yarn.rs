...
//!
//! Same Grid3D dispatch shape as `iron_rope_banded`: one thread per
//! (head, i in 0..head_dim/2), each thread rotating the pair
//! (i, i + half_dim). No reduction, no threadgroup memory.
//!
//! Codegen-only. Validated by the in-source `#[test_kernel]`s + Iron
//! integration tests.

use wh_iron::kernel;

#[kernel]
pub fn iron_rope_yarn<T>(
    qk: Tensor<T>,
    out: Tensor<T>,
...
/// New-syntax correctness + bench for `iron_rope_yarn` (YaRN context-extension
...
pub mod kernel_tests {
    ...
    fn rope_yarn_setup(
        ...
        TestSetup::new(iron_rope_yarn::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_rope_yarn`.
pub mod kernel_benches {
    ...
    fn bench_rope_yarn(dt: DType) -> BenchSetup {
        ...
        let half = head_dim / 2;
        BenchSetup::new(iron_rope_yarn::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
