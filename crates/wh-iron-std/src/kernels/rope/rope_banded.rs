...
//!
//! Different from `iron_rope` (in `kernels/rope/base.rs`):
//!   - generic dtype (iron_rope is f16-only)
...
//!
//! Codegen-only. Validated end-to-end in Iron integration tests.

use wh_iron::kernel;

#[kernel]
pub fn iron_rope_banded<T>(
    qk: Tensor<T>,
...
/// New-syntax correctness for `iron_rope_banded`. Grid3D `[T, n_heads, half_dim]`,
...
pub mod kernel_tests {
    ...
    fn rope_banded_setup(
        ...
        TestSetup::new(iron_rope_banded::kernel_ir_for(dt))
...
/// New-syntax benchmarks for `iron_rope_banded`: decode (T=1) and prefill (T=512).
pub mod kernel_benches {
    ...
    fn rope_banded_bench(dt: DType, t_len: usize, n_heads: usize, head_dim: usize) -> BenchSetup {
        ...
        BenchSetup::new(iron_rope_banded::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
