...
//!
//! Perf follow-up to `iron/patch_embed.rs`. The original kernel is one
...
pub mod kernel_tests {
    ...
    use super::iron_patch_embed_mma;
    ...
    fn mma_setup(
        ...
        TestSetup::new(iron_patch_embed_mma::kernel_ir_for(dt))
...
/// New-syntax bench for `iron_patch_embed_mma` (ViT-L-ish 8×8 patch, hidden 1024).
...
pub mod kernel_benches {
    ...
    fn bench_patch_embed_mma(dt: DType) -> BenchSetup {
        ...
        let patch_dim = in_ch * patch_h * patch_w;
        let n_out = num_patches * hidden;
        BenchSetup::new(iron_patch_embed_mma::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
