...
//! `iron_steel_gemm_splitk_nax` — split-K GEMM via `mpp::tensor_ops::matmul2d`.
//!
...
//!
//! ## Geometry (mirrors `iron_steel_gemm_fused_nax`)
//!
...
pub fn iron_steel_gemm_splitk_nax<T>(
    a: Tensor<T>,
    b: Tensor<T>,
...
#[kernel]
pub fn iron_steel_gemm_splitk_accum_nax<T>(
    partials: Tensor<f32>,
    mut out: Tensor<T>,
    #[constexpr] m: u32,
    #[constexpr] n: u32,
...
pub mod kernel_benches {
    ...
    use super::{iron_steel_gemm_splitk_accum_nax, iron_steel_gemm_splitk_nax};
    ...
    fn bench_splitk_nax(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_steel_gemm_splitk_nax::kernel_ir_for(dt))
    ...
    fn bench_splitk_accum_nax(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_steel_gemm_splitk_accum_nax::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
...
///   - **pass 1** (`iron_steel_gemm_splitk_nax`) — each `tgid_z` K-split
///     writes its partial `[M, N]` product (fp32) to `partials[split]`.
///     Oracle = per-split partial matmul over `[k_start, k_end)`.
///   - **pass 2** (`iron_steel_gemm_splitk_accum_nax`) — sums `n_splits`
...
pub mod kernel_tests {
    ...
    fn test_splitk_nax_pass1(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_steel_gemm_splitk_nax::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn test_splitk_accum_nax(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_steel_gemm_splitk_accum_nax::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
...
mod tests {
    ...
    fn splitk_kernel_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_steel_gemm_splitk_nax::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_steel_gemm_splitk_nax");
    ...
    fn accum_kernel_constructs() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_steel_gemm_splitk_accum_nax::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_steel_gemm_splitk_accum_nax");
    ...
    fn splitk_bf16_stages_through_half() {
        let k = iron_steel_gemm_splitk_nax::kernel_ir_for(DType::BF16);
    ...
    fn codegen_emits_mpp_include_and_kernel_decl() {
        for (dt, t_name) in [(DType::F32, "float"), (DType::F16, "half"), (DType::BF16, "half")] {
            let mut k = iron_steel_gemm_splitk_nax::kernel_ir_for(dt);
            let suffix = match dt {
                DType::F32 => "f32",
                DType::F16 => "f16",
                DType::BF16 => "bf16",
            ...
            k.name = format!("iron_steel_gemm_splitk_nax_{suffix}");
            ...
            assert!(msl.contains(&format!("kernel void iron_steel_gemm_splitk_nax_{suffix}")));
    ...
    fn codegen_emits_accum_kernel_decl() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let mut k = iron_steel_gemm_splitk_accum_nax::kernel_ir_for(dt);
            let suffix = match dt {
                DType::F32 => "f32",
                DType::F16 => "f16",
                DType::BF16 => "bf16",
            ...
            k.name = format!("iron_steel_gemm_splitk_accum_nax_{suffix}");
            let msl = MslGenerator::default().generate(&k).expect("codegen");
            assert!(
                msl.contains(&format!("kernel void iron_steel_gemm_splitk_accum_nax_{suffix}"))
            );
