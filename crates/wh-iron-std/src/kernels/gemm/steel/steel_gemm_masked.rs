...
///
/// Produces: `iron_steel_gemm_masked_64x64x16_2x2`, `_32x32x16_2x2`.
#[kernel(variants(
    BM = [64u32, 32u32],
    BN = [64u32, 32u32],
    WM = [2u32,  2u32],
...
pub fn iron_steel_gemm_masked<T>(
    a: Tensor<T>,
    b: Tensor<T>,
    out_mask: Tensor<T>,
    op_mask: Tensor<T>,
...
pub mod kernel_benches {
    ...
    fn bench_masked_64x64x16_2x2(dt: DType) -> BenchSetup {
        mb(iron_steel_gemm_masked_64x64x16_2x2::kernel_ir_for(dt), 64, 64, 128, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_masked_32x32x16_2x2(dt: DType) -> BenchSetup {
        mb(iron_steel_gemm_masked_32x32x16_2x2::kernel_ir_for(dt), 32, 32, 128, dt)
    }
}
...
pub mod kernel_tests {
    ...
    fn test_masked_64x64x16_2x2(dt: DType) -> TestSetup {
        masked_setup(iron_steel_gemm_masked_64x64x16_2x2::kernel_ir_for(dt), 64, 64, 128, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_masked_32x32x16_2x2(dt: DType) -> TestSetup {
        masked_setup(iron_steel_gemm_masked_32x32x16_2x2::kernel_ir_for(dt), 32, 32, 128, dt)
    }
}
