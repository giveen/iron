...
//! `iron_steel_gemm_gather_nax` — gather GEMM via `mpp::tensor_ops::matmul2d`.
//!
...
//! intrinsics — no `Op::InlineMsl`. It is exactly `iron_steel_gemm_fused_nax`
...
pub fn iron_steel_gemm_gather_nax<T>(
    a: Tensor<T>,
    b: Tensor<T>,
...
pub mod kernel_benches {
    ...
    use super::iron_steel_gemm_gather_nax;
    ...
    fn bench_gather_nax(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_steel_gemm_gather_nax::kernel_ir_for(dt))
...
pub mod kernel_tests {
    ...
    fn test_gather_nax(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_steel_gemm_gather_nax::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
...
mod tests {
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_steel_gemm_gather_nax::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_steel_gemm_gather_nax");
    ...
    fn bf16_stages_through_half() {
        let k = iron_steel_gemm_gather_nax::kernel_ir_for(DType::BF16);
    ...
    fn codegen_emits_mpp_include_and_kernel_decl() {
        for (dt, t_name) in [(DType::F32, "float"), (DType::F16, "half"), (DType::BF16, "half")] {
            let mut k = iron_steel_gemm_gather_nax::kernel_ir_for(dt);
            let suffix = match dt {
                DType::F32 => "f32",
                DType::F16 => "f16",
                DType::BF16 => "bf16",
            ...
            k.name = format!("iron_steel_gemm_gather_nax_{suffix}");
            ...
            assert!(msl.contains(&format!("kernel void iron_steel_gemm_gather_nax_{suffix}")));
