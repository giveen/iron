...
///
/// Produces: `iron_steel_gemm_gather_64x64x16_2x2`, `_32x32x16_2x2`.
#[kernel(variants(
    BM = [64u32, 32u32],
    BN = [64u32, 32u32],
    WM = [2u32,  2u32],
...
pub fn iron_steel_gemm_gather<T>(
    a: Tensor<T>,
    b: Tensor<T>,
...
pub mod kernel_benches {
    ...
    fn bench_gather_64x64x16_2x2(dt: DType) -> BenchSetup {
        gb(iron_steel_gemm_gather_64x64x16_2x2::kernel_ir_for(dt), 64, 64, 128, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_gather_32x32x16_2x2(dt: DType) -> BenchSetup {
        gb(iron_steel_gemm_gather_32x32x16_2x2::kernel_ir_for(dt), 32, 32, 128, dt)
    }
}
...
pub mod kernel_tests {
    ...
    fn test_gather_64x64x16_2x2(dt: DType) -> TestSetup {
        gather_setup(iron_steel_gemm_gather_64x64x16_2x2::kernel_ir_for(dt), 64, 64, 128, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_gather_32x32x16_2x2(dt: DType) -> TestSetup {
        gather_setup(iron_steel_gemm_gather_32x32x16_2x2::kernel_ir_for(dt), 32, 32, 128, dt)
    }
}
