...
///
/// Produces: `iron_steel_gemm_64x64x16_2x2`, `_32x32x16_2x2`, `_64x64x16_1x2`,
...
pub mod kernel_benches {
    ...
    fn bench_fused_64x64x16_2x2(dt: DType) -> BenchSetup {
        fb(iron_steel_gemm_64x64x16_2x2::kernel_ir_for(dt), 64, 64, 128, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fused_32x32x16_2x2(dt: DType) -> BenchSetup {
        fb(iron_steel_gemm_32x32x16_2x2::kernel_ir_for(dt), 32, 32, 128, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fused_64x64x16_1x2(dt: DType) -> BenchSetup {
        fb(iron_steel_gemm_64x64x16_1x2::kernel_ir_for(dt), 64, 64, 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fused_32x64x16_1x2(dt: DType) -> BenchSetup {
        fb(iron_steel_gemm_32x64x16_1x2::kernel_ir_for(dt), 32, 64, 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fused_64x64x16_4x2(dt: DType) -> BenchSetup {
        fb(iron_steel_gemm_64x64x16_4x2::kernel_ir_for(dt), 64, 64, 256, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fused_64x32x16_1x2(dt: DType) -> BenchSetup {
        fb(iron_steel_gemm_64x32x16_1x2::kernel_ir_for(dt), 64, 32, 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fused_32x32x16_1x2(dt: DType) -> BenchSetup {
        fb(iron_steel_gemm_32x32x16_1x2::kernel_ir_for(dt), 32, 32, 64, dt)
    }
}
...
pub mod kernel_tests {
    ...
    fn test_fused_64x64x16_2x2(dt: DType) -> TestSetup {
        fused_setup(iron_steel_gemm_64x64x16_2x2::kernel_ir_for(dt), 64, 64, 128, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fused_32x32x16_2x2(dt: DType) -> TestSetup {
        fused_setup(iron_steel_gemm_32x32x16_2x2::kernel_ir_for(dt), 32, 32, 128, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fused_64x64x16_1x2(dt: DType) -> TestSetup {
        fused_setup(iron_steel_gemm_64x64x16_1x2::kernel_ir_for(dt), 64, 64, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fused_32x64x16_1x2(dt: DType) -> TestSetup {
        fused_setup(iron_steel_gemm_32x64x16_1x2::kernel_ir_for(dt), 32, 64, 64, dt)
    }
}
