...
//! `iron_fp4_qmm_mma` / `iron_fp8_e4m3_qmm_mma` — fp4/fp8 simdgroup-matrix MMA.
//!
//! Simdgroup-matrix MMA prefill path for fp4 (E2M1) and fp8 (E4M3) quantized
//! dense GEMM — the non-NAX counterpart of `iron_fp_qmm_nax`. Falls back from
//! `iron_fp_qmm_nax` on pre-M4 hardware (no Apple tensor cores).
//!
//! ## fp4 E2M1 codebook
//!
...
//! `[sign:1][exp:4][mantissa:3]`. Dequant follows the `iron_fp8_e4m3_quant_dequant`
...
//!   sign: `1 - 2*(code >> 7)`.
//! Scale per group (group_size=32 for fp8, matching `iron_fp8_e4m3_quant_dequant`).
//!
//! ## Geometry (both kernels)
//!
//! Identical to `iron_qmm_mma`:
...
/// on `WDEC` (fp4: 1 pass × 8 nibbles via `iron_decode_e2m1`; fp8: 2 passes × 4
/// bytes, inline E4M3). Produces `iron_fp4_qmm_mma` / `iron_fp8_e4m3_qmm_mma`.
    ...
    for kb in range(0u32, k, 32u32) {
        ...
        if WDEC == 0u32 {
            ...
            for _ci in range(0u32, 8u32, 1u32) {
                ...
                let val = s * iron_decode_e2m1(nibble);
        ...
        threadgroup_barrier();
        // ── 3. MMA inner loop — identical to iron_qmm_mma ──
...
pub mod kernel_tests {
    ...
    use super::{iron_fp4_qmm_mma, iron_fp8_e4m3_qmm_mma};
    ...
    pub(crate) fn fp8_iron_decode_e4m3(code: u32) -> f32 {
            ...
            for nc in 0..n {
                ...
                for d in 0..k {
                    ...
                    let dec = if bits == 4 {
                        ...
                        fp8_iron_decode_e4m3(codes[nc * k + d])
                    };
    ...
    fn test_fp4_qmm_mma(dt: DType) -> TestSetup {
        fp_setup(iron_fp4_qmm_mma::kernel_ir_for(dt), 32, 32, 128, 4, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 1e-2, 5e-2])]
    fn test_fp8_e4m3_qmm_mma(dt: DType) -> TestSetup {
        fp_setup(iron_fp8_e4m3_qmm_mma::kernel_ir_for(dt), 32, 32, 128, 8, dt)
    }
    ...
    fn test_fp4_qmm_mma_multi_tile(dt: DType) -> TestSetup {
        fp_setup(iron_fp4_qmm_mma::kernel_ir_for(dt), 64, 64, 128, 4, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 1e-2, 5e-2])]
    fn test_fp8_e4m3_qmm_mma_multi_tile(dt: DType) -> TestSetup {
        fp_setup(iron_fp8_e4m3_qmm_mma::kernel_ir_for(dt), 64, 64, 128, 8, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_fp4_qmm_mma(dt: DType) -> BenchSetup {
        fpb(iron_fp4_qmm_mma::kernel_ir_for(dt), 32, 4096, 4096, 4, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_qmm_mma(dt: DType) -> BenchSetup {
        fpb(iron_fp8_e4m3_qmm_mma::kernel_ir_for(dt), 32, 4096, 4096, 8, dt)
    }
}
