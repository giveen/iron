...
#[kernel]
pub fn iron_gemv<T>(mat: Tensor<T>, vec: Tensor<T>, out: Tensor<T>, #[constexpr] k: u32) {
...
/// New-syntax correctness for `iron_gemv` (Reduction, one threadgroup per row;
...
pub mod kernel_tests {
    ...
    fn test_iron_gemv(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_gemv::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_gemv` (vs MLX `metal/gemv.metal`).
pub mod kernel_benches {
    ...
    fn bench_gemv(dt: DType) -> BenchSetup {
        ...
        // axpby-only and left unbound. `mat`/`vec` are shared by name with the Iron
        // inputs (placeholders) so both kernels see identical data. tol floor 1e-2
        // is the legacy gemv reduction floor (Iron folds the dot in f32; MLX
        ...
        BenchSetup::new(iron_gemv::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            ...
            .with_reference(
                ...
                // mat[[0]] / in_vec[[1]] shared by name with the Iron inputs.
