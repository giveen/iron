...
/// New-syntax correctness for `iron_gemv_masked` (`out[r] = Σ_j mat[r,j]·vec[j]·mask[j]`).
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_gemv_masked;
    use crate::utils::{pack_f32, unpack_f32};

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 1e-1, 1.0])]
    fn test_iron_gemv_masked(dt: DType) -> TestSetup {
        ...
        let expected: Vec<f32> = (0..m)
            ...
            .collect();
        TestSetup::new(iron_gemv_masked::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_gemv_masked`.
pub mod kernel_benches {
    ...
    fn bench_gemv_masked(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gemv_masked::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
