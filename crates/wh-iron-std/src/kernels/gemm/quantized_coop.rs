...
//! `iron_qmm_coop` - wide-N cooperative-dequant int4 quantized matmul.
//!
//! Isolated benching of `iron_qmm_mma` (manual 8x8 `simdgroup_matmul`) vs
//! `iron_qmm_mma_mpp` (single `mpp::tensor_ops::matmul2d` call per SG per
//! K-block, BN=32) on this M5 Max showed the MPP/tensor-unit path already
//! closes most of the gap to the design target on its own (`iron_qmm_mma`
//! flat ~23% of 57 TFLOP/s peak at every shape; `iron_qmm_mma_mpp` reaches
...
//!      `iron_qmm_mma_mpp` - 128 lanes x 8 contiguous K-elems).
//!   2. W-tile coop-dequant int4, twice the row range of
//!      `iron_qmm_mma_mpp` (`w_row` spans 0..64 via a 2-step loop over
...
//! `iron_qmm_mma`/`iron_qmm_mma_mpp`; `matmul2d`'s cooperative-tensor
...
//! documented on `iron_gated_delta_prep_chunk`/`_fast` - see
//! `crates/wh-iron-std/src/kernels/ssm/gated_delta_prep_chunk.rs`),
//! same as `iron_qmm_mma_mpp`'s single accumulator. `k`/`n`/`gs_per_row`
...
//! tile/loop-bound constants the same way `iron_qmm_mma_mpp`'s already do
...
pub fn iron_qmm_coop<T>(
    ...
    // shape as iron_qmm_mma_mpp, run twice with a 32-row offset.
    ...
    // iron_qmm_mma_mpp's single-accumulator OutScratch size).
...
mod tests {
    ...
    fn kernel_ir_uses_two_coop_tile_ops_no_inline_msl() {
        ...
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_qmm_coop::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_qmm_coop");
            let all_ops =
    ...
    fn dump() {
        ...
        let mut k = iron_qmm_coop::kernel_ir_for(DType::F16);
...
/// New-syntax correctness for `iron_qmm_coop`. Shares the affine-int4 CPU
...
/// kernel is invisible to `iron build --emit` (build discovery walks the
