...
//! dispatch. Sibling of `iron_batched_qkv_qgemv_fast` (3-output) and
//! `iron_batched_qkv_qmm_fast` (3-output, M>1).
//!
...
#[kernel]
pub fn iron_batched_4_qgemv_fast<T>(
    x: Tensor<T>,
    w_a: Tensor<u32>,
    scales_a: Tensor<T>,
    biases_a: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::iron_batched_4_qgemv_fast;
    ...
    fn test_batched_4_qgemv_fast(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_batched_4_qgemv_fast::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_batched_4_qgemv_fast` — MLX-less reduction
...
pub mod kernel_benches {
    ...
    fn bench_batched_4_qgemv_fast(dt: DType) -> BenchSetup {
        ...
        let n_tgs = out_a.max(out_b).max(out_c).max(out_d).div_ceil(8);
        BenchSetup::new(iron_batched_4_qgemv_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
