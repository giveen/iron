...
//!
//! ## DISPATCH INVARIANTS (identical to `iron_qmm_mma`)
//!
    ...
    for kb in range(0u32, k, 32u32) {
        ...
        if WDEC == 0u32 {
            ...
            for i in range(0u32, 8u32, 1u32) {
                ...
                threadgroup_store("ws", ws_base + i, (iron_decode_e2m1(nib) * scale).cast::<T>());
            }
        } else if WDEC == 1u32 {
            for i in range(0u32, 8u32, 1u32) {
...
pub mod kernel_tests {
    ...
    fn test_mxfp4_qmm_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxfp4_qmm_mma::kernel_ir_for(dt), QFormat::Mxfp4, 32, 32, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_nvfp4_qmm_mma(dt: DType) -> TestSetup {
        mma_setup(iron_nvfp4_qmm_mma::kernel_ir_for(dt), QFormat::Nvfp4, 32, 32, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxfp8_e4m3_qmm_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxfp8_e4m3_qmm_mma::kernel_ir_for(dt), QFormat::Mxfp8E4, 32, 32, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxfp8_e5m2_qmm_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxfp8_e5m2_qmm_mma::kernel_ir_for(dt), QFormat::Mxfp8E5, 32, 32, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_nvfp8_qmm_mma(dt: DType) -> TestSetup {
        mma_setup(iron_nvfp8_qmm_mma::kernel_ir_for(dt), QFormat::Nvfp8, 32, 32, 64, dt)
    }
    ...
    fn test_fp4_mma(dt: DType) -> TestSetup {
        mma_setup(iron_fp4_float_qmm_mma::kernel_ir_for(dt), QFormat::Fp4, 32, 32, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_fp8_e4m3_mma(dt: DType) -> TestSetup {
        mma_setup(iron_nvfp8_qmm_mma::kernel_ir_for(dt), QFormat::Fp8E4m3, 32, 32, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_fp8_e5m2_mma(dt: DType) -> TestSetup {
        mma_setup(iron_fp8_e5m2_qmm_mma::kernel_ir_for(dt), QFormat::Fp8E5m2, 32, 32, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int8_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int8_qmm_mma::kernel_ir_for(dt), QFormat::Int8, 32, 32, 64, dt)
    }
    ...
    fn test_int2_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int2_qmm_mma::kernel_ir_for(dt), QFormat::Int2, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int3_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int3_qmm_mma::kernel_ir_for(dt), QFormat::Int3, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int4_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int4_qmm_mma::kernel_ir_for(dt), QFormat::Int4, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int5_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int5_qmm_mma::kernel_ir_for(dt), QFormat::Int5, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int6_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int6_qmm_mma::kernel_ir_for(dt), QFormat::Int6, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxint2_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxint2_qmm_mma::kernel_ir_for(dt), QFormat::Mxint2, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxint3_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxint3_qmm_mma::kernel_ir_for(dt), QFormat::Mxint3, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxint4_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxint4_qmm_mma::kernel_ir_for(dt), QFormat::Mxint4, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxint5_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxint5_qmm_mma::kernel_ir_for(dt), QFormat::Mxint5, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxint6_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxint6_qmm_mma::kernel_ir_for(dt), QFormat::Mxint6, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_mxint8_mma(dt: DType) -> TestSetup {
        mma_setup(iron_mxint8_qmm_mma::kernel_ir_for(dt), QFormat::Mxint8, 32, 32, 64, dt)
    }
    ...
    fn test_nvfp8_f16_qmm_mma(dt: DType) -> TestSetup {
        mma_setup(iron_nvfp8_f16_qmm_mma::kernel_ir_for(dt), QFormat::Nvfp8F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_fp8_e4m3_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_nvfp8_f16_qmm_mma::kernel_ir_for(dt), QFormat::Fp8E4m3F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_fp4_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_fp4_f16_qmm_mma::kernel_ir_for(dt), QFormat::Fp4F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_fp8_e5m2_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_fp8_e5m2_f16_qmm_mma::kernel_ir_for(dt), QFormat::Fp8E5m2F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int2_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int2_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int2F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int3_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int3_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int3F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int4_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int4_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int4F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int5_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int5_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int5F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int6_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int6_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int6F16, 32, 32, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 4e-1])]
    fn test_int8_f16_mma(dt: DType) -> TestSetup {
        mma_setup(iron_int8_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int8F16, 32, 32, 64, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_mxfp4_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_mxfp4_qmm_mma::kernel_ir_for(dt), QFormat::Mxfp4, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp4_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_nvfp4_qmm_mma::kernel_ir_for(dt), QFormat::Nvfp4, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e4m3_mma(dt: DType) -> BenchSetup {
        mma_bench(
            iron_mxfp8_e4m3_qmm_mma::kernel_ir_for(dt),
            QFormat::Mxfp8E4,
            4096,
            4096,
            4096,
    ...
    fn bench_mxfp8_e5m2_mma(dt: DType) -> BenchSetup {
        mma_bench(
            iron_mxfp8_e5m2_qmm_mma::kernel_ir_for(dt),
            QFormat::Mxfp8E5,
            4096,
            4096,
            4096,
    ...
    fn bench_nvfp8_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_nvfp8_qmm_mma::kernel_ir_for(dt), QFormat::Nvfp8, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp4_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_fp4_float_qmm_mma::kernel_ir_for(dt), QFormat::Fp4, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_nvfp8_qmm_mma::kernel_ir_for(dt), QFormat::Fp8E4m3, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_fp8_e5m2_qmm_mma::kernel_ir_for(dt), QFormat::Fp8E5m2, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int8_qmm_mma::kernel_ir_for(dt), QFormat::Int8, 4096, 4096, 4096, dt)
    }
    // Symmetric sub-byte ints (FP32 group scale) + MXINT (E8M0 block scale) +
    // MXINT8 (8-bit, E8M0). K=4096 is a multiple of 32 and every block size.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int2_qmm_mma::kernel_ir_for(dt), QFormat::Int2, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int3_qmm_mma::kernel_ir_for(dt), QFormat::Int3, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int4_qmm_mma::kernel_ir_for(dt), QFormat::Int4, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int5_qmm_mma::kernel_ir_for(dt), QFormat::Int5, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int6_qmm_mma::kernel_ir_for(dt), QFormat::Int6, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint2_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_mxint2_qmm_mma::kernel_ir_for(dt), QFormat::Mxint2, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint3_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_mxint3_qmm_mma::kernel_ir_for(dt), QFormat::Mxint3, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint4_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_mxint4_qmm_mma::kernel_ir_for(dt), QFormat::Mxint4, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint5_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_mxint5_qmm_mma::kernel_ir_for(dt), QFormat::Mxint5, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint6_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_mxint6_qmm_mma::kernel_ir_for(dt), QFormat::Mxint6, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint8_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_mxint8_qmm_mma::kernel_ir_for(dt), QFormat::Mxint8, 4096, 4096, 4096, dt)
    }
    ...
    fn bench_nvfp8_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(
            iron_nvfp8_f16_qmm_mma::kernel_ir_for(dt),
            QFormat::Nvfp8F16,
            4096,
            4096,
            4096,
    ...
    fn bench_fp4_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_fp4_f16_qmm_mma::kernel_ir_for(dt), QFormat::Fp4F16, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(
            iron_fp8_e5m2_f16_qmm_mma::kernel_ir_for(dt),
            QFormat::Fp8E5m2F16,
            4096,
            4096,
            4096,
    ...
    fn bench_int2_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int2_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int2F16, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int3_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int3F16, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int4_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int4F16, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int5_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int5F16, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int6_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int6F16, 4096, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_f16_mma(dt: DType) -> BenchSetup {
        mma_bench(iron_int8_f16_qmm_mma::kernel_ir_for(dt), QFormat::Int8F16, 4096, 4096, 4096, dt)
    }
}
