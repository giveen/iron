...
//! `iron_qmm_nax_int8` — production int8 quantized matmul via `mpp::tensor_ops::matmul2d`.
//!
//! Int8 NAX counterpart of `iron_qmm_nax` (int4). Algorithmically identical
//! to `iron_qmm_mma_mpp_int8` — the two co-exist so consumers can select
...
mod tests {
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_qmm_nax_int8::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_qmm_nax_int8");
    ...
    fn bf16_stages_through_half() {
        let k = iron_qmm_nax_int8::kernel_ir_for(DType::BF16);
    ...
    fn codegen_emits_mpp_include_and_kernel_decl() {
        for (dt, t_name) in [(DType::F32, "float"), (DType::F16, "half"), (DType::BF16, "half")] {
            let mut k = iron_qmm_nax_int8::kernel_ir_for(dt);
            let suffix = match dt {
                DType::F32 => "f32",
                DType::F16 => "f16",
                DType::BF16 => "bf16",
            ...
            k.name = format!("iron_qmm_nax_int8_{suffix}");
            ...
            assert!(msl.contains(&format!("kernel void iron_qmm_nax_int8_{suffix}")));
            assert!(msl.contains(&format!("threadgroup {t_name} Xs")));
        }
    }
}

/// New-syntax correctness + benchmark for `iron_qmm_nax_int8` (MPP/NAX tensor-core
...
/// New-syntax benchmark for `iron_qmm_nax_int8`.
