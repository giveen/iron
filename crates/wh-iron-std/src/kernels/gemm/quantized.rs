...
// ─── iron_qmm ─────────────────────────────────────────────────────────────
//
// Quantized matmul (B>1 / prefill). Same int4 weight layout as `iron_qmv`
// extended along the M axis (token count). Each threadgroup owns 8
// consecutive output columns at one M-row — `iron_qmv`'s 2 SG × 4 N-row
// tile lifted into M via an outer grid axis (`tgid_y = m_row`). The
// inner K-walk is bit-identical to `iron_qmv`: each lane caches 16 X
...
//   out     [m, n]                 T
//
// At M = 1 this is byte-identical to `iron_qmv`. At M > 1 each M-row
...
pub fn iron_qmm<T>(
    ...
    for _b in range(0u32, k, 512u32) {
        ...
        let xi14 = xb + 14u32;
        let xi15 = xb + 15u32;
        // Mask-without-shift constants. Same as iron_qmv.
        let s_16 = 0.0625f32;
        let s_256 = 0.00390625f32;
        let s_4096 = 0.000244140625f32;
        let x0 = load(x[xi0]).cast::<f32>();
        ...
        let x14 = x14_raw * s_256;
        let x15 = x15_raw * s_4096;
        // Group index within this row's K dimension. iron_qmv uses
...
// `iron_qmm`, but lifts BM=2 M-rows into the same threadgroup so the W
...
// ─── iron_qmm_bm3 ─────────────────────────────────────────────────────────
//
// Quantized matmul BM3: BM × BN output tile with 3× W reuse.
//
// Same int4 layout + 8-output 2 SG × 4 N-row geometry as iron_qmm and
// iron_qmm_bm2, but lifts BM=3 M-rows into the same threadgroup so the
// W packs + nibble extractions are loaded ONCE per K-block per N-row
// and reused across all three M-rows. Per K-block per TG: 8 W loads
// (unchanged from v2/bm2) producing 24 outputs (vs bm2's 16, v2's 8).
// W bandwidth per output drops to 1/3 of v2.
//
// Geometry:
//   tpg = 64 = 2 SG × 32 lanes
//   BM = 3 (M-rows per TG)
//   BN = 8 (N-rows per TG, each SG owns 4)
//   24 outputs per TG (BM × BN)
//   Grid: [n / 8, m / 3, 1]
//
// Register footprint per lane (f32):
//   24 X values (8 per M-row × 3 M-rows) = 96 bytes
//   12 accumulators (4 N-rows × 3 M-rows) = 48 bytes
//   8 W nibble extracts (shared) = 32 bytes
//   About 176 bytes before compiler temporaries.
//
// Targets M=3, 6, and 9 without padding to a four-row tile.
#[kernel]
pub fn iron_qmm_bm3<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
    ...
    let sb_base3 = row3 * gs_per_row;
    // BM=3 M-rows per TG.
    let m_row_a = m_tile * 3u32;
    let m_row_b = m_row_a + 1u32;
    let m_row_c = m_row_a + 2u32;
    let x_base_a = m_row_a * k;
    let x_base_b = m_row_b * k;
    let x_base_c = m_row_c * k;
    // 12 accumulators: 4 N-rows × 3 M-rows.
    let mut acc0_a = 0.0f32;
    let mut acc0_b = 0.0f32;
    ...
    for _b in range(0u32, k, 256u32) {
        ...
        let pack_off = _b / 8u32 + lane_pack_off;
        // ── Row 0 (W extracts shared across all 3 M-rows) — 8 nibbles ──
        let p00 = load(w[w_base0 + pack_off]);
        let p00_hi = p00 >> 16u32;
        let s0 = load(scales[sb_base0 + g]).cast::<f32>();
        let bi0 = load(biases[sb_base0 + g]).cast::<f32>();
    ...
    // Cross-lane reduce + lane-0 stores. 24 outputs per TG.
    let r0_a = simd_sum(acc0_a);
    let r0_b = simd_sum(acc0_b);
...
//
// Same int4 layout + 8-output 2 SG × 4 N-row geometry as iron_qmm and
// iron_qmm_bm2, but lifts BM=4 M-rows into the same threadgroup so the
// W packs + nibble extractions are loaded ONCE per K-block per N-row
...
// ─── iron_qmm_bm8 ─────────────────────────────────────────────────────────
//
// Quantized matmul v5 — BM × BN output tile with 8× W reuse. Scalar
// (non-MMA) follow-up motivated by `iron_qmm_mma_bm8`: that simdgroup-
// matrix MMA kernel LOST to `iron_qmm_bm4` in the speculative-decode
// verify γ=7 (T=8) bench (see benchmarks/qwen3x/README.md). This kernel
// tests whether a plain scalar extension of bm4's geometry to BM=8 —
// no MMA, no TG-memory dtype staging — fares better by simply halving
// W traffic again relative to bm4, at the cost of doubled per-lane
// register pressure.
//
// Same int4 weight layout + 8-output 2 SG × 4 N-row geometry as
// `iron_qmm_bm4`, but lifts BM=8 M-rows into the same threadgroup so
// the W packs + nibble extractions are loaded ONCE per K-block per
// N-row and reused across all eight M-rows. Per K-block per TG: 8 W
// loads (unchanged from v2/bm2/bm4) producing 64 outputs (vs bm4's 32,
// bm2's 16, v2's 8). W bandwidth per output drops to 1/8 of v2, half
// of bm4.
//
// Geometry:
//   tpg = 64 = 2 SG × 32 lanes
//   BM = 8 (M-rows per TG)
//   BN = 8 (N-rows per TG, each SG owns 4)
//   64 outputs per TG (BM × BN)
//   Grid: [n / 8, m / 8, 1]
//
// Register footprint per lane (f32):
//   128 X values (16 per M-row × 8 M-rows) = 512 bytes
//   32 accumulators (4 N-rows × 8 M-rows)  = 128 bytes
//   16 W nibble extracts (shared)          =  64 bytes
//   ≈ 800 bytes — near Apple GPU's ~1024 byte/lane register file
//   budget. bm4 sat at ≈440B/lane; bm8 roughly doubles both live-X and
//   accumulator footprint, so occupancy may halve again from bm4's
//   already-reduced level (~6 SGs/SM → ~3 SGs/SM). That's the
//   experiment this kernel runs: does the 8× W-bandwidth reduction
//   outrun a further occupancy hit at T=8, or does bm4 remain the
//   better marginal-cost point for the γ=7 verify batch (as it already
//   beat the MMA-class bm8 attempt)?
#[kernel]
pub fn iron_qmm_bm8<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
    ...
    let sb_base3 = row3 * gs_per_row;
    // BM=8 M-rows per TG.
    let m_row_m0 = m_tile * 8u32;
    let m_row_m1 = m_row_m0 + 1u32;
    let m_row_m2 = m_row_m0 + 2u32;
    let m_row_m3 = m_row_m0 + 3u32;
    let m_row_m4 = m_row_m0 + 4u32;
    let m_row_m5 = m_row_m0 + 5u32;
    let m_row_m6 = m_row_m0 + 6u32;
    let m_row_m7 = m_row_m0 + 7u32;
    let x_base_m0 = m_row_m0 * k;
    let x_base_m1 = m_row_m1 * k;
    let x_base_m2 = m_row_m2 * k;
    let x_base_m3 = m_row_m3 * k;
    let x_base_m4 = m_row_m4 * k;
    let x_base_m5 = m_row_m5 * k;
    let x_base_m6 = m_row_m6 * k;
    let x_base_m7 = m_row_m7 * k;
    // 32 accumulators: 4 N-rows × 8 M-rows.
    let mut acc0_m0 = 0.0f32;
    let mut acc0_m1 = 0.0f32;
    let mut acc0_m2 = 0.0f32;
    let mut acc0_m3 = 0.0f32;
    let mut acc0_m4 = 0.0f32;
    let mut acc0_m5 = 0.0f32;
    let mut acc0_m6 = 0.0f32;
    let mut acc0_m7 = 0.0f32;
    let mut acc1_m0 = 0.0f32;
    let mut acc1_m1 = 0.0f32;
    let mut acc1_m2 = 0.0f32;
    let mut acc1_m3 = 0.0f32;
    let mut acc1_m4 = 0.0f32;
    let mut acc1_m5 = 0.0f32;
    let mut acc1_m6 = 0.0f32;
    let mut acc1_m7 = 0.0f32;
    let mut acc2_m0 = 0.0f32;
    let mut acc2_m1 = 0.0f32;
    let mut acc2_m2 = 0.0f32;
    let mut acc2_m3 = 0.0f32;
    let mut acc2_m4 = 0.0f32;
    let mut acc2_m5 = 0.0f32;
    let mut acc2_m6 = 0.0f32;
    let mut acc2_m7 = 0.0f32;
    let mut acc3_m0 = 0.0f32;
    let mut acc3_m1 = 0.0f32;
    let mut acc3_m2 = 0.0f32;
    let mut acc3_m3 = 0.0f32;
    let mut acc3_m4 = 0.0f32;
    let mut acc3_m5 = 0.0f32;
    let mut acc3_m6 = 0.0f32;
    let mut acc3_m7 = 0.0f32;
    // Same K-block=256 layout as bm4: each lane owns 8 X / 1 W pack per
    // iter; 32 lanes = 256 K per block.
    let lane_x_off = lane * 8u32;
    let lane_pack_off = lane;
    for _b in range(0u32, k, 256u32) {
        let s_16 = 0.0625f32;
        let s_256 = 0.00390625f32;
        let s_4096 = 0.000244140625f32;
        // ── Load 8 X values for M-row m0 ──
        let xb_m0 = x_base_m0 + _b + lane_x_off;
        let x0_m0 = load(x[xb_m0]).cast::<f32>();
        let x1_m0_raw = load(x[xb_m0 + 1u32]).cast::<f32>();
        let x2_m0_raw = load(x[xb_m0 + 2u32]).cast::<f32>();
        let x3_m0_raw = load(x[xb_m0 + 3u32]).cast::<f32>();
        let x4_m0 = load(x[xb_m0 + 4u32]).cast::<f32>();
        let x5_m0_raw = load(x[xb_m0 + 5u32]).cast::<f32>();
        let x6_m0_raw = load(x[xb_m0 + 6u32]).cast::<f32>();
        let x7_m0_raw = load(x[xb_m0 + 7u32]).cast::<f32>();
        let xs_m0 =
            x0_m0 + x1_m0_raw + x2_m0_raw + x3_m0_raw + x4_m0 + x5_m0_raw + x6_m0_raw + x7_m0_raw;
        let x1_m0 = x1_m0_raw * s_16;
        let x2_m0 = x2_m0_raw * s_256;
        let x3_m0 = x3_m0_raw * s_4096;
        let x5_m0 = x5_m0_raw * s_16;
        let x6_m0 = x6_m0_raw * s_256;
        let x7_m0 = x7_m0_raw * s_4096;
        // ── Load 8 X values for M-row m1 ──
        let xb_m1 = x_base_m1 + _b + lane_x_off;
        let x0_m1 = load(x[xb_m1]).cast::<f32>();
        let x1_m1_raw = load(x[xb_m1 + 1u32]).cast::<f32>();
        let x2_m1_raw = load(x[xb_m1 + 2u32]).cast::<f32>();
        let x3_m1_raw = load(x[xb_m1 + 3u32]).cast::<f32>();
        let x4_m1 = load(x[xb_m1 + 4u32]).cast::<f32>();
        let x5_m1_raw = load(x[xb_m1 + 5u32]).cast::<f32>();
        let x6_m1_raw = load(x[xb_m1 + 6u32]).cast::<f32>();
        let x7_m1_raw = load(x[xb_m1 + 7u32]).cast::<f32>();
        let xs_m1 =
            x0_m1 + x1_m1_raw + x2_m1_raw + x3_m1_raw + x4_m1 + x5_m1_raw + x6_m1_raw + x7_m1_raw;
        let x1_m1 = x1_m1_raw * s_16;
        let x2_m1 = x2_m1_raw * s_256;
        let x3_m1 = x3_m1_raw * s_4096;
        let x5_m1 = x5_m1_raw * s_16;
        let x6_m1 = x6_m1_raw * s_256;
        let x7_m1 = x7_m1_raw * s_4096;
        // ── Load 8 X values for M-row m2 ──
        let xb_m2 = x_base_m2 + _b + lane_x_off;
        let x0_m2 = load(x[xb_m2]).cast::<f32>();
        let x1_m2_raw = load(x[xb_m2 + 1u32]).cast::<f32>();
        let x2_m2_raw = load(x[xb_m2 + 2u32]).cast::<f32>();
        let x3_m2_raw = load(x[xb_m2 + 3u32]).cast::<f32>();
        let x4_m2 = load(x[xb_m2 + 4u32]).cast::<f32>();
        let x5_m2_raw = load(x[xb_m2 + 5u32]).cast::<f32>();
        let x6_m2_raw = load(x[xb_m2 + 6u32]).cast::<f32>();
        let x7_m2_raw = load(x[xb_m2 + 7u32]).cast::<f32>();
        let xs_m2 =
            x0_m2 + x1_m2_raw + x2_m2_raw + x3_m2_raw + x4_m2 + x5_m2_raw + x6_m2_raw + x7_m2_raw;
        let x1_m2 = x1_m2_raw * s_16;
        let x2_m2 = x2_m2_raw * s_256;
        let x3_m2 = x3_m2_raw * s_4096;
        let x5_m2 = x5_m2_raw * s_16;
        let x6_m2 = x6_m2_raw * s_256;
        let x7_m2 = x7_m2_raw * s_4096;
        // ── Load 8 X values for M-row m3 ──
        let xb_m3 = x_base_m3 + _b + lane_x_off;
        let x0_m3 = load(x[xb_m3]).cast::<f32>();
        let x1_m3_raw = load(x[xb_m3 + 1u32]).cast::<f32>();
        let x2_m3_raw = load(x[xb_m3 + 2u32]).cast::<f32>();
        let x3_m3_raw = load(x[xb_m3 + 3u32]).cast::<f32>();
        let x4_m3 = load(x[xb_m3 + 4u32]).cast::<f32>();
        let x5_m3_raw = load(x[xb_m3 + 5u32]).cast::<f32>();
        let x6_m3_raw = load(x[xb_m3 + 6u32]).cast::<f32>();
        let x7_m3_raw = load(x[xb_m3 + 7u32]).cast::<f32>();
        let xs_m3 =
            x0_m3 + x1_m3_raw + x2_m3_raw + x3_m3_raw + x4_m3 + x5_m3_raw + x6_m3_raw + x7_m3_raw;
        let x1_m3 = x1_m3_raw * s_16;
        let x2_m3 = x2_m3_raw * s_256;
        let x3_m3 = x3_m3_raw * s_4096;
        let x5_m3 = x5_m3_raw * s_16;
        let x6_m3 = x6_m3_raw * s_256;
        let x7_m3 = x7_m3_raw * s_4096;
        // ── Load 8 X values for M-row m4 ──
        let xb_m4 = x_base_m4 + _b + lane_x_off;
        let x0_m4 = load(x[xb_m4]).cast::<f32>();
        let x1_m4_raw = load(x[xb_m4 + 1u32]).cast::<f32>();
        let x2_m4_raw = load(x[xb_m4 + 2u32]).cast::<f32>();
        let x3_m4_raw = load(x[xb_m4 + 3u32]).cast::<f32>();
        let x4_m4 = load(x[xb_m4 + 4u32]).cast::<f32>();
        let x5_m4_raw = load(x[xb_m4 + 5u32]).cast::<f32>();
        let x6_m4_raw = load(x[xb_m4 + 6u32]).cast::<f32>();
        let x7_m4_raw = load(x[xb_m4 + 7u32]).cast::<f32>();
        let xs_m4 =
            x0_m4 + x1_m4_raw + x2_m4_raw + x3_m4_raw + x4_m4 + x5_m4_raw + x6_m4_raw + x7_m4_raw;
        let x1_m4 = x1_m4_raw * s_16;
        let x2_m4 = x2_m4_raw * s_256;
        let x3_m4 = x3_m4_raw * s_4096;
        let x5_m4 = x5_m4_raw * s_16;
        let x6_m4 = x6_m4_raw * s_256;
        let x7_m4 = x7_m4_raw * s_4096;
        // ── Load 8 X values for M-row m5 ──
        let xb_m5 = x_base_m5 + _b + lane_x_off;
        let x0_m5 = load(x[xb_m5]).cast::<f32>();
        let x1_m5_raw = load(x[xb_m5 + 1u32]).cast::<f32>();
        let x2_m5_raw = load(x[xb_m5 + 2u32]).cast::<f32>();
        let x3_m5_raw = load(x[xb_m5 + 3u32]).cast::<f32>();
        let x4_m5 = load(x[xb_m5 + 4u32]).cast::<f32>();
        let x5_m5_raw = load(x[xb_m5 + 5u32]).cast::<f32>();
        let x6_m5_raw = load(x[xb_m5 + 6u32]).cast::<f32>();
        let x7_m5_raw = load(x[xb_m5 + 7u32]).cast::<f32>();
        let xs_m5 =
            x0_m5 + x1_m5_raw + x2_m5_raw + x3_m5_raw + x4_m5 + x5_m5_raw + x6_m5_raw + x7_m5_raw;
        let x1_m5 = x1_m5_raw * s_16;
        let x2_m5 = x2_m5_raw * s_256;
        let x3_m5 = x3_m5_raw * s_4096;
        let x5_m5 = x5_m5_raw * s_16;
        let x6_m5 = x6_m5_raw * s_256;
        let x7_m5 = x7_m5_raw * s_4096;
        // ── Load 8 X values for M-row m6 ──
        let xb_m6 = x_base_m6 + _b + lane_x_off;
        let x0_m6 = load(x[xb_m6]).cast::<f32>();
        let x1_m6_raw = load(x[xb_m6 + 1u32]).cast::<f32>();
        let x2_m6_raw = load(x[xb_m6 + 2u32]).cast::<f32>();
        let x3_m6_raw = load(x[xb_m6 + 3u32]).cast::<f32>();
        let x4_m6 = load(x[xb_m6 + 4u32]).cast::<f32>();
        let x5_m6_raw = load(x[xb_m6 + 5u32]).cast::<f32>();
        let x6_m6_raw = load(x[xb_m6 + 6u32]).cast::<f32>();
        let x7_m6_raw = load(x[xb_m6 + 7u32]).cast::<f32>();
        let xs_m6 =
            x0_m6 + x1_m6_raw + x2_m6_raw + x3_m6_raw + x4_m6 + x5_m6_raw + x6_m6_raw + x7_m6_raw;
        let x1_m6 = x1_m6_raw * s_16;
        let x2_m6 = x2_m6_raw * s_256;
        let x3_m6 = x3_m6_raw * s_4096;
        let x5_m6 = x5_m6_raw * s_16;
        let x6_m6 = x6_m6_raw * s_256;
        let x7_m6 = x7_m6_raw * s_4096;
        // ── Load 8 X values for M-row m7 ──
        let xb_m7 = x_base_m7 + _b + lane_x_off;
        let x0_m7 = load(x[xb_m7]).cast::<f32>();
        let x1_m7_raw = load(x[xb_m7 + 1u32]).cast::<f32>();
        let x2_m7_raw = load(x[xb_m7 + 2u32]).cast::<f32>();
        let x3_m7_raw = load(x[xb_m7 + 3u32]).cast::<f32>();
        let x4_m7 = load(x[xb_m7 + 4u32]).cast::<f32>();
        let x5_m7_raw = load(x[xb_m7 + 5u32]).cast::<f32>();
        let x6_m7_raw = load(x[xb_m7 + 6u32]).cast::<f32>();
        let x7_m7_raw = load(x[xb_m7 + 7u32]).cast::<f32>();
        let xs_m7 =
            x0_m7 + x1_m7_raw + x2_m7_raw + x3_m7_raw + x4_m7 + x5_m7_raw + x6_m7_raw + x7_m7_raw;
        let x1_m7 = x1_m7_raw * s_16;
        let x2_m7 = x2_m7_raw * s_256;
        let x3_m7 = x3_m7_raw * s_4096;
        let x5_m7 = x5_m7_raw * s_16;
        let x6_m7 = x6_m7_raw * s_256;
        let x7_m7 = x7_m7_raw * s_4096;
        // VARIANT A group index / pack offset — identical to bm4.
        let g = (_b + lane_x_off) / 64u32;
        let pack_off = _b / 8u32 + lane_pack_off;
        // ── Row 0 (W extracts shared across all 8 M-rows) — 8 nibbles ──
        let p00 = load(w[w_base0 + pack_off]);
        let p00_hi = p00 >> 16u32;
        let s0 = load(scales[sb_base0 + g]).cast::<f32>();
        let bi0 = load(biases[sb_base0 + g]).cast::<f32>();
        ...
        let qd0_m7 = q00 * x0_m7
            ...
            + q06 * x6_m7
            + q07 * x7_m7;
        acc0_m0 = acc0_m0 + s0 * qd0_m0 + bi0 * xs_m0;
        acc0_m1 = acc0_m1 + s0 * qd0_m1 + bi0 * xs_m1;
        acc0_m2 = acc0_m2 + s0 * qd0_m2 + bi0 * xs_m2;
        acc0_m3 = acc0_m3 + s0 * qd0_m3 + bi0 * xs_m3;
        acc0_m4 = acc0_m4 + s0 * qd0_m4 + bi0 * xs_m4;
        acc0_m5 = acc0_m5 + s0 * qd0_m5 + bi0 * xs_m5;
        acc0_m6 = acc0_m6 + s0 * qd0_m6 + bi0 * xs_m6;
        acc0_m7 = acc0_m7 + s0 * qd0_m7 + bi0 * xs_m7;
        // ── Row 1 ──
        let p10 = load(w[w_base1 + pack_off]);
        let p10_hi = p10 >> 16u32;
        let s1 = load(scales[sb_base1 + g]).cast::<f32>();
        ...
        let qd1_m7 = q10 * x0_m7
            ...
            + q16 * x6_m7
            + q17 * x7_m7;
        acc1_m0 = acc1_m0 + s1 * qd1_m0 + bi1 * xs_m0;
        acc1_m1 = acc1_m1 + s1 * qd1_m1 + bi1 * xs_m1;
        acc1_m2 = acc1_m2 + s1 * qd1_m2 + bi1 * xs_m2;
        acc1_m3 = acc1_m3 + s1 * qd1_m3 + bi1 * xs_m3;
        acc1_m4 = acc1_m4 + s1 * qd1_m4 + bi1 * xs_m4;
        acc1_m5 = acc1_m5 + s1 * qd1_m5 + bi1 * xs_m5;
        acc1_m6 = acc1_m6 + s1 * qd1_m6 + bi1 * xs_m6;
        acc1_m7 = acc1_m7 + s1 * qd1_m7 + bi1 * xs_m7;
        // ── Row 2 ──
        let p20 = load(w[w_base2 + pack_off]);
        let p20_hi = p20 >> 16u32;
        let s2 = load(scales[sb_base2 + g]).cast::<f32>();
        ...
        let qd2_m7 = q20 * x0_m7
            ...
            + q26 * x6_m7
            + q27 * x7_m7;
        acc2_m0 = acc2_m0 + s2 * qd2_m0 + bi2 * xs_m0;
        acc2_m1 = acc2_m1 + s2 * qd2_m1 + bi2 * xs_m1;
        acc2_m2 = acc2_m2 + s2 * qd2_m2 + bi2 * xs_m2;
        acc2_m3 = acc2_m3 + s2 * qd2_m3 + bi2 * xs_m3;
        acc2_m4 = acc2_m4 + s2 * qd2_m4 + bi2 * xs_m4;
        acc2_m5 = acc2_m5 + s2 * qd2_m5 + bi2 * xs_m5;
        acc2_m6 = acc2_m6 + s2 * qd2_m6 + bi2 * xs_m6;
        acc2_m7 = acc2_m7 + s2 * qd2_m7 + bi2 * xs_m7;
        // ── Row 3 ──
        let p30 = load(w[w_base3 + pack_off]);
        let p30_hi = p30 >> 16u32;
        let s3 = load(scales[sb_base3 + g]).cast::<f32>();
        ...
        let qd3_m7 = q30 * x0_m7
            ...
            + q36 * x6_m7
            + q37 * x7_m7;
        acc3_m0 = acc3_m0 + s3 * qd3_m0 + bi3 * xs_m0;
        acc3_m1 = acc3_m1 + s3 * qd3_m1 + bi3 * xs_m1;
        acc3_m2 = acc3_m2 + s3 * qd3_m2 + bi3 * xs_m2;
        acc3_m3 = acc3_m3 + s3 * qd3_m3 + bi3 * xs_m3;
        acc3_m4 = acc3_m4 + s3 * qd3_m4 + bi3 * xs_m4;
        acc3_m5 = acc3_m5 + s3 * qd3_m5 + bi3 * xs_m5;
        acc3_m6 = acc3_m6 + s3 * qd3_m6 + bi3 * xs_m6;
        acc3_m7 = acc3_m7 + s3 * qd3_m7 + bi3 * xs_m7;
    }
    // Cross-lane reduce + lane-0 stores. 64 outputs per TG.
    let r0_m0 = simd_sum(acc0_m0);
    let r0_m1 = simd_sum(acc0_m1);
    let r0_m2 = simd_sum(acc0_m2);
    let r0_m3 = simd_sum(acc0_m3);
    let r0_m4 = simd_sum(acc0_m4);
    let r0_m5 = simd_sum(acc0_m5);
    let r0_m6 = simd_sum(acc0_m6);
    let r0_m7 = simd_sum(acc0_m7);
    let r1_m0 = simd_sum(acc1_m0);
    let r1_m1 = simd_sum(acc1_m1);
    let r1_m2 = simd_sum(acc1_m2);
    let r1_m3 = simd_sum(acc1_m3);
    let r1_m4 = simd_sum(acc1_m4);
    let r1_m5 = simd_sum(acc1_m5);
    let r1_m6 = simd_sum(acc1_m6);
    let r1_m7 = simd_sum(acc1_m7);
    let r2_m0 = simd_sum(acc2_m0);
    let r2_m1 = simd_sum(acc2_m1);
    let r2_m2 = simd_sum(acc2_m2);
    let r2_m3 = simd_sum(acc2_m3);
    let r2_m4 = simd_sum(acc2_m4);
    let r2_m5 = simd_sum(acc2_m5);
    let r2_m6 = simd_sum(acc2_m6);
    let r2_m7 = simd_sum(acc2_m7);
    let r3_m0 = simd_sum(acc3_m0);
    let r3_m1 = simd_sum(acc3_m1);
    let r3_m2 = simd_sum(acc3_m2);
    let r3_m3 = simd_sum(acc3_m3);
    let r3_m4 = simd_sum(acc3_m4);
    let r3_m5 = simd_sum(acc3_m5);
    let r3_m6 = simd_sum(acc3_m6);
    let r3_m7 = simd_sum(acc3_m7);
    if lane == 0u32 {
        store(out[m_row_m0 * n + row0], r0_m0.cast::<T>());
        store(out[m_row_m0 * n + row1], r1_m0.cast::<T>());
        store(out[m_row_m0 * n + row2], r2_m0.cast::<T>());
        store(out[m_row_m0 * n + row3], r3_m0.cast::<T>());
        store(out[m_row_m1 * n + row0], r0_m1.cast::<T>());
        store(out[m_row_m1 * n + row1], r1_m1.cast::<T>());
        store(out[m_row_m1 * n + row2], r2_m1.cast::<T>());
        store(out[m_row_m1 * n + row3], r3_m1.cast::<T>());
        store(out[m_row_m2 * n + row0], r0_m2.cast::<T>());
        store(out[m_row_m2 * n + row1], r1_m2.cast::<T>());
        store(out[m_row_m2 * n + row2], r2_m2.cast::<T>());
        store(out[m_row_m2 * n + row3], r3_m2.cast::<T>());
        store(out[m_row_m3 * n + row0], r0_m3.cast::<T>());
        store(out[m_row_m3 * n + row1], r1_m3.cast::<T>());
        store(out[m_row_m3 * n + row2], r2_m3.cast::<T>());
        store(out[m_row_m3 * n + row3], r3_m3.cast::<T>());
        store(out[m_row_m4 * n + row0], r0_m4.cast::<T>());
        store(out[m_row_m4 * n + row1], r1_m4.cast::<T>());
        store(out[m_row_m4 * n + row2], r2_m4.cast::<T>());
        store(out[m_row_m4 * n + row3], r3_m4.cast::<T>());
        store(out[m_row_m5 * n + row0], r0_m5.cast::<T>());
        store(out[m_row_m5 * n + row1], r1_m5.cast::<T>());
        store(out[m_row_m5 * n + row2], r2_m5.cast::<T>());
        store(out[m_row_m5 * n + row3], r3_m5.cast::<T>());
        store(out[m_row_m6 * n + row0], r0_m6.cast::<T>());
        store(out[m_row_m6 * n + row1], r1_m6.cast::<T>());
        store(out[m_row_m6 * n + row2], r2_m6.cast::<T>());
        store(out[m_row_m6 * n + row3], r3_m6.cast::<T>());
        store(out[m_row_m7 * n + row0], r0_m7.cast::<T>());
        store(out[m_row_m7 * n + row1], r1_m7.cast::<T>());
        store(out[m_row_m7 * n + row2], r2_m7.cast::<T>());
        store(out[m_row_m7 * n + row3], r3_m7.cast::<T>());
    }
}

// ─── iron_qmv_int8_fast ───────────────────────────────────────────────────
//
// Int8 decode GEMV — mirrors `iron_qmv`'s 8-row-per-TG geometry but for
...
//     block; acc += scale * qdot + bias * xs mirrors `iron_qmv` exactly.
//   - `& 0xFF` mask extracts byte codes 0..255 (unsigned int8).
//
// ## Layouts (same field names as iron_qmv)
...
pub fn iron_qmv_int8_fast<T>(
    ...
    // in a simdgroup, reducing X bandwidth by 4×. Mirrors `iron_qmv` for
...
// ─── iron_qmm_int8_fast ───────────────────────────────────────────────────
//
// Int8 matmul (M=1 batched form) — mirrors `iron_qmm` for int4 but adapted
...
// lanes). Grid: [n / 8, m, 1].
//
// At M = 1 this is byte-identical to `iron_qmv_int8_fast` with the X
// index incorporating the M-row base. Bias hoist, explicit byte shifts,
// and algebraic-split accumulator all match `iron_qmv_int8_fast`.
//
// ## Layouts
...
// ─── iron_qmm_bm2_int8_fast ───────────────────────────────────────────────
//
// Int8 matmul BM=2 — mirrors `iron_qmm_bm2` for int4 but adapted for
...
#[kernel]
pub fn iron_qmm_bm2_int8_fast<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
...
// ─── iron_qmm_bm4_int8_fast ───────────────────────────────────────────────
//
// Int8 matmul BM=4 — mirrors `iron_qmm_bm4` for int4 but adapted for
...
#[kernel]
pub fn iron_qmm_bm4_int8_fast<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
...
// Predicted on the QUANTIZED_SHAPES grid: ≥ 100% Iron MLX at M=32 (M5)
// / ≥ 80% Iron MLX (M2). M=8/16 stays on bm4 — MMA tile is 75%/50% empty
// at those Ms (would waste 1024-output budget).
#[kernel]
pub fn iron_qmm_mma<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
    ...
    // Iron MLX) with and without the upcast, so we keep simpler half MMA.
    ...
    // is patched per-dtype in `iron_qmm_for` (`xs_ld_const`/`ws_ld_const` IR
...
// `iron_qmm_mma` requires `m % 32 == 0`, so M=16 falls through to bm4 in
// the selector. bm4 wins moderate-N at M=16 but loses wide-N on M2 Pro
// (76-94% Iron MLX). This kernel maps M=16 exactly to 2 frag_m positions
...
//   (same MMA work-per-SG as iron_qmm_mma — we halve only the warp-m grid)
...
//      identical body to iron_qmm_mma's per-SG inner loop.
//   5. After K-loop, write each SG's 4 frags to global out.
//
// Frag lane mapping (Apple steel_gemm layout, same as iron_qmm_mma):
//   qid = lane/4, fm = (qid & 4) + ((lane/2) % 4),
//   fn0 = (qid & 2)*2 + (lane%2)*2, fn1 = fn0 + 1
//
// Per-lane register footprint: 4 C f32 frags (16 elems × 4B = 64B) +
// 4 A/B T frags (~16B) + scratch ≈ 256 B — well under the ~1 KB/lane
// register file. Occupancy at 64 tpg is 2× iron_qmm_mma's 128 tpg.
//
// At M < 16 this kernel padding-wastes >= 50% of the tile (would route
// to bm2/bm4). At M = 32 use iron_qmm_mma (full tile, 2× the output
// budget). Selector route: `m == 16` → iron_qmm_mma_m16.
#[kernel]
pub fn iron_qmm_mma_m16<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
    ...
    let sn = sg & 1u32;
    let lane_in_tg = sg * 32u32 + lane;
    // 8×8 frag lane mapping (Apple steel_gemm layout — same as iron_qmm_mma).
    let qid = lane / 4u32;
    let fm = (qid & 4u32) + ((lane / 2u32) % 4u32);
    let fn0 = (qid & 2u32) * 2u32 + (lane % 2u32) * 2u32;
    let fn1 = fn0 + 1u32;
    ...
    // as iron_qmm_mma. Xs size = 16 × 36 = 576 T; Ws size = 32 × 36 = 1152 T.
...
// ─── iron_qmm_mma_bm8 ─────────────────────────────────────────────────────
//
// Quarter-height simdgroup-matrix MMA — the M=8 cell. `iron_qmm_mma`
// requires `m % 32 == 0` and `iron_qmm_mma_m16` is hard-wired to M=16, so
// M=8 falls through to `iron_qmm_bm4` in the selector today. bm4 wins at
// M=8 but its marginal cost per extra row is compute-bound (measured
// ~49.8ms fixed + 10.2ms/token), while an MMA-class kernel's marginal
// cost is bandwidth-bound (~116ms fixed + 0.71ms/token). At small T
// (speculative-decode verify batches, T≈1-8) the MMA-class fixed cost
// dominates today because nothing is sized to M=8 — this kernel closes
// that gap by mapping M=8 to exactly ONE 8-row frag position (no padding
// waste, unlike routing T≤8 through `iron_qmm_mma_m16`, which would waste
// 50% of the BM=16 tile).
//
// Geometry:
//   tpg = 64 = 2 SG × 32 lanes (WM=1, WN=2 warp grid — unchanged from m16)
//   BM = 8, BN = BK = 32 → 8×32 output tile (256 outputs/TG)
//   Grid: [N/32, M/8, 1]
//   Each SG owns an 8×16 sub-tile = 1 frag_m × 2 frag_n = 2 8×8 frags
//   (half m16's frag count — BM=8 fits only one 8-row frag position, so
//   m16's frag_m=1 / row_a1 / a_f1 / c_f10 / c_f11 have no analog here)
//   Per K-block per SG: 2 frags × 4 k-inner = 8 MMAs (16 across TG)
//
// Threadgroup memory (skewed BK+4 = 36 stride to break 32-bank conflicts,
// same rationale as iron_qmm_mma / iron_qmm_mma_m16):
//   Xs[8 × 36] = 288 T   (X tile, 8 rows × 32 cols + 4 skew)
//   Ws[32 × 36] = 1152 T (dequant W tile, 32 rows × 32 cols + 4 skew —
//   identical to m16's Ws; W tile geometry (BN=BK=32) doesn't shrink)
//   Total: 1440 T (5.625 KB f32 / 2.8125 KB f16) — quarter the X tile of
//   iron_qmm_mma, half the X tile of iron_qmm_mma_m16; same Ws as both.
//
// Per K-block:
//   1. Coop X load — 64 lanes × 4 strides each fill 256-elt Xs tile.
//      flat = i*64 + lane_in_tg ∈ 0..255; m_row = flat/32 ∈ 0..7.
//   2. Coop W dequant — 128 packs / 64 lanes = 2 packs per lane. Identical
//      to iron_qmm_mma_m16's dequant block verbatim — W tile geometry is
//      unchanged, so the pack/lane assignment doesn't depend on BM.
//   3. threadgroup_barrier()
//   4. Per SG (sm=0, sn = sg & 1): 1 frag_m × 2 frag_n × 4 k-inner
//      unrolled MMA. Only ONE A-frag (a_f0, no a_f1) since BM=8 has a
//      single frag_m row; two B-frags (b_f0, b_f1) as in m16.
//   5. After K-loop, write each SG's 2 frags to global out.
//
// Frag lane mapping (Apple steel_gemm layout, same as iron_qmm_mma /
// iron_qmm_mma_m16 — fixed hardware layout, does not change with BM):
//   qid = lane/4, fm = (qid & 4) + ((lane/2) % 4),
//   fn0 = (qid & 2)*2 + (lane%2)*2, fn1 = fn0 + 1
//
// Per-lane register footprint: 2 C f32 frags (16 elems × 4B = 32B) + 3
// A/B T frags (a_f0, b_f0, b_f1; ~12B) + scratch ≈ 128 B — roughly half
// iron_qmm_mma_m16's ~256B footprint. Occupancy at 64 tpg matches m16's
// (same tpg, smaller Xs allocation only).
//
// When to route here: T≈1-8 speculative-decode verify batches, where
// `iron_qmm_bm4`'s compute-bound marginal cost loses to this kernel's
// bandwidth-bound marginal cost once more than a handful of tokens are
// batched into one verify dispatch, despite bm8's higher fixed dispatch
// cost. NOT wired into `iron_qmm_for`'s default M-routing table (opt-in
// only) — see that function's doc comment for the current M=8 route
// (bm4); changing the default is out of scope here.
//
// Constraint: BN=32 means callers need `N % 32 == 0`. Every projection
// shape in the Qwen3.6-27B consumer this kernel targets satisfies that
// (hidden=5120, intermediate=17408, vocab=248320, q_proj=6144,
// kv_proj=1024, linear-attn k_proj=2048, linear-attn v_proj=6144 — all
// multiples of 32).
#[kernel]
pub fn iron_qmm_mma_bm8<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
    ...
    let sg = simd_group_id();
    // 2 SGs in 1×2 warp grid: sg ∈ {0,1} → (sm=0, sn=sg).
    // Each SG owns an 8×16 sub-tile at (0, sn*16) inside the 8×32
    // output tile. WM=1 — both SGs cover the full BM=8 rows.
    let sm = 0u32;
    let sn = sg & 1u32;
    let lane_in_tg = sg * 32u32 + lane;
    // 8×8 frag lane mapping (Apple steel_gemm layout — same as iron_qmm_mma).
    ...
    let fn0 = (qid & 2u32) * 2u32 + (lane % 2u32) * 2u32;
    let fn1 = fn0 + 1u32;
    // TG memory for X tile [8 × 32] and dequant W tile [32 × 32].
    // BK + 4 = 36 stride for bank-conflict avoidance — same skew rationale
    // as iron_qmm_mma / iron_qmm_mma_m16. Xs size = 8 × 36 = 288 T; Ws size
    // = 32 × 36 = 1152 T (unchanged from m16 — W tile geometry doesn't
    // shrink with BM).
    threadgroup_alloc("xs", 288, T);
    threadgroup_alloc("ws", 1152, T);
    // ── 2 output frags per SG, init to 0 ── (frag_n=0/1 only; BM=8 has a
    // single frag_m row, so there is no c_f10/c_f11 analog).
    let c_f00 = simdgroup_alloc::<f32, 8, 8>();
    simdgroup_elem_store(c_f00, 0, 0.0f32);
    simdgroup_elem_store(c_f00, 1, 0.0f32);
    let c_f01 = simdgroup_alloc::<f32, 8, 8>();
    simdgroup_elem_store(c_f01, 0, 0.0f32);
    simdgroup_elem_store(c_f01, 1, 0.0f32);
    // A (X) and B (W^T) frag scratch, reused per k_inner. Only ONE A-frag
    // (a_f0) — BM=8 fits a single frag_m row.
    let a_f0 = simdgroup_alloc::<T, 8, 8>();
    let b_f0 = simdgroup_alloc::<T, 8, 8>();
    let b_f1 = simdgroup_alloc::<T, 8, 8>();
    // Coop-load lane assignments.
    // X tile: 8×32 = 256 elements, 64 lanes × 4 strides each → 256 covers.
    //   step i in 0..4: flat = i*64 + lane_in_tg ∈ 0..255.
    //   m_row = flat/32 ∈ 0..7, k_col = flat%32.
    // W tile: 32×32 = 1024 elements, 128 packs, 64 lanes × 2 packs each —
    //   identical to iron_qmm_mma_m16's W dequant (verbatim below).
    let x_m_base = m_tile * 8u32; // first M-row this TG handles
    let w_n_base = n_tile * 32u32; // first N-row this TG handles
    let packs_per_row = k / 8u32;
    // TG row stride = 36 (BK + 4 skew).
    let xs_ld = 36u32;
    let ws_ld = 36u32;
    for kb in range(0u32, k, 32u32) {
        // ── 1. Coop X load — 64 lanes × 4 strides each fill 256-elt tile ──
        let flat0 = lane_in_tg;
        let flat1 = 64u32 + lane_in_tg;
        let flat2 = 128u32 + lane_in_tg;
        let flat3 = 192u32 + lane_in_tg;
        ...
        // ── 2. Coop W dequant — 64 lanes × 2 packs each → 1024 fp T ──
        // Identical to iron_qmm_mma_m16's dequant block — W tile geometry
        // (BN=BK=32) is unchanged by BM.
        let s_16 = 0.0625f32;
        let s_256 = 0.00390625f32;
        let s_4096 = 0.000244140625f32;
        // Pack 0
        ...
        threadgroup_store("ws", ws_base_1 + 7u32, (s_1 * q7_1 + b_1).cast::<T>());
        threadgroup_barrier();
        // ── 3. MMA inner loop — 2 frags × 4 k-inner = 8 MMAs per SG ──
        // sm = 0 fixed (WM=1 → both SGs cover full BM=8). Only ONE
        // frag_m row (row_a0) — BM=8 doesn't fit a second 8-row frag.
        // A-frag (X) elem[i] @ (fm, fn_i) = Xs[(0 + fm) * 36 + (k_inner*8 + fn_i)]
        // B-frag (W^T) elem[i] @ (fm, fn_i) = Ws[(sn*16 + frag_n + fn_i) * 36 + (k_inner*8 + fm)]
        let row_a0 = sm * 8u32 + fm; // frag_m = 0 (only position)
        let col_b0 = sn * 16u32; // frag_n = 0 base offset
        let col_b1 = sn * 16u32 + 8u32; // frag_n = 8 base offset
        // k_inner = 0 (k offset 0..7 inside BK=32)
        simdgroup_elem_store(a_f0, 0, threadgroup_load("xs", row_a0 * xs_ld + fn0));
    ...
    // ── 4. Write 2 C frags to global out ──
    // sm = 0 → out_m_base starts at m_tile*8 (no sub-tile m offset; BM=8
    // fits exactly one frag_m row, so there is no +8 second-frag write).
    let out_m_base = m_tile * 8u32;
    let out_n_base = n_tile * 32u32 + sn * 16u32;
    // c_f00 at (frag_m=0, frag_n=0)
    store(out[(out_m_base + fm) * n + out_n_base + fn0], simdgroup_elem_load(c_f00, 0).cast::<T>());
    store(out[(out_m_base + fm) * n + out_n_base + fn1], simdgroup_elem_load(c_f00, 1).cast::<T>());
...
// ─── iron_qmm_mma_int8 ────────────────────────────────────────────────────
//
// Int8-quantized simdgroup-matrix MMA prefill — mirrors `iron_qmm_mma` but
// operates on int8-packed weights (4 bytes per u32, `pack_factor=4`).
//
// Geometry (identical to iron_qmm_mma):
//   tpg = 128 = 4 SG × 32 lanes (WM=2, WN=2 warp grid)
//   BM = BN = BK = 32, output tile 32×32 (1024 outputs/TG)
//   Grid: [N/32, M/32, 1]
//   Each SG owns a 16×16 sub-tile = 2×2 = 4 8×8 frags
...
// X load: same as iron_qmm_mma (8 contiguous K elems per lane, vec4 fusion).
// MMA inner loop: identical to iron_qmm_mma (4 k-inner × 4 frags = 16 MMAs/SG).
#[kernel]
pub fn iron_qmm_mma_int8<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
    ...
    // TG memory for X tile [BM × BK] and dequant W tile [BN × BK].
    // BK + 4 = 36 stride for bank-conflict avoidance — same skew rationale
    // as iron_qmm_mma. Xs size = 32 × 36 = 1152 T; Ws size = 32 × 36 = 1152 T.
    threadgroup_alloc("xs", 1152, T);
    threadgroup_alloc("ws", 1152, T);
    // ── 4 output frags per SG, init to 0 ──
    let c_f00 = simdgroup_alloc::<f32, 8, 8>();
    ...
    let ws_ld = ws_ld_const;
    // Coop X-load mapping. 32×32 = 1024 elements, 128 lanes × 8 elems each.
    // Same vec4-fusion layout as iron_qmm_mma: lane_in_tg/4 = m_row, lane_in_tg%4 = k_quad.
    let x_m_row = lane_in_tg / 4u32;
    let x_k_quad = lane_in_tg & 3u32;
    let x_k_base = x_k_quad * 8u32;
    ...
    for kb in range(0u32, k, 32u32) {
        ...
        threadgroup_barrier();
        // ── 3. MMA inner loop — 4 frags × 4 k-inner = 16 MMAs per SG ──
        // Identical to iron_qmm_mma: A-load, barrier, B-load, barrier, MMAs,
...
// ─── iron_qmm_mma_m16_int8 ────────────────────────────────────────────────
//
// Int8 half-height MMA — the M=16 cell, int8 weights.
// Mirrors `iron_qmm_mma_m16` but with int8 pack format (4 bytes/u32).
//
// Geometry (identical to iron_qmm_mma_m16):
//   tpg = 64 = 2 SG × 32 lanes (WM=1, WN=2 warp grid)
//   BM = 16, BN = BK = 32 → 16×32 output tile (512 outputs/TG)
//   Grid: [N/32, M/16, 1]
...
//
// X load: iron_qmm_mma_m16 flat-stride pattern (8 strides of 64 lanes covering
// the 512-element Xs tile). Same as int4.
//
// MMA inner loop: identical to iron_qmm_mma_m16 (no A/B barrier; 4 k-inner).
#[kernel]
pub fn iron_qmm_mma_m16_int8<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
...
// ─── iron_qmm_mma_int2 ────────────────────────────────────────────────────
//
// Int2-quantized simdgroup-matrix MMA prefill — mirrors `iron_qmm_mma` but
...
pub fn iron_qmm_mma_int2<T>(
    ...
    for kb in range(0u32, k, 32u32) {
        ...
        threadgroup_barrier();
        // ── 3. MMA inner loop — 4 frags × 4 k-inner = 16 MMAs per SG ──
        // Identical to iron_qmm_mma_int8: A-load, barrier, B-load, barrier,
...
#[kernel]
pub fn iron_affine_dequantize_int4<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    mut out: Tensor<T>,
...
// different constants.
#[kernel]
pub fn iron_affine_quantize_int4<T>(
    w: Tensor<T>,
    mut out: Tensor<u32>,
    mut scales: Tensor<T>,
    mut biases: Tensor<T>,
...
#[kernel]
pub fn iron_affine_dequantize_int8<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    mut out: Tensor<T>,
...
// ─── iron_affine_quantize_int8 ───────────────────────────────────────────
#[kernel]
pub fn iron_affine_quantize_int8<T>(
    w: Tensor<T>,
    mut out: Tensor<u32>,
    mut scales: Tensor<T>,
    mut biases: Tensor<T>,
...
// `quantized.h`.
#[kernel]
pub fn iron_affine_dequantize_int2<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    mut out: Tensor<T>,
...
// ─── iron_affine_quantize_int2 ───────────────────────────────────────────
//
// Inverse of `iron_affine_dequantize_int2`. One threadgroup of 32 threads
...
//
// n_bins = 3 (`2^2 - 1`). Same template as `iron_affine_quantize_int4`
// with the pack width widened from 8 nibbles to 16 two-bit fields.
#[kernel]
pub fn iron_affine_quantize_int2<T>(
    w: Tensor<T>,
    mut out: Tensor<u32>,
    mut scales: Tensor<T>,
    mut biases: Tensor<T>,
...
//
// Bit layouts are the exact inverse of `iron_affine_dequantize_int{3,5,6}`.

// ─── iron_affine_quantize_int3 ──────────────────────────────────────────
//
...
// - Grid: [n_groups, 1, 1].
#[kernel]
pub fn iron_affine_quantize_int3<T>(
    w: Tensor<T>,
    mut out: Tensor<u32>,
    mut scales: Tensor<T>,
    mut biases: Tensor<T>,
...
// - Grid: [n_groups, 1, 1].
#[kernel]
pub fn iron_affine_quantize_int5<T>(
    w: Tensor<T>,
    mut out: Tensor<u32>,
    mut scales: Tensor<T>,
    mut biases: Tensor<T>,
...
// - Grid: [n_groups, 1, 1].
#[kernel]
pub fn iron_affine_quantize_int6<T>(
    w: Tensor<T>,
    mut out: Tensor<u32>,
    mut scales: Tensor<T>,
    mut biases: Tensor<T>,
...
#[kernel]
pub fn iron_affine_dequantize_int3<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    mut out: Tensor<T>,
...
#[kernel]
pub fn iron_affine_dequantize_int5<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    mut out: Tensor<T>,
...
#[kernel]
pub fn iron_affine_dequantize_int6<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    mut out: Tensor<T>,
...
// ═══════════════════════════════════════════════════════════════════════
//
// The hand-unrolled `iron_qmv` / `iron_qmm*` above are int4-only, f32+f16 —
// the production hot path. This section closes the rest of the
// `affine_qmv / qvm / qmm` coverage gap with a clean, generic family:
//
//   - `iron_qmv_b{3,4,5,6,8}`  — quantized matvec, `y = W · x`
//   - `iron_qvm_b{3,4,5,6,8}`  — quantized vecmat, `y = xᵀ · W`
//   - `iron_qmm_b{3,4,5,6,8}`  — quantized matmul (batched matvec)
//
...
// They are not the perf path (the unrolled int4 `iron_qmv`/`iron_qmm*`
...
// wh-iron kernel + a GPU correctness test behind it.
//
// ── Bit-extraction ──
...
//
// `iron_qmm_b*` is the M-batched form of `iron_qmv_b*`; the bodies are
...
/// Produces: `iron_qmv_b4`, `iron_qmv_b8`.
...
/// Produces: `iron_qmv_odd_b3`, `iron_qmv_odd_b5`, `iron_qmv_odd_b6`.
#[kernel(variants(BITS = [3u32, 5u32, 6u32], suffix = "b{BITS}"))]
pub fn iron_qmv_odd<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
...
/// Identical body to `iron_qmv`; registered under `qmm_b*` so the bench
/// scoreboard tracks it separately. Dispatch `grid = [N, M, 1]`.
/// Produces: `iron_qmm_b4`, `iron_qmm_b8`.
#[kernel(variants(BITS = [4u32, 8u32], suffix = "b{BITS}"))]
pub fn iron_qmm<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
...
/// Produces: `iron_qmm_odd_b3`, `iron_qmm_odd_b5`, `iron_qmm_odd_b6`.
#[kernel(variants(BITS = [3u32, 5u32, 6u32], suffix = "b{BITS}"))]
pub fn iron_qmm_odd<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
...
/// Produces: `iron_qvm_b4`, `iron_qvm_b8`.
#[kernel(variants(BITS = [4u32, 8u32], suffix = "b{BITS}"))]
pub fn iron_qvm<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
...
/// bit-stream-packed.
/// Produces: `iron_qvm_odd_b3`, `iron_qvm_odd_b5`, `iron_qvm_odd_b6`.
#[kernel(variants(BITS = [3u32, 5u32, 6u32], suffix = "b{BITS}"))]
pub fn iron_qvm_odd<T>(
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    x: Tensor<T>,
...
/// per TG, mirroring `iron_qmv`'s 8-row geometry with K/N transposed.
///
...
// ─── iron_qmm_mma_b{3,5,6} — bit-stream MMA for odd bit-widths ────────────────
//
// Bit-width-generalized siblings of `iron_qmm_mma` for int3 / int5 / int6
...
//
// Mirrors the `gather_qmm_mma!` macro in `iron/moe.rs` — exactly the same
...
///
/// Produces kernels: `iron_qmm_mma_b3`, `iron_qmm_mma_b5`, `iron_qmm_mma_b6`.
///
...
/// `iron/moe.rs` but applied to dense (non-expert) GEMMs.
///
...
pub fn iron_qmm_mma<T>(
    ...
    // X coop-load lane assignments: same shape as iron_qmm_mma.
    let x_m_row = lane_in_tg / 4u32;
    let x_k_quad = lane_in_tg & 3u32;
    let x_k_base = x_k_quad * 8u32;
    ...
    for kb in range(0u32, k, 32u32) {
        // ── 1. Coop X load — identical to iron_qmm_mma ──
        let x_row_dev_base = (x_m_base + x_m_row) * k + kb + x_k_base;
        let x_ws_base = x_m_row * xs_ld + x_k_base;
        let xv0 = load(x[x_row_dev_base]).cast::<T>();
        let xv1 = load(x[x_row_dev_base + 1u32]).cast::<T>();
        ...
        // Identical to iron_qmm_mma: serpentine (0,0)→(0,1)→(1,1)→(1,0).
...
/// Auto-select the best `iron_qmm*` kernel for a given dtype + M
...
/// | M=1 / odd M          | `iron_qmm`    | v2 (any M, including 1)                            |
///
...
pub fn patch_qmm_mma_dtype_aware_skew(
    kernel: &mut wh_iron::core::ir::Kernel,
    dtype: wh_iron::core::dtype::DType,
) {
    use wh_iron::core::dtype::DType;
...
mod qmm_selector_tests {
    ...
    fn selector_picks_mma_at_m_multiple_of_32() {
        ...
        for m in [32u32, 64, 96, 128] {
            let k = iron_qmm_for(DType::F32, m);
            assert_eq!(k.name, "iron_qmm_mma", "m={m}: multiple of 32 should route to mma");
        }
    }
    ...
    fn selector_picks_mma_m16_at_m_16() {
        ...
        let k = iron_qmm_for(DType::F32, 16);
        assert_eq!(k.name, "iron_qmm_mma_m16");
    }
    ...
    fn selector_picks_bm4_at_m_8_12_20_24_28() {
        ...
        for m in [4u32, 8, 12, 20, 24, 28, 36, 60] {
            let k = iron_qmm_for(DType::F32, m);
            assert_eq!(k.name, "iron_qmm_bm4", "m={m}: m%4==0 not mma should route to bm4");
        }
    }
    ...
    fn selector_picks_bm2_at_even_m_not_multiple_of_4() {
        for m in [2u32, 6, 10, 14, 18, 22, 26, 30] {
            let k = iron_qmm_for(DType::F32, m);
            assert_eq!(k.name, "iron_qmm_bm2", "m={m}: even-not-mod-4 should route to bm2");
        }
    }

    #[test]
    fn selector_picks_v2_at_m_1() {
        let k = iron_qmm_for(DType::F32, 1);
        assert_eq!(k.name, "iron_qmm");
    }
    ...
    fn selector_picks_v2_at_odd_m() {
        for m in [3u32, 5, 7, 9, 15, 31] {
            let k = iron_qmm_for(DType::F32, m);
            assert_eq!(k.name, "iron_qmm", "m={m}: odd M should route to v2");
        }
    }
    ...
    fn selector_picks_bm4_across_dtypes_at_m_8() {
        for dt in [DType::F32, DType::F16] {
            let k = iron_qmm_for(dt, 8);
            assert_eq!(k.name, "iron_qmm_bm4", "dt={dt:?}");
        }
    }
    ...
    fn selector_kernels_carry_reduction_mode() {
        for m in [1u32, 4, 8, 16, 32] {
            ...
            assert_eq!(
                k.mode,
                wh_iron::core::ir::KernelMode::Reduction,
...
pub mod kernel_tests {
    ...
    fn test_affine_dequantize_int4(dt: DType) -> TestSetup {
        dequant_setup(iron_affine_dequantize_int4::kernel_ir_for(dt), 4, 64, 8, dt)
    }
    ...
    fn test_affine_dequantize_int8(dt: DType) -> TestSetup {
        ...
        dequant_setup(iron_affine_dequantize_int8::kernel_ir_for(dt), 8, 64, 4, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-2, 1e-1])]
    fn test_affine_dequantize_int2(dt: DType) -> TestSetup {
        dequant_setup(iron_affine_dequantize_int2::kernel_ir_for(dt), 2, 64, 16, dt)
    }
    ...
    fn test_affine_dequantize_int3(dt: DType) -> TestSetup {
        dequant_odd_setup(iron_affine_dequantize_int3::kernel_ir_for(dt), 3, 8, 32, 4, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 5e-2, 5e-1])]
    fn test_affine_dequantize_int5(dt: DType) -> TestSetup {
        dequant_odd_setup(iron_affine_dequantize_int5::kernel_ir_for(dt), 5, 8, 32, 4, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 5e-2, 5e-1])]
    fn test_affine_dequantize_int6(dt: DType) -> TestSetup {
        dequant_odd_setup(iron_affine_dequantize_int6::kernel_ir_for(dt), 6, 4, 32, 4, dt)
    }
    ...
    fn test_affine_quantize_int2(dt: DType) -> TestSetup {
        quantize_setup(iron_affine_quantize_int2::kernel_ir_for(dt), 2, 64, 8, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-3, 1e-2])]
    fn test_affine_quantize_int3(dt: DType) -> TestSetup {
        quantize_setup(iron_affine_quantize_int3::kernel_ir_for(dt), 3, 32, 8, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-3, 1e-2])]
    fn test_affine_quantize_int4(dt: DType) -> TestSetup {
        quantize_setup(iron_affine_quantize_int4::kernel_ir_for(dt), 4, 64, 8, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-3, 1e-2])]
    fn test_affine_quantize_int5(dt: DType) -> TestSetup {
        quantize_setup(iron_affine_quantize_int5::kernel_ir_for(dt), 5, 32, 8, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-3, 1e-2])]
    fn test_affine_quantize_int6(dt: DType) -> TestSetup {
        quantize_setup(iron_affine_quantize_int6::kernel_ir_for(dt), 6, 32, 8, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-3, 1e-2])]
    fn test_affine_quantize_int8(dt: DType) -> TestSetup {
        quantize_setup(iron_affine_quantize_int8::kernel_ir_for(dt), 8, 64, 8, dt)
    }
    ...
    fn test_qmv(dt: DType) -> TestSetup {
        qx_setup(iron_qmv::kernel_ir_for(dt), 1, 32, 512, 4, 64, false, [4, 1, 1], [64, 1, 1], dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_qmv_int8_fast(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmv_int8_fast::kernel_ir_for(dt),
            1,
            32,
            512,
            8,
    ...
    fn test_qmm(dt: DType) -> TestSetup {
        qx_setup(iron_qmm::kernel_ir_for(dt), 8, 16, 512, 4, 64, true, [2, 8, 1], [64, 1, 1], dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_qmm_bm2(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_bm2::kernel_ir_for(dt),
            8,
            16,
            512,
            4,
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_qmm_bm3(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_bm3::kernel_ir_for(dt),
            6,
            16,
            512,
            4,
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_qmm_bm8(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_bm8::kernel_ir_for(dt),
            8,
            16,
            512,
            4,
    ...
    fn test_qmm_int8_fast(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_int8_fast::kernel_ir_for(dt),
            8,
            16,
            512,
            8,
    ...
    fn test_qmm_bm2_int8_fast(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_bm2_int8_fast::kernel_ir_for(dt),
            8,
            16,
            512,
            8,
    ...
    fn test_qmm_bm4_int8_fast(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_bm4_int8_fast::kernel_ir_for(dt),
            8,
            16,
            512,
            8,
    ...
    fn test_qmm_mma_m16(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_mma_m16::kernel_ir_for(dt),
            16,
            64,
            512,
            4,
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_qmm_mma_bm8(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_mma_bm8::kernel_ir_for(dt),
            8,
            64,
            512,
            4,
    ...
    fn test_qmm_mma_int8(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_mma_int8::kernel_ir_for(dt),
            32,
            64,
            512,
            8,
    ...
    fn test_qmm_mma_m16_int8(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_mma_m16_int8::kernel_ir_for(dt),
            16,
            64,
            512,
            8,
    ...
    fn test_qmm_mma_int2(dt: DType) -> TestSetup {
        qx_setup(
            iron_qmm_mma_int2::kernel_ir_for(dt),
            32,
            64,
            512,
            2,
    ...
    fn test_qvm_int4_fast(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_qvm_int4_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("w", u32_bytes(&w), DType::U32))
            .input(TestBuffer::from_vec("scales", pack_f32(&scales_f, dt), dt))
            .input(TestBuffer::from_vec("biases", pack_f32(&biases_f, dt), dt))
    ...
    //
    // These bit-width-parametric kernels (`iron_qmv_b*`, `iron_qvm_b*`,
    // `iron_qmm_b*`, `iron_qmm_mma_b{3,5,6}`) read a per-row bit-stream weight:
    ...
    qmv_test!(test_qmv_b3, iron_qmv_odd_b3, 3);
    qmv_test!(test_qmv_b4, iron_qmv_b4, 4);
    qmv_test!(test_qmv_b5, iron_qmv_odd_b5, 5);
    qmv_test!(test_qmv_b6, iron_qmv_odd_b6, 6);
    qmv_test!(test_qmv_b8, iron_qmv_b8, 8);
    ...
    qmm_test!(test_qmm_b3, iron_qmm_odd_b3, 3);
    qmm_test!(test_qmm_b4, iron_qmm_b4, 4);
    qmm_test!(test_qmm_b5, iron_qmm_odd_b5, 5);
    qmm_test!(test_qmm_b6, iron_qmm_odd_b6, 6);
    qmm_test!(test_qmm_b8, iron_qmm_b8, 8);
    ...
    qvm_test!(test_qvm_b3, iron_qvm_odd_b3, 3);
    qvm_test!(test_qvm_b4, iron_qvm_b4, 4);
    qvm_test!(test_qvm_b5, iron_qvm_odd_b5, 5);
    qvm_test!(test_qvm_b6, iron_qvm_odd_b6, 6);
    qvm_test!(test_qvm_b8, iron_qvm_b8, 8);
    ...
    qmm_mma_odd_test!(test_qmm_mma_b3, iron_qmm_mma_b3, 3);
    qmm_mma_odd_test!(test_qmm_mma_b5, iron_qmm_mma_b5, 5);
    qmm_mma_odd_test!(test_qmm_mma_b6, iron_qmm_mma_b6, 6);
}
...
pub mod kernel_benches {
    ...
    fn bench_dequant_pow2(dt: DType) -> BenchSetup {
        db(iron_affine_dequantize_intBITS::kernel_ir_for(dt), BITS, 64, 65536, dt)
    }
    ...
    fn bench_dequant_int3(dt: DType) -> BenchSetup {
        db_odd(iron_affine_dequantize_int3::kernel_ir_for(dt), 8, 3, 32, 65536, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_dequant_int5(dt: DType) -> BenchSetup {
        db_odd(iron_affine_dequantize_int5::kernel_ir_for(dt), 8, 5, 32, 65536, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_dequant_int6(dt: DType) -> BenchSetup {
        db_odd(iron_affine_dequantize_int6::kernel_ir_for(dt), 4, 3, 32, 65536, dt)
    }
    ...
    fn bench_quant_pow2(dt: DType) -> BenchSetup {
        qb(iron_affine_quantize_intBITS::kernel_ir_for(dt), BITS, 64, 65536, dt)
    }

    #[bench(dtypes = [f32, f16, bf16],
            variants(BITS = [3, 5, 6], suffix = "int{BITS}"))]
    fn bench_quant_odd(dt: DType) -> BenchSetup {
        qb(iron_affine_quantize_intBITS::kernel_ir_for(dt), BITS, 32, 65536, dt)
    }
    ...
    ///
    /// Why `Tiny`, not `Positive`/`Signed`: Iron folds the dequant dot in f32
    ...
    /// Iron-vs-MLX divergence (which tiny magnitudes can't mask).
    #[allow(clippy::too_many_arguments)]
    fn qmb_ref(
        kernel: Kernel,
        m: usize,
    ...
    // Iron kernels fold the dequant dot in f32 while MLX accumulates in the
    ...
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_bm3(dt: DType) -> BenchSetup {
        qmb_ref(
            iron_qmm_bm3::kernel_ir_for(dt),
            6,
            4096,
    ...
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_bm8(dt: DType) -> BenchSetup {
        qmb_ref(
            iron_qmm_bm8::kernel_ir_for(dt),
            8,
            4096,
    ...
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_mma_bm8(dt: DType) -> BenchSetup {
        qmb_ref(
            iron_qmm_mma_bm8::kernel_ir_for(dt),
            8,
            4096,
    ...
    fn bench_qvm_int4_fast(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_qvm_int4_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    //
    // The generic `iron_q{mv,vm,mm}_b*` kernels (correctness-first scalar bodies)
    ...
    fn bench_qmv_b3(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmv_odd_b3::kernel_ir_for(dt), 1, 4096, 4096, 3, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmv_b4(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmv_b4::kernel_ir_for(dt), 1, 4096, 4096, 4, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmv_b5(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmv_odd_b5::kernel_ir_for(dt), 1, 4096, 4096, 5, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmv_b6(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmv_odd_b6::kernel_ir_for(dt), 1, 4096, 4096, 6, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmv_b8(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmv_b8::kernel_ir_for(dt), 1, 4096, 4096, 8, 64, [4096, 1, 1], dt)
    }
    ...
    fn bench_qvm_b3(dt: DType) -> BenchSetup {
        qmb_gs(iron_qvm_odd_b3::kernel_ir_for(dt), 1, 4096, 4096, 3, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qvm_b4(dt: DType) -> BenchSetup {
        qmb_gs(iron_qvm_b4::kernel_ir_for(dt), 1, 4096, 4096, 4, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qvm_b5(dt: DType) -> BenchSetup {
        qmb_gs(iron_qvm_odd_b5::kernel_ir_for(dt), 1, 4096, 4096, 5, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qvm_b6(dt: DType) -> BenchSetup {
        qmb_gs(iron_qvm_odd_b6::kernel_ir_for(dt), 1, 4096, 4096, 6, 64, [4096, 1, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qvm_b8(dt: DType) -> BenchSetup {
        qmb_gs(iron_qvm_b8::kernel_ir_for(dt), 1, 4096, 4096, 8, 64, [4096, 1, 1], dt)
    }

    // qmm (batched matvec, M=4): grid [N, M, 1].
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_b3(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmm_odd_b3::kernel_ir_for(dt), 4, 4096, 4096, 3, 64, [4096, 4, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_b4(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmm_b4::kernel_ir_for(dt), 4, 4096, 4096, 4, 64, [4096, 4, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_b5(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmm_odd_b5::kernel_ir_for(dt), 4, 4096, 4096, 5, 64, [4096, 4, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_b6(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmm_odd_b6::kernel_ir_for(dt), 4, 4096, 4096, 6, 64, [4096, 4, 1], dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_b8(dt: DType) -> BenchSetup {
        qmb_gs(iron_qmm_b8::kernel_ir_for(dt), 4, 4096, 4096, 8, 64, [4096, 4, 1], dt)
    }
    ...
    fn bench_qmm_mma_b3(dt: DType) -> BenchSetup {
        qmm_mma_b(iron_qmm_mma_b3::kernel_ir_for(dt), 4096, 4096, 3, 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_mma_b5(dt: DType) -> BenchSetup {
        qmm_mma_b(iron_qmm_mma_b5::kernel_ir_for(dt), 4096, 4096, 5, 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_qmm_mma_b6(dt: DType) -> BenchSetup {
        qmm_mma_b(iron_qmm_mma_b6::kernel_ir_for(dt), 4096, 4096, 6, 64, dt)
    }
}
