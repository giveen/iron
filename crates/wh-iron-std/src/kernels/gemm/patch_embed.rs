...
//! the NCHW image convention and writes NCHW output; `iron_patch_embed` takes
...
pub mod kernel_tests {
    ...
    fn patch_embed_setup(
        ...
        TestSetup::new(iron_patch_embed::kernel_ir_for(dt))
...
/// New-syntax bench for `iron_patch_embed` (ViT-L SigLIP stem shape).
...
pub mod kernel_benches {
    ...
    fn bench_patch_embed(dt: DType) -> BenchSetup {
        ...
        let patch_dim = in_ch * patch_h * patch_w;
        let n_out = num_patches * hidden;
        BenchSetup::new(iron_patch_embed::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
