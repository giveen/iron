...
//! - `iron_fp4_quant_dequant` — MXFP4 / NV-FP4 codebook quantization.
//! - `iron_fp8_e4m3_quant_dequant` — E4M3 fp8 (3 mantissa bits, max ±448).
//! - `iron_fp8_e5m2_quant_dequant` — E5M2 fp8 (2 mantissa bits, max ±57344).
//!
...
#[kernel]
pub fn iron_fp4_quant_dequant(inp: Tensor<f32>, out: Tensor<f32>, #[constexpr] n: u32) {
...
// ─── iron_fp8_quant_dequant — fp8 (e4m3 / e5m2) quant + dequant ─────────────
//
// The fp8 counterpart of `iron_fp4_quant_dequant` above. fp8 is the standard
...
/// selected by its compile-time `if`. Produces `iron_fp8_e4m3_quant_dequant` and
/// `iron_fp8_e5m2_quant_dequant`.
#[kernel(variants((FMT, NAME) = [(0u32, e4m3), (1u32, e5m2)], suffix = "{NAME}_quant_dequant"))]
pub fn iron_fp8(inp: Tensor<f32>, out: Tensor<f32>, #[constexpr] n: u32) {
...
/// New-syntax correctness for `iron_fp4_quant_dequant` (Grid3D, one 32-lane
...
pub mod kernel_tests {
    ...
    use super::{iron_fp4_quant_dequant, iron_fp8_e4m3_quant_dequant, iron_fp8_e5m2_quant_dequant};
    ...
    fn test_iron_fp4_quant_dequant(_dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_fp4_quant_dequant::kernel_ir_for())
    ...
    fn test_iron_fp8_e4m3_quant_dequant(_dt: DType) -> TestSetup {
        fp8_setup(iron_fp8_e4m3_quant_dequant::kernel_ir_for(), 3.0, -6.0, 8.0, 448.0)
    }
    // e5m2: 2 mantissa bits, exponent [-14, 15], max ±57344.
    #[test_kernel(dtypes = [f32], tol = 1e-3)]
    fn test_iron_fp8_e5m2_quant_dequant(_dt: DType) -> TestSetup {
        fp8_setup(iron_fp8_e5m2_quant_dequant::kernel_ir_for(), 2.0, -14.0, 15.0, 57344.0)
    }
}
...
pub mod kernel_benches {
    ...
    // consecutive elements — the same element-to-simdgroup grouping the Iron Grid3D
    // dispatch uses. `inp` is shared by name with the Iron input below.
    //
    ...
    // split into two 16-lane amax groups), whereas `iron_fp4_quant_dequant` takes a
    ...
    fn bench_fp4(_dt: DType) -> BenchSetup {
        let n = QUANT_N;
        BenchSetup::new(iron_fp4_quant_dequant::kernel_ir_for())
            .mode(KernelMode::Grid3D)
            ...
            .with_reference(
                ...
                // w[[0]] shared by name with the Iron `inp`; out[[1]] fresh.
    ...
    #[bench(dtypes = [f32])]
    fn bench_fp8_e4m3(_dt: DType) -> BenchSetup { qb(iron_fp8_e4m3_quant_dequant::kernel_ir_for()) }
    #[bench(dtypes = [f32])]
    fn bench_fp8_e5m2(_dt: DType) -> BenchSetup { qb(iron_fp8_e5m2_quant_dequant::kernel_ir_for()) }
}
