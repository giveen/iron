...
//!
//! Closes the bandwidth-bound prefill gap in Iron. The existing
//! `iron_qmm_mma`, `iron_qmm_mma_m16`, `iron_qmm_bm4`, `iron_qmm_bm2`, `iron_qmm`
//! kernels each handle a *fixed* M class — `iron_qmm_mma` requires `M % 32 == 0`,
//! `iron_qmm_mma_m16` is hard-wired to `M = 16`, etc. The model-level
...
//! the next multiple of 32 and routes to `iron_qmm_mma` for the full
...
//! needed — `iron_qmm_mma` is already in the kernel pack at every
//! shipped dtype.

use wh_iron::core::{dtype::DType, ir::Kernel};

use crate::kernels::gemm::quantized::{iron_qmm_mma, patch_qmm_mma_dtype_aware_skew};

/// Tile geometry mirrors `iron_qmm_mma`. Exposed for callers sizing
/// the dispatch grid + the M-padding step.
pub const BM_TILE: u32 = 32;
pub const BN_TILE: u32 = 32;
pub const BK_TILE: u32 = 32;
/// Threads per group — 4 SG × 32 lanes — matches `iron_qmm_mma`.
...
/// `iron_qmm_mma::kernel_ir_for(dtype)` with the dtype-aware TG skew
/// patch applied (matches the path in `iron_qmm_for` for `M % 32 == 0`).
/// The caller dispatches with grid `[N / 32, m_padded / 32, 1]`
/// and `tpg = 128`.
pub fn kernel_ir_for(dtype: DType) -> Kernel {
    let mut k = iron_qmm_mma::kernel_ir_for(dtype);
    patch_qmm_mma_dtype_aware_skew(&mut k, dtype);
    k.mode = wh_iron::core::ir::KernelMode::Reduction;
    k
}
...
mod tests {
    ...
    #[test]
    fn kernel_ir_for_returns_iron_qmm_mma_per_dtype() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = kernel_ir_for(dt);
            assert_eq!(
                k.name, "iron_qmm_mma",
                "dynamic-M routes to iron_qmm_mma for dtype {:?}",
                dt
            );
            assert_eq!(k.mode, wh_iron::core::ir::KernelMode::Reduction);
        }
    }
}
