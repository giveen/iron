...
//! `iron_qmm_mma_mpp` — production int4 quantized matmul via `mpp::tensor_ops::matmul2d`.
//!
//! This is the MPP (MetalPerformancePrimitives) counterpart of
//! `iron_qmm_mma`. It mirrors the same algorithm — int4 weights dequantized
...
mod tests {
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_qmm_mma_mpp::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_qmm_mma_mpp");
    ...
    fn bf16_stages_through_half() {
        let k = iron_qmm_mma_mpp::kernel_ir_for(DType::BF16);
    ...
    fn codegen_emits_mpp_include_and_kernel_decl() {
        for (dt, t_name) in [(DType::F32, "float"), (DType::F16, "half"), (DType::BF16, "half")] {
            let mut k = iron_qmm_mma_mpp::kernel_ir_for(dt);
            let suffix = match dt {
                DType::F32 => "f32",
                DType::F16 => "f16",
                DType::BF16 => "bf16",
            ...
            k.name = format!("iron_qmm_mma_mpp_{suffix}");
            ...
            assert!(msl.contains(&format!("kernel void iron_qmm_mma_mpp_{suffix}")));
            assert!(msl.contains(&format!("threadgroup {t_name} Xs")));
        }
    }
}

/// New-syntax correctness + benchmark for `iron_qmm_mma_mpp` (MPP/NAX tensor-core
...
/// New-syntax benchmark for `iron_qmm_mma_mpp`.
