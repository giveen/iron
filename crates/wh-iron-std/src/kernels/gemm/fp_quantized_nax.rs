...
//! `iron_fp_qmm_nax` — fp4 (E2M1) quantized matmul via `mpp::tensor_ops::matmul2d`.
//!
...
//!
//! Fp4 counterpart of `iron_qmm_nax`. Mirrors the same coop-load /
...
mod tests {
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_fp_qmm_nax::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_fp_qmm_nax");
    ...
    fn bf16_stages_through_half() {
        let k = iron_fp_qmm_nax::kernel_ir_for(DType::BF16);
    ...
    fn codegen_emits_mpp_include_and_kernel_decl() {
        for (dt, t_name) in [(DType::F32, "float"), (DType::F16, "half"), (DType::BF16, "half")] {
            let mut k = iron_fp_qmm_nax::kernel_ir_for(dt);
            let suffix = match dt {
                DType::F32 => "f32",
                DType::F16 => "f16",
                DType::BF16 => "bf16",
            ...
            k.name = format!("iron_fp_qmm_nax_{suffix}");
            ...
            assert!(msl.contains(&format!("kernel void iron_fp_qmm_nax_{suffix}")));
            assert!(msl.contains(&format!("threadgroup {t_name} Xs")));
        }
    }
}

/// New-syntax correctness + benchmark for `iron_fp_qmm_nax` — the MPP
/// (matmul2d) fp4 E2M1 matmul. Same scale-only E2M1 dequant as
/// `iron_fp4_qmm_mma`, so this reuses that file's shared oracle / bench builder.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_fp_qmm_nax;
    ...
    fn test_fp_qmm_nax(dt: DType) -> TestSetup {
        fp_setup(iron_fp_qmm_nax::kernel_ir_for(dt), 32, 32, 128, 4, dt)
    }
    // Multi-tile (64×64) — cross-threadgroup tile indexing over the MPP path.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 1e-2, 5e-2])]
    fn test_fp_qmm_nax_multi_tile(dt: DType) -> TestSetup {
        fp_setup(iron_fp_qmm_nax::kernel_ir_for(dt), 64, 64, 128, 4, dt)
    }
}

/// New-syntax benchmark for `iron_fp_qmm_nax`.
pub mod kernel_benches {
    ...
    fn bench_fp_qmm_nax(dt: DType) -> BenchSetup {
        fpb(iron_fp_qmm_nax::kernel_ir_for(dt), 32, 4096, 4096, 4, dt)
    }
}
