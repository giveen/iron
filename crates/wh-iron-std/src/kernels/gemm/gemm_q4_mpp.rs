...
//! for Nemotron prefill. The MMA twin of `iron_gemm_q8_mpp`, but reading the
...
//! scalar `iron_gemm` (which sat at ~0.1% of the tensor-core peak).
//!
//! Same 64×64×32 coop_tile geometry as `iron_gemm_q8_mpp` (4 simdgroups,
//! 2×2 warp grid, 128 threads/tg). Only the weight-dequant block differs.
//!
//! ## Q4 weight layout (matches `iron_ops::quantize_q4`)
...
pub mod kernel_benches {
    ...
    fn bench_gemm_q4_mpp(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gemm_q4_mpp::kernel_ir_for(dt))
...
pub mod kernel_tests {
    ...
    fn setup(n_rows: usize, out_dim: usize, k_in: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_gemm_q4_mpp::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
