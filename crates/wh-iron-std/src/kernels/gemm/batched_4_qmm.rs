...
//! `iron_batched_4_qgemv_fast` (M=1, 4-output) and the 4-output cousin
//! of `iron_batched_qkv_qmm_fast` (M>1, 3-output).
//!
...
//!
//! Dispatch geometry mirrors `iron_batched_4_qgemv_fast`:
...
/// rows no-op.
#[kernel]
pub fn iron_batched_4_qmm_fast<T>(
    x: Tensor<T>,
    w_a: Tensor<u32>,
    scales_a: Tensor<T>,
    biases_a: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::iron_batched_4_qmm_fast;
    ...
    fn test_batched_4_qmm_fast(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_batched_4_qmm_fast::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_batched_4_qmm_fast` — MLX-less reduction
...
pub mod kernel_benches {
    ...
    fn bench_batched_4_qmm_fast(dt: DType) -> BenchSetup {
        ...
        let n_tgs = out_a.max(out_b).max(out_c).max(out_d).div_ceil(8);
        BenchSetup::new(iron_batched_4_qmm_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
