...
/// Decodes through the shared `kernels/primitives.rs`. Produces `iron_<FMT>_qmm`.
#[kernel(variants(
...
pub mod kernel_tests {
    ...
    fn test_mxfp4_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxfp4_qmm::kernel_ir_for(dt), QFormat::Mxfp4, 3, 4, 256, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_nvfp4_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_nvfp4_qmm::kernel_ir_for(dt), QFormat::Nvfp4, 3, 4, 256, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxfp8_e4m3_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxfp8_e4m3_qmm::kernel_ir_for(dt), QFormat::Mxfp8E4, 3, 4, 256, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxfp8_e5m2_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxfp8_e5m2_qmm::kernel_ir_for(dt), QFormat::Mxfp8E5, 3, 4, 256, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_nvfp8_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_nvfp8_qmm::kernel_ir_for(dt), QFormat::Nvfp8, 3, 4, 256, dt)
    }
    ...
    fn test_fp4_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_fp4_qmm::kernel_ir_for(dt), QFormat::Fp4, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_nvfp8_qmm::kernel_ir_for(dt), QFormat::Fp8E4m3, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_fp8_e5m2_qmm::kernel_ir_for(dt), QFormat::Fp8E5m2, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int8_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int8_qmm::kernel_ir_for(dt), QFormat::Int8, 3, 4, 256, dt)
    }
    ...
    fn test_int2_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int2_qmm::kernel_ir_for(dt), QFormat::Int2, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int3_qmm::kernel_ir_for(dt), QFormat::Int3, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int4_qmm::kernel_ir_for(dt), QFormat::Int4, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int5_qmm::kernel_ir_for(dt), QFormat::Int5, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int6_qmm::kernel_ir_for(dt), QFormat::Int6, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint2_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxint2_qmm::kernel_ir_for(dt), QFormat::Mxint2, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint3_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxint3_qmm::kernel_ir_for(dt), QFormat::Mxint3, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint4_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxint4_qmm::kernel_ir_for(dt), QFormat::Mxint4, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint5_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxint5_qmm::kernel_ir_for(dt), QFormat::Mxint5, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint6_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxint6_qmm::kernel_ir_for(dt), QFormat::Mxint6, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint8_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_mxint8_qmm::kernel_ir_for(dt), QFormat::Mxint8, 3, 4, 256, dt)
    }
    ...
    fn test_nvfp8_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_nvfp8_f16_qmm::kernel_ir_for(dt), QFormat::Nvfp8F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_nvfp8_f16_qmm::kernel_ir_for(dt), QFormat::Fp8E4m3F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp4_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_fp4_f16_qmm::kernel_ir_for(dt), QFormat::Fp4F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_fp8_e5m2_f16_qmm::kernel_ir_for(dt), QFormat::Fp8E5m2F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int2_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int2_f16_qmm::kernel_ir_for(dt), QFormat::Int2F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int3_f16_qmm::kernel_ir_for(dt), QFormat::Int3F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int4_f16_qmm::kernel_ir_for(dt), QFormat::Int4F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int5_f16_qmm::kernel_ir_for(dt), QFormat::Int5F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int6_f16_qmm::kernel_ir_for(dt), QFormat::Int6F16, 3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int8_f16_qmm(dt: DType) -> TestSetup {
        qmm_setup(iron_int8_f16_qmm::kernel_ir_for(dt), QFormat::Int8F16, 3, 4, 256, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_mxfp4_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxfp4_qmm::kernel_ir_for(dt), QFormat::Mxfp4, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp4_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_nvfp4_qmm::kernel_ir_for(dt), QFormat::Nvfp4, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e4m3_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxfp8_e4m3_qmm::kernel_ir_for(dt), QFormat::Mxfp8E4, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e5m2_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxfp8_e5m2_qmm::kernel_ir_for(dt), QFormat::Mxfp8E5, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp8_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_nvfp8_qmm::kernel_ir_for(dt), QFormat::Nvfp8, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp4_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_fp4_qmm::kernel_ir_for(dt), QFormat::Fp4, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_nvfp8_qmm::kernel_ir_for(dt), QFormat::Fp8E4m3, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_fp8_e5m2_qmm::kernel_ir_for(dt), QFormat::Fp8E5m2, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int8_qmm::kernel_ir_for(dt), QFormat::Int8, 32, 4096, 4096, dt)
    }
    // Symmetric sub-byte ints (FP32 group scale) + MXINT (E8M0 block scale).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int2_qmm::kernel_ir_for(dt), QFormat::Int2, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int3_qmm::kernel_ir_for(dt), QFormat::Int3, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int4_qmm::kernel_ir_for(dt), QFormat::Int4, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int5_qmm::kernel_ir_for(dt), QFormat::Int5, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int6_qmm::kernel_ir_for(dt), QFormat::Int6, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint2_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxint2_qmm::kernel_ir_for(dt), QFormat::Mxint2, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint3_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxint3_qmm::kernel_ir_for(dt), QFormat::Mxint3, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint4_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxint4_qmm::kernel_ir_for(dt), QFormat::Mxint4, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint5_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxint5_qmm::kernel_ir_for(dt), QFormat::Mxint5, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint6_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxint6_qmm::kernel_ir_for(dt), QFormat::Mxint6, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint8_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_mxint8_qmm::kernel_ir_for(dt), QFormat::Mxint8, 32, 4096, 4096, dt)
    }
    // FP16-scale twins. `fp8_e4m3_f16` reuses the `nvfp8_f16` kernel.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp8_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_nvfp8_f16_qmm::kernel_ir_for(dt), QFormat::Nvfp8F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_nvfp8_f16_qmm::kernel_ir_for(dt), QFormat::Fp8E4m3F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp4_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_fp4_f16_qmm::kernel_ir_for(dt), QFormat::Fp4F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_fp8_e5m2_f16_qmm::kernel_ir_for(dt), QFormat::Fp8E5m2F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int2_f16_qmm::kernel_ir_for(dt), QFormat::Int2F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int3_f16_qmm::kernel_ir_for(dt), QFormat::Int3F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int4_f16_qmm::kernel_ir_for(dt), QFormat::Int4F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int5_f16_qmm::kernel_ir_for(dt), QFormat::Int5F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int6_f16_qmm::kernel_ir_for(dt), QFormat::Int6F16, 32, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_f16_qmm(dt: DType) -> BenchSetup {
        qmm_bench(iron_int8_f16_qmm::kernel_ir_for(dt), QFormat::Int8F16, 32, 4096, 4096, dt)
    }
}
