...
//!
//! This is the M>1 sibling of `iron_batched_qkv_qgemv_fast`. At
...
//!
//! Dispatch geometry mirrors `iron_batched_qkv_qgemv_fast`:
...
#[kernel]
pub fn iron_batched_qkv_qmm_fast<T>(
    x: Tensor<T>,
    w_q: Tensor<u32>,
    scales_q: Tensor<T>,
    biases_q: Tensor<T>,
...
mod dump_audit {
    ...
    /// runtime status. This differs from `iron_gated_delta_prep_chunk`'s
    /// `state_reg`, whose touching loop bound (`n_per_t`) is itself
    /// constexpr-derived.
    /// `cargo test -p wh-iron-std --lib --release -- kernels::gemm::batched_qkv_qmm::dump_audit::dump --nocapture`
    #[test]
    fn dump() {
        use wh_iron::codegen::msl::MslGenerator;
        let mut k = iron_batched_qkv_qmm_fast::kernel_ir_for(DType::F32);
...
pub mod kernel_tests {
    ...
    use super::iron_batched_qkv_qmm_fast;
    ...
    fn test_batched_qkv_qmm_fast(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_batched_qkv_qmm_fast::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_batched_qkv_qmm_fast` — MLX-less reduction
...
pub mod kernel_benches {
    ...
    fn bench_batched_qkv_qmm_fast(dt: DType) -> BenchSetup {
        ...
        let n_tgs = out_q.max(out_k).max(out_v).div_ceil(8);
        BenchSetup::new(iron_batched_qkv_qmm_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
