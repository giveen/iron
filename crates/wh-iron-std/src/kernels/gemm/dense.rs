...
// i.e. the SAME `out = input · weightᵀ` contraction as `iron_gemm`, replicated
...
// `iron_gemm`; the batch index rides the 3rd grid axis (`tgid_z`).
//
// ## DISPATCH INVARIANTS (identical to `iron_gemm` plus the batch axis)
...
/// New-syntax correctness tests for `iron_gemm` — the multi-row 32×32-tiled
...
pub mod kernel_tests {
    ...
    fn gemm_setup(n_rows: usize, in_dim: usize, out_dim: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_gemm::kernel_ir_for(dt))
    ...
    fn gemm_batched_setup(
        ...
        TestSetup::new(iron_gemm_batched::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("weight", pack_f32(&weight_f, dt), dt))
            .input(TestBuffer::from_vec("input", pack_f32(&input_f, dt), dt))
...
/// New-syntax benchmark for `iron_gemm`. Nemotron-class block-diffusion shape:
...
pub mod kernel_benches {
    ...
    fn bench_gemm(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gemm::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
