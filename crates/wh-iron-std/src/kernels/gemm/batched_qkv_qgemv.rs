...
//! Two variants:
//!
//! **`iron_batched_qkv_qgemv`** — one output row per TG (original
//! correctness-first variant). Grid: `[max(out_q,out_k,out_v), 1, 3]`;
//! `program_id::<0>()` = output row, `program_id::<2>()` = matrix.
//!
//! **`iron_batched_qkv_qgemv_fast`** — 8 output rows per TG, mirroring
//! `iron_qmv`'s geometry. Each TG computes 8 output rows of the matrix
//! selected by `program_id::<2>()`. Grid:
//! `[ceil(max(out_q,out_k,out_v)/8), 1, 3]`, TPG = 64 (2 simdgroups ×
//! 32 lanes). Uses `iron_qmv`'s mask-without-shift trick + algebraic-split
//! accumulator (`s*q_dot + b*xs`) — identical inner loop to
//! `iron_rms_norm_qgemv_fast` but without the RMSNorm phase.
...
#[kernel]
pub fn iron_batched_qkv_qgemv<T>(
    x: Tensor<T>,
    w_q: Tensor<u32>,
    scales_q: Tensor<T>,
    biases_q: Tensor<T>,
...
/// computes 4 output rows (row0..row3). Uses `iron_qmv`'s mask-without-shift
...
#[kernel]
pub fn iron_batched_qkv_qgemv_fast<T>(
    x: Tensor<T>,
    w_q: Tensor<u32>,
    scales_q: Tensor<T>,
    biases_q: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::{iron_batched_qkv_qgemv, iron_batched_qkv_qgemv_fast};
    ...
    fn test_batched_qkv_qgemv(dt: DType) -> TestSetup {
        ...
        setup(iron_batched_qkv_qgemv::kernel_ir_for(dt), dt, in_dim, gs, out_q, out_k, out_v)
    ...
    fn test_batched_qkv_qgemv_fast(dt: DType) -> TestSetup {
        ...
        setup(iron_batched_qkv_qgemv_fast::kernel_ir_for(dt), dt, in_dim, gs, out_q, out_k, out_v)
...
pub mod kernel_benches {
    ...
    fn bench_batched_qkv_qgemv(dt: DType) -> BenchSetup {
        ...
        let max_rows = out_q.max(out_k).max(out_v);
        let s =
            BenchSetup::new(iron_batched_qkv_qgemv::kernel_ir_for(dt)).mode(KernelMode::Reduction);
    ...
    fn bench_batched_qkv_qgemv_fast(dt: DType) -> BenchSetup {
        let (in_dim, gs, out_q, out_k, out_v) =
            (4096usize, 64usize, 4096usize, 1024usize, 1024usize);
        let n_tgs = out_q.max(out_k).max(out_v).div_ceil(8);
        let s = BenchSetup::new(iron_batched_qkv_qgemv_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction);
