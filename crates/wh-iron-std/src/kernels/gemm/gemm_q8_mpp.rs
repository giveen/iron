...
//! SCALAR `iron_gemm_q8` (which does `acc += w*x` on the ALUs, ~12× slower
...
//! Name contains `_mpp_` so Iron's PSOCache LIVE-COMPILES it (the offline
...
/// `iron_grouped_gemm_q8` (which is scalar). For prefill O-LoRA-A: output
...
/// MMA as iron_gemm_q8_mpp. Live-compiled (name has _mpp_).
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_grouped_gemm_q8_mpp<T>(
    x: Tensor<T>,
    qs: Tensor<u32>,
    d_f32: Tensor<f32>,
    mut out: Tensor<T>,
...
pub mod kernel_benches {
    ...
    use super::{iron_gemm_q8_mpp, iron_grouped_gemm_q8_mpp};
    ...
    fn bench_gemm_q8_mpp(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gemm_q8_mpp::kernel_ir_for(dt))
    ...
    fn bench_grouped_gemm_q8_mpp(dt: DType) -> BenchSetup {
        ...
        let n_blocks = out_dim * k_in / 32;
        BenchSetup::new(iron_grouped_gemm_q8_mpp::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
