...
//!
//! Unlike the `iron_aura_flash_p1` + `iron_aura_flash_pass2` pair, this does the
...
//!
//! Layout (matches `iron_aura_flash_p1` / `iron_aura_score` / `iron_aura_value` — all
...
//! sibling `iron_aura_flash_p1` / `iron_aura_flash_pass2` / `iron_aura_score` /
//! `iron_aura_value` kernels were genericised to `Tensor<T>` during the bf16
//! coverage rollout; this kernel was the last laggard. Unifying the
//! dtype contract lets Iron's cache store norms+codebook in the
...
//! shape of `iron_aura_flash_p1` (correctness-equivalent; token-parallelism
//! is a perf follow-up).
//!
//! ## DISPATCH INVARIANTS
//!
...
pub mod kernel_tests {
    ...
    use super::iron_aura_flash_sdpa_kb4_vb4_d128;
    ...
    fn sdpa_setup(dt: DType, has_sinks: bool, window_size: usize) -> TestSetup {
        ...
        TestSetup::new(iron_aura_flash_sdpa_kb4_vb4_d128::kernel_ir_for(dt))
    ...
    fn test_iron_aura_flash_sdpa_kb4_vb4_d128(dt: DType) -> TestSetup { sdpa_setup(dt, false, 0) }
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_iron_aura_flash_sdpa_sinks(dt: DType) -> TestSetup { sdpa_setup(dt, true, 0) }
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_iron_aura_flash_sdpa_window(dt: DType) -> TestSetup { sdpa_setup(dt, false, 4) }

    // Sinks AND sliding window together — the GPT-OSS sink-attention recipe.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_iron_aura_flash_sdpa_sinks_window(dt: DType) -> TestSetup { sdpa_setup(dt, true, 4) }
}
...
pub mod kernel_benches {
    ...
    fn bench_sdpa(dt: DType) -> BenchSetup {
        setup(iron_aura_flash_sdpa_kb4_vbVB_dDIM::kernel_ir_for(dt), DIM, 4, VB, dt)
    }
}
