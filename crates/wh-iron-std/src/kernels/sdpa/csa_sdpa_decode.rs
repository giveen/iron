...
mod tests {
    ...
    use super::iron_csa_sdpa_decode;

    fn msl_for(dt: DType) -> String {
        let mut k = iron_csa_sdpa_decode::kernel_ir_for(dt);
        k.mode = KernelMode::Reduction;
        MslGenerator::default().generate(&k).expect("iron_csa_sdpa_decode codegen succeeds")
    }
    ...
    fn codegen_produces_nonempty_msl_for_all_float_dtypes() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            ...
            assert!(
                src.contains("kernel void iron_csa_sdpa_decode"),
                "MSL for {dt:?} should declare iron_csa_sdpa_decode:\n{src}",
            );
        }
    }
}
...
pub mod kernel_tests {
    ...
    fn test_dsv4_csa_sdpa_decode(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_csa_sdpa_decode::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_csa(dt: DType) -> BenchSetup {
        ...
        let bytes = (2 * n_q_heads * head_dim + 2 * n_kv_heads * kv_stride * head_dim)
            * dt.size_bytes()
            + n_selected * 4;
        BenchSetup::new(iron_csa_sdpa_decode::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
