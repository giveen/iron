...
//! by `iron_aura_flash_p1` (one tuple per (q_idx, block_idx) pair) into a
...
///
/// Produces kernels: `iron_aura_flash_pass2_d64`, `_d80`, `_d96`, `_d128`,
/// `_d256`, `_d512`.
///
...
pub fn iron_aura_flash_pass2<T>(
    o_partials: Tensor<T>,
    m_partials: Tensor<T>,
    l_partials: Tensor<T>,
    mut output: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::iron_aura_flash_pass2_d128;
    ...
    // non-causal `iron_aura_flash_p1` per-block online-softmax on the CPU
    ...
    /// Emulate `iron_aura_flash_p1` (non-causal): per (q_head, block)
    ...
    fn test_iron_aura_flash_pass2_combined(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_aura_flash_pass2_d128::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_flash_pass2(dt: DType) -> BenchSetup {
        flash_pass2(BenchSetup::new(iron_aura_flash_pass2_dDIM::kernel_ir_for(dt)), DIM, dt)
    }
}
