...
//!
//! This file ships the K=2 (`iron_sdpa_decode_batched_q2`) and K=4
//! (`iron_sdpa_decode_batched_q4`) decode-form specializations. K=8 and
//! K=16 land separately via prefill-tile reuse through
//! `iron_sdpa_prefill_mma` (the FA-2 tile already implements the
//! KV-reuse pattern at BQ × BK; the batched variant arm dispatches it
//! with `q_len = K, k_len = n_kv`). See
//! `wh-iron-planning/M7-batched-q-sdpa-plan.md`.
//!
//! ## DISPATCH INVARIANTS
//!
//! - **K=2 (`iron_sdpa_decode_batched_q2`):** TPG = 1024 (32 simdgroups × 32
...
//! - **K=4 (`iron_sdpa_decode_batched_q4`):** TPG ≤ 768; recommended 512
...
#[kernel]
pub fn iron_sdpa_decode_batched_q2<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_batched_q4<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_batched_q8<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
mod tests {
    ...
    use super::iron_sdpa_decode_batched_q2;

    fn msl_for(dt: DType) -> String {
        let mut k = iron_sdpa_decode_batched_q2::kernel_ir_for(dt);
        k.mode = KernelMode::Reduction;
        MslGenerator::default().generate(&k).expect("iron_sdpa_decode_batched_q2 codegen succeeds")
    }
    ...
    fn codegen_produces_nonempty_msl_for_all_float_dtypes() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            ...
            assert!(
                src.contains("kernel void iron_sdpa_decode_batched_q2"),
    ...
    fn msl_for_q4(dt: DType) -> String {
        let mut k = super::iron_sdpa_decode_batched_q4::kernel_ir_for(dt);
        k.mode = KernelMode::Reduction;
        MslGenerator::default().generate(&k).expect("iron_sdpa_decode_batched_q4 codegen succeeds")
    }
    ...
    fn q4_codegen_produces_nonempty_msl_for_all_float_dtypes() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            ...
            assert!(!src.trim().is_empty(), "MSL for {dt:?} should not be empty");
            assert!(
                src.contains("kernel void iron_sdpa_decode_batched_q4"),
                "MSL for {dt:?} should declare the kernel function:\n{src}",
            );
        }
    }
...
pub mod kernel_tests {
    ...
    use super::{
        iron_sdpa_decode_batched_q2,
        iron_sdpa_decode_batched_q4,
        iron_sdpa_decode_batched_q8,
    };
    ...
    fn test_sdpa_decode_batched_q2(dt: DType) -> TestSetup {
        batched_setup(iron_sdpa_decode_batched_q2::kernel_ir_for(dt), 2, 1024, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_sdpa_decode_batched_q4(dt: DType) -> TestSetup {
        batched_setup(iron_sdpa_decode_batched_q4::kernel_ir_for(dt), 4, 512, dt)
    }
    ...
    fn test_sdpa_decode_batched_q8(dt: DType) -> TestSetup {
        ...
        batched_setup(iron_sdpa_decode_batched_q8::kernel_ir_for(dt), 8, 256, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn setup(ir: wh_iron::core::ir::Kernel, batch_q: usize, dt: DType) -> BenchSetup {
    ...
    fn bench_q2(dt: DType) -> BenchSetup {
        setup(iron_sdpa_decode_batched_q2::kernel_ir_for(dt), 2, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_q4(dt: DType) -> BenchSetup {
        setup(iron_sdpa_decode_batched_q4::kernel_ir_for(dt), 4, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_q8(dt: DType) -> BenchSetup {
        setup(iron_sdpa_decode_batched_q8::kernel_ir_for(dt), 8, dt)
    }
}
