...
/// New-syntax correctness + benchmark for `iron_sdpa` — the baseline decode SDPA
...
/// `sdpa_vector` (gqa=1). Pre-this migration `iron_sdpa` had no CPU-oracle test
/// — validation flowed only through the `iron bench` head-to-head against MLX.
pub mod kernel_tests {
    ...
    fn setup(n_heads: usize, n_kv: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_sdpa` (decode attention, head_dim 128).
pub mod kernel_benches {
    ...
    fn bench_sdpa(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_sdpa::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
