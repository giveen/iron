...
//! `iron_sdpa_multi`, generalising `iron_sdpa_decode_d256` with a
...
//! whose prefill path in Iron (`Qwen35AttentionMixer.forwardMany`)
...
//! machine-freeze hazard as `iron_sdpa_decode_d256` and
//! `iron_sdpa_multi`. Consumers MUST encode these as preconditions
//! in their wrappers.
//!
...
//! This mirrors `iron_sdpa_decode_d256` exactly.

use wh_iron::kernel;

#[kernel]
pub fn iron_sdpa_multi_d256<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
mod tests {
    ...
    use super::iron_sdpa_multi_d256;

    fn msl_for(dt: DType) -> String {
        let mut k = iron_sdpa_multi_d256::kernel_ir_for(dt);
        k.mode = KernelMode::Reduction;
        MslGenerator::default().generate(&k).expect("iron_sdpa_multi_d256 codegen succeeds")
    }
    ...
    fn codegen_produces_nonempty_msl_for_all_float_dtypes() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            ...
            assert!(
                src.contains("kernel void iron_sdpa_multi_d256"),
                "MSL for {dt:?} should declare iron_sdpa_multi_d256:\n{src}",
            );
        }
    }
}
...
pub mod kernel_tests {
    ...
    fn multi_d256_setup(dt: DType, causal: bool) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_multi_d256::kernel_ir_for(dt))
    ...
    fn test_iron_sdpa_multi_d256(dt: DType) -> TestSetup { multi_d256_setup(dt, false) }

    // Causal: query row `r` attends `[0, base_kv + r + 1)`.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 3e-3, 1.5e-2])]
    fn test_iron_sdpa_multi_d256_causal(dt: DType) -> TestSetup { multi_d256_setup(dt, true) }
}
...
pub mod kernel_benches {
    ...
    fn bench_sdpa_multi_d256(dt: DType) -> BenchSetup {
        ...
        let bytes = (2 * n_query * n_q_heads * head_dim + 2 * n_kv_heads * n_kv * head_dim)
            * dt.size_bytes();
        BenchSetup::new(iron_sdpa_multi_d256::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    // Append-shape bench: matches `iron_sdpa_prefill_qtiled_d256`'s bench
    // shape exactly (1024-token chunk landing on an 8192 prefix, causal,
    // GQA 32/8, the real Qwen3.6-A3B config) so `iron bench --diff` can
