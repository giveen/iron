...
//! a time.
//!
//! This is `iron_sdpa_decode` generalised with a query dimension: one
...
//! machine-freeze hazard as `iron_sdpa_decode`. Consumers MUST encode
...
pub fn iron_sdpa_multi<T>(
    ...
    // consecutive Op::Load (same constraint as iron_sdpa_decode).
    ...
    // hit distinct threadgroup-memory banks (see iron_sdpa_decode).
    let stride = ns + 1u32;
...
// ─── Tree-causal variant: `iron_sdpa_multi_tree_mask` ───────────────
//
// Identical to `iron_sdpa_multi` except the `causal: u32` constexpr is
...
//
// Equivalence: `treeCausalMask` from Iron's `DraftTreeNode` is the
// canonical mask producer.
//
// Dispatch invariants — IDENTICAL to `iron_sdpa_multi`. TPG=1024,
// head_dim=128 hard, grid `[n_q_heads * n_query * 1024, 1, 1]`.
#[kernel]
pub fn iron_sdpa_multi_tree_mask<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    mask: Tensor<T>,
...
pub mod kernel_tests {
    ...
    fn multi_setup(dt: DType, causal: bool) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_multi::kernel_ir_for(dt))
    ...
    fn test_iron_sdpa_multi(dt: DType) -> TestSetup { multi_setup(dt, false) }

    // Causal: query row `r` attends `[0, base_kv + r + 1)` — exercises the
    // within-block causal mask the full-mode test leaves dormant.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_multi_causal(dt: DType) -> TestSetup { multi_setup(dt, true) }
}

/// New-syntax benchmark for `iron_sdpa_multi` (`class=GenericEmpty`).
pub mod kernel_benches {
    ...
    fn bench_sdpa_multi(dt: DType) -> BenchSetup {
        ...
        let bytes = (2 * n_query * n_q_heads * head_dim + 2 * n_kv_heads * n_kv * head_dim)
            * dt.size_bytes();
        BenchSetup::new(iron_sdpa_multi::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn bench_sdpa_multi_tree_mask(dt: DType) -> BenchSetup {
        use super::iron_sdpa_multi_tree_mask;
        let (n_q_heads, n_kv_heads, head_dim) = (32usize, 8usize, 128usize);
        let (base_kv, n_query) = (4096usize, 8usize);
        let kv_stride = base_kv + n_query;
        let heads_per_group = n_q_heads / n_kv_heads;
        ...
        let bytes = (2 * n_query * n_q_heads * head_dim
            ...
            + n_query * n_query)
            * dt.size_bytes();
        BenchSetup::new(iron_sdpa_multi_tree_mask::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("q", n_query * n_q_heads * head_dim, dt))
            .buffer(BenchBuffer::random("k", n_kv_heads * kv_stride * head_dim, dt))
            .buffer(BenchBuffer::random("v", n_kv_heads * kv_stride * head_dim, dt))
