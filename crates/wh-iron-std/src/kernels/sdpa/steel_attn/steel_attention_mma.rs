...
//! Prefill SDPA via `simdgroup_multiply_accumulate` (MMA) — `iron_sdpa_prefill_mma`.
//!
//! Mirrors `iron_sdpa_prefill`'s outer geometry (BQ=32, BK=16, BD=128, WM=4,
...
#[kernel]
pub fn iron_sdpa_prefill_mma<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
    ...
    #[constexpr] scale: f32,
    // W48 direct-KV extension (sdpa_kv_compact elimination, see
    // Laguna Parity compendium §16-TargetA): `k_len` now means the
    // K/V ADDRESSING STRIDE (row pitch of the k/v buffers), which for
    // the legacy compacted-KV callers equals the exact packed length
    // (so nothing changes for them — see below) but for the new
    // direct-KV caller is the raw KV cache's `maxSeq` capacity.
    // `position` is the absolute sequence position of this Q chunk's
    // first row (previously implicitly derived as `k_len - q_len`,
    // which only worked because compaction always sized the packed
    // buffer to exactly `position + q_len`). `k_real` is the number
    // of REAL (non-pad) KV rows; any k >= k_real is masked to -inf
    // alongside the existing causal test, so a direct-strided read
    // over a raw, not-necessarily-zeroed cache tail can never leak
    // pad/stale content into the softmax denominator.
    //
    // Legacy (compacted-KV) call sites are byte-identical no-ops:
    // pass `position = k_len - q_len` (the same value the kernel used
    // to derive internally) and `k_real = k_len` (the compacted
    // buffer's length IS the real length — the new mask never trips
    // since the loop never reaches `k_len` in the first place).
    #[constexpr] position: u32,
    #[constexpr] k_real: u32,
) {
    let q_tile = tgid_x;
    let q_head = tgid_y;
    let batch = tgid_z;
    ...
    let causal_kb_lim = (q_tile_last_abs / bk) + 1u32;
    // Never scan past the real KV length (the pad-mask boundary) or
    // past the addressing stride (the buffer's actual row capacity —
    // `k_len` blocks floor-divided so `addr_kb_lim * bk <= k_len`
    // always, guarding the direct-KV caller's raw, non-bk-aligned
    // `maxSeq` against an out-of-bounds coop-load read). Legacy
    // compacted-KV callers pass `k_real == k_len` and a `bk`-aligned
    // `k_len` (both q_len/k_len are always 32-tile-aligned there), so
    // both extra clamps are no-ops — `min` never picks them over
    // `causal_kb_lim` — preserving byte-identical behavior.
    let real_kb_lim = (k_real + bk - 1u32) / bk;
    let addr_kb_lim = k_len / bk;
    let kb_lim = min(min(causal_kb_lim, real_kb_lim), addr_kb_lim);
    for kb in range(0u32, kb_lim, 1u32) {
        ...
        // Causal OR pad-region (k >= k_real): either masks to -inf.
        // Legacy compacted callers have k_real == k_len == the loop's
        // own upper bound, so `kb_off + fnX >= k_real` can never be
        // true inside this loop — pure causal, unchanged.
        let s00 = select(kb_off + fn0 > q_abs || kb_off + fn0 >= k_real, neg_infinity(), raw_s00);
        let s01 = select(kb_off + fn1 > q_abs || kb_off + fn1 >= k_real, neg_infinity(), raw_s01);
        let s10 = select(
            kb_off + 8u32 + fn0 > q_abs || kb_off + 8u32 + fn0 >= k_real,
            neg_infinity(),
            raw_s10,
        );
        let s11 = select(
            kb_off + 8u32 + fn1 > q_abs || kb_off + 8u32 + fn1 >= k_real,
            neg_infinity(),
            raw_s11,
        );
...
#[kernel]
pub fn iron_sdpa_prefill_mma_transbf16(
    q: Tensor<f16>,
    k: Tensor<f16>,
    v: Tensor<f16>,
    out: Tensor<bf16>,
    ...
    let q_tile_first = q_tile * bq + sg * bq_sg;
    let q_row_base = q_head_row_off + q_tile_first * head_dim;
    // W58 fused epilogue: write O straight to the TRANSPOSED bf16 layout
    // out[t, q_head*head_dim + c] (row-major [q_len, n_q_heads*head_dim]),
    // eliminating the separate transpose + f16->bf16 cast dispatches.
    let out_row_stride = n_q_heads * head_dim;
    let out_head_off = q_head * head_dim;
    // kv_ld = head_dim + 8 = 136 bank-skew pad on the column-major K^T
    // reads (`tg_ks[fn * kv_ld + fm]` strides by kv_ld across lanes).
    // Median-of-5 sweep (2026-05-19, see selector docstring) confirmed
    // +8 wins on the f16 4-byte-load bank pattern. Real wins: M2 f16
    ...
    // ── Preload 16 Q frags (one per d_frag of head_dim=128), pre-scaled ──
    let q_f0 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f0, 0, load(q[q_row_base + fm * head_dim + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f0, 1, load(q[q_row_base + fm * head_dim + fn1]).cast::<f16>());
    let q_f1 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f1, 0, load(q[q_row_base + fm * head_dim + 8u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f1, 1, load(q[q_row_base + fm * head_dim + 8u32 + fn1]).cast::<f16>());
    let q_f2 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f2, 0, load(q[q_row_base + fm * head_dim + 16u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f2, 1, load(q[q_row_base + fm * head_dim + 16u32 + fn1]).cast::<f16>());
    let q_f3 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f3, 0, load(q[q_row_base + fm * head_dim + 24u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f3, 1, load(q[q_row_base + fm * head_dim + 24u32 + fn1]).cast::<f16>());
    let q_f4 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f4, 0, load(q[q_row_base + fm * head_dim + 32u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f4, 1, load(q[q_row_base + fm * head_dim + 32u32 + fn1]).cast::<f16>());
    let q_f5 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f5, 0, load(q[q_row_base + fm * head_dim + 40u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f5, 1, load(q[q_row_base + fm * head_dim + 40u32 + fn1]).cast::<f16>());
    let q_f6 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f6, 0, load(q[q_row_base + fm * head_dim + 48u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f6, 1, load(q[q_row_base + fm * head_dim + 48u32 + fn1]).cast::<f16>());
    let q_f7 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f7, 0, load(q[q_row_base + fm * head_dim + 56u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f7, 1, load(q[q_row_base + fm * head_dim + 56u32 + fn1]).cast::<f16>());
    let q_f8 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f8, 0, load(q[q_row_base + fm * head_dim + 64u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f8, 1, load(q[q_row_base + fm * head_dim + 64u32 + fn1]).cast::<f16>());
    let q_f9 = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_f9, 0, load(q[q_row_base + fm * head_dim + 72u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_f9, 1, load(q[q_row_base + fm * head_dim + 72u32 + fn1]).cast::<f16>());
    let q_fa = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_fa, 0, load(q[q_row_base + fm * head_dim + 80u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_fa, 1, load(q[q_row_base + fm * head_dim + 80u32 + fn1]).cast::<f16>());
    let q_fb = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_fb, 0, load(q[q_row_base + fm * head_dim + 88u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_fb, 1, load(q[q_row_base + fm * head_dim + 88u32 + fn1]).cast::<f16>());
    let q_fc = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_fc, 0, load(q[q_row_base + fm * head_dim + 96u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_fc, 1, load(q[q_row_base + fm * head_dim + 96u32 + fn1]).cast::<f16>());
    let q_fd = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_fd, 0, load(q[q_row_base + fm * head_dim + 104u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_fd, 1, load(q[q_row_base + fm * head_dim + 104u32 + fn1]).cast::<f16>());
    let q_fe = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_fe, 0, load(q[q_row_base + fm * head_dim + 112u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_fe, 1, load(q[q_row_base + fm * head_dim + 112u32 + fn1]).cast::<f16>());
    let q_ff = simdgroup_alloc::<T, 8, 8>();
    simdgroup_elem_store(q_ff, 0, load(q[q_row_base + fm * head_dim + 120u32 + fn0]).cast::<f16>());
    simdgroup_elem_store(q_ff, 1, load(q[q_row_base + fm * head_dim + 120u32 + fn1]).cast::<f16>());
    // ── Init 16 O frags to zero ──
    let o_f0 = simdgroup_alloc::<f32, 8, 8>();
    simdgroup_elem_store(o_f0, 0, 0.0f32);
    simdgroup_elem_store(o_f0, 1, 0.0f32);
    ...
    for kb in range(0u32, kb_lim, 1u32) {
        ...
        for kr in range(0u32, bk, 1u32) {
            let kv_off = kv_row_base + (kb_off + kr) * head_dim + lane_in_tg;
            let kr_off = kr * kv_ld;
            threadgroup_store("tg_ks", kr_off + lane_in_tg, load(k[kv_off]).cast::<f16>());
            threadgroup_store("tg_vs", kr_off + lane_in_tg, load(v[kv_off]).cast::<f16>());
        }
        threadgroup_barrier();
        // ── S = Q · K^T (32 matmuls per SG: 16 d_frags × 2 k_chunks) ──
        simdgroup_elem_store(s_f0, 0, 0.0f32);
        ...
        // Write P into distinct p_f0/p_f1 (not s_f0/s_f1) so P·V matmul reads
        // from a frag that wasn't just the Q·K^T accumulator.
        simdgroup_elem_store(p_f0, 0, p00.cast::<f16>());
        simdgroup_elem_store(p_f0, 1, p01.cast::<f16>());
        simdgroup_elem_store(p_f1, 0, p10.cast::<f16>());
        simdgroup_elem_store(p_f1, 1, p11.cast::<f16>());
        // ── Scale all 16 O frags by m_diff ──
        simdgroup_elem_store(o_f0, 0, simdgroup_elem_load(o_f0, 0) * m_diff);
        simdgroup_elem_store(o_f0, 1, simdgroup_elem_load(o_f0, 1) * m_diff);
        simdgroup_elem_store(o_f1, 0, simdgroup_elem_load(o_f1, 0) * m_diff);
    ...
    // ── Final normalize + write O (fused transpose + f16->bf16) ──
    let is_row = select(s_row > 0.0f32, 1.0f32 / s_row, 0.0f32);
    let out_row = q_tile_first + fm;
    let out_base = out_row * out_row_stride + out_head_off;
    store(
        out[out_base + fn0],
        (simdgroup_elem_load(o_f0, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + fn1],
        (simdgroup_elem_load(o_f0, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 8u32 + fn0],
        (simdgroup_elem_load(o_f1, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 8u32 + fn1],
        (simdgroup_elem_load(o_f1, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 16u32 + fn0],
        (simdgroup_elem_load(o_f2, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 16u32 + fn1],
        (simdgroup_elem_load(o_f2, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 24u32 + fn0],
        (simdgroup_elem_load(o_f3, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 24u32 + fn1],
        (simdgroup_elem_load(o_f3, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 32u32 + fn0],
        (simdgroup_elem_load(o_f4, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 32u32 + fn1],
        (simdgroup_elem_load(o_f4, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 40u32 + fn0],
        (simdgroup_elem_load(o_f5, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 40u32 + fn1],
        (simdgroup_elem_load(o_f5, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 48u32 + fn0],
        (simdgroup_elem_load(o_f6, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 48u32 + fn1],
        (simdgroup_elem_load(o_f6, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 56u32 + fn0],
        (simdgroup_elem_load(o_f7, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 56u32 + fn1],
        (simdgroup_elem_load(o_f7, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 64u32 + fn0],
        (simdgroup_elem_load(o_f8, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 64u32 + fn1],
        (simdgroup_elem_load(o_f8, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 72u32 + fn0],
        (simdgroup_elem_load(o_f9, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 72u32 + fn1],
        (simdgroup_elem_load(o_f9, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 80u32 + fn0],
        (simdgroup_elem_load(o_fa, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 80u32 + fn1],
        (simdgroup_elem_load(o_fa, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 88u32 + fn0],
        (simdgroup_elem_load(o_fb, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 88u32 + fn1],
        (simdgroup_elem_load(o_fb, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 96u32 + fn0],
        (simdgroup_elem_load(o_fc, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 96u32 + fn1],
        (simdgroup_elem_load(o_fc, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 104u32 + fn0],
        (simdgroup_elem_load(o_fd, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 104u32 + fn1],
        (simdgroup_elem_load(o_fd, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 112u32 + fn0],
        (simdgroup_elem_load(o_fe, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 112u32 + fn1],
        (simdgroup_elem_load(o_fe, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 120u32 + fn0],
        (simdgroup_elem_load(o_ff, 0) * is_row).cast::<f16>().cast::<bf16>(),
    );
    store(
        out[out_base + 120u32 + fn1],
        (simdgroup_elem_load(o_ff, 1) * is_row).cast::<f16>().cast::<bf16>(),
    );
}

/// New-syntax correctness for the prefill-tile SDPA kernel
/// (`iron_sdpa_prefill_mma`). This MMA kernel powers M7's K=8/16 batched-Q
...
pub mod kernel_tests {
    ...
    use super::iron_sdpa_prefill_mma;
    use crate::{
        kernels::sdpa::steel_attn::steel_attention_mma_bf16::iron_sdpa_prefill_mma_bf16,
    ...
    /// Causal-prefix SDPA reference for the W48 direct-KV extension:
    /// query row `qi` attends `[0, min(position + qi + 1, k_real))` —
    /// same as `naive_causal_prefix` except the visible bound is
    /// `k_real` (real KV length) rather than `k_len` (addressing
    /// stride/buffer capacity), and `position` is explicit rather than
    /// derived as `k_len - q_len`. Reduces to exactly
    /// `naive_causal_prefix`'s math when `k_real == k_len` and
    /// `position == k_len - q_len`.
    #[allow(clippy::too_many_arguments)]
    fn naive_causal_prefix_padded(
        q: &[f32],
        k: &[f32],
        v: &[f32],
        n_q_heads: usize,
        ...
        for qh in 0..n_q_heads {
            ...
            for qi in 0..q_len {
                let q_off = q_head_off + qi * head_dim;
                let visible_end = (position + qi + 1).min(k_real);
                let mut scores = vec![0.0f32; visible_end];
                for (t, score) in scores.iter_mut().enumerate() {
                    let k_off = kv_slab + t * head_dim;
                    let mut dot = 0.0f32;
    ...
    /// `iron_sdpa_prefill_mma`-only setup (the W48 direct-KV extension;
    /// `iron_sdpa_prefill_mma_bf16` keeps the original 10-param signature
    /// and still uses `prefill_setup` above). `k_len` is the K/V
    /// ADDRESSING STRIDE (buffer row pitch); `k_real` is the real
    /// (unpadded) KV length. The `[k_real, k_len)` tail of the K/V
    /// buffers is poisoned with large out-of-distribution values so any
    /// leak past the pad mask produces a large, easy-to-see divergence
    /// rather than a silent near-zero one (a mutation check, not just a
    /// shape check).
    #[allow(clippy::too_many_arguments)]
    fn prefill_setup_direct(
        ir: wh_iron::core::ir::Kernel,
        dt: DType,
        n_q_heads: usize,
        n_kv_heads: usize,
        ...
        let q = unpack_f32(&pack_f32(&ramp(n_q_heads * Q_LEN * HEAD_DIM, 17, 8.0), dt), dt);
        let mut k = unpack_f32(&pack_f32(&ramp(n_kv_heads * k_len * HEAD_DIM, 13, 6.0), dt), dt);
        let mut v = unpack_f32(&pack_f32(&ramp(n_kv_heads * k_len * HEAD_DIM, 19, 9.0), dt), dt);
        for kvh in 0..n_kv_heads {
            let slab = kvh * k_len * HEAD_DIM;
            for t in k_real..k_len {
                for d in 0..HEAD_DIM {
                    k[slab + t * HEAD_DIM + d] = 999.0;
                    v[slab + t * HEAD_DIM + d] = 999.0;
                }
            }
        }
        let expected = naive_causal_prefix_padded(
            &q, &k, &v, n_q_heads, n_kv_heads, Q_LEN, k_len, HEAD_DIM, position, k_real, scale,
        );
        ...
        TestSetup::new(ir)
            ...
            .constexpr("k_real", k_real as u32)
            .expect(TestBuffer::from_vec("out", pack_f32(&expected, dt), dt))
            .grid_3d(1, n_q_heads as u32, 1, [128, 1, 1])
    }

    // MHA, short prefix: k_len = 96 (6 KV-tiles of 16), 2 heads, 1 KV head.
    // `position = k_len - Q_LEN`, `k_real = k_len`: reproduces the legacy
    // compacted-KV call contract exactly (see `prefill_setup_direct`'s
    // doc comment) — same numbers as before the W48 signature change.
    #[test_kernel(dtypes = [f32], tol = [5e-3])]
    fn test_iron_sdpa_prefill_mma(dt: DType) -> TestSetup {
        prefill_setup_direct(iron_sdpa_prefill_mma::kernel_ir_for(dt), dt, 2, 1, 96, 64, 96)
    }

    // GQA fan-out 4: 8 Q heads over 2 KV heads — exercises the kv-head
    // mapping in the prefill tile.
    #[test_kernel(dtypes = [f32], tol = [5e-3])]
    fn test_iron_sdpa_prefill_mma_gqa(dt: DType) -> TestSetup {
        prefill_setup_direct(iron_sdpa_prefill_mma::kernel_ir_for(dt), dt, 8, 2, 96, 64, 96)
    }

    // W48 direct-KV / pad-mask coverage (Laguna compendium §16-TargetA):
    // K/V buffer is allocated at ADDRESSING STRIDE k_len=160 (10 KV-tiles
    // of 16) but only the first k_real=101 rows are real content —
    // `prefill_setup_direct` poisons `[k_real, k_len)` with large
    // out-of-distribution values (999.0). `position=90` puts the Q
    // tile's rows at absolute positions [90, 121]: rows past 101 are
    // causally ALLOWED to see the poisoned [101, 111] region (which
    // `real_kb_lim`/`addr_kb_lim` still scan into, since 111 < the
    // 112-row block boundary) — so this is a genuine mutation check,
    // not a vacuous one: a kernel that dropped the `k >= k_real` guard
    // (falling back to causal-only masking) would let 999.0 leak into
    // those rows' softmax and diverge far outside tolerance from the
    // oracle, which correctly excludes it. `k_real=101` is deliberately
    // NOT a multiple of `bk=16` (nor of 32) to exercise the partial-block
    // boundary the compendium flagged ("the `kReal not %32` edge").
    #[test_kernel(dtypes = [f32, bf16], tol = [5e-3, 1e-1])]
    fn test_iron_sdpa_prefill_mma_direct_kv_padded(dt: DType) -> TestSetup {
        prefill_setup_direct(iron_sdpa_prefill_mma::kernel_ir_for(dt), dt, 2, 1, 160, 90, 101)
    }
    #[test_kernel(dtypes = [f32, bf16], tol = [5e-3, 1e-1])]
    fn test_iron_sdpa_prefill_mma_direct_kv_padded_gqa(dt: DType) -> TestSetup {
        prefill_setup_direct(iron_sdpa_prefill_mma::kernel_ir_for(dt), dt, 8, 2, 160, 90, 101)
    }
    // No-pad direct-KV sanity: k_real == k_len (addressing stride equals
    // real length, like a chunk that exactly fills the cache) — confirms
    // the pad guard is a true no-op at the boundary, not just when there's
    // slack.
    #[test_kernel(dtypes = [f32, bf16], tol = [5e-3, 1e-1])]
    fn test_iron_sdpa_prefill_mma_direct_kv_exact(dt: DType) -> TestSetup {
        prefill_setup_direct(iron_sdpa_prefill_mma::kernel_ir_for(dt), dt, 2, 1, 96, 64, 96)
    }
    ...
    // Unaffected by the W48 direct-KV extension (that kernel is untouched —
    // production's `Ops.sdpaPrefillMma` dispatches `iron_sdpa_prefill_mma`,
    // not this sibling; see LagunaSdpa.swift's `prefillMmaBf16`).
    #[test_kernel(dtypes = [f32, bf16], tol = [5e-3, 1e-1])]
    fn test_iron_sdpa_prefill_mma_bf16(dt: DType) -> TestSetup {
        prefill_setup(iron_sdpa_prefill_mma_bf16::kernel_ir_for(dt), dt, 2, 1, 96)
    }
    #[test_kernel(dtypes = [f32, bf16], tol = [5e-3, 1e-1])]
    fn test_iron_sdpa_prefill_mma_bf16_gqa(dt: DType) -> TestSetup {
        prefill_setup(iron_sdpa_prefill_mma_bf16::kernel_ir_for(dt), dt, 8, 2, 96)
    }
}
