...
//! `iron_sdpa_prefill_nax` — flash attention via `mpp::tensor_ops::matmul2d`.
//!
...
//! `iron_sdpa_prefill_mma`: the standard FlashAttention-2 online-softmax
...
/// should prefer `iron_sdpa_prefill_nax_d64` / `_d128` / `_d256` for
...
pub fn iron_sdpa_prefill_nax<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    mut out: Tensor<T>,
...
// ── D-chunk macro: generates iron_sdpa_prefill_nax_d{64,128,256} ────────────
//
...
macro_rules! sdpa_prefill_nax_wide {
    ($name:ident, $head_dim:literal, $n_chunks:literal, $os_slots:literal) => {
        #[doc = concat!(
            ...
            stringify!($head_dim),
            ".\n\n",
            "Same algorithm as `iron_sdpa_prefill_nax` (d=32) but loops the QK and PV\n",
...
// d128: 16 × 132 = 2112
// d256: 16 × 260 = 4160
sdpa_prefill_nax_wide!(iron_sdpa_prefill_nax_d64, 64u32, 2u32, 1088);
sdpa_prefill_nax_wide!(iron_sdpa_prefill_nax_d128, 128u32, 4u32, 2112);
sdpa_prefill_nax_wide!(iron_sdpa_prefill_nax_d256, 256u32, 8u32, 4160);
...
pub mod kernel_benches {
    ...
    use super::{BQ, TPG, iron_sdpa_prefill_nax};
    ...
    fn bench_sdpa_prefill_nax(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_sdpa_prefill_nax::kernel_ir_for(dt))
...
pub mod kernel_tests {
    ...
    use super::{
        ...
        iron_sdpa_prefill_nax_d64,
        iron_sdpa_prefill_nax_d128,
        iron_sdpa_prefill_nax_d256,
    };
    ...
    fn nax_setup(kernel: wh_iron::core::ir::Kernel, head_dim: usize, dt: DType) -> TestSetup {
        let scale = 1.0f32 / (head_dim as f32).sqrt();
    ...
    fn test_sdpa_prefill_nax(dt: DType) -> TestSetup {
        nax_setup(iron_sdpa_prefill_nax::kernel_ir_for(dt), 32, dt)
    }

    // Wide head-dim variants — D-chunk loop over head_dim/32 32-wide chunks.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [2e-2, 5e-2, 2e-1])]
    fn test_sdpa_prefill_nax_d64(dt: DType) -> TestSetup {
        nax_setup(iron_sdpa_prefill_nax_d64::kernel_ir_for(dt), 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [2e-2, 5e-2, 2e-1])]
    fn test_sdpa_prefill_nax_d128(dt: DType) -> TestSetup {
        nax_setup(iron_sdpa_prefill_nax_d128::kernel_ir_for(dt), 128, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [2e-2, 5e-2, 2e-1])]
    fn test_sdpa_prefill_nax_d256(dt: DType) -> TestSetup {
        nax_setup(iron_sdpa_prefill_nax_d256::kernel_ir_for(dt), 256, dt)
    }
}

#[cfg(test)]
mod tests {
    use wh_iron::core::{dtype::DType, ir::Op};
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_sdpa_prefill_nax::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_sdpa_prefill_nax");
    ...
    fn wide_kernels_ir_constructs_and_uses_coop_tile_ops() {
        ...
        for dt in [DType::F32, DType::F16, DType::BF16] {
            for (name, kernel_ir) in [
                (
                    "iron_sdpa_prefill_nax_d64",
                    iron_sdpa_prefill_nax_d64::kernel_ir_for as fn(_) -> _,
                ),
                ("iron_sdpa_prefill_nax_d128", iron_sdpa_prefill_nax_d128::kernel_ir_for),
                ("iron_sdpa_prefill_nax_d256", iron_sdpa_prefill_nax_d256::kernel_ir_for),
            ] {
                let k = kernel_ir(dt);
    ...
    fn codegen_emits_mpp_include_and_kernel_decl() {
        ...
        for (dt, t_name) in [(DType::F32, "float"), (DType::F16, "half"), (DType::BF16, "half")] {
            let mut k = iron_sdpa_prefill_nax::kernel_ir_for(dt);
            let suffix = match dt {
                DType::F32 => "f32",
                DType::F16 => "f16",
                _ => "bf16",
            };
            k.name = format!("iron_sdpa_prefill_nax_{suffix}");
            ...
            assert!(msl.contains(&format!("kernel void iron_sdpa_prefill_nax_{suffix}")));
    ...
    fn wide_codegen_emits_mpp_include_and_kernel_decl() {
        ...
        for (dt, t_name) in [(DType::F32, "float"), (DType::F16, "half"), (DType::BF16, "half")] {
            for (dim, kernel_ir) in [
                (64usize, iron_sdpa_prefill_nax_d64::kernel_ir_for as fn(_) -> _),
                (128, iron_sdpa_prefill_nax_d128::kernel_ir_for),
                (256, iron_sdpa_prefill_nax_d256::kernel_ir_for),
            ] {
                let suffix = match dt {
                    DType::F32 => "f32",
                    DType::F16 => "f16",
                ...
                k.name = format!("iron_sdpa_prefill_nax_d{dim}_{suffix}");
                let msl = MslGenerator::default().generate(&k).expect("codegen");
                assert!(
                ...
                assert!(
                    msl.contains(&format!("kernel void iron_sdpa_prefill_nax_d{dim}_{suffix}")),
