...
///
/// Heuristic:
/// - bf16 + Apple gen-8 (M2): use `iron_sdpa_prefill_mma_bf16` — single-Q
...
/// shell sessions, M2 mini canonical per `feedback_wh_iron_bench_on_m2_mini`**:
///
...
pub fn sdpa_prefill_mma_for(dtype: DType, family: Option<u32>) -> Kernel {
    ...
    let mut k = if is_pre_m3_bf16 {
        steel_attention_mma_bf16::iron_sdpa_prefill_mma_bf16::kernel_ir_for(dtype)
    } else {
        steel_attention_mma::iron_sdpa_prefill_mma::kernel_ir_for(dtype)
    };
    ...
    fn auto_select_picks_sibling_for_f32_and_f16_on_any_family() {
        for family in [None, Some(7), Some(8), Some(9), Some(10)] {
            for dt in [DType::F32, DType::F16] {
                ...
                assert_eq!(k.name, "iron_sdpa_prefill_mma", "dt={dt:?} family={family:?}");
            }
        }
    }
