...
//! Prefill scaled-dot-product attention — `iron_sdpa_prefill`.
//!
...
pub mod kernel_benches {
    ...
    // bq=32, bk=16 (bd=128 ⇒ bk=16), wm=4, wn=1. Our Iron BQ is a separate
    // tuning knob; the MLX dispatch always uses its bq=32/bk=16 tile.
    const MLX_BQ: usize = 32;
    const MLX_BK: usize = 16;
    const MLX_WM: u32 = 4;
    const MLX_WN: u32 = 1;
    /// Tolerance floor — legacy steel_attention bench `tol=2e-2`. Iron bakes
    ...
    fn sdpa_b(kernel: wh_iron::core::ir::Kernel, dt: DType) -> BenchSetup {
...
/// All three variants (`iron_sdpa_prefill` scalar flash, `_mma`, and
...
pub mod kernel_tests {
    ...
    fn test_sdpa_prefill_mma_bf16(dt: DType) -> TestSetup {
        sdpa_setup(iron_sdpa_prefill_mma_bf16::kernel_ir_for(dt), dt)
    }
}
