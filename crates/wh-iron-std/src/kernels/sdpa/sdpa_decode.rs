...
//! |-------------------------|-------|--------|------|--------|
//! | `iron_sdpa_decode_d64`  |   2   |   1×2  |  ✓  |        |
//! | `iron_sdpa_decode_d96`  |   3   |   1×3  |      |        |
//! | `iron_sdpa_decode`      |   4   |   1×4  |  ✓  |   ✓    |
//! | `iron_sdpa_decode_d256` |   8   |   2×4  |  ✓  |        |
//! | `iron_sdpa_decode_d512` |  16   |   4×4  |      |        |
//!
...
#[kernel]
pub fn iron_sdpa_decode_d256<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_d512<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
mod tests {
    ...
    use super::{
        iron_sdpa_decode,
        iron_sdpa_decode_d64,
        iron_sdpa_decode_d96,
        iron_sdpa_decode_d256,
        iron_sdpa_decode_d512,
    };
    ...
    fn codegen_d64() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let mut k = iron_sdpa_decode_d64::kernel_ir_for(dt);
            k.mode = KernelMode::Reduction;
            let src = MslGenerator::default().generate(&k).unwrap();
            check("iron_sdpa_decode_d64", &src);
        }
    }
    ...
    fn codegen_d96() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let mut k = iron_sdpa_decode_d96::kernel_ir_for(dt);
            k.mode = KernelMode::Reduction;
            let src = MslGenerator::default().generate(&k).unwrap();
            check("iron_sdpa_decode_d96", &src);
        }
    }
    ...
    fn codegen_d128() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let mut k = iron_sdpa_decode::kernel_ir_for(dt);
            k.mode = KernelMode::Reduction;
            let src = MslGenerator::default().generate(&k).unwrap();
            check("iron_sdpa_decode", &src);
    ...
    fn codegen_d256() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let mut k = iron_sdpa_decode_d256::kernel_ir_for(dt);
            k.mode = KernelMode::Reduction;
            let src = MslGenerator::default().generate(&k).unwrap();
            check("iron_sdpa_decode_d256", &src);
        }
    }
    ...
    fn codegen_d512() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let mut k = iron_sdpa_decode_d512::kernel_ir_for(dt);
            k.mode = KernelMode::Reduction;
            let src = MslGenerator::default().generate(&k).unwrap();
            check("iron_sdpa_decode_d512", &src);
        }
    }
}
...
pub mod kernel_tests {
    ...
    fn setup_d64(dt: DType, has_sink: bool) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_decode_d64::kernel_ir_for(dt))
    ...
    fn test_iron_sdpa_decode_d64(dt: DType) -> TestSetup { setup_d64(dt, false) }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_d64_sink(dt: DType) -> TestSetup { setup_d64(dt, true) }

    // ── d96 ──────────────────────────────────────────────────────────

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_d96(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_decode_d96::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("q", pack_f32(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack_f32(&k, dt), dt))
            .input(TestBuffer::from_vec("v", pack_f32(&v, dt), dt))
    ...
    fn setup_d128(
        ...
        TestSetup::new(iron_sdpa_decode::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("q", pack_f32(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack_f32(&k, dt), dt))
            .input(TestBuffer::from_vec("v", pack_f32(&v, dt), dt))
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode(dt: DType) -> TestSetup { setup_d128(dt, 64, 64, 0, 0, false, 0.0) }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_swa_sink(dt: DType) -> TestSetup {
        setup_d128(dt, 16, 16, 2, 8, false, 0.0)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_swa(dt: DType) -> TestSetup {
        setup_d128(dt, 16, 16, 0, 8, false, 0.0)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_learned_sink(dt: DType) -> TestSetup {
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_strided(dt: DType) -> TestSetup {
    ...
    fn setup_d256(dt: DType, has_sink: bool) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_decode_d256::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("q", pack_f32(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack_f32(&k, dt), dt))
            .input(TestBuffer::from_vec("v", pack_f32(&v, dt), dt))
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_d256(dt: DType) -> TestSetup { setup_d256(dt, false) }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_decode_d256_sink(dt: DType) -> TestSetup { setup_d256(dt, true) }

    // ── d512 ─────────────────────────────────────────────────────────

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 3e-3, 1.5e-2])]
    fn test_iron_sdpa_decode_d512(dt: DType) -> TestSetup {
        ...
        let expected =
            naive_sdpa(&q, &k, &v, nqh, nkh, hd, n_kv, kv_stride, 0, 0, false, 0.0, scale);
        TestSetup::new(iron_sdpa_decode_d512::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("q", pack_f32(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack_f32(&k, dt), dt))
            .input(TestBuffer::from_vec("v", pack_f32(&v, dt), dt))
...
pub mod kernel_benches {
    ...
    fn bench_sdpa_decode_d64(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_sdpa_decode_d64::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn bench_sdpa_decode_d96(dt: DType) -> BenchSetup {
        ...
        let (n_kv, kv_stride) = (4096usize, 4096usize);
        let bytes = (2 * nqh * hd + 2 * nkh * n_kv * hd) * dt.size_bytes();
        BenchSetup::new(iron_sdpa_decode_d96::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("q", nqh * hd, dt))
            .buffer(BenchBuffer::random("k", nkh * kv_stride * hd, dt))
            .buffer(BenchBuffer::random("v", nkh * kv_stride * hd, dt))
    ...
    fn bench_sdpa_decode(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_sdpa_decode::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("q", nqh * hd, dt))
            .buffer(BenchBuffer::random("k", nkh * kv_stride * hd, dt))
            .buffer(BenchBuffer::random("v", nkh * kv_stride * hd, dt))
    ...
    fn bench_sdpa_decode_d256(dt: DType) -> BenchSetup {
        ...
        let (n_kv, kv_stride) = (4096usize, 4096usize);
        let bytes = (2 * nqh * hd + 2 * nkh * n_kv * hd) * dt.size_bytes();
        BenchSetup::new(iron_sdpa_decode_d256::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("q", nqh * hd, dt))
            .buffer(BenchBuffer::random("k", nkh * kv_stride * hd, dt))
            .buffer(BenchBuffer::random("v", nkh * kv_stride * hd, dt))
    ...
    fn bench_sdpa_decode_d512(dt: DType) -> BenchSetup {
        ...
        let (n_kv, kv_stride) = (4096usize, 4096usize);
        let bytes = (2 * nqh * hd + 2 * nkh * n_kv * hd) * dt.size_bytes();
        BenchSetup::new(iron_sdpa_decode_d512::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("q", nqh * hd, dt))
            .buffer(BenchBuffer::random("k", nkh * kv_stride * hd, dt))
            .buffer(BenchBuffer::random("v", nkh * kv_stride * hd, dt))
