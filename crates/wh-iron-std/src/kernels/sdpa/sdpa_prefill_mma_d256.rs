...
//! (IRON_OP_ATTRIB diagnostic, iron-rebaseline @ 592b0de) showed the
...
//! consistent with `iron_sdpa_prefill_qtiled_d256`'s pure scalar
...
//! simdgroup-matrix fragments (see `iron_sdpa_prefill_mma`, the existing
...
#[kernel]
pub fn iron_sdpa_prefill_mma_d256<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
/// Iron dispatches so `iron build --emit` picks the kernel up for all
/// three.
pub mod kernel_benches {
    use wh_iron::{bench, test::*};

    use super::iron_sdpa_prefill_mma_d256;
    ...
    fn bench_sdpa_prefill_mma_d256(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_sdpa_prefill_mma_d256::kernel_ir_for(dt))
...
/// `iron test`-harness CPU-oracle coverage, following the same
...
pub mod kernel_tests {
    ...
    fn mma_d256_setup(dt: DType, causal: bool) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_prefill_mma_d256::kernel_ir_for(dt))
            .mode(KernelMode::SimdGroup2D)
    ...
    fn test_iron_sdpa_prefill_mma_d256_causal(dt: DType) -> TestSetup { mma_d256_setup(dt, true) }

    // Full (bidirectional): every row attends `[0, base_kv + n_query)`.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 1e-2, 5e-2])]
    fn test_iron_sdpa_prefill_mma_d256_full(dt: DType) -> TestSetup { mma_d256_setup(dt, false) }
}
