...
//!
//! `iron_aura_flash_pass2` later reduces the partials cross-block.
//!
//! ## Dispatch
//!
...
///
/// Same compressed-domain online-softmax as `iron_aura_flash_p1`, with
...
///
/// Produces kernels: `iron_aura_flash_p1_causal_kb4_vb2_d64`,
/// `_causal_kb4_vb2_d128`.
#[kernel(variants(DIM = [64, 128], DPL = [2, 4], suffix = "kb4_vb2_d{DIM}"))]
pub fn iron_aura_flash_p1_causal<T>(
    q_rot: Tensor<T>,
    key_packed: Tensor<u32>,
    key_norms: Tensor<T>,
    key_codebook: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::{iron_aura_flash_p1_causal_kb4_vb2_d128, iron_aura_flash_p1_kb4_vb2_d128};
    use crate::utils::{pack_f32, unpack_f32};

    // ── GPU pass-1 partials, validated directly ─────────────────────────
    //
    // The companion `iron_aura_flash_pass2` test CPU-emulates pass 1 to stage
    // the partials it feeds the GPU reducer — so the GPU pass-1 path itself
    // was never exercised. This test dispatches the real `iron_aura_flash_p1`
    ...
    // AURA-codebook-decoded K,V that `iron_aura_flash_pass2`'s `emulate_p1`
    ...
    /// CPU oracle for `iron_aura_flash_p1` (non-causal): per (q_head, block)
    ...
    /// Mirrors `iron_aura_flash_pass2::kernel_tests::emulate_p1`.
    ...
    fn test_iron_aura_flash_p1_kb4_vb2_d128(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_aura_flash_p1_kb4_vb2_d128::kernel_ir_for(dt))
    ...
    //
    // The causal sibling `iron_aura_flash_p1_causal_kb4_vb2_d128` shares the whole
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-2, 1e-1])]
    fn test_iron_aura_flash_p1_causal_kb4_vb2_d128(dt: DType) -> TestSetup {
        let (q_heads, kv_heads, tokens, dim) = (2usize, 1usize, 8usize, DIM);
        let repeat = q_heads / kv_heads;
        let block_size = 4usize;
        let num_blocks = tokens.div_ceil(block_size); // 2
        ...
        TestSetup::new(iron_aura_flash_p1_causal_kb4_vb2_d128::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("q_rot", pack_f32(&q_rot, dt), dt))
            .input(TestBuffer::from_vec("key_packed", pack_u32(&key_packed), DType::U32))
            .input(TestBuffer::from_vec("key_norms", pack_f32(&key_norms, dt), dt))
...
pub mod kernel_benches {
    ...
    fn bench_flash_p1(dt: DType) -> BenchSetup {
        flash_p1(
            BenchSetup::new(iron_aura_flash_p1_kb4_vbVB_dDIM::kernel_ir_for(dt)),
            dt,
            DIM,
            4,
            VB,
    ...
    fn bench_flash_p1_causal(dt: DType) -> BenchSetup {
        flash_p1(
            BenchSetup::new(iron_aura_flash_p1_causal_kb4_vb2_dDIM::kernel_ir_for(dt)),
            dt,
            DIM,
            4,
            2,
