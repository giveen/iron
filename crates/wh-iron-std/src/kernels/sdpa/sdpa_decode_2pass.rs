...
//!   (MLX uses tid.z; wh-iron's DSL doesn't expose tgid_z, so we
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass1<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass1_bc4<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    mut partial_o: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass2<T>(
    partial_o: Tensor<T>,
...
//
// The strided access pattern in `iron_sdpa_decode_2pass_pass1` (each block reads
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass1_tiled<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    mut partial_o: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass1_d64<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    mut partial_o: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass2_d64<T>(
    partial_o: Tensor<T>,
    partial_m: Tensor<f32>,
    partial_l: Tensor<f32>,
    mut out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass1_d96<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    mut partial_o: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass2_d96<T>(
    partial_o: Tensor<T>,
    partial_m: Tensor<f32>,
    partial_l: Tensor<f32>,
    mut out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_decode_2pass_pass1_d256<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    mut partial_o: Tensor<T>,
...
/// (dims 0..3 then 4..7) — same strategy as `iron_sdpa_decode_d256`.
#[kernel]
pub fn iron_sdpa_decode_2pass_pass2_d256<T>(
    partial_o: Tensor<T>,
    partial_m: Tensor<f32>,
    partial_l: Tensor<f32>,
    mut out: Tensor<T>,
...
// Bagel 2 / ITER 53: standalone registration for pass2 so `iron build
...
// Iron consumes the pass2 wrapper to chain pass1 → pass2 in
// `Ops.sdpaDecode2Pass`.
//
// Note: only the base `iron_sdpa_decode_2pass_pass2` kernel gets a
...
// at this point. Add per-dim registrations here as Iron starts
// dispatching them.

pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_sdpa_decode_2pass_pass2;
    ...
    fn test_iron_sdpa_decode_2pass_combined(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_decode_2pass_pass2::kernel_ir_for(dt))
...
// (`run_reference_compare` in run_kernel.rs) times exactly ONE Iron kernel against
...
pub mod kernel_benches {
    ...
    use super::{
        iron_sdpa_decode_2pass_pass1,
        iron_sdpa_decode_2pass_pass1_d64,
        iron_sdpa_decode_2pass_pass1_d96,
        iron_sdpa_decode_2pass_pass1_d256,
        iron_sdpa_decode_2pass_pass2,
        iron_sdpa_decode_2pass_pass2_d64,
        iron_sdpa_decode_2pass_pass2_d96,
        iron_sdpa_decode_2pass_pass2_d256,
    };
    ...
    fn pass1(ir: wh_iron::core::ir::Kernel, head_dim: usize, dt: DType) -> BenchSetup {
        let gqa_factor = N_Q_HEADS / N_KV_HEADS;
        let kv_stride = N_KV;
        let scale = 1.0f32 / (head_dim as f32).sqrt();
    ...
    fn pass2(ir: wh_iron::core::ir::Kernel, head_dim: usize, dt: DType) -> BenchSetup {
        let partial_len = N_Q_HEADS * BLOCKS * head_dim;
        let ml_len = N_Q_HEADS * BLOCKS;
    ...
    fn bench_pass1(dt: DType) -> BenchSetup {
        pass1(iron_sdpa_decode_2pass_pass1::kernel_ir_for(dt), 128, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pass2(dt: DType) -> BenchSetup {
        pass2(iron_sdpa_decode_2pass_pass2::kernel_ir_for(dt), 128, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pass1_d64(dt: DType) -> BenchSetup {
        pass1(iron_sdpa_decode_2pass_pass1_d64::kernel_ir_for(dt), 64, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pass2_d64(dt: DType) -> BenchSetup {
        pass2(iron_sdpa_decode_2pass_pass2_d64::kernel_ir_for(dt), 64, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pass1_d96(dt: DType) -> BenchSetup {
        pass1(iron_sdpa_decode_2pass_pass1_d96::kernel_ir_for(dt), 96, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pass2_d96(dt: DType) -> BenchSetup {
        pass2(iron_sdpa_decode_2pass_pass2_d96::kernel_ir_for(dt), 96, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pass1_d256(dt: DType) -> BenchSetup {
        pass1(iron_sdpa_decode_2pass_pass1_d256::kernel_ir_for(dt), 256, dt)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pass2_d256(dt: DType) -> BenchSetup {
        pass2(iron_sdpa_decode_2pass_pass2_d256::kernel_ir_for(dt), 256, dt)
    }
}
