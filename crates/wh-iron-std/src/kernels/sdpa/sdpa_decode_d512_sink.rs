...
#[kernel]
pub fn iron_sdpa_decode_d512_sink<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
...
mod tests {
    ...
    use super::iron_sdpa_decode_d512_sink;

    fn msl_for(dt: DType) -> String {
        let mut k = iron_sdpa_decode_d512_sink::kernel_ir_for(dt);
        k.mode = KernelMode::Reduction;
        MslGenerator::default().generate(&k).expect("iron_sdpa_decode_d512_sink codegen succeeds")
    }
    ...
    fn codegen_produces_nonempty_msl_for_all_float_dtypes() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            ...
            assert!(
                src.contains("kernel void iron_sdpa_decode_d512_sink"),
                "MSL for {dt:?} should declare iron_sdpa_decode_d512_sink:\n{src}",
            );
        }
    }
}
...
pub mod kernel_tests {
    ...
    fn test_iron_sdpa_decode_d512_sink(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_sdpa_decode_d512_sink::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_sdpa_decode_d512_sink(dt: DType) -> BenchSetup {
        ...
        let bytes = (2 * n_q_heads * head_dim + 2 * n_kv_heads * n_kv * head_dim) * dt.size_bytes()
            + n_q_heads * 4;
        BenchSetup::new(iron_sdpa_decode_d512_sink::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
