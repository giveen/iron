...
//! aggregate scores.
//!
//! Sits downstream of [`iron_indexer_score`]. Takes the
...
//! [`crate::kernels::sampling::sort::iron_sort`] but with parallel storage for an
...
//!   path is `iron_sort` per chunk + a `iron_merge`-style co-rank merge
...
#[kernel]
pub fn iron_indexer_topk_block(
    score: Tensor<f32>,
    ...
    // iron_sort kernel's barrier discipline (`if flip >= 7`) is reused:
...
pub mod kernel_tests {
    ...
    use super::iron_indexer_topk_block;
    ...
    fn setup(n_kv: usize, k: usize) -> TestSetup {
        ...
        TestSetup::new(iron_indexer_topk_block::kernel_ir())
...
pub mod kernel_benches {
    ...
    fn bench_topk(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_indexer_topk_block::kernel_ir())
            .mode(KernelMode::Reduction)
