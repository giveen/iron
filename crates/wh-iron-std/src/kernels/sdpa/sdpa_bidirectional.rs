...
//! ## Naming
//!
//! `iron_sdpa_bidirectional_dN<T>` — N is the constexpr head_dim,
...
//! `iron_sdpa_decode` / `iron_sdpa_multi`.
//!
...
#[kernel]
pub fn iron_sdpa_bidirectional_d64<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_bidirectional_d32<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_bidirectional_d72<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_bidirectional_d80<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_sdpa_bidirectional_d96<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::{
        iron_sdpa_bidirectional_d32,
        iron_sdpa_bidirectional_d64,
        iron_sdpa_bidirectional_d72,
        iron_sdpa_bidirectional_d80,
        iron_sdpa_bidirectional_d96,
    };
    ...
    fn setup(ir: wh_iron::core::ir::Kernel, head_dim: usize, dt: DType) -> TestSetup {
    ...
    fn test_iron_sdpa_bidirectional_d32(dt: DType) -> TestSetup {
        setup(iron_sdpa_bidirectional_d32::kernel_ir_for(dt), 32, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_bidirectional_d64(dt: DType) -> TestSetup {
        setup(iron_sdpa_bidirectional_d64::kernel_ir_for(dt), 64, dt)
    }
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_bidirectional_d72(dt: DType) -> TestSetup {
        setup(iron_sdpa_bidirectional_d72::kernel_ir_for(dt), 72, dt)
    }

    // head_dim 80 — ragged: 80 = 32·2 + 16, partial-lane bounds masking.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_bidirectional_d80(dt: DType) -> TestSetup {
        setup(iron_sdpa_bidirectional_d80::kernel_ir_for(dt), 80, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_iron_sdpa_bidirectional_d96(dt: DType) -> TestSetup {
        setup(iron_sdpa_bidirectional_d96::kernel_ir_for(dt), 96, dt)
    }

    // Production vision-tower self-attention shape that previously
    // CPU-pinned in Iron (PaliGemma SigLIP / FastVLM): n_q_heads=16,
    ...
    fn test_iron_sdpa_bidirectional_d64_vision_tower(dt: DType) -> TestSetup {
        setup_prod(iron_sdpa_bidirectional_d64::kernel_ir_for(dt), 64, 16, 16, 576, dt)
    }

    // d80 (Qwen2.5-VL vision tower) at production shape — ragged-lane variant.
    #[test_kernel(dtypes = [f32], tol = [2e-3])]
    fn test_iron_sdpa_bidirectional_d80_vision_tower(dt: DType) -> TestSetup {
        setup_prod(iron_sdpa_bidirectional_d80::kernel_ir_for(dt), 80, 16, 16, 576, dt)
    }

    // d96 (Phi-3-vision / Qwen variants) at production shape — ragged-lane.
    #[test_kernel(dtypes = [f32], tol = [2e-3])]
    fn test_iron_sdpa_bidirectional_d96_vision_tower(dt: DType) -> TestSetup {
        setup_prod(iron_sdpa_bidirectional_d96::kernel_ir_for(dt), 96, 16, 16, 576, dt)
    }
    ...
    #[test_kernel(dtypes = [f32], tol = [2e-3])]
    fn test_iron_sdpa_bidirectional_d64_audio_encoder(dt: DType) -> TestSetup {
        setup_prod(iron_sdpa_bidirectional_d64::kernel_ir_for(dt), 64, 16, 16, 1500, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn setup(ir: wh_iron::core::ir::Kernel, head_dim: usize, dt: DType) -> BenchSetup {
