...
//! Decode-form scaled dot-product attention — `iron_sdpa_vector` family.
//!
...
//! |---------------------------|----------|------------|--------|
//! | `iron_sdpa_vector_d64`      | 64       | 2          | 2      |
//! | `iron_sdpa_vector_d96`      | 96       | 3          | 3      |
//! | `iron_sdpa_vector` (d=128)  | 128      | 4          | 4      |
//! | `iron_sdpa_vector_d192`     | 192      | 6          | 6      |
//! | `iron_sdpa_vector_d256`     | 256      | 8          | 8      |
//!
...
//!
//! `iron/sdpa_decode.rs` is a sibling kernel with the same dispatch +
//! reduction shape but extra Iron-specific surface
//! (`kv_stride`, `heads_per_group`, `sink_end`, `window_start`). The
//! split is deliberate: this file's charter is a 1:1 MLX port for the
//! `iron bench` head-to-head, so additions that diverge from MLX's
//! `sdpa_vector` template live in `iron/`. Bandwidth fixes that apply
...
pub fn iron_sdpa_vector<T>(
    ...
    // occupancy factor that capped bf16 at 62% Iron despite vectorized loads.
    ...
    // `iron_sdpa_decode_2pass_pass1`. Inline'd loads + casts broke the run before.
...
/// higher than d=128 but below the d=256 bound that `iron_sdpa_decode_d256`
/// confirmed fits in 1024 threads/TG (8 accumulators).
#[kernel]
pub fn iron_sdpa_vector_d192<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
/// Generic `T ∈ {f32, f16, bf16}`. TPG=1024; grid=[n_q_heads,1,1].
///
/// 8 live K accumulators + 8 V accumulators per lane. `iron_sdpa_decode_d256`
...
#[kernel]
pub fn iron_sdpa_vector_d256<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    out: Tensor<T>,
...
/// kernels (`iron_sdpa_vector` at head_dim 128 plus the d64/d96/d192/d256
...
/// `scaled_dot_product_attention`'s `iron_sdpa`. Inputs are dtype-rounded so the
...
pub mod kernel_tests {
    ...
    use super::{
        iron_sdpa_vector,
        iron_sdpa_vector_d64,
        iron_sdpa_vector_d96,
        iron_sdpa_vector_d192,
        iron_sdpa_vector_d256,
    };
    ...
    fn test_sdpa_vector(dt: DType) -> TestSetup {
        sdpa_setup(iron_sdpa_vector::kernel_ir_for(dt), 128, 64, 8, 2, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_sdpa_vector_d64(dt: DType) -> TestSetup {
        sdpa_setup(iron_sdpa_vector_d64::kernel_ir_for(dt), 64, 64, 8, 2, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_sdpa_vector_d96(dt: DType) -> TestSetup {
        sdpa_setup(iron_sdpa_vector_d96::kernel_ir_for(dt), 96, 64, 8, 2, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_sdpa_vector_d192(dt: DType) -> TestSetup {
        sdpa_setup(iron_sdpa_vector_d192::kernel_ir_for(dt), 192, 64, 8, 2, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 2e-3, 1e-2])]
    fn test_sdpa_vector_d256(dt: DType) -> TestSetup {
        sdpa_setup(iron_sdpa_vector_d256::kernel_ir_for(dt), 256, 64, 8, 2, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_sdpa_vector(dt: DType) -> BenchSetup {
        sb(iron_sdpa_vector::kernel_ir_for(dt), 128, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_sdpa_vector_d64(dt: DType) -> BenchSetup {
        sb(iron_sdpa_vector_d64::kernel_ir_for(dt), 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_sdpa_vector_d96(dt: DType) -> BenchSetup {
        sb(iron_sdpa_vector_d96::kernel_ir_for(dt), 96, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_sdpa_vector_d192(dt: DType) -> BenchSetup {
        sb(iron_sdpa_vector_d192::kernel_ir_for(dt), 192, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_sdpa_vector_d256(dt: DType) -> BenchSetup {
        sb(iron_sdpa_vector_d256::kernel_ir_for(dt), 256, dt)
    }
}
