...
//! `out[token, d]`. The block-scaled counterpart of `iron/iron_dequant_gather.rs`
...
#[kernel]
pub fn iron_mxfp4_dequant_gather<T>(
    weight: Tensor<u32>,
    scales: Tensor<u8>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_nvfp4_dequant_gather<T>(
    weight: Tensor<u32>,
    scales: Tensor<u8>,
    indices: Tensor<u32>,
    out: Tensor<T>,
    ...
    let val = iron_decode_e2m1(nib);
    let scale =
        iron_decode_e4m3(load(scales[token_id * blocks_per_row + d / block_size]).cast::<u32>())
            * global;
    store(out[idx], (val * scale).cast::<T>());
}
...
#[kernel]
pub fn iron_mxfp8_e4m3_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<u8>,
    indices: Tensor<u32>,
    out: Tensor<T>,
    ...
    let token_id = load(indices[token]);
    let blocks_per_row = hidden / block_size;
    let elem = iron_decode_e4m3(load(weight[token_id * hidden + d]).cast::<u32>());
    let sbits = load(scales[token_id * blocks_per_row + d / block_size]).cast::<f32>();
    let scale = exp2(sbits - 127.0f32);
...
#[kernel]
pub fn iron_mxfp8_e5m2_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<u8>,
    indices: Tensor<u32>,
    out: Tensor<T>,
    ...
    let token_id = load(indices[token]);
    let blocks_per_row = hidden / block_size;
    let elem = iron_decode_e5m2(load(weight[token_id * hidden + d]).cast::<u32>());
    let sbits = load(scales[token_id * blocks_per_row + d / block_size]).cast::<f32>();
    let scale = exp2(sbits - 127.0f32);
    store(out[idx], (elem * scale).cast::<T>());
}
...
#[kernel]
pub fn iron_nvfp8_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<f32>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
// + f32 scale), so it reuses `iron_nvfp8_dequant_gather`; only fp4 (4-bit E2M1),
...
#[kernel]
pub fn iron_fp4_dequant_gather<T>(
    weight: Tensor<u32>,
    scales: Tensor<f32>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_fp8_e5m2_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<f32>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_int8_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<f32>,
    indices: Tensor<u32>,
    out: Tensor<T>,
    ...
    let token_id = load(indices[token]);
    let blocks_per_row = hidden / block_size;
    let elem = iron_decode_int8(load(weight[token_id * hidden + d]).cast::<u32>());
    let scale = load(scales[token_id * blocks_per_row + d / block_size]);
    store(out[idx], (elem * scale).cast::<T>());
}
...
// of the DSL shift operands, matching `iron/dequant_gemv.rs` style.
...
int_dequant_gather_f32!(iron_int2_dequant_gather, 2u32, 2u32, 4.0f32);
int_dequant_gather_f32!(iron_int3_dequant_gather, 3u32, 4u32, 8.0f32);
int_dequant_gather_f32!(iron_int4_dequant_gather, 4u32, 8u32, 16.0f32);
int_dequant_gather_f32!(iron_int5_dequant_gather, 5u32, 16u32, 32.0f32);
int_dequant_gather_f32!(iron_int6_dequant_gather, 6u32, 32u32, 64.0f32);
...
int_dequant_gather_e8m0!(iron_mxint2_dequant_gather, 2u32, 2u32, 4.0f32);
int_dequant_gather_e8m0!(iron_mxint3_dequant_gather, 3u32, 4u32, 8.0f32);
int_dequant_gather_e8m0!(iron_mxint4_dequant_gather, 4u32, 8u32, 16.0f32);
int_dequant_gather_e8m0!(iron_mxint5_dequant_gather, 5u32, 16u32, 32.0f32);
int_dequant_gather_e8m0!(iron_mxint6_dequant_gather, 6u32, 32u32, 64.0f32);

/// MXINT8 dequantizing gather — 8-bit codes (byte layout, block 32), E8M0 pow-2
/// block scale. Decode is sign-extend → `code · 2^(sbits-127)`.
#[kernel]
pub fn iron_mxint8_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<u8>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
// Mirrors `mlx/block_scaled_dequant.rs` (`iron_nvfp8_f16_dequant`, etc.).
...
#[kernel]
pub fn iron_nvfp8_f16_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<f16>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_fp4_f16_dequant_gather<T>(
    weight: Tensor<u32>,
    scales: Tensor<f16>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
/// scale.
#[kernel]
pub fn iron_fp8_e5m2_f16_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<f16>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
int_dequant_gather_f16!(iron_int2_f16_dequant_gather, 2u32, 2u32, 4.0f32);
int_dequant_gather_f16!(iron_int3_f16_dequant_gather, 3u32, 4u32, 8.0f32);
int_dequant_gather_f16!(iron_int4_f16_dequant_gather, 4u32, 8u32, 16.0f32);
int_dequant_gather_f16!(iron_int5_f16_dequant_gather, 5u32, 16u32, 32.0f32);
int_dequant_gather_f16!(iron_int6_f16_dequant_gather, 6u32, 32u32, 64.0f32);
...
#[kernel]
pub fn iron_int8_f16_dequant_gather<T>(
    weight: Tensor<u8>,
    scales: Tensor<f16>,
    indices: Tensor<u32>,
    out: Tensor<T>,
...
pub mod kernel_tests {
    ...
    fn test_mxfp4_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxfp4_dequant_gather::kernel_ir_for(dt), QFormat::Mxfp4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_nvfp4_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_nvfp4_dequant_gather::kernel_ir_for(dt), QFormat::Nvfp4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxfp8_e4m3_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxfp8_e4m3_dequant_gather::kernel_ir_for(dt), QFormat::Mxfp8E4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxfp8_e5m2_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxfp8_e5m2_dequant_gather::kernel_ir_for(dt), QFormat::Mxfp8E5, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_nvfp8_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_nvfp8_dequant_gather::kernel_ir_for(dt), QFormat::Nvfp8, 256, dt)
    }
    ...
    fn test_fp4_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_fp4_dequant_gather::kernel_ir_for(dt), QFormat::Fp4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_nvfp8_dequant_gather::kernel_ir_for(dt), QFormat::Fp8E4m3, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_fp8_e5m2_dequant_gather::kernel_ir_for(dt), QFormat::Fp8E5m2, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int8_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int8_dequant_gather::kernel_ir_for(dt), QFormat::Int8, 256, dt)
    }
    ...
    fn test_int2_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int2_dequant_gather::kernel_ir_for(dt), QFormat::Int2, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int3_dequant_gather::kernel_ir_for(dt), QFormat::Int3, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int4_dequant_gather::kernel_ir_for(dt), QFormat::Int4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int5_dequant_gather::kernel_ir_for(dt), QFormat::Int5, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int6_dequant_gather::kernel_ir_for(dt), QFormat::Int6, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint2_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxint2_dequant_gather::kernel_ir_for(dt), QFormat::Mxint2, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint3_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxint3_dequant_gather::kernel_ir_for(dt), QFormat::Mxint3, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint4_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxint4_dequant_gather::kernel_ir_for(dt), QFormat::Mxint4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint5_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxint5_dequant_gather::kernel_ir_for(dt), QFormat::Mxint5, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint6_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxint6_dequant_gather::kernel_ir_for(dt), QFormat::Mxint6, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint8_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_mxint8_dequant_gather::kernel_ir_for(dt), QFormat::Mxint8, 256, dt)
    }
    ...
    fn test_nvfp8_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_nvfp8_f16_dequant_gather::kernel_ir_for(dt), QFormat::Nvfp8F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_nvfp8_f16_dequant_gather::kernel_ir_for(dt), QFormat::Fp8E4m3F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp4_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_fp4_f16_dequant_gather::kernel_ir_for(dt), QFormat::Fp4F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(
            iron_fp8_e5m2_f16_dequant_gather::kernel_ir_for(dt),
            QFormat::Fp8E5m2F16,
            256,
            dt,
        )
    ...
    fn test_int2_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int2_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int2F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int3_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int3F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int4_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int4F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int5_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int5F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int6_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int6F16, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int8_f16_dequant_gather(dt: DType) -> TestSetup {
        gather_setup(iron_int8_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int8F16, 256, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_mxfp4_gather(dt: DType) -> BenchSetup {
        gb(iron_mxfp4_dequant_gather::kernel_ir_for(dt), QFormat::Mxfp4, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp4_gather(dt: DType) -> BenchSetup {
        gb(iron_nvfp4_dequant_gather::kernel_ir_for(dt), QFormat::Nvfp4, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e4m3_gather(dt: DType) -> BenchSetup {
        gb(iron_mxfp8_e4m3_dequant_gather::kernel_ir_for(dt), QFormat::Mxfp8E4, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e5m2_gather(dt: DType) -> BenchSetup {
        gb(iron_mxfp8_e5m2_dequant_gather::kernel_ir_for(dt), QFormat::Mxfp8E5, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp8_gather(dt: DType) -> BenchSetup {
        gb(iron_nvfp8_dequant_gather::kernel_ir_for(dt), QFormat::Nvfp8, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp4_gather(dt: DType) -> BenchSetup {
        gb(iron_fp4_dequant_gather::kernel_ir_for(dt), QFormat::Fp4, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_gather(dt: DType) -> BenchSetup {
        gb(iron_nvfp8_dequant_gather::kernel_ir_for(dt), QFormat::Fp8E4m3, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_gather(dt: DType) -> BenchSetup {
        gb(iron_fp8_e5m2_dequant_gather::kernel_ir_for(dt), QFormat::Fp8E5m2, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_gather(dt: DType) -> BenchSetup {
        gb(iron_int8_dequant_gather::kernel_ir_for(dt), QFormat::Int8, 4096, dt)
    }
    // Symmetric sub-byte ints (FP32 group scale) + MXINT (E8M0 block scale).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2_gather(dt: DType) -> BenchSetup {
        gb(iron_int2_dequant_gather::kernel_ir_for(dt), QFormat::Int2, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_gather(dt: DType) -> BenchSetup {
        gb(iron_int3_dequant_gather::kernel_ir_for(dt), QFormat::Int3, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_gather(dt: DType) -> BenchSetup {
        gb(iron_int4_dequant_gather::kernel_ir_for(dt), QFormat::Int4, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_gather(dt: DType) -> BenchSetup {
        gb(iron_int5_dequant_gather::kernel_ir_for(dt), QFormat::Int5, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_gather(dt: DType) -> BenchSetup {
        gb(iron_int6_dequant_gather::kernel_ir_for(dt), QFormat::Int6, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint2_gather(dt: DType) -> BenchSetup {
        gb(iron_mxint2_dequant_gather::kernel_ir_for(dt), QFormat::Mxint2, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint3_gather(dt: DType) -> BenchSetup {
        gb(iron_mxint3_dequant_gather::kernel_ir_for(dt), QFormat::Mxint3, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint4_gather(dt: DType) -> BenchSetup {
        gb(iron_mxint4_dequant_gather::kernel_ir_for(dt), QFormat::Mxint4, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint5_gather(dt: DType) -> BenchSetup {
        gb(iron_mxint5_dequant_gather::kernel_ir_for(dt), QFormat::Mxint5, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint6_gather(dt: DType) -> BenchSetup {
        gb(iron_mxint6_dequant_gather::kernel_ir_for(dt), QFormat::Mxint6, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint8_gather(dt: DType) -> BenchSetup {
        gb(iron_mxint8_dequant_gather::kernel_ir_for(dt), QFormat::Mxint8, 4096, dt)
    }
    // FP16-scaled twins. fp8_e4m3_f16 reuses the nvfp8_f16 kernel.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp8_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_nvfp8_f16_dequant_gather::kernel_ir_for(dt), QFormat::Nvfp8F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_nvfp8_f16_dequant_gather::kernel_ir_for(dt), QFormat::Fp8E4m3F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp4_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_fp4_f16_dequant_gather::kernel_ir_for(dt), QFormat::Fp4F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_fp8_e5m2_f16_dequant_gather::kernel_ir_for(dt), QFormat::Fp8E5m2F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_int2_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int2F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_int3_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int3F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_int4_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int4F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_int5_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int5F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_int6_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int6F16, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_f16_gather(dt: DType) -> BenchSetup {
        gb(iron_int8_f16_dequant_gather::kernel_ir_for(dt), QFormat::Int8F16, 4096, dt)
    }
}
