...
//! ## Macro structure
//!
//! `iron_dequant_gather_kernel!` emits the entire `#[kernel] pub fn …`
...
///
/// Produces kernels: `iron_dequant_gather_int2`, `_int3`, `_int4`, `_int5`,
/// `_int6`, `_int8`.
///
...
/// New-syntax correctness tests for the `iron_dequant_gather_int{2,3,4,5,6,8}`
...
pub mod kernel_tests {
    ...
    fn gather_setup(
        ...
        let expected = iron_dequant_gather_oracle(&w, &s, &b, &indices, hidden, group_size, bits);
    ...
    fn test_dequant_gather_pow2(dt: DType) -> TestSetup {
        gather_setup(iron_dequant_gather_intBITS::kernel_ir_for(dt), BITS, 256, 64, dt)
    }
    ...
    fn test_dequant_gather_odd(dt: DType) -> TestSetup {
        gather_setup(iron_dequant_gather_intBITS::kernel_ir_for(dt), BITS, 64, 32, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_dequant_gather(dt: DType) -> BenchSetup {
        gb(iron_dequant_gather_intBITS::kernel_ir_for(dt), BITS, 4096, 64, dt)
    }
}
