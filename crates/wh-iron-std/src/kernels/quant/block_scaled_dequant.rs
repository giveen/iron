...
pub fn iron_nvfp4_dequant<T>(
    ...
    if i < n {
        let word = load(codes[i / 8u32]);
        let nib = (word >> ((i & 7u32) * 4u32)) & 0xFu32;
        let elem = iron_decode_e2m1(nib);
        // E4M3 micro-scale × global.
        let block_scale = iron_decode_e4m3(load(scales[i / block_size]).cast::<u32>()) * global;
...
#[kernel]
pub fn iron_mxfp8_e4m3_dequant<T>(
    codes: Tensor<u8>,
    scales: Tensor<u8>,
    out: Tensor<T>,
    #[constexpr] n: u32,
    ...
    let i = program_id::<0>();
    if i < n {
        let elem = iron_decode_e4m3(load(codes[i]).cast::<u32>());
        let sbits = load(scales[i / block_size]).cast::<f32>();
...
#[kernel]
pub fn iron_mxfp8_e5m2_dequant<T>(
    codes: Tensor<u8>,
    scales: Tensor<u8>,
    out: Tensor<T>,
    #[constexpr] n: u32,
    ...
    let i = program_id::<0>();
    if i < n {
        let elem = iron_decode_e5m2(load(codes[i]).cast::<u32>());
        let sbits = load(scales[i / block_size]).cast::<f32>();
        let scale = exp2(sbits - 127.0f32);
        store(out[i], (elem * scale).cast::<T>());
    }
...
// E4M3 + f32 scale), so it reuses `iron_nvfp8_dequant` — only fp4 (4-bit E2M1),
...
#[kernel]
pub fn iron_fp8_e5m2_dequant<T>(
    codes: Tensor<u8>,
    scales: Tensor<f32>,
    out: Tensor<T>,
    #[constexpr] n: u32,
...
pub fn iron_int8_dequant<T>(
    ...
    let i = program_id::<0>();
    if i < n {
        let elem = iron_decode_int8(load(codes[i]).cast::<u32>());
        let scale = load(scales[i / block_size]);
        store(out[i], (elem * scale).cast::<T>());
    }
}
...
// (straddle-aware two-word read, mirroring `iron/dequant_gemv.rs`), sign-extend
...
int_dequant_f32!(iron_int2_dequant, 2u32, 2u32, 4.0f32);
int_dequant_f32!(iron_int3_dequant, 3u32, 4u32, 8.0f32);
int_dequant_f32!(iron_int4_dequant, 4u32, 8u32, 16.0f32);
int_dequant_f32!(iron_int5_dequant, 5u32, 16u32, 32.0f32);
int_dequant_f32!(iron_int6_dequant, 6u32, 32u32, 64.0f32);
...
int_dequant_e8m0!(iron_mxint2_dequant, 2u32, 2u32, 4.0f32);
int_dequant_e8m0!(iron_mxint3_dequant, 3u32, 4u32, 8.0f32);
int_dequant_e8m0!(iron_mxint4_dequant, 4u32, 8u32, 16.0f32);
int_dequant_e8m0!(iron_mxint5_dequant, 5u32, 16u32, 32.0f32);
int_dequant_e8m0!(iron_mxint6_dequant, 6u32, 32u32, 64.0f32);

/// MXINT8 — 8-bit codes (byte layout, block 32), E8M0 pow-2 block scale.
#[kernel]
pub fn iron_mxint8_dequant<T>(
    codes: Tensor<u8>,
    scales: Tensor<u8>,
    out: Tensor<T>,
    #[constexpr] n: u32,
...
#[kernel]
pub fn iron_nvfp8_f16_dequant<T>(
    codes: Tensor<u8>,
    scales: Tensor<f16>,
    out: Tensor<T>,
    #[constexpr] n: u32,
...
#[kernel]
pub fn iron_fp8_e5m2_f16_dequant<T>(
    codes: Tensor<u8>,
    scales: Tensor<f16>,
    out: Tensor<T>,
    #[constexpr] n: u32,
...
int_dequant_f16!(iron_int2_f16_dequant, 2u32, 2u32, 4.0f32);
int_dequant_f16!(iron_int3_f16_dequant, 3u32, 4u32, 8.0f32);
int_dequant_f16!(iron_int4_f16_dequant, 4u32, 8u32, 16.0f32);
int_dequant_f16!(iron_int5_f16_dequant, 5u32, 16u32, 32.0f32);
int_dequant_f16!(iron_int6_f16_dequant, 6u32, 32u32, 64.0f32);

/// int8 (FP16 scale) — 8-bit codes (byte layout, group 64), per-group FP16 scale.
#[kernel]
pub fn iron_int8_f16_dequant<T>(
    codes: Tensor<u8>,
    scales: Tensor<f16>,
    out: Tensor<T>,
    #[constexpr] n: u32,
...
pub mod kernel_tests {
    ...
    fn test_nvfp4_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_nvfp4_dequant::kernel_ir_for(dt), QFormat::Nvfp4, 4, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 5e-2, 2e-1])]
    fn test_mxfp8_e4m3_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxfp8_e4m3_dequant::kernel_ir_for(dt), QFormat::Mxfp8E4, 4, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 5e-2, 2e-1])]
    fn test_mxfp8_e5m2_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxfp8_e5m2_dequant::kernel_ir_for(dt), QFormat::Mxfp8E5, 4, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 5e-2, 2e-1])]
    fn test_nvfp8_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_nvfp8_dequant::kernel_ir_for(dt), QFormat::Nvfp8, 4, 64, dt)
    }
    ...
    fn test_fp4_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_fp4_dequant::kernel_ir_for(dt), QFormat::Fp4, 4, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_nvfp8_dequant::kernel_ir_for(dt), QFormat::Fp8E4m3, 4, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_fp8_e5m2_dequant::kernel_ir_for(dt), QFormat::Fp8E5m2, 4, 64, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int8_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int8_dequant::kernel_ir_for(dt), QFormat::Int8, 4, 64, dt)
    }
    ...
    fn test_int2_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int2_dequant::kernel_ir_for(dt), QFormat::Int2, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int3_dequant::kernel_ir_for(dt), QFormat::Int3, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int4_dequant::kernel_ir_for(dt), QFormat::Int4, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int5_dequant::kernel_ir_for(dt), QFormat::Int5, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int6_dequant::kernel_ir_for(dt), QFormat::Int6, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint2_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxint2_dequant::kernel_ir_for(dt), QFormat::Mxint2, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint3_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxint3_dequant::kernel_ir_for(dt), QFormat::Mxint3, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint4_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxint4_dequant::kernel_ir_for(dt), QFormat::Mxint4, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint5_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxint5_dequant::kernel_ir_for(dt), QFormat::Mxint5, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint6_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxint6_dequant::kernel_ir_for(dt), QFormat::Mxint6, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint8_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_mxint8_dequant::kernel_ir_for(dt), QFormat::Mxint8, 4, 64, dt)
    }
    ...
    fn test_nvfp8_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_nvfp8_f16_dequant::kernel_ir_for(dt), QFormat::Nvfp8F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_nvfp8_f16_dequant::kernel_ir_for(dt), QFormat::Fp8E4m3F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp4_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_fp4_f16_dequant::kernel_ir_for(dt), QFormat::Fp4F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_fp8_e5m2_f16_dequant::kernel_ir_for(dt), QFormat::Fp8E5m2F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int2_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int2_f16_dequant::kernel_ir_for(dt), QFormat::Int2F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int3_f16_dequant::kernel_ir_for(dt), QFormat::Int3F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int4_f16_dequant::kernel_ir_for(dt), QFormat::Int4F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int5_f16_dequant::kernel_ir_for(dt), QFormat::Int5F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int6_f16_dequant::kernel_ir_for(dt), QFormat::Int6F16, 4, 64, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int8_f16_dequant(dt: DType) -> TestSetup {
        dequant_setup(iron_int8_f16_dequant::kernel_ir_for(dt), QFormat::Int8F16, 4, 64, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_nvfp4_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_nvfp4_dequant::kernel_ir_for(dt), QFormat::Nvfp4, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e4m3_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxfp8_e4m3_dequant::kernel_ir_for(dt), QFormat::Mxfp8E4, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxfp8_e5m2_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxfp8_e5m2_dequant::kernel_ir_for(dt), QFormat::Mxfp8E5, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp8_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_nvfp8_dequant::kernel_ir_for(dt), QFormat::Nvfp8, 4096, 4096, dt)
    }
    // Legacy float-scale fp4 / fp8 + symmetric int8. fp8_e4m3 reuses the nvfp8
    // kernel (same 8-bit-E4M3 + f32-scale shape); the others decode in their own.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp4_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_fp4_dequant::kernel_ir_for(dt), QFormat::Fp4, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_nvfp8_dequant::kernel_ir_for(dt), QFormat::Fp8E4m3, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_fp8_e5m2_dequant::kernel_ir_for(dt), QFormat::Fp8E5m2, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int8_dequant::kernel_ir_for(dt), QFormat::Int8, 4096, 4096, dt)
    }
    // Symmetric sub-byte ints (FP32 group scale) + MXINT (E8M0 block scale).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int2_dequant::kernel_ir_for(dt), QFormat::Int2, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int3_dequant::kernel_ir_for(dt), QFormat::Int3, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int4_dequant::kernel_ir_for(dt), QFormat::Int4, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int5_dequant::kernel_ir_for(dt), QFormat::Int5, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int6_dequant::kernel_ir_for(dt), QFormat::Int6, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint2_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxint2_dequant::kernel_ir_for(dt), QFormat::Mxint2, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint3_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxint3_dequant::kernel_ir_for(dt), QFormat::Mxint3, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint4_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxint4_dequant::kernel_ir_for(dt), QFormat::Mxint4, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint5_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxint5_dequant::kernel_ir_for(dt), QFormat::Mxint5, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint6_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxint6_dequant::kernel_ir_for(dt), QFormat::Mxint6, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint8_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_mxint8_dequant::kernel_ir_for(dt), QFormat::Mxint8, 4096, 4096, dt)
    }
    // FP16-scale twins (fp8_e4m3_f16 reuses the nvfp8_f16 kernel).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_nvfp8_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_nvfp8_f16_dequant::kernel_ir_for(dt), QFormat::Nvfp8F16, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e4m3_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(
            iron_nvfp8_f16_dequant::kernel_ir_for(dt),
            QFormat::Fp8E4m3F16,
            4096,
            4096,
            dt,
    ...
    fn bench_fp4_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_fp4_f16_dequant::kernel_ir_for(dt), QFormat::Fp4F16, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_fp8_e5m2_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(
            iron_fp8_e5m2_f16_dequant::kernel_ir_for(dt),
            QFormat::Fp8E5m2F16,
            4096,
            4096,
            dt,
    ...
    fn bench_int2_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int2_f16_dequant::kernel_ir_for(dt), QFormat::Int2F16, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int3_f16_dequant::kernel_ir_for(dt), QFormat::Int3F16, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int4_f16_dequant::kernel_ir_for(dt), QFormat::Int4F16, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int5_f16_dequant::kernel_ir_for(dt), QFormat::Int5F16, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int6_f16_dequant::kernel_ir_for(dt), QFormat::Int6F16, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int8_f16_dequant(dt: DType) -> BenchSetup {
        dequant_bench(iron_int8_f16_dequant::kernel_ir_for(dt), QFormat::Int8F16, 4096, 4096, dt)
    }
}
