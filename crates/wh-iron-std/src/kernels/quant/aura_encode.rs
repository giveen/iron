...
//!   bandwidth payback. See Iron's `AuraKLDIntegrationTests` for the gate.
...
//! (worst case — see Iron post-mortem 2026-05-19). Consumers MUST
...
//! ## Macro structure
//!
//! `iron_aura_encode_kernel!` wraps a single `#[kernel] pub fn …`
...
///
/// Produces kernels: `iron_aura_encode_int2`, `iron_aura_encode_int3`, `iron_aura_encode_int4`,
/// `iron_aura_encode_int6`, `iron_aura_encode_int8`.
///
...
// Iron's KLD harness (Qwen3-0.6B-4bit, 61-position): T-typed
...
pub fn iron_aura_encode<T>(
    ...
    // indices.  Caught by the iron_aura_encode GPU correctness
    // test.  See Iron post-mortem 2026-05-19 — a wh-iron
...
pub mod kernel_tests {
    ...
    use super::iron_aura_encode_int4;
    ...
    // `iron_aura_encode_int4_srht_rotation_f32` A/B test).
...
pub mod kernel_benches {
    ...
    fn bench_aura_encode(dt: DType) -> BenchSetup {
        setup(BenchSetup::new(iron_aura_encode_intBITS::kernel_ir_for(dt)), 128, BITS, 256, dt)
    }
}
