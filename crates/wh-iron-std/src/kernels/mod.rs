//! Copyright 2026 Eric Kryski (@ekryski) and Tom Turney (@TheTom)
//! SPDX-License-Identifier: Apache-2.0
//! Kernel standard library, grouped by **operation family** — the consolidation
//! target (see `docs/specs/KERNEL_CONSOLIDATION_PLAN.md`). Families migrate here
//! one PR at a time from the legacy `mlx/` + `iron/` folders. Kernels are named
//! for the **operation / layout** they implement (`iron_<op>`), never for a model;
//! model-specific usage notes live in comments above the kernel. The Iron emit
//! consumer is regenerated from the new inventory after each family lands.

pub mod audio;
pub mod convolution;
pub mod gemm;
pub mod hyper_connections;
pub mod kv_cache;
pub mod moe;
pub mod norm;
pub mod ops;
pub mod primitives;
pub mod quant;
pub mod rope;
pub mod sampling;
pub mod sdpa;
pub mod ssm;
pub mod vision;
