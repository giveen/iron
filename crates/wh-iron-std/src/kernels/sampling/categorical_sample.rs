...
//! by Iron's `gpu-categorical` decode path (T > 0, no filters). The
//! greedy fast path uses `argmax` instead.
//!
//! Codegen-only. End-to-end sampling correctness lives in Iron's
//! harness.
...
#[kernel]
pub fn iron_softmax_categorical_sample<T>(
    inp: Tensor<T>,
    out: Tensor<u32>,
...
pub mod kernel_tests {
    ...
    use super::iron_softmax_categorical_sample;
    ...
    fn test_softmax_categorical_sample(dt: DType) -> TestSetup {
        ...
        logits[idx] = spike;
        TestSetup::new(iron_softmax_categorical_sample::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_softmax_categorical_sample` at Qwen3 vocab
...
pub mod kernel_benches {
    ...
    fn bench_softmax_categorical_sample(dt: DType) -> BenchSetup {
        let n = 152_064usize;
        BenchSetup::new(iron_softmax_categorical_sample::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
