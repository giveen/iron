...
#[kernel]
pub fn iron_softmax<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
...
/// New-syntax correctness for `iron_softmax` (Reduction mode, one threadgroup per
...
pub mod kernel_tests {
    ...
    fn setup_with(
        ...
        TestSetup::new(iron_softmax::kernel_ir_for(dt))
    ...
    fn test_iron_softmax(dt: DType) -> TestSetup { setup(4, 1024, dt) }
    ...
    fn test_iron_softmax_large_values(dt: DType) -> TestSetup {
        setup_with(2, 1024, dt, |_r, i| 50.0 + (i % 7) as f32 * 10.0)
    }
}

/// New-syntax benchmark for `iron_softmax` (vs MLX `metal/softmax.metal`).
pub mod kernel_benches {
    ...
    // `axis_size`(int)[[buffer(2)]]. `inp` is shared by name with the Iron input.
    //
    // MLX dispatch geometry: one threadgroup per row (grid=[rows,1,1]) at
    // tpg=1024 — NOT the Iron tpg=256. `softmax_looped` uses
    ...
    // these at 1024 threads (n_simd==32, every slot live). At Iron's tpg=256
    ...
    fn bench_softmax(dt: DType) -> BenchSetup {
        ...
        let tn = mlx_tname(dt);
        BenchSetup::new(iron_softmax::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
