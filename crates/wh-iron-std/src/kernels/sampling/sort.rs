...
//!
//!   1. **`iron_sort<T>`** — single-block bitonic sort. Each threadgroup
...
//!
//!   2. **`iron_merge<T>`** — one bottom-up merge pass. Given sorted runs
...
//!
//! ## DISPATCH INVARIANTS (iron_merge)
...
#[kernel]
pub fn iron_sort<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
...
//
// `iron_sort_segmented<T>` sorts each row of a `[batch, n]` matrix
// independently. One threadgroup per row; each threadgroup uses a
// single-block bitonic sort identical to `iron_sort`, covering rows of up
...
#[kernel]
pub fn iron_sort_segmented<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    ...
    threadgroup_barrier();
    // Bitonic sort network — identical structure to `iron_sort`.
...
/// New-syntax correctness for the sort family. `iron_sort` (single-block bitonic,
/// Reduction, one TG per 1024-block), `iron_merge` (one bottom-up merge pass,
/// Grid3D — input holds sorted runs of `run`, output sorts each `2*run` block),
/// and `iron_sort_segmented` (Reduction, per-row sort, n ≤ 1024). All exact on the
/// multiset; oracles sort the relevant chunk.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::{iron_merge, iron_sort, iron_sort_segmented};
    ...
    fn test_iron_merge(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_merge::kernel_ir_for(dt))
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = 1e-6)]
    fn test_iron_sort_segmented(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_sort_segmented::kernel_ir_for(dt))
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = 1e-6)]
    fn test_iron_sort(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_sort::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("inp", pack_f32(&inp, dt), dt))
...
/// New-syntax benchmark for `iron_sort` (vs MLX `metal/sort.metal`).
pub mod kernel_benches {
    ...
    fn bench_sort(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_sort::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            ...
            // Iron `iron_sort` layout (`bn256_tn4` → BLOCK_THREADS=256, N_PER_THREAD=4,
            ...
            // on the Y axis (`[1, n_blocks, 1]`) — unlike the Iron kernel which
    ...
    fn bench_merge(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_merge::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_segmented(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_sort_segmented::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
