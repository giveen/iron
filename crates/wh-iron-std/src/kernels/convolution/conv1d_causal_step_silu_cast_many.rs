...
//! `ssm.rs`) + `iron_silu_cast_to_f32` (in `mlx/unary.rs`) looped T times,
...
//! same dispatch-saving pattern as `iron_rope_banded` and
//! `iron_kv_cache_update_many`.
//!
//! Layout:
//!
//!   src       [T, conv_dim]                T
...
//! production user of `conv1d_causal_step` in the Iron tree today —
...
//! Identical to `iron_silu_cast_to_f32` (`x * sigmoid(x)` with
//! `sigmoid(x) = 1 / (1 + exp(-x))`).
//!
...
#[kernel]
pub fn iron_conv1d_causal_step_silu_cast_many<T>(
    src: Tensor<T>,
    w: Tensor<T>,
    b: Tensor<T>,
    state_in: Tensor<T>,
    ...
    for r in range(0u32, t_len, 1u32) {
        ...
        // SiLU + cast — identical form to `iron_silu_cast_to_f32`:
...
mod dump_audit {
    ...
    /// compiler. Unlike `iron_gated_delta_prep_chunk`'s `state_reg`, though,
    ...
    /// `cargo test -p wh-iron-std --lib --release -- kernels::convolution::conv1d_causal_step_silu_cast_many::dump_audit::dump --nocapture`
    #[test]
    fn dump() {
        use wh_iron::codegen::msl::MslGenerator;
        let mut k = iron_conv1d_causal_step_silu_cast_many::kernel_ir_for(DType::F32);
...
pub mod kernel_tests {
    ...
    use super::iron_conv1d_causal_step_silu_cast_many;
...
pub mod kernel_benches {
    ...
    fn bench_shape(t_len: usize, conv_dim: usize, dt: DType) -> BenchSetup {
        let state_rows = (CONV_KERNEL - 1) as usize;
        let kernel: Kernel = iron_conv1d_causal_step_silu_cast_many::kernel_ir_for(dt);
