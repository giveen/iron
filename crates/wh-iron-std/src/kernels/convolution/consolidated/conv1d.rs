...
//! crate:
//!
//! * [`iron_conv1d_dense`](self::iron_conv1d_dense) — the four dense 1D convs
//!   (direct with/without dilation, transpose full/depthwise) in one
//!   function. Four named variants via `VARIANT` axis:
//!   `iron_conv1d_dense_direct` (audio_conv1d),
//!   `iron_conv1d_dense_dilated` (conv1d_dilated),
//!   `iron_conv1d_dense_transpose` (conv1d_transpose),
//!   `iron_conv1d_dense_depthwise` (iron_conv1d_transpose_depthwise).
...
//!
//! * [`iron_conv1d_quant`](self::iron_conv1d_quant) — the 19
...
//!   `iron_conv1d_quant_{audio,fishspeech}_{mxfp4,nvfp4,…,int8}`. The `dilation` constexpr is
...
//!
//! The decode primitives (`iron_decode_e2m1`, `iron_decode_e4m3`,
//! `iron_decode_e5m2`, `iron_decode_int8`, `iron_decode_e8m0`,
//! `iron_unpack_nbit`) live in `crate::kernels::primitives` and are inlined at
...
//! always 0 in that case. The straddle-aware `iron_unpack_nbit` primitive
...
//   (TRANSPOSE=1, DILATED=0, DEPTHWISE=1) → iron_conv1d_transpose_depthwise (transpose, depthwise)
//
...
#[kernel(variants(
    ...
    // Named labels give readable names: iron_conv1d_quant_audio_mxfp4, …fishspeech_int8.
...
pub fn iron_conv1d_quant<T>(
    ...
    // dedicated `iron_decode_e2m1` / `iron_decode_e4m3` / `iron_decode_e5m2` /
    // `iron_decode_int8` primitives instead. The half/full terms remain live but
    ...
    for ic in range(0u32, in_ch, 1u32) {
        ...
        for kx in range(0u32, k, 1u32) {
            ...
            //     `iron_unpack_nbit` extracts the BITS-wide code; nibble formats
            //     (FMT 0,1,7) then go through `iron_decode_e2m1`, sub-byte ints
            //     sign-extend via `half`/`full` below.
            //   * FMT 13..18: u8 weight, one code per byte. Routed to
            //     `iron_decode_e4m3` / `iron_decode_e5m2` / `iron_decode_int8` by
            //     FMT.
            ...
            //     (`iron_decode_e8m0` → 2^(b-127)).
            //   * ST=u8, FMT 13-15 (mxfp8 / mxint8): E8M0.
            //   * ST=f32 (FMT 7-12, 16-18): direct load, no decode.
            let scale = if FMT <= 6u32 {
                if FMT == 1u32 {
                    iron_decode_e4m3(load(scales[w_row_blk + col / block_size]).cast::<u32>())
                        * global
                } else {
                    iron_decode_e8m0(load(scales[w_row_blk + col / block_size]).cast::<u32>())
                }
...
pub mod kernel_tests {
    ...
    // (`iron_conv1d_dense_fmt*`, `iron_conv1d_block_scaled_*`) into the test
    ...
    fn test_dense_conv1d(dt: DType) -> TestSetup {
        if TRANSPOSE == 0u32 {
            if DILATED == 0u32 {
                ...
                direct_setup(iron_conv1d_dense_direct::kernel_ir_for(dt), 1, 8, 50, 16, 3, 1, 1, dt)
            } else {
                // dilated — MRF ResBlock, dilation 3.
                dilated_setup(
                    iron_conv1d_dense_dilated::kernel_ir_for(dt),
                    1,
                    12,
                    60,
                    12,
            ...
            transpose_setup(
                iron_conv1d_dense_transpose::kernel_ir_for(dt),
                1,
                8,
                16,
                6,
            ...
            depthwise_setup(iron_conv1d_dense_depthwise::kernel_ir_for(dt), 6, 13, 3, 2, 1, dt)
        }
    }
    ...
    fn test_blockscaled_conv1d(dt: DType) -> TestSetup {
        ...
        let kernel = if DIL == 0u32 {
            if FMT == 0u32 {
                iron_conv1d_quant_audio_mxfp4::kernel_ir_for(dt)
            } else if FMT == 1u32 {
                iron_conv1d_quant_audio_nvfp4::kernel_ir_for(dt)
            } else if FMT == 2u32 {
                iron_conv1d_quant_audio_fp8_e5m2::kernel_ir_for(dt)
            } else {
                iron_conv1d_quant_audio_int8::kernel_ir_for(dt)
            }
        } else {
            if FMT == 0u32 {
                iron_conv1d_quant_fishspeech_mxfp4::kernel_ir_for(dt)
            } else if FMT == 1u32 {
                iron_conv1d_quant_fishspeech_nvfp4::kernel_ir_for(dt)
            } else if FMT == 2u32 {
                iron_conv1d_quant_fishspeech_fp8_e5m2::kernel_ir_for(dt)
            } else {
                iron_conv1d_quant_fishspeech_int8::kernel_ir_for(dt)
            }
        };
...
pub mod kernel_benches {
    ...
    fn bench_dense_conv1d(dt: DType) -> BenchSetup {
        ...
        let kernel = if TRANSPOSE == 0u32 {
            if DILATED == 0u32 {
                iron_conv1d_dense_direct::kernel_ir_for(dt)
            } else {
                iron_conv1d_dense_dilated::kernel_ir_for(dt)
            }
        } else if DEPTHWISE == 0u32 {
            ...
            iron_conv1d_dense_depthwise::kernel_ir_for(dt)
        };
