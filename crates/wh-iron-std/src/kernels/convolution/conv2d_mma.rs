...
//!
//! Perf follow-up to `iron/conv2d.rs` — the direct-conv kernels there are
...
//!
//! ## Tile geometry (mirrors `iron_qmm_mma`)
//!
...
//!   by 32.  Padding extensions are a follow-up (same as `iron_qmm_mma` →
//!   `iron_qmm_mma_m16`).
...
//!
//! The cooperative weight load mirrors the X-load in `iron_qmm_mma`: the B
...
//! rows of B as K-vectors (no transpose needed vs `iron_qmm_mma`'s W^T).
//!
...
pub fn conv2d_mma<T>(
    ...
    // Lane mapping for coop B-load (weight): same as X-load in iron_qmm_mma.
    ...
    for kb in range(0u32, total_k, 32u32) {
        ...
        // [BK × BM_oc] B-matrix — same swap as iron_qmm_mma's W^T read.)
