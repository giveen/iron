...
//! wh-iron DSL has no complex type — but it does not need one: a
...
///
/// Produces kernels: `iron_fft_n32`, `_n64`, `_n128`, `_n256`, `_n512`, `_n1024`.
...
//      `iron_fft_n*` kernel on the padded M-length sequence (and on the
...
// convolution) uses the existing `iron_fft_n1024` (M=1024 covers both
...
pub fn iron_fft_bluestein_preprocess<T>(
    in_re: Tensor<T>,
    in_im: Tensor<T>,
    mut out_re: Tensor<T>,
    mut out_im: Tensor<T>,
    ...
    if idx < rows * m_len {
        ...
        // `iron_fft_bluestein_chirp_filter`). Inverse flips both signs.
...
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_fft_bluestein_cmul<T>(
    y_re: Tensor<T>,
    y_im: Tensor<T>,
...
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_fft_bluestein_postprocess<T>(
    conv_re: Tensor<T>,
    conv_im: Tensor<T>,
    mut out_re: Tensor<T>,
    mut out_im: Tensor<T>,
...
pub mod kernel_tests {
    ...
    fft_test!(test_fft_n32, iron_fft_n32, 32);
    fft_test!(test_fft_n64, iron_fft_n64, 64);
    fft_test!(test_fft_n128, iron_fft_n128, 128);
    fft_test!(test_fft_n256, iron_fft_n256, 256);
    fft_test!(test_fft_n512, iron_fft_n512, 512);
    fft_test!(test_fft_n1024, iron_fft_n1024, 1024);
    ...
    fn test_fft_inv_n64(dt: DType) -> TestSetup {
        fft_setup(iron_fft_n64::kernel_ir_for(dt), 64, true, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-3, 8e-3, 4e-2])]
    fn test_fft_inv_n256(dt: DType) -> TestSetup {
        fft_setup(iron_fft_n256::kernel_ir_for(dt), 256, true, dt)
    }
    ...
    fn test_fft_bluestein_chirp_filter(_dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_fft_bluestein_chirp_filter::kernel_ir_for())
    ...
    fn bluestein_pre_setup(dt: DType, inv: bool) -> TestSetup {
        ...
        TestSetup::new(iron_fft_bluestein_preprocess::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bluestein_cmul_setup(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_fft_bluestein_cmul::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bluestein_post_setup(dt: DType, inv: bool) -> TestSetup {
        ...
        TestSetup::new(iron_fft_bluestein_postprocess::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
...
pub mod kernel_benches {
    ...
    fft_bench!(bench_fft_n32, iron_fft_n32, 32);
    fft_bench!(bench_fft_n64, iron_fft_n64, 64);
    fft_bench!(bench_fft_n128, iron_fft_n128, 128);
    fft_bench!(bench_fft_n256, iron_fft_n256, 256);
    fft_bench!(bench_fft_n512, iron_fft_n512, 512);
    fft_bench!(bench_fft_n1024, iron_fft_n1024, 1024);
    ...
    fn bench_bluestein_chirp_filter(_dt: DType) -> BenchSetup {
        BenchSetup::new(iron_fft_bluestein_chirp_filter::kernel_ir_for())
            .mode(KernelMode::Grid3D)
    ...
    fn bench_bluestein_preprocess(dt: DType) -> BenchSetup {
        BenchSetup::new(iron_fft_bluestein_preprocess::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_bluestein_cmul(dt: DType) -> BenchSetup {
        BenchSetup::new(iron_fft_bluestein_cmul::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_bluestein_postprocess(dt: DType) -> BenchSetup {
        BenchSetup::new(iron_fft_bluestein_postprocess::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
