...
//! `iron_rope_banded` — see that file's docstring for the broader
...
#[kernel]
pub fn iron_kv_cache_update_many<T>(
    src: Tensor<T>,
    ...
    // Per-row position lookup — same shape as `iron_rope_banded`.
...
pub mod kernel_tests {
    ...
    use super::iron_kv_cache_update_many;
    ...
    fn test_kv_cache_update_many(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_kv_cache_update_many::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_kv_cache_update_many` — a Qwen-class prefill
...
pub mod kernel_benches {
    ...
    fn bench_kv_cache_update_many(dt: DType) -> BenchSetup {
        ...
        let total = n_tokens * n_kv_heads * head_dim;
        BenchSetup::new(iron_kv_cache_update_many::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
