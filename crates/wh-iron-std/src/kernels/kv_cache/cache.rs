...
//! group-quant (int4 / int8) variants used by Iron's
...
//! "int{BITS}")]` so `iron_quantize_kv_int4/8` and `iron_bulk_dequant_kv_int4/8`
...
//!
//! Codegen-only. End-to-end correctness validated in Iron integration
...
///
/// Produces `iron_quantize_kv_int4` and `iron_quantize_kv_int8`. One thread per group.
#[kernel(variants(BITS = [4, 8], suffix = "int{BITS}"))]
pub fn iron_quantize_kv<T>(
    src: Tensor<T>,
...
///
/// Produces `iron_bulk_dequant_kv_int4` and `iron_bulk_dequant_kv_int8`.
/// One thread per output element.
#[kernel(variants(BITS = [4, 8], suffix = "int{BITS}"))]
pub fn iron_bulk_dequant_kv<T>(
    in_w: Tensor<u32>,
    in_s: Tensor<T>,
    in_b: Tensor<T>,
    mut out: Tensor<T>,
...
#[kernel]
pub fn iron_quantize_kv_fp8_e4m3<T>(
    src: Tensor<T>,
    mut out_w: Tensor<u32>,
    mut out_s: Tensor<T>,
    #[constexpr] head_dim: u32,
...
#[kernel]
pub fn iron_quantize_kv_fp8_e5m2<T>(
    src: Tensor<T>,
    mut out_w: Tensor<u32>,
    mut out_s: Tensor<T>,
    #[constexpr] head_dim: u32,
...
#[kernel]
pub fn iron_bulk_dequant_kv_fp8_e4m3<T>(
    in_w: Tensor<u32>,
    in_s: Tensor<T>,
    mut out: Tensor<T>,
    #[constexpr] head_dim: u32,
...
#[kernel]
pub fn iron_bulk_dequant_kv_fp8_e5m2<T>(
    in_w: Tensor<u32>,
    in_s: Tensor<T>,
    mut out: Tensor<T>,
    #[constexpr] head_dim: u32,
...
pub mod kernel_tests {
    ...
    use super::{
        iron_bulk_dequant_kv_fp8_e4m3,
        iron_bulk_dequant_kv_fp8_e5m2,
        iron_bulk_dequant_kv_int4,
        iron_bulk_dequant_kv_int8,
        iron_kv_cache_update,
        iron_quantize_kv_fp8_e4m3,
        iron_quantize_kv_fp8_e5m2,
        iron_quantize_kv_int4,
        iron_quantize_kv_int8,
    };
    ...
    fn test_kv_cache_update(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_kv_cache_update::kernel_ir_for(dt))
    ...
    // ── iron_quantize_kv_int4 / int8 — scale + bias check ─────────────────
    //
    ...
    // ── iron_bulk_dequant_kv_int4 / int8 — exact dequant oracle ───────────
    //
    ...
    fn dequant_setup(kernel: wh_iron::core::ir::Kernel, bits: u32, dt: DType) -> TestSetup {
    ...
    fn test_bulk_dequant_kv(dt: DType) -> TestSetup {
        dequant_setup(iron_bulk_dequant_kv_intBITS::kernel_ir_for(dt), BITS, dt)
    }

    // ── iron_quantize_kv_fp8_e4m3 / e5m2 — scale check ────────────────────
    //
    ...
    fn test_quantize_kv_fp8_e4m3(dt: DType) -> TestSetup {
        quant_fp8_scale_setup(iron_quantize_kv_fp8_e4m3::kernel_ir_for(dt), 240.0, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-3, 1e-2])]
    fn test_quantize_kv_fp8_e5m2(dt: DType) -> TestSetup {
        quant_fp8_scale_setup(iron_quantize_kv_fp8_e5m2::kernel_ir_for(dt), 57344.0, dt)
    }

    // ── iron_bulk_dequant_kv_fp8_e4m3 / e5m2 — exact fp8 decode oracle ─────────
    //
    ...
    fn test_bulk_dequant_kv_fp8_e4m3(dt: DType) -> TestSetup {
        ...
        dequant_fp8_setup(iron_bulk_dequant_kv_fp8_e4m3::kernel_ir_for(dt), &palette, dt)
    }
    ...
    fn test_bulk_dequant_kv_fp8_e5m2(dt: DType) -> TestSetup {
        ...
        dequant_fp8_setup(iron_bulk_dequant_kv_fp8_e5m2::kernel_ir_for(dt), &palette, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_kv_cache_update(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_kv_cache_update::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn quant_bench(kernel: wh_iron::core::ir::Kernel, bits: u32, dt: DType) -> BenchSetup {
        let groups_per_head = HEAD_DIM / GROUP_SIZE;
        let total_groups = N_KV_HEADS * groups_per_head;
        let vals_per_pack = 32 / bits as usize;
        let n_packed = N_KV_HEADS * MAX_SEQ * (HEAD_DIM / vals_per_pack);
    ...
    fn bench_quantize_kv(dt: DType) -> BenchSetup {
        quant_bench(iron_quantize_kv_intBITS::kernel_ir_for(dt), BITS, dt)
    }

    fn dequant_bench(kernel: wh_iron::core::ir::Kernel, bits: u32, dt: DType) -> BenchSetup {
        let groups_per_head = HEAD_DIM / GROUP_SIZE;
        let vals_per_pack = 32 / bits as usize;
        let n_packed = N_KV_HEADS * MAX_SEQ * (HEAD_DIM / vals_per_pack);
        let s_total = N_KV_HEADS * MAX_SEQ * groups_per_head;
    ...
    fn bench_bulk_dequant_kv(dt: DType) -> BenchSetup {
        dequant_bench(iron_bulk_dequant_kv_intBITS::kernel_ir_for(dt), BITS, dt)
    }

    // fp8 quantize is scale-only (no out_b buffer).
    fn quant_fp8_bench(kernel: wh_iron::core::ir::Kernel, dt: DType) -> BenchSetup {
        let groups_per_head = HEAD_DIM / GROUP_SIZE;
        let total_groups = N_KV_HEADS * groups_per_head;
        let n_packed = N_KV_HEADS * MAX_SEQ * (HEAD_DIM / 4);
        let s_total = N_KV_HEADS * MAX_SEQ * groups_per_head;
    ...
    fn bench_quantize_kv_fp8_e4m3(dt: DType) -> BenchSetup {
        quant_fp8_bench(iron_quantize_kv_fp8_e4m3::kernel_ir_for(dt), dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_quantize_kv_fp8_e5m2(dt: DType) -> BenchSetup {
        quant_fp8_bench(iron_quantize_kv_fp8_e5m2::kernel_ir_for(dt), dt)
    }

    // fp8 dequant is scale-only (no in_b buffer).
    fn dequant_fp8_bench(kernel: wh_iron::core::ir::Kernel, dt: DType) -> BenchSetup {
        let groups_per_head = HEAD_DIM / GROUP_SIZE;
        let n_packed = N_KV_HEADS * MAX_SEQ * (HEAD_DIM / 4);
        let s_total = N_KV_HEADS * MAX_SEQ * groups_per_head;
        let total_out = N_KV_HEADS * N_POSITIONS * HEAD_DIM;
    ...
    fn bench_bulk_dequant_kv_fp8_e4m3(dt: DType) -> BenchSetup {
        dequant_fp8_bench(iron_bulk_dequant_kv_fp8_e4m3::kernel_ir_for(dt), dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_bulk_dequant_kv_fp8_e5m2(dt: DType) -> BenchSetup {
        dequant_fp8_bench(iron_bulk_dequant_kv_fp8_e5m2::kernel_ir_for(dt), dt)
    }
}
