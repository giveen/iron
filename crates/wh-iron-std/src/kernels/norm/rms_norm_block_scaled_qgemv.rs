...
//! fusion of `kernels/norm/rms_norm_qgemv.rs` (`iron_rms_norm_qgemv`, the simple
...
//! - Phase 1: per-thread Σx², `iron_rms_inv_scalar` does the TG reduce + rsqrt.
...
/// through `kernels/primitives.rs`. Produces `iron_<FMT>_rms_norm_qgemv`.
#[kernel(variants(
...
pub fn iron<T>(
    ...
    let inv_rms = iron_rms_inv_scalar(ssq, eps_buf, in_dim);
...
pub mod kernel_tests {
    ...
    fn test_mxfp4_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_mxfp4_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Mxfp4, 4, 256, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_nvfp4_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_nvfp4_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Nvfp4, 4, 256, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxfp8_e4m3_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_mxfp8_e4m3_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxfp8E4,
            4,
            256,
            dt,
    ...
    fn test_mxfp8_e5m2_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_mxfp8_e5m2_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxfp8E5,
            4,
            256,
            dt,
    ...
    fn test_nvfp8_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_nvfp8_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Nvfp8, 4, 256, dt)
    }
    ...
    fn test_fp4_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_fp4_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Fp4, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e4m3_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_nvfp8_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Fp8E4m3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_fp8_e5m2_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Fp8E5m2,
            4,
            256,
            dt,
    ...
    fn test_int8_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_int8_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int8, 4, 256, dt)
    }
    ...
    fn test_int2_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_int2_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int2, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int3_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_int3_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int4_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_int4_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int4, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int5_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_int5_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int5, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_int6_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_int6_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int6, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint2_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_mxint2_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Mxint2, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint3_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_mxint3_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Mxint3, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint4_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_mxint4_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Mxint4, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint5_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_mxint5_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Mxint5, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint6_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_mxint6_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Mxint6, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_mxint8_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_mxint8_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Mxint8, 4, 256, dt)
    }
    ...
    fn test_nvfp8_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_nvfp8_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Nvfp8F16,
            4,
            256,
            dt,
    ...
    fn test_fp4_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(iron_fp4_f16_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Fp4F16, 4, 256, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_fp8_e5m2_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_fp8_e5m2_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Fp8E5m2F16,
            4,
            256,
            dt,
    ...
    fn test_int2_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_int2_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Int2F16,
            4,
            256,
            dt,
    ...
    fn test_int3_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_int3_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Int3F16,
            4,
            256,
            dt,
    ...
    fn test_int4_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_int4_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Int4F16,
            4,
            256,
            dt,
    ...
    fn test_int5_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_int5_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Int5F16,
            4,
            256,
            dt,
    ...
    fn test_int6_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_int6_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Int6F16,
            4,
            256,
            dt,
    ...
    fn test_int8_f16_rms_norm_qgemv(dt: DType) -> TestSetup {
        rms_qgemv_setup(
            iron_int8_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Int8F16,
            4,
            256,
            dt,
...
pub mod kernel_benches {
    ...
    fn bench_mxfp4_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_mxfp4_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxfp4,
            4096,
            4096,
            dt,
    ...
    fn bench_nvfp4_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_nvfp4_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Nvfp4,
            4096,
            4096,
            dt,
    ...
    fn bench_nvfp8_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_nvfp8_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Nvfp8,
            4096,
            4096,
            dt,
    ...
    fn bench_fp4_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(iron_fp4_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Fp4, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    ...
    fn bench_int8_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(iron_int8_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int8, 4096, 4096, dt)
    }
    // Symmetric sub-byte ints (FP32 group scale) + MXINT (E8M0 block scale).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int2_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(iron_int2_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int2, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int3_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(iron_int3_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int3, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int4_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(iron_int4_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int4, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int5_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(iron_int5_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int5, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_int6_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(iron_int6_rms_norm_qgemv::kernel_ir_for(dt), QFormat::Int6, 4096, 4096, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_mxint2_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_mxint2_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxint2,
            4096,
            4096,
            dt,
    ...
    fn bench_mxint3_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_mxint3_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxint3,
            4096,
            4096,
            dt,
    ...
    fn bench_mxint4_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_mxint4_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxint4,
            4096,
            4096,
            dt,
    ...
    fn bench_mxint5_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_mxint5_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxint5,
            4096,
            4096,
            dt,
    ...
    fn bench_mxint6_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_mxint6_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxint6,
            4096,
            4096,
            dt,
    ...
    fn bench_mxint8_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_mxint8_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Mxint8,
            4096,
            4096,
            dt,
    ...
    fn bench_fp4_f16_rms_norm_qgemv(dt: DType) -> BenchSetup {
        rms_qgemv_bench(
            iron_fp4_f16_rms_norm_qgemv::kernel_ir_for(dt),
            QFormat::Fp4F16,
            4096,
            4096,
            dt,
