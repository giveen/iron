...
//! Two variants:
//!
//! **`iron_rms_norm_qgemv`** — one output row per TG (the original port).
...
//! TPG ≥ 32.
//!
//! **`iron_rms_norm_qgemv_fast`** — 8 output rows per TG, mirroring
//! `iron_qmv`'s geometry. Phase 1 (SSQ → `inv_rms`) is shared across all
//! 8 rows — the TG-wide reduce amortizes the RMSNorm over 8 outputs.
//! Phase 2 uses the `iron_qmv` mask-without-shift trick (X pre-scaled by
...
pub fn iron_rms_norm_qgemv<T>(
    ...
    // call to iron_rms_inv_scalar for the threadgroup reduce + rsqrt.
...
/// Phase 1 (SSQ for RMSNorm) is shared — `iron_rms_inv_scalar` performs a
/// TG-wide reduce, so the same `inv_rms` is broadcast to all 8 rows.
/// Phase 2 reuses `iron_qmv`'s two-pass algebraic split:
...
pub fn iron_rms_norm_qgemv_fast<T>(
    ...
    // All 64 threads cooperate — `iron_rms_inv_scalar` performs the full
    ...
    let inv_rms = iron_rms_inv_scalar(ssq, eps_buf, in_dim);
    // Phase 2: 4-row int4 GEMV per simdgroup, mirroring `iron_qmv`.
    ...
    // Mirrors `iron_qmv` and MLX `qdot` (quantized.h:235-244).
    let s_16 = 0.0625f32;
...
// ─── iron_rms_norm_qgemv_int8_fast ───────────────────────────────────────────
//
// Fused RMSNorm + int8-quantized GEMV — 8-row-per-TG perf variant.
//
// Mirrors `iron_rms_norm_qgemv_fast` (int4, 8-row-per-TG, 2 SG × 32 lanes)
...
//
// Phase 1 (TG-wide SSQ → inv_rms via `iron_rms_inv_scalar`) is identical to
...
///
/// int8 variant of `iron_rms_norm_qgemv_fast`. Byte-extract (4 vals/pack),
/// algebraic-split accumulator. Grid: `[out_dim/8, 1, 1]`.
#[kernel]
pub fn iron_rms_norm_qgemv_int8_fast<T>(
    x: Tensor<T>,
    norm_weight: Tensor<T>,
    weight: Tensor<u32>,
    scales: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::{iron_rms_norm_qgemv, iron_rms_norm_qgemv_fast, iron_rms_norm_qgemv_int8_fast};
    ...
    /// the byte-strided layout `iron_rms_norm_qgemv_int8_fast` decodes.
    ...
    fn test_rms_norm_qgemv(dt: DType) -> TestSetup {
        ...
        setup(iron_rms_norm_qgemv::kernel_ir_for(dt), dt, in_dim, gs, out_dim, false).grid_3d(
            out_dim as u32,
            1,
            1,
            [128, 1, 1],
    ...
    fn test_rms_norm_qgemv_fast(dt: DType) -> TestSetup {
        ...
        setup(iron_rms_norm_qgemv_fast::kernel_ir_for(dt), dt, in_dim, gs, out_dim, false).grid_3d(
            (out_dim / 8) as u32,
            1,
            1,
            [64, 1, 1],
    ...
    fn test_rms_norm_qgemv_int8_fast(dt: DType) -> TestSetup {
        let (in_dim, gs, out_dim) = (512usize, 64usize, 16usize);
        setup(iron_rms_norm_qgemv_int8_fast::kernel_ir_for(dt), dt, in_dim, gs, out_dim, true)
...
pub mod kernel_benches {
    ...
    fn bench_rms_norm_qgemv(dt: DType) -> BenchSetup {
        ...
        let s = BenchSetup::new(iron_rms_norm_qgemv::kernel_ir_for(dt)).mode(KernelMode::Reduction);
    ...
    fn bench_rms_norm_qgemv_fast(dt: DType) -> BenchSetup {
        let (in_dim, gs, out_dim) = (4096usize, 64usize, 4096usize);
        let s = BenchSetup::new(iron_rms_norm_qgemv_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction);
    ...
    fn bench_rms_norm_qgemv_int8_fast(dt: DType) -> BenchSetup {
        let (in_dim, gs, out_dim) = (4096usize, 64usize, 4096usize);
        let s = BenchSetup::new(iron_rms_norm_qgemv_int8_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction);
