...
//! recurrence (`iron_gated_delta_step` / `_chunk`) produces the linear-
...
//! ```
//!
//! The distinguishing feature versus the plain `iron_rms_norm` is the
...
//! `T` at the store.
//!
//! Algorithm-identical reduction to `mlx/rms_norm.rs`'s `iron_rms_norm`
...
pub fn iron_gated_rmsnorm<T>(
    ...
    if in_bounds {
        ...
        // iron_swiglu) to keep the gate precise before the round to T.
...
/// New-syntax correctness for `iron_gated_rmsnorm` (Reduction mode, one
...
pub mod kernel_tests {
    ...
    fn setup(rows: usize, n: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_gated_rmsnorm::kernel_ir_for(dt))
    ...
    fn test_iron_gated_rmsnorm(dt: DType) -> TestSetup { setup(4, 512, dt) }
}

/// New-syntax benchmark for `iron_gated_rmsnorm` (fused GDN post-step, fp32 `y`,
...
pub mod kernel_benches {
    ...
    fn bench_gated_rmsnorm(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gated_rmsnorm::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
