...
//! [`iron_rms_norm_small`] instead, which uses a 2-elements-per-thread
...
//! case — see Iron post-mortem 2026-05-19). Consumers MUST encode
...
/// let inv_rms = iron_rms_inv_scalar(partial_ssq, eps_buf, n);
/// ```
///
...
///
/// `iron_rms_inv_scalar` is a **valid standalone kernel**: `partial_ssq` is a
...
///
/// When called via the cross-kernel DSL (`let inv = iron_rms_inv_scalar(g, ...)`)
...
/// [`iron_rms_norm`]. Where `iron_rms_norm` fixes `N = TPG * 4` (so a
...
/// held in registers across the reduction the way `iron_rms_norm` does
...
///   stride walks past `n` simply stop. Unlike `iron_rms_norm` there is
///   no 128-alignment or `n ≤ 4096` requirement.
#[kernel]
pub fn iron_rms_norm_wide<T>(
    x: Tensor<T>,
    w: Tensor<T>,
    out: Tensor<T>,
    eps_buf: Tensor<f32>,
...
/// across `[Hv, Dv]` — one row per threadgroup. Used by the Iron
...
///
/// Same `N = TPG * 4` invariant as `iron_rms_norm` — Dv is multiple of
/// 4 on every shipped Qwen3 hybrid (128 / 256 / 512). One thread owns
/// 4 consecutive `Dv`-axis elements; the OOB clamp + mask copies the
/// `iron_rms_norm` template so a wrong-TPG dispatch fails loudly rather
/// than silently miscomputing.
#[kernel]
pub fn iron_gated_mixer_norm<T>(
    y: Tensor<f32>,
    z: Tensor<T>,
    w: Tensor<T>,
    out: Tensor<T>,
    ...
    let safe_base = rs + safe_col;
    let base = rs + col;
    // y is already fp32, but mirror the iron_rms_norm load pattern
    ...
    // in wh-iron codegen `vectorize.rs`).
    ...
    if in_bounds {
        ...
        // silu(z) = z / (1 + exp(-z)). Inlined per the `iron_sigmoid`
        // precedent — Activation::Sigmoid folds into FusedElementwise
        // and the per-kernel feature analyzer would miss it, so the
        // emitted MSL stays self-contained without an `iron_sigmoid`
        // helper. Same as `iron_gated_delta_prep_step`'s `beta` path.
...
mod wide_tests {
    ...
    use super::iron_rms_norm_wide;

    fn msl_for(dt: DType) -> String {
        let mut k = iron_rms_norm_wide::kernel_ir_for(dt);
        k.mode = KernelMode::Reduction;
        MslGenerator::default().generate(&k).expect("iron_rms_norm_wide codegen succeeds")
    }
    ...
    fn codegen_produces_nonempty_msl_for_all_float_dtypes() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            ...
            assert!(
                src.contains("kernel void iron_rms_norm_wide"),
                "MSL for {dt:?} should declare iron_rms_norm_wide:\n{src}",
            );
        }
    }
}

/// New-syntax correctness for `iron_rms_norm` (Reduction mode, one threadgroup
...
pub mod kernel_tests {
    ...
    use super::{iron_gated_mixer_norm, iron_rms_norm, iron_rms_norm_wide};
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 2e-2, 1e-1])]
    fn test_iron_rms_norm_npot_tpg(dt: DType) -> TestSetup { setup(3, 1536, dt) }

    // Non-128-aligned wide-row coverage (SmolVLM2 d=960; 960 = 7·128 +
    // 64 is NOT a multiple of 128, so `iron_rms_norm` can't dispatch it).
    // `iron_rms_norm_wide` strides over the row with a fixed TPG=1024 and
    ...
    fn setup_wide(rows: usize, n: usize, dt: DType) -> TestSetup {
        ...
        let w_dt = unpack_f32(&pack_f32(&w, dt), dt);
        let expected = expected_rms(&x, &w_dt, rows, n, eps, dt);
        TestSetup::new(iron_rms_norm_wide::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("x", pack_f32(&x, dt), dt))
            .input(TestBuffer::from_vec("w", pack_f32(&w, dt), dt))
            .input(TestBuffer::zeros("out", rows * n, dt))
    ...
    fn test_iron_rms_norm_wide_d960(dt: DType) -> TestSetup { setup_wide(3, 960, dt) }
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-4, 2e-2, 1e-1])]
    fn test_iron_rms_norm_wide_multi_iter(dt: DType) -> TestSetup { setup_wide(1, 5376, dt) }

    // ── iron_gated_mixer_norm: out = (y · rms(y) · w) · silu(z) ──────────────
    //
    ...
    fn setup_mixer(rows: usize, n: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_gated_mixer_norm::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn test_iron_gated_mixer_norm(dt: DType) -> TestSetup { setup_mixer(8, 128, dt) }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 5e-3, 2e-2])]
    fn test_iron_gated_mixer_norm_dv256(dt: DType) -> TestSetup { setup_mixer(4, 256, dt) }
}

/// New-syntax benchmark for `iron_rms_norm` (vs MLX `metal/rms_norm.metal`).
pub mod kernel_benches {
    use wh_iron::{bench, test::*};

    use super::{iron_gated_mixer_norm, iron_rms_norm, iron_rms_norm_small, iron_rms_norm_wide};
    ...
    // `w_stride`(uint)[[buffer(5)]]. x/w/eps_buf are shared by name with the Iron
    // inputs; w_stride=1 (contiguous per-channel weight, the legacy `U32V(1)`).
    // `eps` reuses the Iron `eps_buf` (1e-5, F32) so both sides match.
    //
    ...
    fn rms_ref(rows: usize, n: usize, dt: DType) -> RefKernel {
        ...
        // x/w/eps_buf shared by name with the Iron inputs (placeholders).
    ...
    fn bench_rms_norm(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_rms_norm::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    // the parent bench; only the Iron-side per-thread layout differs.
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_rms_norm_small(dt: DType) -> BenchSetup {
        let (rows, n) = (1024usize, 64usize);
        BenchSetup::new(iron_rms_norm_small::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(input_buffer("x", rows * n, dt, InputDomain::Signed))
            .buffer(input_buffer("w", n, dt, InputDomain::Positive))
            .buffer(BenchBuffer::zeros("out", rows * n, dt).output())
    ...
    // row. Handles rows wider than the 4096 cap of iron_rms_norm — use a
    ...
    fn bench_rms_norm_wide(dt: DType) -> BenchSetup {
        ...
        const TPG: u32 = 1024;
        BenchSetup::new(iron_rms_norm_wide::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn bench_gated_mixer_norm(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gated_mixer_norm::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
