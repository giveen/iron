...
//!
//! Uses `iron_rms_inv_scalar` (from `mlx/rms_norm.rs`) via cross-kernel
//! call for the shared reduction phase: each thread computes its
//! `partial_ssq`, then calls `iron_rms_inv_scalar(partial_ssq, eps_buf, n)`
...
#[kernel]
pub fn iron_rms_norm_residual<T>(
    x: Tensor<T>,
    residual: Tensor<T>,
    w: Tensor<T>,
    out: Tensor<T>,
    ...
    // and skip their stores, mirroring `iron_rms_norm`'s freeze-safe guard.
    let col = tid * 4u32;
    ...
    // Cross-kernel call: KernelInlinePass splices iron_rms_inv_scalar's body
    ...
    let rms = iron_rms_inv_scalar(partial_ssq, eps_buf, n);
    if in_bounds {
...
/// New-syntax correctness for `iron_rms_norm_residual` (Reduction mode, one
...
pub mod kernel_tests {
    ...
    use super::iron_rms_norm_residual;
    ...
    fn setup(rows: usize, n: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_rms_norm_residual::kernel_ir_for(dt))
    ...
    fn test_iron_rms_norm_residual(dt: DType) -> TestSetup { setup(4, 512, dt) }
}

/// New-syntax benchmark for `iron_rms_norm_residual` (fused RMSNorm + residual
...
pub mod kernel_benches {
    ...
    fn bench_rms_norm_residual(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_rms_norm_residual::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
