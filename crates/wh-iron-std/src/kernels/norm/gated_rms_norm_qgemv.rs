...
//!
//!   1. `iron_gated_rmsnorm`: per-row RMSNorm with SiLU gate.
...
//!
//!   2. `iron_dequant_gemv_int4` (the GDN out projection):
...
//!
//! ```text
//!   inner = iron_gated_rmsnorm(y, z, w, eps)        // [Hv, Dv]
//!   out   = iron_dequant_gemv_int4(inner, Wq, S, B)  // [out_dim]
//! ```
//!
...
pub fn iron_gated_rms_norm_qgemv_int4_fast<T>(
    y: Tensor<f32>,
    z: Tensor<T>,
    ...
    for r_it in range(0u32, row_iters, 1u32) {
        ...
        for k in range(0u32, dv_per_lane, 1u32) {
            ...
            // `iron_gated_rmsnorm` / `moe_down_swiglu_accum`.
    ...
    //
    // Mirrors `iron_rms_norm_qgemv_fast` Phase 2 verbatim, except the
    ...
    // `_m{16,32}` notes in `iron/moe.rs` for the same constraint).
    ...
    for _b in range(0u32, in_dim, 512u32) {
        ...
        // and the `stack_alloc + range` pattern.
        for _r in range(0u32, 4u32, 1u32) {
            let row = base_row + _r;
...
pub mod kernel_tests {
    ...
    use super::{
        iron_gated_rms_norm_qgemv_int4_fast,
    ...
    fn setup(hv: usize, dv: usize, out_dim: usize, group_size: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_gated_rms_norm_qgemv_int4_fast::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_gated_rms_norm_qgemv_int4_fast` at the
...
pub mod kernel_benches {
    ...
    use super::iron_gated_rms_norm_qgemv_int4_fast;
    ...
    fn bench_gated_rms_norm_qgemv_int4_fast(dt: DType) -> BenchSetup {
        ...
        let u32_per_row = in_dim / 8;
        let n_groups = in_dim / group_size;
        BenchSetup::new(iron_gated_rms_norm_qgemv_int4_fast::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
