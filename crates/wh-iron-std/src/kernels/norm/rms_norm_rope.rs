...
//!
//! Phase 1 — `inv_rms = rsqrt(mean(x²) + eps)` via `iron_rms_inv_scalar`
...
pub fn iron_rms_norm_rope<T>(
    ...
    // names are substituted into iron_rms_inv_scalar's callee loads.
    let v1 = load(x[rs + lid]).cast::<f32>();
    let v2 = load(x[rs + lid + half]).cast::<f32>();
    let partial_ssq = v1 * v1 + v2 * v2;
    let inv_rms = iron_rms_inv_scalar(partial_ssq, eps_buf, axis_size);
...
/// New-syntax correctness for `iron_rms_norm_rope` (Reduction mode, one
...
pub mod kernel_tests {
    ...
    fn test_iron_rms_norm_rope(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_rms_norm_rope::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_rms_norm_rope` (fused RMSNorm + RoPE, one
...
pub mod kernel_benches {
    ...
    fn bench_rms_norm_rope(dt: DType) -> BenchSetup {
        ...
        let inv_freqs: Vec<f32> =
            (0..half).map(|i| 1.0 / 10000.0_f32.powf(i as f32 / half as f32)).collect();
        BenchSetup::new(iron_rms_norm_rope::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
