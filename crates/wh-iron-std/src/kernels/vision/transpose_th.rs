...
//! `iron_rope_2d` — emits Q/K/V in **token-major** layout
//! `[n_tokens, n_heads, head_dim]` (one contiguous head block per token).
//! But `iron_sdpa_bidirectional` reads K/V **head-major**
...
/// New-syntax correctness for `iron_transpose_th`. Grid3D, grid
...
pub mod kernel_tests {
    ...
    fn test_transpose_th(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_transpose_th::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_transpose_th` at the SigLIP production
...
pub mod kernel_benches {
    ...
    fn bench_transpose_th(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_transpose_th::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
