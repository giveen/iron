...
#[kernel]
pub fn iron_resize_normalize<T>(
    input: Tensor<T>,
    mean: Tensor<f32>,
    std: Tensor<f32>,
    out: Tensor<T>,
...
#[kernel]
pub fn iron_resize_normalize_bicubic<T>(
    input: Tensor<T>,
    mean: Tensor<f32>,
    std: Tensor<f32>,
    out: Tensor<T>,
...
/// New-syntax correctness for `iron_resize_normalize` vs a CPU bilinear
/// reference. Grid3D, grid `[target_w, target_h, 3]`, tpg `[1,1,1]`.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::{iron_resize_normalize, iron_resize_normalize_bicubic};
    ...
    fn test_resize_normalize(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_resize_normalize::kernel_ir_for(dt))
    ...
    fn test_resize_normalize_bicubic(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_resize_normalize_bicubic::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("input", pack_f32(&src_f, dt), dt))
            .input(TestBuffer::from_vec(
                "mean",
...
/// New-syntax benchmark for `iron_resize_normalize` at a representative VL
...
pub mod kernel_benches {
    ...
    fn bench_resize_normalize(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_resize_normalize::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_resize_normalize_bicubic(dt: DType) -> BenchSetup {
        let (sw, sh, tw, th) = (640usize, 480usize, 448usize, 448usize);
        BenchSetup::new(iron_resize_normalize_bicubic::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .buffer(BenchBuffer::random("input", sh * sw * 3, dt))
            .buffer(BenchBuffer::random("mean", 3, DType::F32))
            .buffer(BenchBuffer::random("std", 3, DType::F32))
