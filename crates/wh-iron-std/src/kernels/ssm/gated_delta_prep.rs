...
//!
//! `iron_gated_delta_prep_step` extends the recurrence-only
//! [`iron_gated_delta_step`](super::gated_delta::iron_gated_delta_step) by
...
//!   - `y`            : Tensor<T> [B, Hv, Dv]
//!
//! ## DISPATCH INVARIANTS (identical to `iron_gated_delta_step`)
//!
...
#[kernel]
pub fn iron_gated_delta_prep_step<T>(
    ...
    // Mirrors `iron_gated_delta_step` exactly.
    ...
    // simply go unread. Same convention as `iron_gated_delta_step`.
    ...
    //                  emitted MSL self-contained — no `iron_sigmoid` helper
    //                  required.
    ...
    //
    // Same shape as `iron_gated_delta_step::phase_1` but reads q/k from the
...
mod tests {
    ...
    /// `cargo test -p wh-iron-std --lib --release -- kernels::ssm::gated_delta_prep::tests::dump --nocapture`
    #[test]
    fn dump() {
        use wh_iron::codegen::msl::MslGenerator;
        let mut k = iron_gated_delta_prep_step::kernel_ir_for(DType::F32);
...
/// (`iron_gated_delta_prep_step`). Oracle is the legacy
...
pub mod kernel_tests {
    ...
    use super::iron_gated_delta_prep_step;
    ...
    fn setup(
        ...
        TestSetup::new(iron_gated_delta_prep_step::kernel_ir_for(dt))
    ...
    fn test_iron_gated_delta_prep_step_gqa(dt: DType) -> TestSetup {
        setup(2, 4, 2, 8, 64, 0.7, dt)
    }

    // Hv == Hk (no key-sharing) at the minimum dk=32, single batch.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_iron_gated_delta_prep_step_no_gqa(dt: DType) -> TestSetup {
...
pub mod kernel_benches {
    ...
    // `iron_gated_delta_step`. conv_out is the fused q|k|v slab of width
    // `2·Hk·Dk + Hv·Dv`.
    ...
    fn bench_gated_delta_prep_step(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gated_delta_prep_step::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
