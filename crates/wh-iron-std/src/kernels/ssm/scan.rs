...
//! `iron_ssm_step_a2d` — the Mamba 1 (Jamba) variant carrying a 2-D
//! per-(channel, state) `A_log` instead of the scalar-per-head `A`.
//!
//! `iron_ssm_step_grouped` is a faithful port of MLX's `iron_ssm_step<T, Dh, Ds, H, G>`
//! from ekryski's `mlx` fork (`alpha` branch) — semantically MLX-aligned
//! but mainline MLX (pinned by `wh-iron-std/build.rs`) doesn't ship
...
//! file (or just `iron_ssm_step_grouped` alone) graduates to `mlx/ssm.rs` and
...
//!
//! Codegen-only. Correctness validated end-to-end in Iron integration
...
// 2D-`A_log` variant of `iron_ssm_step` above.
//
// The scalar `iron_ssm_step` bakes in a per-channel scalar `A` (`a[h_id]`),
...
// Mamba `A = -exp(A_log)` reparam (matching `iron_ssm_step_grouped`). Per state
// element `(h, d, n)`:
//
...
//
// One thread per `(head, d)` — same Grid3D geometry as `iron_ssm_step`; no
...
// Faithful port of MLX's `iron_ssm_step<T, Dh, Ds, H, G>` (alpha branch). One
...
#[kernel]
pub fn iron_ssm_step_grouped<T>(
...
#[kernel]
pub fn iron_gated_group_rmsnorm<T>(
    y: Tensor<f32>,
    z: Tensor<T>,
    w: Tensor<T>,
    mut out: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::{iron_ssm_step, iron_ssm_step_a2d, iron_ssm_step_grouped};
    use crate::utils::pack_f32;

    // ── SSD portable-scan kernels: cross-backend codegen smoke ────────────
    // The Mamba2 SSD chunked-matmul prefill scan (iron-ops
    // `ssm_prefill_scan_ssd_portable`) is built from these `iron_ssd_*` #[kernel]
    // ops + `iron_gemm_batched`. The whole point is PORTABILITY — they must
    ...
    #[test]
    fn iron_ssd_portable_kernels_codegen_all_backends() {
        use wh_iron::{
            codegen::{
                CudaGenerator,
                GlslGenerator,
        ...
        let kernels: Vec<(&str, wh_iron::core::Kernel)> = vec![
            ("iron_gemm_batched", {
                let mut k =
                    crate::kernels::gemm::dense::iron_gemm_batched::kernel_ir_for(DType::F32);
                k.mode = KernelMode::Reduction;
                k
            }),
            ("iron_ssd_lcs", {
                let mut k = super::iron_ssd_lcs::kernel_ir_for();
                k.mode = KernelMode::Grid3D;
                k
            }),
            ("iron_ssd_gather_bc", {
                let mut k = super::iron_ssd_gather_bc::kernel_ir_for();
                k.mode = KernelMode::Grid3D;
                k
            }),
            ("iron_ssd_xt", {
                let mut k = super::iron_ssd_xt::kernel_ir_for();
                k.mode = KernelMode::Grid3D;
                k
            }),
            ("iron_ssd_mmask", {
                let mut k = super::iron_ssd_mmask::kernel_ir_for();
                k.mode = KernelMode::Grid3D;
                k
            }),
            ("iron_ssd_bdt", {
                let mut k = super::iron_ssd_bdt::kernel_ir_for();
                k.mode = KernelMode::Grid3D;
                k
            }),
            ("iron_ssd_recur", {
                let mut k = super::iron_ssd_recur::kernel_ir_for();
                k.mode = KernelMode::Grid3D;
                k
            }),
            ("iron_ssd_combine", {
                let mut k = super::iron_ssd_combine::kernel_ir_for();
                k.mode = KernelMode::Grid3D;
                k
            }),
            ("iron_ssd_g1_cb", {
                let mut k = super::iron_ssd_g1_cb::kernel_ir_for();
                k.mode = KernelMode::Reduction;
                k
            }),
            ("iron_ssd_g4_cs", {
                let mut k = super::iron_ssd_g4_cs::kernel_ir_for();
                k.mode = KernelMode::Reduction;
                k
            }),
        ];
    ...
    fn ssm_step_setup(n_heads: usize, head_dim: usize, state_dim: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_ssm_step::kernel_ir_for(dt))
    ...
    // ── iron_ssm_step_a2d (Mamba 1 / Jamba: 2-D per-(channel,state) A_log) ────
    ...
    fn ssm_step_a2d_setup(
        ...
        TestSetup::new(iron_ssm_step_a2d::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("x", pack_f32(&x, dt), dt))
    ...
    // ── iron_ssm_step_grouped (MLX-aligned iron_ssm_step<T,Dh,Ds,H,G>) ─────────────────
    //
    // Distinct from `iron_ssm_step`: separate `state_in`/`state_out` buffers (not
    ...
    /// CPU oracle mirroring `iron_ssm_step_grouped` exactly (batch folded into `n`).
    #[allow(clippy::too_many_arguments)]
    fn iron_ssm_step_oracle(
        x: &[f32],
        a_log: &[f32],
        b_mat: &[f32],
        c_mat: &[f32],
    ...
    fn iron_ssm_step_setup(
        ...
        let (out_exp, state_exp) = iron_ssm_step_oracle(
            &x,
            &a_log,
            &b_mat,
            &c_mat,
        ...
        TestSetup::new(iron_ssm_step_grouped::kernel_ir_for(dt))
    ...
    fn test_iron_ssm_step(dt: DType) -> TestSetup { iron_ssm_step_setup(4, 16, 32, 4, 2, dt) }

    // ── iron_strided_col_copy ──────────────────────────────────────────────────
    ...
    fn strided_col_copy_setup(s: usize, stride: usize, col_off: usize, width: usize) -> TestSetup {
        ...
        use super::iron_strided_col_copy;
        TestSetup::new(iron_strided_col_copy::kernel_ir_for())
            .mode(KernelMode::Grid3D)
    ...
    fn softplus_add_rows_setup(s: usize, n: usize) -> TestSetup {
        ...
        use super::iron_softplus_add_rows;
        TestSetup::new(iron_softplus_add_rows::kernel_ir_for())
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("src", pack_f32(&src, dt), dt))
...
/// New-syntax benchmarks for the selective-scan step kernels. `iron_ssm_step` is
/// also correctness-tested above; `iron_ssm_step_a2d` (2-D
/// per-(channel,state) A_log) and `iron_ssm_step_grouped` (MLX-aligned reduction form)
...
pub mod kernel_benches {
    ...
    fn bench_ssm_step(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_ssm_step::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_ssm_step_a2d(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_ssm_step_a2d::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_iron_ssm_step(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_ssm_step_grouped::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
...
//
// Replaces the 3 sequential `iron_strided_col_copy` calls that carve z, xbc, and
...
//
// Replaces the 3 sequential `iron_strided_col_copy` calls that carve x_ssm, b,
...
#[kernel]
pub fn iron_gated_group_rmsnorm_batched(
...
// helper kernels in `iron-ops/src/iron_ssd_scan.rs`. They prepare the operands for
// the 4 batched GEMMs (run via `iron_gemm_batched`) and combine the result.
...
//
// Same 32×32 / 16-K Reduction-mode tiling as `iron_gemm_batched`; the batch
...
// → writes M [nc*H, L, L] directly. Fuses the separate `iron_ssd_mmask` [L,L]
...
// Reads B,C directly from [T, G, ds]; equivalent to iron_gemm_batched on
// (b_g, c_g) but without ever materializing them or CB.
#[kernel]
pub fn iron_ssd_g1_cb(
...
// review): `iron_mamba_split_proj`/`iron_mamba_split_conv` desugar their
...
mod if_conversion_regression {
    ...
    use super::{iron_mamba_split_conv, iron_mamba_split_proj};

    fn count_ifs_after_passes(mut k: wh_iron::core::Kernel) -> usize {
        wh_iron::codegen::passes::run_passes_with_stats(
            &mut k,
            &wh_iron::codegen::passes::standard_pipeline(),
        )
    ...
    fn split_proj_keeps_both_nested_ifs() {
        assert_eq!(count_ifs_after_passes(iron_mamba_split_proj::kernel_ir_for()), 2);
    }

    #[test]
    fn split_conv_keeps_both_nested_ifs() {
        assert_eq!(count_ifs_after_passes(iron_mamba_split_conv::kernel_ir_for()), 2);
    }
}
