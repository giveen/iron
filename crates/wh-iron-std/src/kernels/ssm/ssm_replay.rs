...
//! Mamba / Mamba 2 state replay — port of `iron_ssm_replay.metal`
//! (spec 040). The speculative-decode rollback companion to
//! `ssm.rs`'s `ssm_step`. Two kernels:
//!
//!   - `iron_ssm_step_record` — the sequential SSD forward over `t_total`
//!     steps, capturing each step's `(dA, dBx)` into delta logs
//!     alongside the standard `(y, state_out)`.
//!   - `iron_ssm_replay` — re-folds the first `k` log entries onto a
...
/// verbatim from the legacy `tests/iron_ssm_replay_gpu_correctness.rs`
/// (since removed):
///
...
pub mod kernel_tests {
    ...
    use super::{iron_ssm_replay_d16_64_4, iron_ssm_step_record_d16_64_4_2};
    ...
    fn record_setup(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_ssm_step_record_d16_64_4_2::kernel_ir_for(dt))
    ...
    fn replay_setup(dt: DType, mask: Vec<u32>, has_mask: bool) -> TestSetup {
        ...
        TestSetup::new(iron_ssm_replay_d16_64_4::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
...
pub mod kernel_benches {
    ...
    use super::{
        iron_ssm_replay_d16_64_4,
        iron_ssm_replay_d128_128_32,
        iron_ssm_step_record_d16_64_4_2,
        iron_ssm_step_record_d128_128_32_2,
    };
    ...
    fn bench_ssm_record(dt: DType) -> BenchSetup {
        ...
        let n_total = batch * H;
        BenchSetup::new(iron_ssm_step_record_d16_64_4_2::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_ssm_replay(dt: DType) -> BenchSetup {
        ...
        let n_total = batch * H;
        BenchSetup::new(iron_ssm_replay_d16_64_4::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_ssm_record_d128_128_32(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_ssm_step_record_d128_128_32_2::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_ssm_replay_d128_128_32(dt: DType) -> BenchSetup {
        let (batch, t, k_steps) = (1usize, 8usize, 8usize);
        let n_total = batch * P_H;
        BenchSetup::new(iron_ssm_replay_d128_128_32::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
