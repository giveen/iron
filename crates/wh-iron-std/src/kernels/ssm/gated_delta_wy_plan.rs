...
//! Chunked-WY GDN prefill — **plan** kernel (pass 1 of 2) — `iron_gdn_wy_plan`.
//!
//! Two-kernel split of the dormant [`iron_gated_delta_wy_chunk`](super::gated_delta_wy)
...
//! sibling [`iron_gdn_wy_scan`](super::gated_delta_wy_scan) (`_scan`) fix that
...
//! [`iron_gdn_wy_scan`](super::gated_delta_wy_scan) for the derivation check
...
//!     Iron computes `g`/`beta` upstream (the qknorm/prep pass) before this
//!     pipeline runs.
//!   - `t_len`: `[1]` u32 — runtime chunk length. **`t_len % c == 0`
//!     required for v1** (Iron pads a short prefill tail up to a multiple of
...
/// (`iron_gdn_wy_plan`). Oracle mirrors
...
pub mod kernel_tests {
    ...
    fn setup_with_gates(
        ...
        TestSetup::new(iron_gdn_wy_plan::kernel_ir_for(dt))
    ...
    fn test_iron_gdn_wy_plan_c64_dk32(dt: DType) -> TestSetup { setup(64, 1, 1, 32, 32, 64, dt) }
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [3e-3, 5e-3, 1e-2])]
    fn test_iron_gdn_wy_plan_multi_chunk_gqa(dt: DType) -> TestSetup {
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [3e-3, 5e-3, 1e-2])]
    fn test_iron_gdn_wy_plan_harsh_gates(dt: DType) -> TestSetup {
...
pub mod kernel_benches {
    ...
    fn bench_gdn_wy_plan_t2048(dt: DType) -> BenchSetup {
        ...
        let n_total = b * hv;
        let nc = t / c;
        BenchSetup::new(iron_gdn_wy_plan::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
