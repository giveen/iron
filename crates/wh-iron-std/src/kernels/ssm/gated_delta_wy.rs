...
//! Chunked-WY Gated DeltaNet prefill kernel — `iron_gated_delta_wy_chunk`.
//!
...
//! ~480 simdgroup slots).
//!
//! ## Layouts (match `iron_gated_delta_chunk`)
//!
//!   - `q, k`:    [B, T, Hk, Dk]
//!   - `v, y`:    [B, T, Hv, Dv]
//!   - `g, beta`: [B, T, Hv]
...
//! ## Numerical precision
//!
//! Matches `iron_gated_delta_chunk`: accumulators in f32, state in f32 too.
...
#[kernel]
pub fn iron_gated_delta_wy_chunk<T>(
    q: Tensor<T>,
    k: Tensor<T>,
    v: Tensor<T>,
    g: Tensor<T>,
...
/// (`iron_gated_delta_wy_chunk`). Oracle is the plain `sequential_gdn` per-token
...
pub mod kernel_tests {
    ...
    use super::iron_gated_delta_wy_chunk;
    ...
    fn setup(
        ...
        TestSetup::new(iron_gated_delta_wy_chunk::kernel_ir_for(dt))
    ...
    fn test_iron_gated_delta_wy_chunk_one_chunk(dt: DType) -> TestSetup {
    ...
    fn test_iron_gated_delta_wy_chunk_multi_chunk(dt: DType) -> TestSetup {
...
pub mod kernel_benches {
    ...
    fn bench_gated_delta_wy_chunk(dt: DType) -> BenchSetup {
        ...
        let n_total = b * hv;
        BenchSetup::new(iron_gated_delta_wy_chunk::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
