...
//!
//!   - `iron_gated_delta_step`  — single-token decode (`T = 1`)
//!   - `iron_gated_delta_chunk` — multi-token chunked prefill (`T > 1`); the
...
#[kernel]
pub fn iron_gated_delta_step<T>(
...
/// `iron_gated_delta_chunk` — multi-token GDN forward over `T` tokens.
///
/// Same recurrence math as `iron_gated_delta_step`, wrapped in an inner
...
///   scheduler picks.
#[kernel]
pub fn iron_gated_delta_chunk<T>(
...
pub mod kernel_tests {
    ...
    use super::{iron_gated_delta_chunk, iron_gated_delta_step};
    ...
    fn setup(b: usize, hv: usize, hk: usize, dv: usize, dk: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_gated_delta_step::kernel_ir_for(dt))
    ...
    fn test_iron_gated_delta_step_gqa(dt: DType) -> TestSetup { setup(2, 4, 2, 8, 64, dt) }

    // Hv == Hk (no key-sharing) at the minimum dk=32, single batch.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-5, 5e-2, 2e-1])]
    fn test_iron_gated_delta_step_no_gqa(dt: DType) -> TestSetup { setup(1, 4, 4, 4, 32, dt) }

    // ── iron_gated_delta_chunk: multi-token prefill recurrence ───────────────
    //
    // Same per-step recurrence as `iron_gated_delta_step`, but the kernel threads
    ...
    fn chunk_setup(
        ...
        TestSetup::new(iron_gated_delta_chunk::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("q", pack_f32(&q, dt), dt))
            .input(TestBuffer::from_vec("k", pack_f32(&k, dt), dt))
            .input(TestBuffer::from_vec("v", pack_f32(&v, dt), dt))
    ...
    fn test_iron_gated_delta_chunk(dt: DType) -> TestSetup { chunk_setup(1, 16, 4, 2, 8, 64, dt) }
}

/// New-syntax benchmarks for the GDN decode + chunked-prefill kernels.
/// `iron_gated_delta_step` (decode) and `iron_gated_delta_chunk` (multi-token
...
pub mod kernel_benches {
    ...
    fn bench_gated_delta_step(dt: DType) -> BenchSetup {
        ...
        let n_total = b * hv;
        BenchSetup::new(iron_gated_delta_step::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn bench_gated_delta_chunk(dt: DType) -> BenchSetup {
        ...
        let n_total = b * hv;
        BenchSetup::new(iron_gated_delta_chunk::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
