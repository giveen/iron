...
//! Lands in `iron/` so the bare-`#[kernel]` form emits cleanly into
//! `IronKernels.swift` for Iron callers.

use wh_iron::kernel;

#[kernel]
pub fn iron_vector_add<T>(a: Tensor<T>, b: Tensor<T>, mut out: Tensor<T>) {
    let i = tid;
...
pub mod kernel_tests {
    ...
    fn setup(n: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_vector_add::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_add(dt: DType) -> BenchSetup {
        let n = 4096usize;
        BenchSetup::new(iron_vector_add::kernel_ir_for(dt))
