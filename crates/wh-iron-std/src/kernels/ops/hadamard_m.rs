...
//! The wh-iron-std version ships a **standalone** kernel for the pure M-element
//! Hadamard of any batch of M-vectors.
//!
//! Expressed in the `#[kernel]` DSL — no `Op::InlineMsl`. The three M values
//! get their own monomorphized DSL function (`iron_hadamard_m12`, `_m20`, `_m28`)
...
#[kernel]
pub fn iron_hadamard_m12<T>(inp: Tensor<T>, mut out: Tensor<T>, #[constexpr] scale: f32) {
...
#[kernel]
pub fn iron_hadamard_m20<T>(inp: Tensor<T>, mut out: Tensor<T>, #[constexpr] scale: f32) {
...
#[kernel]
pub fn iron_hadamard_m28<T>(inp: Tensor<T>, mut out: Tensor<T>, #[constexpr] scale: f32) {
...
/// Build the kernel IR for `iron_hadamard_m{M}` with M ∈ {12, 20, 28}.
/// Dispatches to the appropriate monomorphized DSL function.
pub fn kernel_ir_for(m: u32, dt: DType) -> Kernel {
    assert!(matches!(m, 12 | 20 | 28), "iron_hadamard_m only supports M ∈ {{12, 20, 28}}, got {m}");
    assert!(
        matches!(dt, DType::F32 | DType::F16 | DType::BF16),
        "iron_hadamard_m only supports F32/F16/BF16, got {dt:?}"
    );
    match m {
        12 => iron_hadamard_m12::kernel_ir_for(dt),
        20 => iron_hadamard_m20::kernel_ir_for(dt),
        28 => iron_hadamard_m28::kernel_ir_for(dt),
        _ => unreachable!(),
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_hadamard_m12(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(super::iron_hadamard_m12::kernel_ir_for(dt))
    ...
    fn bench_hadamard_m20(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(super::iron_hadamard_m20::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("inp", rows * m, dt))
            .buffer(BenchBuffer::zeros("out", rows * m, dt).output())
            .constexpr("scale", 1.0f32 / (m as f32).sqrt())
    ...
    fn bench_hadamard_m28(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(super::iron_hadamard_m28::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("inp", rows * m, dt))
            .buffer(BenchBuffer::zeros("out", rows * m, dt).output())
            .constexpr("scale", 1.0f32 / (m as f32).sqrt())
...
pub mod kernel_tests {
    ...
    fn test_hadamard_m12(dt: DType) -> TestSetup {
        ...
        TestSetup::new(super::iron_hadamard_m12::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn test_hadamard_m20(dt: DType) -> TestSetup {
        ...
        TestSetup::new(super::iron_hadamard_m20::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("inp", pack_f32(&data, dt), dt))
            .input(TestBuffer::zeros("out", rows * m, dt))
            .constexpr("scale", scale)
    ...
    fn test_hadamard_m28(dt: DType) -> TestSetup {
        ...
        TestSetup::new(super::iron_hadamard_m28::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("inp", pack_f32(&data, dt), dt))
            .input(TestBuffer::zeros("out", rows * m, dt))
            .constexpr("scale", scale)
...
mod tests {
    use wh_iron::{codegen::msl::MslGenerator, core::ir::Op};
    ...
    fn kernel_ir_constructs_for_all_m_and_dtypes() {
        for m in [12u32, 20, 28] {
            for dt in [DType::F32, DType::F16, DType::BF16] {
                ...
                assert_eq!(k.name, format!("iron_hadamard_m{m}"));
    ...
    fn codegen_emits_kernel_decl() {
        for m in [12u32, 20, 28] {
            for dt in [DType::F32, DType::F16, DType::BF16] {
                ...
                k.name = format!("iron_hadamard_m{m}_{suffix}");
                let msl = MslGenerator::default().generate(&k).expect("codegen");
                assert!(msl.contains(&format!("kernel void iron_hadamard_m{m}_{suffix}")));
