...
#[kernel]
pub fn iron_leaky_relu<T>(input: Tensor<T>, out: Tensor<T>, slope: Tensor<f32>) {
    let i = program_id::<0>();
    let s = load(slope[0]);
...
/// New-syntax correctness for `iron_leaky_relu`. Grid3D, grid `[n,1,1]`, tpg
/// `[1,1,1]`. Oracle applies the leaky rectifier per element.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_leaky_relu;
    ...
    fn test_leaky_relu(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_leaky_relu::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_leaky_relu(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_leaky_relu::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
