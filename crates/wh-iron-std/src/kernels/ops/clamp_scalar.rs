...
#[kernel]
pub fn iron_clamp_scalar<T>(input: Tensor<T>, out: Tensor<T>, lo: Tensor<f32>, hi: Tensor<f32>) {
    let i = program_id::<0>();
    let lo_v = load(lo[0]);
    let hi_v = load(hi[0]);
...
/// New-syntax correctness for `iron_clamp_scalar`. Grid3D, grid `[n,1,1]`,
/// tpg `[1,1,1]`. Oracle clamps each element to `[lo, hi]`.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_clamp_scalar;
    ...
    fn test_clamp_scalar(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_clamp_scalar::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_clamp_scalar`.
pub mod kernel_benches {
    ...
    fn bench_clamp_scalar(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_clamp_scalar::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
