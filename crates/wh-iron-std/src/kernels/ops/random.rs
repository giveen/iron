...
#[kernel]
pub fn iron_random_hash(out: Tensor<u32>, #[constexpr] n: u32) {
...
/// New-syntax correctness for `iron_random_hash` — a deterministic xorshift of
...
pub mod kernel_tests {
    ...
    use super::iron_random_hash;

    fn u32_bytes(v: &[u32]) -> Vec<u8> { v.iter().flat_map(|x| x.to_le_bytes()).collect() }

    #[test_kernel(dtypes = [f32], tol = 0.5)]
    fn test_iron_random_hash(_dt: DType) -> TestSetup {
        let n = 1024usize;
        let expected: Vec<u32> = (0..n)
            ...
            .collect();
        TestSetup::new(iron_random_hash::kernel_ir_for())
...
/// New-syntax benchmark for `iron_random_hash`.
///
/// Iron-only: there is **no A/B reference attached**. MLX's nearest
...
/// `iron_random_hash` is a per-element xorshift of `gid + 1` producing a `[n]`
...
/// is therefore left Iron-only (its xorshift is pinned by the
...
pub mod kernel_benches {
    ...
    fn bench_random_hash(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_random_hash::kernel_ir_for())
