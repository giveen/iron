...
#[kernel]
pub fn iron_mul<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], load(a[idx]) * load(b[idx]));
}

#[kernel]
pub fn iron_sub<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], load(a[idx]) - load(b[idx]));
}

#[kernel]
pub fn iron_div<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], load(a[idx]) / load(b[idx]));
}

#[kernel]
pub fn iron_max_elem<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], max(load(a[idx]), load(b[idx])));
}

#[kernel]
pub fn iron_min_elem<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], min(load(a[idx]), load(b[idx])));
}

#[kernel]
pub fn iron_pow<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], pow(load(a[idx]), load(b[idx])));
}

#[kernel]
pub fn iron_atan2<T>(y: Tensor<T>, x: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], atan2(load(y[idx]), load(x[idx])));
}

#[kernel]
pub fn iron_remainder<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
    store(out[idx], remainder(load(a[idx]), load(b[idx])));
}

#[kernel]
pub fn iron_logaddexp<T>(a: Tensor<T>, b: Tensor<T>, out: Tensor<T>) {
    let idx = program_id(0);
...
pub mod kernel_tests {
    ...
    /// ref_setup path): `iron_mul(a, b)` checked against `iron_div(a, 1/b)`
    ...
    fn test_binary_mul_vs_div_twin(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_mul::kernel_ir_for(dt))
            .input(TestBuffer::from_vec("a", pack_f32(&a, dt), dt))
            .input(TestBuffer::from_vec("b", pack_f32(&b, dt), dt))
            .input(TestBuffer::zeros("out", n, dt))
            .grid_1d(n, 256)
            .compare_against(
                TestSetup::new(iron_div::kernel_ir_for(dt))
                    .input(TestBuffer::from_vec("a", pack_f32(&a, dt), dt))
    ...
    fn test_binary_max(dt: DType) -> TestSetup {
        bin(
            iron_max_elem::kernel_ir_for(dt),
            "out",
            &ramp(17, 0.05, -0.4, 512),
    ...
    fn test_binary_min(dt: DType) -> TestSetup {
        bin(
            iron_min_elem::kernel_ir_for(dt),
            "out",
            &ramp(17, 0.05, -0.4, 512),
            &ramp(13, 0.06, -0.35, 512),
            f32::min,
    ...
    fn test_binary_atan2(dt: DType) -> TestSetup {
        // iron_atan2(y, x, out): first input is y. `bin` packs the first slice as "a"
        ...
        TestSetup::new(iron_atan2::kernel_ir_for(dt))
    ...
    fn test_binary_logaddexp(dt: DType) -> TestSetup {
        bin(
            iron_logaddexp::kernel_ir_for(dt),
            "out",
            &ramp(11, 0.3, -1.5, 512),
            &ramp(7, 0.4, -1.0, 512),
            |x, y| (x.exp() + y.exp()).ln(),
...
pub mod kernel_benches {
    ...
    /// y/x/out; the rest a/b/out) so Iron buffers bind correctly; the reference
    ...
    fn setup_ref(
        ...
        BenchSetup::new(kernel)
            ...
            .with_reference(
                ...
                // a/b shared by name with the Iron inputs above (placeholders).
    ...
    fn bench_mul(dt: DType) -> BenchSetup {
        setup_ref(iron_mul::kernel_ir_for(dt), ["a", "b", "out"], dt, "Multiply", 0.0)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_sub(dt: DType) -> BenchSetup {
        setup_ref(iron_sub::kernel_ir_for(dt), ["a", "b", "out"], dt, "Subtract", 0.0)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_div(dt: DType) -> BenchSetup {
        setup_ref(iron_div::kernel_ir_for(dt), ["a", "b", "out"], dt, "Divide", 0.0)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_max(dt: DType) -> BenchSetup {
        setup_ref(iron_max_elem::kernel_ir_for(dt), ["a", "b", "out"], dt, "Maximum", 0.0)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_min(dt: DType) -> BenchSetup {
        setup_ref(iron_min_elem::kernel_ir_for(dt), ["a", "b", "out"], dt, "Minimum", 0.0)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_pow(dt: DType) -> BenchSetup {
        setup_ref(iron_pow::kernel_ir_for(dt), ["a", "b", "out"], dt, "Power", 1e-4)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_atan2(dt: DType) -> BenchSetup {
        setup_ref(iron_atan2::kernel_ir_for(dt), ["y", "x", "out"], dt, "ArcTan2", 1e-3)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_remainder(dt: DType) -> BenchSetup {
        setup_ref(iron_remainder::kernel_ir_for(dt), ["a", "b", "out"], dt, "Remainder", 1e-4)
    }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_logaddexp(dt: DType) -> BenchSetup {
        setup_ref(iron_logaddexp::kernel_ir_for(dt), ["a", "b", "out"], dt, "LogAddExp", 1e-2)
    }
}
