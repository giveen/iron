...
//! `iron_gelu` is the tanh approximation; over PLBERT's 12 shared layers the
...
#[kernel]
pub fn iron_gelu_erf<T>(input: Tensor<T>, out: Tensor<T>) {
    let i = program_id::<0>();
...
pub mod kernel_tests {
    ...
    fn test_gelu_erf(dt: DType) -> TestSetup {
        ...
        let exp: Vec<f32> = input
            ...
            .collect();
        TestSetup::new(iron_gelu_erf::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_gelu_erf(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_gelu_erf::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
