...
#[kernel]
pub fn iron_strided_copy<T>(#[strided] src: Tensor<T>, out: Tensor<T>, #[constexpr] cols: u32) {
...
// ─── iron_strided_copy_nd ──────────────────────────────────────────────────
//
// General N-D strided copy — the MLX `copy_g` / `copy_g_nd{1,2,3}`
// counterpart. The 2-D `iron_strided_copy` above only handles a
...
//   - padded copies         (the 2-D `iron_strided_copy` case),
...
/// index math the kernel uses. `iron_strided_copy` exercises the `#[strided]`
...
pub mod kernel_tests {
    ...
    use super::{iron_strided_copy, iron_strided_copy_nd};
    ...
    fn test_strided_copy_nd(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_strided_copy_nd::kernel_ir_for(dt))
    ...
    fn test_strided_copy(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_strided_copy::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("src", pack_f32(&src_f, dt), dt))
...
pub mod kernel_benches {
    ...
    /// by name with the Iron `#[strided]` source.
    ...
    fn bench_strided_copy(dt: DType) -> BenchSetup {
        ...
        let tn = mlx_tname(dt);
        BenchSetup::new(iron_strided_copy::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            ...
            .with_reference(
                ...
                // src[0] shared by name with the Iron `#[strided] src` above.
    ...
    fn bench_strided_copy_nd(dt: DType) -> BenchSetup {
        ...
        let n_out = d0 * d1;
        BenchSetup::new(iron_strided_copy_nd::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
