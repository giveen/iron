...
//! axis:
//!   - **inclusive** — `iron_scan`: `out[i] = Σ_{j≤i} inp[j]`.
//!   - **exclusive** — `iron_scan_exclusive`: `out[i] = Σ_{j<i} inp[j]`
...
#[kernel]
pub fn iron_scan<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
...
// ── Exclusive scan ───────────────────────────────────────────────────────
//
// Identical machinery to `iron_scan`; only the store stage differs — each
...
#[kernel]
pub fn iron_scan_exclusive<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<1>();
    let lid = tid;
    let lane = simd_lane;
    let sg = simd_id;
...
// machinery from `iron_scan` / `iron_scan_exclusive`, parameterised by a
// different binary operation and identity element:
//
//   | kernel                  | op  | identity |
//   |-------------------------|-----|----------|
//   | iron_scan_prod            | ×   | 1.0      |
//   | iron_scan_prod_exclusive  | ×   | 1.0      |
//   | iron_scan_max             | max | -∞       |
//   | iron_scan_max_exclusive   | max | -∞       |
//   | iron_scan_min             | min | +∞       |
//   | iron_scan_min_exclusive   | min | +∞       |
//
// The exclusive variant stores the running prefix *before* the current
// element — identical in structure to `iron_scan_exclusive`.
//
...
//
// DISPATCH INVARIANTS (same as iron_scan):
//  - Reduction mode, `grid = [1, rows, 1]`, `tg = [tpg, 1, 1]`.
...
#[kernel]
pub fn iron_scan_prod<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<1>();
    let lid = tid;
    let ns = n_simd;
    let row_off = row * n;
...
#[kernel]
pub fn iron_scan_prod_exclusive<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<1>();
    let lid = tid;
    let ns = n_simd;
    let row_off = row * n;
...
#[kernel]
pub fn iron_scan_max<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<1>();
    let lid = tid;
    let ns = n_simd;
    let row_off = row * n;
...
#[kernel]
pub fn iron_scan_max_exclusive<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<1>();
    let lid = tid;
    let ns = n_simd;
    let row_off = row * n;
...
#[kernel]
pub fn iron_scan_min<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<1>();
    let lid = tid;
    let ns = n_simd;
    let row_off = row * n;
...
#[kernel]
pub fn iron_scan_min_exclusive<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<1>();
    let lid = tid;
    let ns = n_simd;
    let row_off = row * n;
...
pub mod kernel_tests {
    ...
    fn test_scan_sum(dt: DType) -> TestSetup {
        scan_setup(iron_scan::kernel_ir_for(dt), 4, 1024, &small, 0.0, |a, b| a + b, false, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1.0, 8.0])]
    fn test_scan_exclusive(dt: DType) -> TestSetup {
        scan_setup(
            iron_scan_exclusive::kernel_ir_for(dt),
            4,
            1024,
            &small,
            0.0,
    ...
    fn test_scan_prod(dt: DType) -> TestSetup {
        scan_setup(iron_scan_prod::kernel_ir_for(dt), 4, 1024, &near1, 1.0, |a, b| a * b, false, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-2, 1e-1, 1.0])]
    fn test_scan_prod_exclusive(dt: DType) -> TestSetup {
        scan_setup(
            iron_scan_prod_exclusive::kernel_ir_for(dt),
            4,
            1024,
            &near1,
            1.0,
    ...
    fn test_scan_max(dt: DType) -> TestSetup {
        scan_setup(
            iron_scan_max::kernel_ir_for(dt),
            4,
            1024,
            &spread,
            f32::NEG_INFINITY,
    ...
    fn test_scan_min(dt: DType) -> TestSetup {
        scan_setup(
            iron_scan_min::kernel_ir_for(dt),
            4,
            1024,
            &spread,
            f32::INFINITY,
...
pub mod kernel_benches {
    ...
    // Legacy `tol=1e-3` floor for the scan family. Iron accumulates the running
    ...
    /// Iron-only scan bench (prod/max/min and the exclusive variants have
    ...
    /// puts rows on the Y axis, matching Iron's `grid_3d(1, rows, 1, …)`), reads
    /// `in[buffer(0)]`, writes `out[buffer(1)]`, and takes `axis_size` as a
    /// `size_t` (8-byte) scalar — unlike Iron's `u32` `n` constexpr.
    ///
    ...
    fn bench_scan_sum(dt: DType) -> BenchSetup { sb_sum_ref(iron_scan::kernel_ir_for(dt), dt) }
    ...
    sbench!(bench_scan_excl, iron_scan_exclusive);
    sbench!(bench_scan_prod, iron_scan_prod);
    sbench!(bench_scan_prod_excl, iron_scan_prod_exclusive);
    sbench!(bench_scan_max, iron_scan_max);
    sbench!(bench_scan_max_excl, iron_scan_max_exclusive);
    sbench!(bench_scan_min, iron_scan_min);
    sbench!(bench_scan_min_excl, iron_scan_min_exclusive);
}
