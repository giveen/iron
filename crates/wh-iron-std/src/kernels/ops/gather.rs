...
/// New-syntax correctness for `iron_gather` (Grid3D, one thread per output
...
pub mod kernel_tests {
    ...
    use super::iron_gather;
    use crate::utils::{pack_f32, unpack_f32};

    fn u32_bytes(v: &[u32]) -> Vec<u8> { v.iter().flat_map(|x| x.to_le_bytes()).collect() }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = 0.0)]
    fn test_iron_gather(dt: DType) -> TestSetup {
        ...
        let expected: Vec<f32> = (0..n_elems)
            ...
            .collect();
        TestSetup::new(iron_gather::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_gather` (embedding-table lookup, Qwen-class
...
pub mod kernel_benches {
    ...
    fn bench_gather(dt: DType) -> BenchSetup {
        ...
        let n_elems = n_tokens * dim;
        BenchSetup::new(iron_gather::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
