...
/// New-syntax correctness for `iron_scatter_axis` (exact). Indices are an
...
pub mod kernel_tests {
    ...
    use super::iron_scatter_axis;
    use crate::utils::{pack_f32, unpack_f32};

    fn u32_bytes(v: &[u32]) -> Vec<u8> { v.iter().flat_map(|x| x.to_le_bytes()).collect() }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = 1e-6)]
    fn test_iron_scatter_axis(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_scatter_axis::kernel_ir_for(dt))
...
/// New-syntax benchmark for `iron_scatter_axis`.
pub mod kernel_benches {
    ...
    fn bench_scatter_axis(dt: DType) -> BenchSetup {
        ...
        let indices: Vec<u8> = (0..upd_len)
            ...
            .collect();
        BenchSetup::new(iron_scatter_axis::kernel_ir_for(dt))
