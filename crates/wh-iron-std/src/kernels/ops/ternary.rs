...
#[kernel]
pub fn iron_select<T>(cond: Tensor<u8>, on_true: Tensor<T>, on_false: Tensor<T>, out: Tensor<T>) {
...
/// New-syntax correctness for `iron_select` (elementwise, exact — picks an input
/// verbatim, so the result is bit-exact in every dtype).
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_select;
    ...
    fn setup(n: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_select::kernel_ir_for(dt))
    ...
    fn test_iron_select(dt: DType) -> TestSetup { setup(512, dt) }
}

/// New-syntax benchmark for `iron_select` (vs MLX `metal/ternary.metal`).
pub mod kernel_benches {
    ...
    // The reference binds cond/on_true/on_false by name (shared with the Iron
    ...
    fn bench_select(dt: DType) -> BenchSetup {
        ...
        let tn = mlx_tname(dt);
        BenchSetup::new(iron_select::kernel_ir_for(dt))
            ...
            .with_reference(
                ...
                // cond/on_true/on_false shared by name with the Iron inputs above
                // (placeholders; the runner overrides them with the Iron bytes).
