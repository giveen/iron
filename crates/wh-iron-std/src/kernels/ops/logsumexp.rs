...
#[kernel]
pub fn iron_logsumexp<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
...
/// New-syntax correctness for `iron_logsumexp` (Reduction, one threadgroup per
/// row, tpg=256). Per-row oracle: `max + log(sum(exp(x - max)))`; one output
/// element per row.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_logsumexp;
    ...
    fn setup_with(
        ...
        TestSetup::new(iron_logsumexp::kernel_ir_for(dt))
    ...
    fn test_iron_logsumexp(dt: DType) -> TestSetup { setup(4, 1024, dt) }
    ...
    fn test_iron_logsumexp_large_values(dt: DType) -> TestSetup {
        setup_with(2, 1024, dt, |_r, i| 50.0 + (i % 7) as f32 * 10.0)
    }
}

/// New-syntax benchmark for `iron_logsumexp` (vs MLX `metal/logsumexp.metal`).
pub mod kernel_benches {
    ...
    // `inp` is shared by name with the Iron input.
    //
    // MLX dispatch geometry: one threadgroup per row (grid=[rows,1,1]) at
    // tpg=1024 — NOT the Iron tpg=256. Same threadgroup-array hazard as
    // `looped_softmax_*`: `local_max[32]`/`local_normalizer[32]` are only valid
    // when n_simd==32 (tpg=1024); at Iron's tpg=256 the stale slots produce NaN.
    ...
    fn bench_logsumexp(dt: DType) -> BenchSetup {
        ...
        let tn = mlx_tname(dt);
        BenchSetup::new(iron_logsumexp::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            ...
            .with_reference(
                ...
                // `inp` shared by name with the Iron input above (placeholder).
