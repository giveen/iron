...
//!   - **all-reduce** — `iron_all_reduce*`: one threadgroup folds the
//!     whole input to a scalar (Reduction mode).
//!   - **row-reduce** — `iron_row_reduce*`: one threadgroup per row of a
//!     `[rows, n]` input (Reduction mode).
//!   - **column-reduce** — `iron_col_reduce*`: one thread per column of a
...
//!   - **segmented-reduce** — `iron_seg_reduce*`: one thread per segment
...
#[kernel]
pub fn iron_all_reduce<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let zero = 0;
...
#[kernel]
pub fn iron_all_reduce_prod<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let off = 0;
...
#[kernel]
pub fn iron_all_reduce_max<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let zero = 0;
...
#[kernel]
pub fn iron_all_reduce_min<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let zero = 0;
...
#[kernel]
pub fn iron_row_reduce<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
...
#[kernel]
pub fn iron_row_reduce_prod<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<0>();
    let rs = row * n;
    let re = rs + n;
...
#[kernel]
pub fn iron_row_reduce_max<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<0>();
    let rs = row * n;
    let re = rs + n;
...
#[kernel]
pub fn iron_row_reduce_min<T>(inp: Tensor<T>, out: Tensor<T>, #[constexpr] n: u32) {
    let row = program_id::<0>();
    let rs = row * n;
    let re = rs + n;
...
//
// Unlike the Reduction-mode `iron_row_reduce`, NO `reduce_*(acc)`
...
// (stride = 1).
//
// This is the one-thread-per-segment counterpart to `iron_row_reduce`'s
...
pub mod kernel_tests {
    ...
    fn test_all_reduce_sum(dt: DType) -> TestSetup {
        all_setup_for(
            iron_all_reduce::kernel_ir_for(dt),
            2048,
            &sum_vals(2048),
            0.0,
            |a, b| a + b,
    ...
    fn test_all_reduce_prod(dt: DType) -> TestSetup {
        all_setup_for(
            iron_all_reduce_prod::kernel_ir_for(dt),
            512,
            &prod_vals(512),
            1.0,
            |a, b| a * b,
    ...
    fn test_all_reduce_max(dt: DType) -> TestSetup {
        all_setup_for(
            iron_all_reduce_max::kernel_ir_for(dt),
            2048,
            &ext_vals(2048),
            f32::NEG_INFINITY,
            f32::max,
    ...
    fn test_all_reduce_min(dt: DType) -> TestSetup {
        all_setup_for(
            iron_all_reduce_min::kernel_ir_for(dt),
            2048,
            &ext_vals(2048),
            f32::INFINITY,
            f32::min,
    ...
    fn test_row_reduce_sum(dt: DType) -> TestSetup {
        row_setup_for(
            iron_row_reduce::kernel_ir_for(dt),
            4,
            1024,
    ...
    fn test_row_reduce_max(dt: DType) -> TestSetup {
        row_setup_for(
            iron_row_reduce_max::kernel_ir_for(dt),
            4,
            1024,
    ...
    fn test_row_reduce_min(dt: DType) -> TestSetup {
        row_setup_for(
            iron_row_reduce_min::kernel_ir_for(dt),
            4,
            1024,
            &|r| (0..1024).map(|i| ((i * 7919 % 1000) as f32) * 0.01 - 5.0 + r as f32).collect(),
            f32::INFINITY,
    ...
    fn test_col_reduce_sum(dt: DType) -> TestSetup {
        col_setup_for(iron_col_reduce::kernel_ir_for(dt), 37, 100, 0.0, |a, b| a + b, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = 1e-3)]
    fn test_col_reduce_max(dt: DType) -> TestSetup {
        col_setup_for(
            iron_col_reduce_max::kernel_ir_for(dt),
            50,
            70,
            f32::NEG_INFINITY,
            f32::max,
    ...
    fn test_col_reduce_min(dt: DType) -> TestSetup {
        col_setup_for(iron_col_reduce_min::kernel_ir_for(dt), 50, 70, f32::INFINITY, f32::min, dt)
    }
    ...
    fn test_seg_reduce_sum(dt: DType) -> TestSetup {
        seg_setup_for(iron_seg_reduce::kernel_ir_for(dt), 64, 48, 0.0, |a, b| a + b, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = 1e-3)]
    fn test_seg_reduce_max(dt: DType) -> TestSetup {
        seg_setup_for(
            iron_seg_reduce_max::kernel_ir_for(dt),
            64,
            48,
            f32::NEG_INFINITY,
            f32::max,
    ...
    fn test_seg_reduce_min(dt: DType) -> TestSetup {
        seg_setup_for(iron_seg_reduce_min::kernel_ir_for(dt), 64, 48, f32::INFINITY, f32::min, dt)
    }
    ...
    fn test_col_reduce_prod(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_col_reduce_prod::kernel_ir_for(dt))
            .input(TestBuffer::from_vec("inp", pack_f32(&inp, dt), dt))
            .input(TestBuffer::zeros("out", cols, dt))
            .constexpr("rows", rows as u32)
            .constexpr("cols", cols as u32)
    ...
    fn test_seg_reduce_prod(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_seg_reduce_prod::kernel_ir_for(dt))
            .input(TestBuffer::from_vec("inp", pack_f32(&inp, dt), dt))
            .input(TestBuffer::zeros("out", n_segments, dt))
            .constexpr("n_segments", n_segments as u32)
            .constexpr("seg_len", seg_len as u32)
...
pub mod kernel_benches {
    ...
    // a single block when `in_size == row_size == N` (matching Iron's single
    // threadgroup); both are `size_t` (8-byte) scalars.
    //
    // `inp` is shared by name with the reference (the runner injects the Iron
    // bytes), so both kernels reduce identical `Positive` data. `tol_floor` is
    // the legacy reduction floor — large for sum/prod because Iron accumulates in
    ...
    fn all_ref(
        ...
        // `sum` folds 16M elements: Iron accumulates in f32 but MLX accumulates in
        ...
        base.with_reference(
                ...
                // in[0] shared by name with the Iron `inp` above (placeholder).
    ...
    fn bench_all_sum(dt: DType) -> BenchSetup {
        all_ref(iron_all_reduce::kernel_ir_for(dt), dt, "sum", 256.0, true)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_all_prod(dt: DType) -> BenchSetup {
        all_ref(iron_all_reduce_prod::kernel_ir_for(dt), dt, "prod", 1024.0, false)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_all_max(dt: DType) -> BenchSetup {
        all_ref(iron_all_reduce_max::kernel_ir_for(dt), dt, "max", 0.0, false)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_all_min(dt: DType) -> BenchSetup {
        all_ref(iron_all_reduce_min::kernel_ir_for(dt), dt, "min", 0.0, false)
    }
    ...
    // axis (`[1, rows, 1]`) — unlike the Iron kernel which uses `program_id::<0>()`
    ...
    fn bench_row_sum(dt: DType) -> BenchSetup {
        row_ref(iron_row_reduce::kernel_ir_for(dt), dt, "sum", 128.0)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_row_prod(dt: DType) -> BenchSetup {
        row_ref(iron_row_reduce_prod::kernel_ir_for(dt), dt, "prod", 32.0)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_row_max(dt: DType) -> BenchSetup {
        row_ref(iron_row_reduce_max::kernel_ir_for(dt), dt, "max", 0.0)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_row_min(dt: DType) -> BenchSetup {
        row_ref(iron_row_reduce_min::kernel_ir_for(dt), dt, "min", 0.0)
    }
    ...
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_col_sum(dt: DType) -> BenchSetup { col_b(iron_col_reduce::kernel_ir_for(dt), dt) }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_col_prod(dt: DType) -> BenchSetup {
        col_b(iron_col_reduce_prod::kernel_ir_for(dt), dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_col_max(dt: DType) -> BenchSetup { col_b(iron_col_reduce_max::kernel_ir_for(dt), dt) }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_col_min(dt: DType) -> BenchSetup { col_b(iron_col_reduce_min::kernel_ir_for(dt), dt) }
    ...
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_seg_sum(dt: DType) -> BenchSetup { seg_b(iron_seg_reduce::kernel_ir_for(dt), dt) }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_seg_prod(dt: DType) -> BenchSetup {
        seg_b(iron_seg_reduce_prod::kernel_ir_for(dt), dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_seg_max(dt: DType) -> BenchSetup { seg_b(iron_seg_reduce_max::kernel_ir_for(dt), dt) }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_seg_min(dt: DType) -> BenchSetup { seg_b(iron_seg_reduce_min::kernel_ir_for(dt), dt) }
}
