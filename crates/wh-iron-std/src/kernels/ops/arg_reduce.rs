...
#[kernel]
pub fn iron_argmax<T>(inp: Tensor<T>, out: Tensor<u32>, #[constexpr] n: u32) {
    let lid = tid;
...
#[kernel]
pub fn iron_argmin<T>(inp: Tensor<T>, out: Tensor<u32>, #[constexpr] n: u32) {
    let lid = tid;
...
pub mod kernel_tests {
    ...
    use super::{iron_argmax, iron_argmin};
    ...
    fn test_iron_argmax(dt: DType) -> TestSetup {
        setup(iron_argmax::kernel_ir_for(dt), 1000, 813, 2.0, dt)
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = 0.5)]
    fn test_iron_argmin(dt: DType) -> TestSetup {
        setup(iron_argmin::kernel_ir_for(dt), 1000, 271, -2.0, dt)
    }
    ...
    fn test_iron_argmax_ties_take_smallest(dt: DType) -> TestSetup {
        let _ = dt;
        tie_setup(iron_argmax::kernel_ir_for(DType::F32), 1024, 200, 600, 5.0)
    }

    #[test_kernel(dtypes = [f32], tol = 0.5)]
    fn test_iron_argmin_ties_take_smallest(dt: DType) -> TestSetup {
        let _ = dt;
        tie_setup(iron_argmin::kernel_ir_for(DType::F32), 1024, 200, 600, -5.0)
    }
}
...
pub mod kernel_benches {
    ...
    // (`arg_reduce_general`) reference. Both Iron and MLX emit the winning index as
    ...
    fn ab(kernel: Kernel, dt: DType, mlx_op: &str) -> BenchSetup {
        ...
        BenchSetup::new(kernel)
            ...
            .with_reference(
                ...
                // One threadgroup of 256 threads, matching Iron.
    ...
    fn bench_argmax(dt: DType) -> BenchSetup { ab(iron_argmax::kernel_ir_for(dt), dt, "argmax") }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_argmin(dt: DType) -> BenchSetup { ab(iron_argmin::kernel_ir_for(dt), dt, "argmin") }
}
