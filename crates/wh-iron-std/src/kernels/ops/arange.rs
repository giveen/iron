...
/// Correctness tests for `iron_arange` in the new `#[test_kernel]` syntax.
///
/// These run via `iron test` (and the `kernel_tests_harness` cargo bridge).
...
pub mod kernel_tests {
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = 1e-6)]
    fn test_iron_arange_descending(dt: DType) -> TestSetup { setup(16.0, -1.0, 16, dt) }
...
/// Benchmark for `iron_arange` in the new `#[bench]` syntax. Registered
/// alongside the legacy `#[kernel]` above, so it appears in
/// `iron bench` next to the legacy `arange` row for A/B comparison.
