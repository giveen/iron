...
//!
//!   - `iron_swiglu` — `silu(gate) · up` (Llama/Qwen/Gemma/Mistral)
//!   - `iron_clamped_swiglu` — `clip(silu(gate), L) · clip(up, ±L)`; a per-layer
//!     activation-clip limit (gpt-oss, StepFun Step-3). `limit <= 0` ⇒ plain SwiGLU
//!   - `iron_fused_gate_gelu` — `gelu_approx(gate) · up`
//!   - `iron_fused_gate_clipped_swiglu` — GPT-OSS clipped: clamp `±7`,
//!     `sigmoid(1.702·g)` gate, `+1` up bias
//!
//! All are one-thread-per-output elementwise kernels computed in f32. The plain
//! ungated `silu` activation lives in `ops/unary.rs` (`iron_silu`); `iron_swiglu`
//! cross-kernel-calls it (inlined at codegen by `KernelInlinePass`).
//!
//! MLX reference: `mx.fast.swiglu` + `fused_gate_activation.metal`
//! (`apply_gate<activation_type ∈ {0,1,2}>`; type 0 = silu = `iron_swiglu`).
...
#[kernel]
pub fn iron_swiglu<T>(gate: Tensor<T>, up: Tensor<T>, out: Tensor<T>) {
    let idx = tid;
    let g = load(gate[idx]).cast::<f32>();
    let u = load(up[idx]).cast::<f32>();
    // Cross-kernel call: KernelInlinePass splices iron_silu's scalar body here.
    // iron_silu's input-param load is replaced by g (already f32), so silu runs in
...
#[kernel]
pub fn iron_fused_gate_gelu<T>(gate: Tensor<T>, up: Tensor<T>, out: Tensor<T>) {
...
#[kernel]
pub fn iron_fused_gate_clipped_swiglu<T>(gate: Tensor<T>, up: Tensor<T>, out: Tensor<T>) {
    let idx = program_id::<0>();
...
pub mod kernel_tests {
    ...
    use super::{
        iron_clamped_swiglu,
        iron_fused_gate_clipped_swiglu,
        iron_fused_gate_gelu,
        iron_swiglu,
    };
    ...
    fn swiglu_setup(n: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_swiglu::kernel_ir_for(dt))
    ...
    fn test_iron_swiglu(dt: DType) -> TestSetup { swiglu_setup(1024, dt) }
    ...
    fn clamped_setup(n: usize, limit: f32, dt: DType) -> TestSetup {
        ...
        let expected: Vec<f32> = g_dt
            ...
            .collect();
        TestSetup::new(iron_clamped_swiglu::kernel_ir_for(dt))
            .input(TestBuffer::from_vec("gate", pack_f32(&gate, dt), dt))
            .input(TestBuffer::from_vec("up", pack_f32(&up, dt), dt))
            .input(TestBuffer::zeros("out", n, dt))
    ...
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 5e-3, 5e-2])]
    fn test_iron_clamped_swiglu_active(dt: DType) -> TestSetup { clamped_setup(1024, 7.0, dt) }

    /// `limit == 0` collapses to plain SwiGLU — equivalence with `iron_swiglu` is
    /// the invariant the per-layer dispatch ships on.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 5e-3, 5e-2])]
    fn test_iron_clamped_swiglu_zero_limit_equals_plain(dt: DType) -> TestSetup {
        clamped_setup(1024, 0.0, dt)
    }

    // ── iron_fused_gate_{gelu,clipped_swiglu} ───────────────────────────────────
    ...
    fn test_fused_gate_gelu(dt: DType) -> TestSetup {
        ...
        let expected: Vec<f32> = g
            ...
            .collect();
        build(iron_fused_gate_gelu::kernel_ir_for(dt), &gate, &up, &expected, dt)
    }
    ...
    fn test_fused_gate_clipped_swiglu(dt: DType) -> TestSetup {
        ...
        let expected: Vec<f32> = g
            ...
            .collect();
        build(iron_fused_gate_clipped_swiglu::kernel_ir_for(dt), &gate, &up, &expected, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_swiglu(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_swiglu::kernel_ir_for(dt))
    ...
    fn bench_clamped_swiglu(dt: DType) -> BenchSetup {
        let n = 1024 * 1024usize;
        BenchSetup::new(iron_clamped_swiglu::kernel_ir_for(dt))
            .buffer(BenchBuffer::random("gate", n, dt))
            .buffer(BenchBuffer::random("up", n, dt))
            .buffer(BenchBuffer::zeros("out", n, dt).output())
    ...
    fn fb(kernel: wh_iron::core::ir::Kernel, dt: DType) -> BenchSetup {
    ...
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_gelu(dt: DType) -> BenchSetup { fb(iron_fused_gate_gelu::kernel_ir_for(dt), dt) }

    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_clipped(dt: DType) -> BenchSetup {
        fb(iron_fused_gate_clipped_swiglu::kernel_ir_for(dt), dt)
    }
}
