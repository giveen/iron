...
#[kernel]
pub fn iron_binary_two<T>(a: Tensor<T>, b: Tensor<T>, mut c: Tensor<T>, mut d: Tensor<T>) {
...
/// New-syntax correctness for `iron_binary_two` (fused two-output elementwise:
/// `c = a + b`, `d = a * b`). Exercises multiple `.expect()` buffers.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_binary_two;
    ...
    fn setup(n: usize, dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_binary_two::kernel_ir_for(dt))
    ...
    fn test_iron_binary_two(dt: DType) -> TestSetup { setup(512, dt) }
}

/// New-syntax benchmark for `iron_binary_two` — reads a+b, writes c+d.
pub mod kernel_benches {
    ...
    fn bench_binary_two(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_binary_two::kernel_ir_for(dt))
