...
//! prosody / duration predictors need (no Iron model used an LSTM before).
//! This file holds two forms:
//!   * [`iron_lstm`] — runs the **whole sequence** recurrence on the GPU in
...
/// per-step host control is wanted; use [`iron_lstm`] above to run the whole
/// sequence on the GPU in one dispatch. Takes the `bias_ih` / `bias_hh`
/// split separately (vs. `iron_lstm`'s precombined `bias`).
#[kernel]
pub fn lstm_cell<T>(
    x: Tensor<T>,
    h_prev: Tensor<T>,
...
pub mod kernel_tests {
    ...
    fn setup(
        ...
        TestSetup::new(iron_lstm::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_lstm(dt: DType) -> BenchSetup {
        ...
        let out_stride = hidden;
        BenchSetup::new(iron_lstm::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
