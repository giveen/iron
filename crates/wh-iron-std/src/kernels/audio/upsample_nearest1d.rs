...
#[kernel]
pub fn iron_upsample_nearest1d<T>(
    input: Tensor<T>,
    mut out: Tensor<T>,
...
/// New-syntax correctness for `iron_upsample_nearest1d`. Grid3D, grid
/// `[C·factor·in_len,1,1]`, tpg `[1,1,1]`. Oracle repeats each sample
/// `factor` times.
pub mod kernel_tests {
    use wh_iron::{test::*, test_kernel};

    use super::iron_upsample_nearest1d;
    ...
    fn setup(c: usize, in_len: usize, factor: usize, dt: DType) -> TestSetup {
        ...
        let exp: Vec<f32> = (0..n_out)
            ...
            .collect();
        TestSetup::new(iron_upsample_nearest1d::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_upsample_nearest1d(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_upsample_nearest1d::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
