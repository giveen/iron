...
//
// `iron_mel_spectrogram` does a direct DFT *inside every (frame, mel_bin)
...
//
//   1. `iron_mel_stft_window`  — extract + window each frame into FFT input
//                           planes (real = windowed sample, imag = 0).
//   2. `iron_fft_n{n_fft}`  — one radix-2 FFT per frame (O(n_fft·log n_fft)).
//   3. `iron_mel_filterbank`   — power = re²+im², Mel-weight, log.
//
...
pub fn iron_mel_stft_window<T>(
    ...
    // (see `iron_mel_spectrogram`) from OOB `audio` reads / `out_re`/`out_im` writes.
    let idx = program_id::<0>();
    if idx < n_out {
        let t = idx % n_fft;
        let frame = idx / n_fft;
...
/// `re`/`im` are `fft_re`/`fft_im` from `iron_fft_n{n_fft}`. One thread per
/// `(frame, mel)`; dispatch flat over `n_frames * n_mels`. Output is
/// bit-identical in form to `iron_mel_spectrogram` — only the spectrum source
/// (FFT vs in-thread DFT) differs.
#[kernel]
pub fn iron_mel_filterbank<T>(
    fft_re: Tensor<T>,
    fft_im: Tensor<T>,
    mel_weight: Tensor<T>,
    out: Tensor<T>,
    ...
    // (see `iron_mel_spectrogram`) from OOB `fft_re`/`fft_im` reads / `out` writes.
    let idx = program_id::<0>();
    if idx < n_out {
        let mel_bin = idx % n_mels;
        let frame = idx / n_mels;
...
/// filterbank, vs the power (`re²+im²`) front-end of `iron_mel_spectrogram` above.
...
#[kernel]
pub fn iron_mel_spectrogram_magnitude<T>(
    audio: Tensor<T>,
    window: Tensor<T>,
    mel_weight: Tensor<T>,
    out: Tensor<T>,
    ...
    // `n_out = n_frames * n_mels`. Guard the threadgroup-rounded dispatch tail
    // (see `iron_mel_spectrogram`) from OOB `audio`/`mel_weight` reads / `out` writes.
    let idx = program_id::<0>();
    if idx < n_out {
        let mel_bin = idx % n_mels;
        let frame = idx / n_mels;
...
pub mod kernel_tests {
    ...
    use super::{
        iron_mel_filterbank,
        iron_mel_spectrogram,
        iron_mel_spectrogram_magnitude,
        iron_mel_stft_window,
    };
    ...
    // — so correctness is gated at f32; the post-FFT `iron_mel_filterbank` test below
    ...
    fn test_mel_spectrogram(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_mel_spectrogram::kernel_ir_for(dt))
    ...
    fn test_mel_stft_window(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_mel_stft_window::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("audio", pack_f32(&audio, dt), dt))
            .input(TestBuffer::from_vec("window", pack_f32(&window, dt), dt))
    ...
    fn test_mel_filterbank(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_mel_filterbank::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    // sharpening the null sensitivity); the post-FFT `iron_mel_filterbank` test keeps
    ...
    fn test_mel_spectrogram_magnitude(dt: DType) -> TestSetup {
        ...
        let n_out = n_frames * n_mels;
        TestSetup::new(iron_mel_spectrogram_magnitude::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .input(TestBuffer::from_vec("audio", pack_f32(&audio, dt), dt))
            .input(TestBuffer::from_vec("window", pack_f32(&window, dt), dt))
            .input(TestBuffer::from_vec("mel_weight", pack_f32(&mel_weight, dt), dt))
...
pub mod kernel_benches {
    ...
    fn bench_mel_spectrogram(dt: DType) -> BenchSetup {
        let n_out = N_FRAMES * N_MELS;
        BenchSetup::new(iron_mel_spectrogram::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_mel_stft_window(dt: DType) -> BenchSetup {
        let n = N_FRAMES * N_FFT;
        BenchSetup::new(iron_mel_stft_window::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .buffer(BenchBuffer::random("audio", n_samples(), dt))
            .buffer(BenchBuffer::random("window", N_FFT, dt))
    ...
    fn bench_mel_filterbank(dt: DType) -> BenchSetup {
        let n_out = N_FRAMES * N_MELS;
        BenchSetup::new(iron_mel_filterbank::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
    ...
    fn bench_mel_spectrogram_magnitude(dt: DType) -> BenchSetup {
        ...
        let n_out = n_frames * n_mels;
        BenchSetup::new(iron_mel_spectrogram_magnitude::kernel_ir_for(dt))
            .mode(KernelMode::Grid3D)
            .buffer(BenchBuffer::random("audio", n_samples, dt))
            .buffer(BenchBuffer::random("window", n_fft, dt))
            .buffer(BenchBuffer::random("mel_weight", n_mels * n_freq, dt))
