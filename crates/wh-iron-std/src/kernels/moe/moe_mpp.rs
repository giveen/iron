...
//! MPP-backed MoE grouped BGEMM, BM=16 — `iron_moe_gather_qmm_mma_int{2,4,8}_bm16_mpp`.
//!
...
/// selects the weight precision; produces `iron_moe_gather_qmm_mma_int{2,4,8}_bm16_mpp`.
///
...
mod tests {
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_moe_gather_qmm_mma_int4_bm16_mpp");
    ...
    fn bf16_stages_through_half() {
        let k = iron_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(DType::BF16);
    ...
    fn codegen_emits_mpp_include() {
        for (mut k, name) in [
            (
                iron_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(DType::F32),
                "iron_moe_gather_qmm_mma_int4_bm16_mpp_f32",
            ),
            (
                iron_moe_gather_qmm_mma_int8_bm16_mpp::kernel_ir_for(DType::F32),
                "iron_moe_gather_qmm_mma_int8_bm16_mpp_f32",
            ),
        ] {
            k.name = name.into();
...
pub mod kernel_tests {
    ...
    use super::{
        iron_moe_gather_qmm_mma_int2_bm16_mpp,
        iron_moe_gather_qmm_mma_int4_bm16_mpp,
        iron_moe_gather_qmm_mma_int8_bm16_mpp,
    };
    ...
    fn test_moe_gather_qmm_mma_int2_bm16_mpp(dt: DType) -> TestSetup {
        int2_indexed_setup(
            iron_moe_gather_qmm_mma_int2_bm16_mpp::kernel_ir_for(dt),
    ...
    fn test_moe_gather_qmm_mma_int4_bm16_mpp(dt: DType) -> TestSetup {
        int4_indexed_setup(
            iron_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(dt),
            MmaTestShape { n_experts: 4, m_total: 64, n_out: 64, k_in: 64, group_size: 32 },
            32, // bn
            16, // bm
            32, // tpg
    ...
    fn test_moe_gather_qmm_mma_int8_bm16_mpp(dt: DType) -> TestSetup {
        int8_indexed_setup(
            iron_moe_gather_qmm_mma_int8_bm16_mpp::kernel_ir_for(dt),
            MmaTestShape { n_experts: 4, m_total: 64, n_out: 64, k_in: 64, group_size: 32 },
            32, // bn
            16, // bm
            32, // tpg
