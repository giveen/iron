...
//! `iron_moe_gather_qmm_coop_paired` - two-expert PAIRED tile variant of
//! the coop-core MoE gather GEMM (`iron_moe_gather_qmm_coop`,
...
//! `iron_moe_gather_qmm_coop`, not a generalization of it - see
...
//! `1..=16`-row pairing-candidate tiles `iron_moe_build_tile_plan_bm32_paired`
//! produces; every other row is handled by the completely unmodified
//! `iron_moe_gather_qmm_coop` dispatch, at its existing cost.
//!
...
//!   2-3 to M-rows 16..32 ("B half") - see `iron_moe_gather_qmm_coop`'s
...
//! step-counter) are otherwise identical to `iron_moe_gather_qmm_coop` -
...
//!   on the host or `iron_moe_build_tile_plan_bm32_paired` on device.
...
/// Paired variant of `iron_moe_gather_qmm_coop`. Params: `x [n_x_rows,
...
pub fn iron_moe_gather_qmm_coop_paired<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
mod tests {
    ...
    fn kernel_ir_uses_two_coop_tile_ops_no_inline_msl() {
        ...
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_moe_gather_qmm_coop_paired::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_moe_gather_qmm_coop_paired");
            let all_ops =
    ...
    fn bf16_stages_through_half() {
        use wh_iron::core::ir::Op;
        let k = iron_moe_gather_qmm_coop_paired::kernel_ir_for(DType::BF16);
...
/// New-syntax correctness for `iron_moe_gather_qmm_coop_paired` against a
...
pub mod kernel_tests {
    ...
    use super::iron_moe_gather_qmm_coop_paired;
    use crate::{
    ...
    fn test_paired_exact_16_16_boundary_different_experts(dt: DType) -> TestSetup {
        ...
        setup(
            iron_moe_gather_qmm_coop_paired::kernel_ir_for(dt),
            &tiles,
            2,
            32,
            32,
    ...
    fn test_paired_odd_leftover_inert_b_half(dt: DType) -> TestSetup {
        ...
        setup(iron_moe_gather_qmm_coop_paired::kernel_ir_for(dt), &tiles, 1, 9, 9, 64, 128, 64, dt)
    }
    ...
    fn test_paired_xrows_gather(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_moe_gather_qmm_coop_paired::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("x", pack_f32(&x_f, dt), dt))
            .input(TestBuffer::from_vec("w", u32_bytes(&weight_packed), DType::U32))
            .input(TestBuffer::from_vec("scales", pack_f32(&scales_f, dt), dt))
...
/// New-syntax benchmarks for `iron_moe_gather_qmm_coop_paired`. Without a
/// `#[bench]` entry the kernel is invisible to `iron build --emit`, same
...
pub mod kernel_benches {
    ...
    fn moe_coop_paired_bench(
        ...
        BenchSetup::new(iron_moe_gather_qmm_coop_paired::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
...
/// `iron_moe_gather_qmm_coop` kernel, fed by `build_own_tile_plan_bm32`
...
pub mod kernel_benches_own {
    ...
    use crate::kernels::moe::{
        moe_gather_qmm_coop::iron_moe_gather_qmm_coop,
        moe_mpp_shared::zipfish_counts,
    ...
    fn moe_own_bench(
        ...
        BenchSetup::new(iron_moe_gather_qmm_coop::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random("x", m_total * k_in, dt))
            .buffer(BenchBuffer::random("w", n_experts * n_out * words_per_row, DType::U32))
            .buffer(BenchBuffer::random("scales", n_experts * n_out * groups_per_row, dt))
