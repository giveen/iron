...
//! MoE permute / unpermute — `iron_moe_permute` gathers token rows into
//! expert-sorted order ahead of the grouped expert matmul; `iron_moe_unpermute`
...
pub mod kernel_tests {
    ...
    // iron_moe_sort_plan: device-built expert-sorted dispatch plan.
    ...
    fn test_moe_sort_plan(_dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_moe_sort_plan::kernel_ir())
    ...
    // iron_moe_permute: gather token rows into expert-sorted order.
    ...
    fn test_moe_permute(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_moe_permute::kernel_ir_for(dt))
    ...
    // iron_moe_unpermute: weighted sum of k expert outputs back to token order.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [1e-4, 1e-3, 1e-2])]
    fn test_moe_unpermute(dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_moe_unpermute::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
...
pub mod kernel_benches {
    ...
    fn bench_moe_sort_plan(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_sort_plan::kernel_ir())
    ...
    fn bench_moe_permute(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_permute::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn bench_moe_unpermute(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_unpermute::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
