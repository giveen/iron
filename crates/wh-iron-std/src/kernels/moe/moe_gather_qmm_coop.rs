...
//! `iron_moe_gather_qmm_coop` - MoE gather variant of the cooperative-
//! staging wide-N int4 dense GEMM core (`iron_qmm_coop`,
...
//! `moe_mpp_tileplan.rs`'s `iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan`
...
//!      `moe_tile_plan_builder_bm32.rs`'s `iron_moe_build_tile_plan_bm32` on
...
//!      `iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan`'s X-stage step
//!      uses.
...
//! `iron_qmm_coop` - see that kernel's module doc for the full staging
//! walkthrough.
//!
...
//! mirrors `iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan`'s own
...
//!   device `iron_moe_build_tile_plan_bm32`.
...
/// MoE gather variant of `iron_qmm_coop`. Params: `x [n_x_rows, k_in]`,
...
pub fn iron_moe_gather_qmm_coop<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    ...
    // BM*BK, identical to `iron_qmm_coop`. `x_m_row` is the TILE-LOCAL row
    ...
    // half (w_row 32..64) of the BN=64 tile - identical to `iron_qmm_coop`,
    ...
    // sequencing to `iron_qmm_coop`; only the store is now guarded by the
...
mod tests {
    ...
    fn kernel_ir_uses_two_coop_tile_ops_no_inline_msl() {
        ...
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_moe_gather_qmm_coop::kernel_ir_for(dt);
            assert_eq!(k.name, "iron_moe_gather_qmm_coop");
            let all_ops =
    ...
    fn bf16_stages_through_half() {
        use wh_iron::core::ir::Op;
        let k = iron_moe_gather_qmm_coop::kernel_ir_for(DType::BF16);
    ...
    fn dump() {
        ...
        let mut k = iron_moe_gather_qmm_coop::kernel_ir_for(DType::F16);
...
/// New-syntax correctness for `iron_moe_gather_qmm_coop` against a CPU
...
pub mod kernel_tests {
    ...
    use super::iron_moe_gather_qmm_coop;
    use crate::{
    ...
    fn test_moe_gather_qmm_coop_skewed_ragged(dt: DType) -> TestSetup {
        ...
        let m_total: usize = counts.iter().sum();
        setup(
            iron_moe_gather_qmm_coop::kernel_ir_for(dt),
            &counts,
...
/// New-syntax benchmarks for `iron_moe_gather_qmm_coop`. Without a
/// `#[bench]` entry the kernel is invisible to `iron build --emit` (build
/// discovery walks the bench registry, not the test registry), so this is
/// required for the kernel to ever reach `IronKernels.swift`, not just for
