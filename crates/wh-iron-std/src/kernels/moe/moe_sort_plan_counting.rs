...
//! Bucketed counting-sort replacement for `iron_moe_sort_plan`
...
//!
//!   1. `iron_moe_sort_plan_hist`    - per-(block, expert) row counts.
//!   2. `iron_moe_sort_plan_offsets` - counts -> each block's per-expert
//!      write offset (base-by-expert prefix sum, then a running
//!      per-block offset within that expert's segment).
//!   3. `iron_moe_sort_plan_scatter` - each row looks up its block's
...
//! result is byte-identical to `iron_moe_sort_plan`'s stable order:
...
//! the caller's command buffer (see `Ops.moeSortPlanCounting` in Iron).
//! `n_experts` is capped at 256 (this model's expert count; matches the
//! existing scope limit on `iron_moe_build_tile_plan`'s threadgroup
//! scratch).
...
#[kernel]
pub fn iron_moe_sort_plan_hist(
    topk_ids: Tensor<u32>,
...
#[kernel]
pub fn iron_moe_sort_plan_offsets(
    ...
    // as `iron_moe_build_tile_plan`'s threadgroup scratch.
...
#[kernel]
pub fn iron_moe_sort_plan_scatter(
    topk_ids: Tensor<u32>,
...
pub mod kernel_tests {
    ...
    // iron_moe_sort_plan_hist: single-block sanity (block_size >= m_total,
    ...
    fn test_moe_sort_plan_hist(_dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_moe_sort_plan_hist::kernel_ir())
    ...
    // iron_moe_sort_plan_offsets: two blocks, hand-worked expected offsets.
    #[test_kernel(dtypes = [f32], tol = 0.0)]
    fn test_moe_sort_plan_offsets(_dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_moe_sort_plan_offsets::kernel_ir())
            .mode(KernelMode::Reduction)
    ...
    // iron_moe_sort_plan_scatter: two blocks of 6, block_offsets precomputed
    ...
    fn test_moe_sort_plan_scatter(_dt: DType) -> TestSetup {
        ...
        TestSetup::new(iron_moe_sort_plan_scatter::kernel_ir())
            .input(TestBuffer::from_vec("topk_ids", u32_bytes(&ids), DType::U32))
...
pub mod kernel_benches {
    ...
    fn bench_moe_sort_plan_hist(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_sort_plan_hist::kernel_ir())
            .mode(KernelMode::Reduction)
    ...
    fn bench_moe_sort_plan_offsets(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_sort_plan_offsets::kernel_ir())
            .mode(KernelMode::Reduction)
    ...
    fn bench_moe_sort_plan_scatter(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_sort_plan_scatter::kernel_ir())
            .buffer(BenchBuffer::from_vec("topk_ids", u32_bytes(&ids), DType::U32))
