...
//! MoE grouped-gather quantized matmul — the per-expert weight×activation
//! cell that runs inside each routed expert call. CSR-routed scalar forms
//! (int4 + b{3,5,6,8}), the m8/m16/m32 short-prefill tilings, and the
//! simdgroup-MMA forms (int4 / int8 / b{3,5,6,8}, bm16). Per-row `indices`
//! or CSR `expert_offsets` select each token's expert.

use wh_iron::kernel;

// ── iron_moe_gather_qmm_int4 ────────────────────────────────────────────────
//
...
#[kernel]
pub fn iron_moe_gather_qmm_int4<T>(
    x: Tensor<T>,
...
// ── iron_moe_gather_qmm_int2 ────────────────────────────────────────────────
//
// Scalar m1 gather for affine int2 (Hy3 oQ2). Per-row `expert_ids[row]`
// selects the expert slab (token-order or expert-sorted). Sibling of
// `iron_moe_gather_qmm_int4` but indices are a flat `[T]` u32 vector, not CSR
// offsets — matches the Path B sorted plan consumed by Iron.
//
// Packing: 16 int2 codes per u32, LSB-first. Affine dequant `s*q+b`.
// DISPATCH: Reduction; grid `[m_out, T, 1]`; TG `[32, 1, 1]`.
#[kernel]
pub fn iron_moe_gather_qmm_int2<T>(
    x: Tensor<T>,
    weight_packed: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    ...
    let row = tgid_y;
    let lane = tid;
    let _ = n_experts; // bounds are caller's responsibility
    let expert = load(expert_ids[row]);
    let total_packs = k_in / 16u32;
    ...
    for pack_idx in range(lane, total_packs, 32u32) {
        let packed = load(weight_packed[weight_row_base + pack_idx]);
        let k_first = pack_idx * 16u32;
        let g = k_first / group_size;
        let scale = load(scales[scale_row_base + g]).cast::<f32>();
        let bias = load(biases[scale_row_base + g]).cast::<f32>();
        for i in range(0u32, 16u32, 1u32) {
            let q = (packed >> (i * 2u32)) & 3u32;
...
// ── iron_moe_gather_qmm_b{3,5,6,8} — wider-precision gather matmul ──────────
//
// `iron_moe_gather_qmm_int4` above is int4-only (MLX's MoE quantization
...
//
// The body is identical to `iron_moe_gather_qmm_int4` except for the
...
// the same split as `dequant_gemv.rs` / the `iron_qmv_b*` family. The
// per-output-element routing, CSR `expert_offsets` walk, group-indexed
// scale/bias, and `simd_sum` reduction are unchanged.
//
// `iron_moe_gather_qmv_b*` aliases register the same kernels under a
...
// `iron_moe_gather_qmm_int4`; see its doc block. `k_in` must be a
...
#[kernel]
pub fn iron_moe_gather_qmm_b8<T>(
    x: Tensor<T>,
    weight_packed: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
///
/// Produces kernels: `iron_moe_gather_qmm_b3`, `_b5`, `_b6`.
...
// ── iron_moe_gather_qmm_int4_m8 ────────────────────────────────────────────
//
// Same recurrence as `iron_moe_gather_qmm_int4`, but each TG produces 8
...
pub fn iron_moe_gather_qmm_int4_m8<T>(
    x: Tensor<T>,
    weight_packed: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
// ── iron_moe_gather_qmm_int4_m16 ────────────────────────────────────────────
//
// Same pattern as `iron_moe_gather_qmm_int4_m8`, extended to 16
...
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_moe_gather_qmm_int4_m16<T>(
    x: Tensor<T>,
    weight_packed: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
// ── iron_moe_gather_qmm_int4_m32 ────────────────────────────────────────────
//
// Same pattern as `iron_moe_gather_qmm_int4_m8`, extended to 32
...
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_moe_gather_qmm_int4_m32<T>(
    x: Tensor<T>,
    weight_packed: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
//   - bits = 4 only
//
// We use 4 SGs (vs MLX's 2) because the existing iron_qmm_mma proves the
// 4-SG 2×2 warp grid hits ~95% of MLX throughput on the same 8×8 frag
// path. MoE inherits that geometry.
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_moe_gather_qmm_mma_int4<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    ...
    // 4 SGs in 2×2 warp grid (matches iron_qmm_mma).
    let sm = sg / 2u32;
    let sn = sg & 1u32;
    ...
    // Skew = +4 on BK for bank-conflict avoidance (see iron_qmm_mma).
...
// ── iron_moe_gather_qmm_mma_b{3,5,6,8} — wider-precision MMA gather matmul ──
//
// Bit-width-generalized siblings of `iron_moe_gather_qmm_mma_int4`. Same
...
///
/// Produces kernels: `iron_moe_gather_qmm_mma_b3`, `_b5`, `_b6`, `_b8`.
/// Same BM=BN=BK=32, 4-SG, 2×2 warp-grid geometry as `iron_moe_gather_qmm_mma_int4`.
...
#[kernel(variants(BITS = [2, 3, 5, 6, 8], suffix = "b{BITS}"))]
#[allow(clippy::too_many_arguments)]
...
// ── iron_moe_gather_qmm_mma_int4_bm16 ────────────────────────────────────────
//
// Half-height MMA grouped quantized matmul — BM=16 variant of
// `iron_moe_gather_qmm_mma_int4`. Matches MLX's `affine_gather_qmm_rhs_nt`
...
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_moe_gather_qmm_mma_int4_bm16<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    ...
    // 64 lanes × 8 strides each (= matches iron_qmm_mma_m16).
...
mod dump_audit_bm16 {
    ...
    /// differs from `iron_gated_delta_prep_chunk`'s `state_reg`: the
    ...
    /// `cargo test -p wh-iron-std --lib --release -- kernels::moe::moe_gather_qmm::dump_audit_bm16::dump --nocapture`
    #[test]
    fn dump() {
        use wh_iron::codegen::msl::MslGenerator;
        let mut k = iron_moe_gather_qmm_mma_int4_bm16::kernel_ir_for(DType::F16);
...
// ── iron_moe_gather_qmm_mma_int2_bm16 ────────────────────────────────────────
//
// Half-height MMA grouped quantized matmul — BM=16 variant of
// `iron_moe_gather_qmm_mma_int4`. Matches MLX's `affine_gather_qmm_rhs_nt`
// at WM=1 WN=2 (2 SGs, 64 tpg).
...
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_moe_gather_qmm_mma_int2_bm16<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
    ...
    // Coop-load lane assignments — X tile is 16×32 = 512 elements,
    // 64 lanes × 8 strides each (= matches iron_qmm_mma_m16).
    // W tile is 32×32 = 1024 elements / 16 int2 codes per pack = 64 packs,
    // 64 lanes x 1 pack each.
    // int2: one pack (16 codes) per lane; 64 lanes cover BN*2 packs for BK=32.
    let w_row_in_tg = lane_in_tg / 2u32;
    let pack_in_row = lane_in_tg & 1u32;
    let xs_ld = 40u32; // BK_padded — see threadgroup_alloc comment above
    let ws_ld = 40u32;
    let m_tile_base = m_tile * 16u32;
    let n_tile_base = n_tile * 32u32;
    let packs_per_row = k_in / 16u32;
    let groups_per_row = k_in / group_size;
    // Walk the TG's BM=16 rows in contiguous expert runs. Up to 16
    // sub-runs (one per row worst case); typical 1-2 at production
    // T*topk shapes.
    ...
    for _sub_iter in range(0u32, 16u32, 1u32) {
        ...
        if cur_valid {
            ...
            for kb in range(0u32, k_in, 32u32) {
                ...
                threadgroup_store("xs", mr6 * xs_ld + kc6, xv6 * m6);
                threadgroup_store("xs", mr7 * xs_ld + kc7, xv7 * m7);
                // W dequant — 64 lanes x 1 int2 pack (16 codes, affine s*q+b).
                let pack_row = n_tile_base + w_row_in_tg;
                let pack_dev = w_expert_base + pack_row * packs_per_row + kb / 16u32 + pack_in_row;
                let p = load(w[pack_dev]);
                let k_off = kb + pack_in_row * 16u32;
                let g = k_off / group_size;
                let sb_base = sb_expert_base + pack_row * groups_per_row;
                let s = load(scales[sb_base + g]).cast::<f32>();
                let b = load(biases[sb_base + g]).cast::<f32>();
                let q0 = (p & 3u32).cast::<f32>();
                let q1 = ((p >> 2u32) & 3u32).cast::<f32>();
                let q2 = ((p >> 4u32) & 3u32).cast::<f32>();
                let q3 = ((p >> 6u32) & 3u32).cast::<f32>();
                let q4 = ((p >> 8u32) & 3u32).cast::<f32>();
                let q5 = ((p >> 10u32) & 3u32).cast::<f32>();
                let q6 = ((p >> 12u32) & 3u32).cast::<f32>();
                let q7 = ((p >> 14u32) & 3u32).cast::<f32>();
                let q8 = ((p >> 16u32) & 3u32).cast::<f32>();
                let q9 = ((p >> 18u32) & 3u32).cast::<f32>();
                let q10 = ((p >> 20u32) & 3u32).cast::<f32>();
                let q11 = ((p >> 22u32) & 3u32).cast::<f32>();
                let q12 = ((p >> 24u32) & 3u32).cast::<f32>();
                let q13 = ((p >> 26u32) & 3u32).cast::<f32>();
                let q14 = ((p >> 28u32) & 3u32).cast::<f32>();
                let q15 = ((p >> 30u32) & 3u32).cast::<f32>();
                let wb = w_row_in_tg * ws_ld + pack_in_row * 16u32;
                threadgroup_store("ws", wb, (s * q0 + b).cast::<T>());
                threadgroup_store("ws", wb + 1u32, (s * q1 + b).cast::<T>());
                threadgroup_store("ws", wb + 2u32, (s * q2 + b).cast::<T>());
                threadgroup_store("ws", wb + 3u32, (s * q3 + b).cast::<T>());
                threadgroup_store("ws", wb + 4u32, (s * q4 + b).cast::<T>());
                threadgroup_store("ws", wb + 5u32, (s * q5 + b).cast::<T>());
                threadgroup_store("ws", wb + 6u32, (s * q6 + b).cast::<T>());
                threadgroup_store("ws", wb + 7u32, (s * q7 + b).cast::<T>());
                threadgroup_store("ws", wb + 8u32, (s * q8 + b).cast::<T>());
                threadgroup_store("ws", wb + 9u32, (s * q9 + b).cast::<T>());
                threadgroup_store("ws", wb + 10u32, (s * q10 + b).cast::<T>());
                threadgroup_store("ws", wb + 11u32, (s * q11 + b).cast::<T>());
                threadgroup_store("ws", wb + 12u32, (s * q12 + b).cast::<T>());
                threadgroup_store("ws", wb + 13u32, (s * q13 + b).cast::<T>());
                threadgroup_store("ws", wb + 14u32, (s * q14 + b).cast::<T>());
                threadgroup_store("ws", wb + 15u32, (s * q15 + b).cast::<T>());
                threadgroup_barrier();
                // MMA inner — 4 frags × 4 k_inner = 16 MMAs per SG.
                // sm=0 (WM=1 → both SGs share rows 0..15).
                let row_a0 = sm * 16u32 + fm;
...
// ── iron_moe_gather_qmm_mma_int8 — pack-aligned int8 MoE MMA BGEMM ────────
//
// Simdgroup-matrix MoE BGEMM for int8-quantized weights. Same tiled-MMA
// geometry as `iron_moe_gather_qmm_mma_int4` (BM=BN=BK=32, 4 SGs, 2×2 warp
...
#[kernel]
#[allow(clippy::too_many_arguments)]
pub fn iron_moe_gather_qmm_mma_int8<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
/// (`iron_moe_gather_qmm_int4` + the `m8` multi-cell sibling). The wider
...
pub mod kernel_tests {
    ...
    fn test_moe_gather_qmm_int4(dt: DType) -> TestSetup {
        int4_setup(iron_moe_gather_qmm_int4::kernel_ir_for(dt), 8, dt)
    }

    // m8: grid_x = m_out / 8 = 8 / 8 = 1 TG of 8 m-cells per x row.
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_moe_gather_qmm_int4_m8(dt: DType) -> TestSetup {
        int4_setup(iron_moe_gather_qmm_int4_m8::kernel_ir_for(dt), 1, dt)
    }

    /// Hy3 scalar m1 int2 gather (per-row expert_ids, not CSR).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_moe_gather_qmm_int2(dt: DType) -> TestSetup {
        let n_experts = 3usize;
        let k_in = 64usize;
        let m_out = 8usize;
        let group_size = 32usize;
        let t_rows = 6usize;
        // Rows [0..2)→e0, [2..5)→e1, [5..6)→e2.
        let expert_ids: Vec<u32> = vec![0, 0, 1, 1, 1, 2];
        let mut weight_unpacked = vec![0u32; n_experts * m_out * k_in];
        for (i, w) in weight_unpacked.iter_mut().enumerate() {
            *w = ((i as u32) * 7 + 3) & 0x3;
        }
        ...
        let b = unpack_f32(&pack_f32(&biases_f, dt), dt);
        let x = unpack_f32(&pack_f32(&x_f, dt), dt);
        // Oracle: per-row expert_ids form of int2 gather.
        let mut expected = vec![0.0f32; t_rows * m_out];
        let packs_per_row = k_in / 16;
        for row in 0..t_rows {
            let expert = expert_ids[row] as usize;
            for m in 0..m_out {
                let wrb = expert * m_out * packs_per_row + m * packs_per_row;
                let srb = expert * m_out * n_groups + m * n_groups;
                let mut acc = 0.0f32;
                for pack_idx in 0..packs_per_row {
                    let packed = weight_packed[wrb + pack_idx];
                    ...
                    for i in 0..16 {
                        ...
                        acc += (q * scale + bias) * x[row * k_in + k_first + i];
                    }
                }
                expected[row * m_out + m] = acc;
            }
        }
        TestSetup::new(iron_moe_gather_qmm_int2::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .input(TestBuffer::from_vec("x", pack_f32(&x_f, dt), dt))
            .input(TestBuffer::from_vec("weight_packed", u32_bytes(&weight_packed), DType::U32))
            .input(TestBuffer::from_vec("scales", pack_f32(&scales_f, dt), dt))
            .input(TestBuffer::from_vec("biases", pack_f32(&biases_f, dt), dt))
            .input(TestBuffer::from_vec("expert_ids", u32_bytes(&expert_ids), DType::U32))
            .input(TestBuffer::zeros("out", t_rows * m_out, dt))
            .constexpr("k_in", k_in as u32)
            .constexpr("m_out", m_out as u32)
            .constexpr("n_experts", n_experts as u32)
    ...
    fn test_moe_gather_qmm(dt: DType) -> TestSetup {
        gather_qmm_bits_setup(iron_moe_gather_qmm_bBITS::kernel_ir_for(dt), BITS, dt)
    }
    ...
    fn test_moe_gather_qmm_mma_int4(dt: DType) -> TestSetup {
        mma_setup(iron_moe_gather_qmm_mma_int4::kernel_ir_for(dt), 4, 32, 128, dt)
    }
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_moe_gather_qmm_mma_int4_bm16(dt: DType) -> TestSetup {
        mma_setup(iron_moe_gather_qmm_mma_int4_bm16::kernel_ir_for(dt), 4, 16, 64, dt)
    }
    /// Hy3 oQ2 affine int2 BM=16 gather (sibling of int4_bm16).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_moe_gather_qmm_mma_int2_bm16(dt: DType) -> TestSetup {
        mma_setup(iron_moe_gather_qmm_mma_int2_bm16::kernel_ir_for(dt), 2, 16, 64, dt)
    }
    ...
    fn test_moe_gather_qmm_mma_int8(dt: DType) -> TestSetup {
        mma_setup(iron_moe_gather_qmm_mma_int8::kernel_ir_for(dt), 8, 32, 128, dt)
    }
    /// Bitstream `b2` MMA gather (sibling of b3/b5/b6/b8).
    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_moe_gather_qmm_mma_b2(dt: DType) -> TestSetup {
        mma_setup(iron_moe_gather_qmm_mma_b2::kernel_ir_for(dt), 2, 32, 128, dt)
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_moe_gather_qmm_int4(dt: DType) -> BenchSetup {
        csr_bench(iron_moe_gather_qmm_int4::kernel_ir_for(dt), 4, 1, 64, 2048, 256, 128, 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_moe_gather_qmm_int4_m8(dt: DType) -> BenchSetup {
        csr_bench(iron_moe_gather_qmm_int4_m8::kernel_ir_for(dt), 4, 8, 64, 2048, 256, 128, 64, dt)
    }
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_moe_gather_qmm_int4_m16(dt: DType) -> BenchSetup {
        csr_bench(
            iron_moe_gather_qmm_int4_m16::kernel_ir_for(dt),
            4,
            16,
            64,
            2048,
    ...
    fn bench_moe_gather_qmm_int4_m32(dt: DType) -> BenchSetup {
        csr_bench(
            iron_moe_gather_qmm_int4_m32::kernel_ir_for(dt),
            4,
            32,
            64,
            2048,
    ...
    fn bench_moe_gather_qmm(dt: DType) -> BenchSetup {
        csr_bench(iron_moe_gather_qmm_bBITS::kernel_ir_for(dt), BITS, 1, 64, 2048, 256, 128, 64, dt)
    }
    ...
    fn bench_moe_gather_qmm_mma_int4(dt: DType) -> BenchSetup {
        mma_bench(
            iron_moe_gather_qmm_mma_int4::kernel_ir_for(dt),
            4,
            32,
            32,
            128,
    ...
    fn bench_moe_gather_qmm_mma(dt: DType) -> BenchSetup {
        mma_bench(
            iron_moe_gather_qmm_mma_bBITS::kernel_ir_for(dt),
            BITS,
            32,
            32,
            128,
    ...
    fn bench_moe_gather_qmm_mma_int8(dt: DType) -> BenchSetup {
        mma_bench(
            iron_moe_gather_qmm_mma_int8::kernel_ir_for(dt),
            8,
            32,
            32,
            128,
    ...
    fn bench_moe_gather_qmm_mma_int4_bm16(dt: DType) -> BenchSetup {
        mma_bench(
            iron_moe_gather_qmm_mma_int4_bm16::kernel_ir_for(dt),
            4,
            32,
            16,
            64,
    ...
    /// Hy3 oQ2 affine int2 BM=16 gather (sibling of int4_bm16).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_moe_gather_qmm_mma_int2_bm16(dt: DType) -> BenchSetup {
        mma_bench(
            iron_moe_gather_qmm_mma_int2_bm16::kernel_ir_for(dt),
            2,
            32,
            16,
            64,
    ...
    // ── Per-row `expert_ids` reduction gather (int2 M1 fallback) ──────────
    // ABI: x, weight_packed, scales, biases, expert_ids, out + {k_in, m_out,
    // n_experts, group_size}. Grid [m_out, T, 1], tpg [32,1,1]. Distinct from
    // `csr_bench` above (that ABI uses `expert_offsets`, not per-row ids).
    #[allow(clippy::too_many_arguments)]
    fn indexed_bench(
        kernel: Kernel,
        bits: u32,
        ...
        BenchSetup::new(kernel)
            ...
            .buffer(BenchBuffer::random(
                "weight_packed",
                n_experts * m_out * words_per_row,
                DType::U32,
            ))
            .buffer(BenchBuffer::random("scales", n_experts * m_out * groups_per_row, dt))
            .buffer(BenchBuffer::random("biases", n_experts * m_out * groups_per_row, dt))
            .buffer(BenchBuffer::zeros("expert_ids", t_rows, DType::U32))
            .buffer(BenchBuffer::zeros("out", t_rows * m_out, dt).output())
            .constexpr("k_in", k_in as u32)
            .constexpr("m_out", m_out as u32)
            .constexpr("n_experts", n_experts as u32)
    ...
    /// Hy3 oQ2 affine int2 reduction-style gather (non-tiled M1 fallback).
    #[bench(dtypes = [f32, f16, bf16])]
    fn bench_moe_gather_qmm_int2(dt: DType) -> BenchSetup {
        indexed_bench(iron_moe_gather_qmm_int2::kernel_ir_for(dt), 2, 64, 2048, 256, 128, 64, dt)
    }
}
