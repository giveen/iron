...
//! Active-only expert-outer MPP gather — `iron_moe_gather_qmm_mma_eg_int2_active_mpp`.
//!
//! Like `moe_mpp_expert_grid` but `tgid_y` indexes a compact active list
//! (`active_experts` / `row_lo` / `row_hi` from `iron_moe_compact_active_experts`)
...
pub fn iron_moe_gather_qmm_mma_eg_int2_active_mpp<T>(
    x: Tensor<T>,
    w: Tensor<u32>,
    scales: Tensor<T>,
    biases: Tensor<T>,
...
pub mod kernel_tests {
    ...
    use super::iron_moe_gather_qmm_mma_eg_int2_active_mpp;
    use crate::{
    ...
    fn test_moe_gather_qmm_mma_eg_int2_active_mpp(dt: DType) -> TestSetup {
        ...
        let base = int2_indexed_setup_with_indices(
            iron_moe_gather_qmm_mma_eg_int2_active_mpp::kernel_ir_for(dt),
        ...
        TestSetup::new(iron_moe_gather_qmm_mma_eg_int2_active_mpp::kernel_ir_for(dt))
...
pub mod kernel_benches {
    ...
    fn bench_moe_gather_qmm_mma_eg_int2_active_mpp(dt: DType) -> BenchSetup {
        ...
        let bytes = n_experts * n_out * words_per_row * 4
            ...
            + m_total * k_in * sz
            + m_total * n_out * sz;
        BenchSetup::new(iron_moe_gather_qmm_mma_eg_int2_active_mpp::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
