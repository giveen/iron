...
//! Device-side tile-plan builder for `iron_moe_gather_qmm_coop`
//! (`moe_gather_qmm_coop.rs`, BM=32). `iron_moe_build_tile_plan_bm32`.
//!
//! BM=32 sibling of `iron_moe_build_tile_plan` (`moe_tile_plan_builder.rs`).
...
//! path is selected (see `IRON_MOE_COOP` in the Swift wiring).
//!
...
//! `MTLDispatchThreadgroupsIndirectArguments` for `iron_moe_gather_qmm_coop`'s
...
/// with.
#[kernel]
pub fn iron_moe_build_tile_plan_bm32(
...
mod tests {
    ...
    fn kernel_ir_constructs_without_inline_msl() {
        let k = iron_moe_build_tile_plan_bm32::kernel_ir();
        assert_eq!(k.name, "iron_moe_build_tile_plan_bm32");
        let all_ops =
...
pub mod kernel_benches {
    ...
    use super::iron_moe_build_tile_plan_bm32;
    ...
    fn bench_moe_build_tile_plan_bm32(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_build_tile_plan_bm32::kernel_ir())
