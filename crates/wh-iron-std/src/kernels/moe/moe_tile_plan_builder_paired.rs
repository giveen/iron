...
//! feeds `iron_moe_gather_qmm_coop_paired` (`moe_gather_qmm_coop_paired.rs`).
...
//!    the NEW `iron_moe_build_tile_plan_bm32_own` device kernel) - every
//!    exact-multiple-of-32 run and every `17..=31`-row remainder, fed to
//!    the COMPLETELY UNCHANGED existing `iron_moe_gather_qmm_coop` GEMM
...
//!    NEW `iron_moe_gather_qmm_coop_paired` kernel via a SECOND, separate
//!    dispatch.
//!
...
/// Host mirror of the NEW `iron_moe_build_tile_plan_bm32_own` device
...
/// oracle for the existing `iron_moe_gather_qmm_coop` GEMM kernel.
...
/// Host mirror of `iron_moe_build_tile_plan_bm32_paired`: the `1..=16`-row
