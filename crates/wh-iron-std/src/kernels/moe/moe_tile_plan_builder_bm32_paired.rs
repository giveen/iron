...
//! Device-side PAIR-ONLY tile-plan builder for `iron_moe_gather_qmm_coop_paired`
//! (`moe_gather_qmm_coop_paired.rs`). `iron_moe_build_tile_plan_bm32_paired`.
//!
...
//! (feeding only the `iron_moe_gather_qmm_coop_paired` kernel) rather than
//! folding into the existing `iron_moe_build_tile_plan_bm32` device
...
//!   capacity.
//! - The companion "own" dispatch (`iron_moe_gather_qmm_coop` fed by
//!   `iron_moe_build_tile_plan_bm32` over `own_counts`) MUST use the SAME
...
pub fn iron_moe_build_tile_plan_bm32_paired(
...
mod tests {
    ...
    fn kernel_ir_constructs_without_inline_msl() {
        let k = iron_moe_build_tile_plan_bm32_paired::kernel_ir();
        assert_eq!(k.name, "iron_moe_build_tile_plan_bm32_paired");
        let all_ops =
...
pub mod kernel_benches {
    ...
    use super::iron_moe_build_tile_plan_bm32_paired;
    ...
    fn bench_moe_build_tile_plan_bm32_paired(_dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_build_tile_plan_bm32_paired::kernel_ir())
