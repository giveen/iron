...
//!
//! ## Inputs (split format — produced by the Iron loader)
//!
//! ```text
...
//! `gguf_dequant_iq2_xxs::iron_gguf_dequant_iq2_xxs` (the proven,
//! production reference) — see that file for the per-element derivation.

use wh_iron::kernel;

#[kernel]
pub fn iron_moe_gather_gemv_iq2xxs<T>(
    x: Tensor<T>,
    qs_all: Tensor<u32>,
    d_all: Tensor<f32>,
...
pub mod kernel_tests {
    ...
    use super::iron_moe_gather_gemv_iq2xxs;

    #[test]
    fn codegen_gather_gemv_iq2xxs_smoke() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let ir = iron_moe_gather_gemv_iq2xxs::kernel_ir_for(dt);
...
pub mod kernel_benches {
    ...
    fn bench_gather_gemv_iq2xxs(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_moe_gather_gemv_iq2xxs::kernel_ir_for(dt))
