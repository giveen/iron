...
//! MPP-backed MoE grouped int4 BGEMM — `iron_moe_gather_qmm_mma_int4_bm64_mpp`.
//!
...
/// → `iron_moe_gather_qmm_mma_int{2,4,8}_bm64_mpp`. Same weight-unpack
    ...
    for _sub_iter in range(0u32, 64u32, 1u32) {
        ...
        if cur_valid {
            ...
            for kb in range(0u32, k_in, 32u32) {
                ...
                // Dequant W → Ws. 128 lanes × `packs_per_lane` packs/lane;
                // `vals_per_pack` codes/pack over the BK=32 slice.
                let packs_in_row = 32u32 / vals_per_pack; // BK=32 → packs per BN row
                let packs_per_lane = 16u32 / vals_per_pack; // (BN*BK/vals)/128 lanes
                let mask = (1u32 << BITS) - 1u32;
                for _pi in range(0u32, packs_per_lane, 1u32) {
                    let pack_id = lane_in_tg * packs_per_lane + _pi;
                    let w_row = pack_id / packs_in_row; // 0..63 (BN rows)
                    let pack_in_row = pack_id % packs_in_row; // which u32 in the BK=32 slice
...
mod tests {
    ...
    fn kernel_ir_constructs_and_uses_coop_tile_ops() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            let k = iron_moe_gather_qmm_mma_int4_bm64_mpp::kernel_ir_for(dt);
    ...
    fn bf16_stages_through_half() {
        let k = iron_moe_gather_qmm_mma_int4_bm64_mpp::kernel_ir_for(DType::BF16);
...
pub mod kernel_tests {
    ...
    use super::{
        iron_moe_gather_qmm_mma_int2_bm64_mpp,
        iron_moe_gather_qmm_mma_int4_bm64_mpp,
        iron_moe_gather_qmm_mma_int8_bm64_mpp,
    };
    ...
    fn test_moe_gather_qmm_mma_int2_bm64_mpp(dt: DType) -> TestSetup {
        int2_indexed_setup(
            iron_moe_gather_qmm_mma_int2_bm64_mpp::kernel_ir_for(dt),
    ...
    fn test_moe_gather_qmm_mma_int4_bm64_mpp(dt: DType) -> TestSetup {
        int4_indexed_setup(
            iron_moe_gather_qmm_mma_int4_bm64_mpp::kernel_ir_for(dt),
            MmaTestShape { n_experts: 4, m_total: 64, n_out: 64, k_in: 64, group_size: 32 },
            64,
            64,
            128,
    ...
    fn test_moe_gather_qmm_mma_int8_bm64_mpp(dt: DType) -> TestSetup {
        int8_indexed_setup(
            iron_moe_gather_qmm_mma_int8_bm64_mpp::kernel_ir_for(dt),
            MmaTestShape { n_experts: 4, m_total: 64, n_out: 64, k_in: 64, group_size: 32 },
            64,
            64,
            128,
