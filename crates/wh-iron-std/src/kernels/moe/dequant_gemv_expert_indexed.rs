...
#[kernel]
pub fn iron_dequant_gemv_int4_expert_indexed<T>(
...
#[kernel]
pub fn iron_dequant_gemv_int2_expert_indexed<T>(
    weights_stacked: Tensor<u32>,
    scales_stacked: Tensor<T>,
    biases_stacked: Tensor<T>,
    input: Tensor<T>,
...
pub mod kernel_tests {
    use wh_iron::{prelude::Kernel, test::*, test_kernel};

    use super::{iron_dequant_gemv_int2_expert_indexed, iron_dequant_gemv_int4_expert_indexed};
    ...
    fn test_dequant_gemv_int4_expert_indexed(dt: DType) -> TestSetup {
        setup_bits(4, dt, iron_dequant_gemv_int4_expert_indexed::kernel_ir_for(dt))
    }

    #[test_kernel(dtypes = [f32, f16, bf16], tol = [5e-3, 5e-2, 2e-1])]
    fn test_dequant_gemv_int2_expert_indexed(dt: DType) -> TestSetup {
        setup_bits(2, dt, iron_dequant_gemv_int2_expert_indexed::kernel_ir_for(dt))
    }
}
...
pub mod kernel_benches {
    ...
    fn bench_dequant_gemv_int4_expert_indexed(dt: DType) -> BenchSetup {
        ...
        BenchSetup::new(iron_dequant_gemv_int4_expert_indexed::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
    ...
    fn bench_dequant_gemv_int2_expert_indexed(dt: DType) -> BenchSetup {
        ...
        let bytes =
            out_dim * packs_per_row * 4 + 2 * out_dim * n_groups * sz + in_dim * sz + out_dim * sz;
        BenchSetup::new(iron_dequant_gemv_int2_expert_indexed::kernel_ir_for(dt))
            .mode(KernelMode::Reduction)
            .buffer(BenchBuffer::random(
                "weights_stacked",
                n_experts * out_dim * packs_per_row,
