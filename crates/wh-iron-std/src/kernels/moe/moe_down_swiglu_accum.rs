...
//!
//! Collapses the three back-to-back dispatches Iron's MoE GPU-router
//! decode path runs per layer (under `IRON_MOE_GPU_ROUTER`, default-on
//! as of ITER 80) into ONE kernel launch:
//!
//!   1. `iron_swiglu` (many=8): `inner[k][d] = silu(gate[k][d]) * up[k][d]`
//!   2. `iron_dequant_gemv_int4_expert_indexed` (many=8): per slot k,
//!      `down_out[k] = W_down[expert[k]] · inner[k]`  (out_dim = hidden)
//!   3. `iron_scalar_fma_chain8`:
...
//! `iron_moe_router_topk` (k=8, packed). The 16 gate/up tensors mirror
//! Iron's per-slot scratch caches: one `Tensor.empty([moeIntermediate])`
...
//!
//!   for k in 0..8: tmp_k = iron_swiglu(gate_k, up_k)
//!   for k in 0..8: down_k = iron_dequant_gemv_int4_expert_indexed(
//!                              W, S, B, tmp_k, expert_indices[k:k+1])
//!   out = iron_scalar_fma_chain8(slot_weights[0:1], down_0, ...,
...
//! `wh-iron-macros/src/body_parser.rs:210`) — they'd silently
...
///   (verified via `iron build --emit` diff across f32/f16/bf16).
...
macro_rules! define_moe_down_swiglu_accum_chain8 {
    (
        ...
        pub fn iron_moe_down_swiglu_accum_int4_chain8<T>(
            gate_0: Tensor<T>,
            up_0: Tensor<T>,
            gate_1: Tensor<T>,
            up_1: Tensor<T>,
            ...
            // Int4 dequant constants, match `iron_dequant_gemv_int4_expert_indexed`.
...
/// (`iron_moe_down_swiglu_accum_int4_chain8`). The 8-way fusion has a clean
...
pub mod kernel_tests {
    ...
    use super::iron_moe_down_swiglu_accum_int4_chain8;
    ...
    fn setup(
        ...
        let mut su = TestSetup::new(iron_moe_down_swiglu_accum_int4_chain8::kernel_ir_for(dt))
...
/// (`iron_moe_down_swiglu_accum_int4_chain8`). Bench-only: the 8-way
...
/// Iron integration tests and by the in-source `#[test_kernel]`s against the
...
pub mod kernel_benches {
    ...
    fn bench_moe_down_swiglu_accum_int4_chain8(dt: DType) -> BenchSetup {
        ...
        let mut bs = BenchSetup::new(iron_moe_down_swiglu_accum_int4_chain8::kernel_ir_for(dt))
            .mode(KernelMode::Reduction);
