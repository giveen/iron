...
use wh_iron_std::kernels::moe::moe_gather_gemv_iq2xxs::iron_moe_gather_gemv_iq2xxs;
...
/// `iron_gguf_dequant_iq2_xxs` exactly.
fn reference(
    x: &[f32],
    qs_all: &[u32],
    d_all: &[f32],
...
fn run_gpu(
    ...
    let mut kernel = iron_moe_gather_gemv_iq2xxs::kernel_ir_for(dt.to_dtype());
