...
//! Batched partial RoPE (iron_partial_rope) must equal the per-token
...
use wh_iron_std::kernels::rope::partial_rope::iron_partial_rope;
...
fn partial_rope_matches_per_token_oracle() {
    ...
    let mut k = iron_partial_rope::kernel_ir_for(Dt::F32.to_dtype());
