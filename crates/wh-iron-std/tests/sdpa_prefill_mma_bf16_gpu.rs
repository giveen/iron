...
//! GPU correctness oracle for `iron_sdpa_prefill_mma_bf16` — the
//! bf16-emulated MMA prefill variant in
//! `crates/wh-iron-std/src/mlx/steel/attn/steel_attention_mma_bf16.rs`.
//!
//! The bf16 variant is the M2-family bf16 routing target of
//! `sdpa_prefill_mma_for` (`is_pre_m3_bf16` arm). Its sibling
//! `iron_sdpa_prefill_mma` already has long-T + batched coverage in
...
//! file closes it.
//!
//! The kernel signature mirrors `iron_sdpa_prefill_mma`: same Q/K/V/O
...
use wh_iron::Context;
use wh_iron_std::kernels::sdpa::steel_attn::steel_attention_mma_bf16::iron_sdpa_prefill_mma_bf16;
...
fn run_sdpa_prefill_bf16(
    ...
    let mut kernel = iron_sdpa_prefill_mma_bf16::kernel_ir_for(dt.to_dtype());
    ...
    kernel.mode = wh_iron::core::ir::KernelMode::SimdGroup2D;
...
#[test]
fn iron_sdpa_prefill_mma_bf16_matches_cpu_reference_bf16_t512() {
    let _g = gpu_lock();
...
#[test]
fn iron_sdpa_prefill_mma_bf16_gqa_factor_4_bf16_t512() {
    let _g = gpu_lock();
...
#[test]
fn iron_sdpa_prefill_mma_bf16_kernel_side_b2_t512_bf16() {
    let _g = gpu_lock();
