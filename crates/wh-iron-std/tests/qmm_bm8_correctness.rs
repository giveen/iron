...
//! iron_qmm_bm8` — the scalar (non-MMA) BM=8 extension of `iron_qmm_bm4`.
//! Sized for T≈1-8 speculative-decode verify batches: motivated by
//! `iron_qmm_mma_bm8` (the simdgroup-matrix MMA BM=8 kernel) LOSING to
//! `iron_qmm_bm4` at T=8 — this kernel tests whether a plain scalar
//! extension of bm4's own geometry to BM=8 (halving W traffic again,
//! no MMA / no TG-memory staging) does better.
//!
//! Mirrors `qmm_mma_bm8_correctness.rs`'s structure but at this kernel's
//! actual BN=8 tile (not BN=32 — that kernel is simdgroup-matrix, this
//! one is the scalar 2 SG × 4 N-row geometry shared by bm2/bm4/bm8).
//!
//! Coverage matrix:
...
//!
//! Run:
//!   cargo test --release -p wh-iron-std --test qmm_bm8_correctness -- --nocapture
...
use wh_iron_std::kernels::gemm::quantized::iron_qmm_bm8;

// ── Tile geometry — this kernel's actual BN=8 (2 SG × 4 N-rows), not the
//    BN=32 simdgroup-matrix tile of `iron_qmm_mma_bm8`. ──────────────────
const BM_TILE: usize = 8;
const BN_TILE: usize = 8;
...
/// Dispatch grid `[N/8, m_padded/8, 1]` for a logical token count `t`.
...
// ── Triple-loop CPU oracle — same algorithm as `qmm_mma_bm8_correctness
//    .rs::cpu_qmm_reference`, replicated for test-file isolation per this
//    crate's integration-test convention. ─────────────────────────────────
...
// ── Host-side dispatcher exercising `iron_qmm_bm8` directly. ─────────────
//
// Pads X (zero-fill) to m_padded = ceil(T/8)*8, dispatches `iron_qmm_bm8`
// with grid `[N/8, m_padded/8, 1]`, then slices the first `T * N`
// element-bytes of the output.
...
fn run_bm8(
    ...
    assert!(n.is_multiple_of(BN_TILE), "n must be multiple of 8 (BN tile)");
    assert!(k.is_multiple_of(256), "k must be multiple of 256 (BK step)");
    ...
    // kernel body references — mirrors `iron_qmm_bm4` / `iron_qmm_mma_bm8`.
    let mut kernel = iron_qmm_bm8::kernel_ir_for(dtype);
...
fn bm8_f16_t8_single_tile() {
    ...
    assert_eq!(dispatch_grid_bm8(t, n), [2, 1, 1], "T=8 should dispatch a single BM tile");
