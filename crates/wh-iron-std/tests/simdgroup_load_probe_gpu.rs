...
//!
//! The kernel under test (`iron_simdgroup_load_probe` in `probe::simdgroup_load_probe`)
...
use wh_iron_std::probe::simdgroup_load_probe::iron_simdgroup_load_probe;
...
#[test]
fn iron_simdgroup_load_probe_emits_simdgroup_load_instruction_f32() {
    let kernel = iron_simdgroup_load_probe::kernel_ir_for(DType::F32);
...
/// drop the primitive.
#[test]
fn iron_simdgroup_load_probe_emits_simdgroup_load_instruction_f16() {
    let kernel = iron_simdgroup_load_probe::kernel_ir_for(DType::F16);
    let msl_gen = MslGenerator::new(MslConfig::default());
    let msl = msl_gen.generate(&kernel).expect("MSL emit should succeed");
...
fn run_round_trip(dtype: Dt) {
    ...
    let kernel = iron_simdgroup_load_probe::kernel_ir_for(dtype.to_dtype());
...
#[test]
fn iron_simdgroup_load_probe_round_trips_8x8_tile_f32() { run_round_trip(Dt::F32); }

#[test]
fn iron_simdgroup_load_probe_round_trips_8x8_tile_f16() { run_round_trip(Dt::F16); }
