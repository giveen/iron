...
//! pipeline: `iron_gdn_wy_plan` (chunk-parallel) -> `iron_gdn_wy_scan`
//! (sequential state scan).
//!
//! Two independent checks:
//!
//! 1. **Pipeline vs. CPU oracle** at a production-ish shape (`Dv=Dk=128`,
//!    the shape the monolithic `iron_gated_delta_wy_chunk` kernel cannot
...
use wh_iron_std::kernels::ssm::{
    gated_delta_wy::iron_gated_delta_wy_chunk,
    gated_delta_wy_plan::iron_gdn_wy_plan,
    gated_delta_wy_scan::iron_gdn_wy_scan,
};
...
/// Runs `iron_gdn_wy_plan` then `iron_gdn_wy_scan` on the given inputs,
...
fn run_pipeline(
    ...
    let mut scan_k = iron_gdn_wy_scan::kernel_ir_for(dtype);
    ...
    let scan_r = ctx
        ...
        .expect("iron_gdn_wy_scan dispatch");
...
/// `iron_gated_delta_wy_chunk` kernel cannot dispatch — its `[Dv,Dk]`
...
/// `iron_gated_delta_wy_chunk` kernel, GPU output vs. GPU output, at a
...
fn gdn_wy_pipeline_matches_monolithic_kernel() {
    ...
    let dt = Dt::F32;
    let dtype = dt.to_dtype();
    // Dv must be a multiple of 32 for `iron_gdn_wy_scan`'s tile dispatch
...
/// Iron e2e prefill-quality campaign (dump-prefill-logits cosine gate
...
/// frontier). `IRON_GDN_WY` stays opt-in (not the prefill default)
/// because of it.
///
...
fn gdn_wy_pipeline_low_rank_correlated_t1024_no_blowup() {
    ...
    assert!(
        all_finite(&y_got),
        "iron_gdn_wy_plan/scan produced non-finite y on low-rank-correlated T=1024 input, \
         the catastrophic inf/NaN blowup class the plan-kernel f32-stack precision fix targeted \
         has regressed"
    );
    assert!(
        all_finite(&state_got),
        "iron_gdn_wy_plan/scan produced non-finite state_out on low-rank-correlated T=1024 \
...
/// with the same no-op-token convention Iron's `Ops.gatedDeltaWYPrefill`
