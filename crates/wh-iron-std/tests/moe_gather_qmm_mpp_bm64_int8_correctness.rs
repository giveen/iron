...
//! GPU correctness for `kernels::moe::moe_mpp_bm64::iron_moe_gather_qmm_mma_int8_bm64_mpp`.
//!
...
//! Validated against the scalar `iron_moe_gather_qmm_b8` oracle on clean-tile,
...
use wh_iron_std::kernels::moe::{moe_gather_qmm::iron_moe_gather_qmm_b8, moe_mpp_bm64};
...
fn moe_gather_qmm_mma_int8_bm64_mpp_matches_b8_clean_tile() {
    ...
    let y_ref = {
        ...
        let mut k = iron_moe_gather_qmm_b8::kernel_ir_for(Dt::F32.to_dtype());
    ...
    let y_mpp = {
        ...
        let ctx = Context::new().unwrap();
        let mut k =
            moe_mpp_bm64::iron_moe_gather_qmm_mma_int8_bm64_mpp::kernel_ir_for(Dt::F32.to_dtype());
        k.mode = KernelMode::Reduction;
    ...
    const CAL_MAX_DIFF: f32 = 3.0e-6; // observed 9.536743e-7
...
fn moe_gather_qmm_mma_int8_bm64_mpp_matches_b8_multi_tile() {
    ...
    // cosine alone would miss. Calibrated 2026-08-06 on Metal (M5 Max),
    // ~3x observed max|Δ| (filled after run).
    const CAL_MAX_DIFF: f32 = 8.0e-6; // observed 2.384186e-6
...
fn moe_gather_qmm_mma_int8_bm64_mpp_bf16_matches_b8_clean_tile() {
    ...
    let y_mpp = {
        ...
        let ctx = Context::new().unwrap();
        let mut k =
            moe_mpp_bm64::iron_moe_gather_qmm_mma_int8_bm64_mpp::kernel_ir_for(Dt::Bf16.to_dtype());
        k.mode = KernelMode::Reduction;
        let r = ctx
            .dispatch_with_grid(
                &k,
    ...
    // bf16 rounding dominates the error budget. Calibrated 2026-08-06 on
    // Metal (M5 Max), ~3x observed max|Δ| (filled after run).
    const CAL_BF16_MAX_DIFF: f32 = 4.0e-2; // observed 1.326013e-2
    assert!(
