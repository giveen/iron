...
//! GPU correctness for `iron_grouped_gemv_q8_rows_tiled` — the
//! token-TILED grouped Q8 gemv (8-fold weight-DRAM amortization). It must
//! equal the proven `iron_grouped_gemv_q8_rows` row-by-row: same grouped Q8
...
use wh_iron_std::kernels::gemm::gemv_quantized::{
...
fn grouped_gemv_q8_rows_tiled_matches_rows() {
    ...
    let mut kt = iron_grouped_gemv_q8_rows_tiled::kernel_ir_for(Dt::F32.to_dtype());
