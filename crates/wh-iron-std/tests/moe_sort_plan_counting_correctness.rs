...
//! replacement of `iron_moe_sort_plan` (`moe_sort_plan_counting.rs`):
//! `iron_moe_sort_plan_hist` -> `iron_moe_sort_plan_offsets` ->
//! `iron_moe_sort_plan_scatter`.
//!
//! `iron_moe_sort_plan`'s output feeds every downstream gather-GEMM, so
...
//! live GPU dispatch of the original `iron_moe_sort_plan` kernel on the
...
use wh_iron_std::kernels::moe::{
    ...
    moe_permute::iron_moe_sort_plan,
...
/// CPU stable-sort oracle: the exact semantics `iron_moe_sort_plan` (and
...
fn run_original(ctx: &Context, ids: &[u32], k: usize) -> (Vec<u32>, Vec<u32>, Vec<u32>) {
    ...
    let k_ir = iron_moe_sort_plan::kernel_ir();
...
fn check_case(ids: &[u32], k: usize, n_experts: usize, block_size: usize, label: &str) {
    ...
    assert_eq!(
        new_se, orig_se,
        "{label}: sorted_experts diverges from iron_moe_sort_plan's own output"
    );
    assert_eq!(
        new_st, orig_st,
        "{label}: source_tokens diverges from iron_moe_sort_plan's own output"
    );
    assert_eq!(new_ip, orig_ip, "{label}: inv_perm diverges from iron_moe_sort_plan's own output");
}
