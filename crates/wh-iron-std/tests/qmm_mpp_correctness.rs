...
//! GPU correctness oracle for `iron_qmm_mma_mpp` — the production int4
//! quantized matmul backed by `mpp::tensor_ops::matmul2d` (NAX path).
//!
//! Dispatches `iron_qmm_mma_mpp_{f32,f16}` over a small set of shapes
//! (single 32×32 tile + multi-tile / multi-K-block) and validates against
//! the same triple-loop CPU oracle once used by the legacy
//! `tests/qmm_gpu_correctness.rs` (since removed) for
//! `iron_qmm_mma`. Requires macOS 26+ / Metal 4 — the kernel includes
...
//!
//! Run:
//!   cargo test --release -p wh-iron-std --test qmm_mpp_correctness -- --nocapture
//!
//! Bench: on M2 mini per `feedback_wh_iron_bench_on_m2_mini.md`, never
//! on M5 Max under load. The bench harness lives in `wh-iron-cli` and is
...
use wh_iron_std::kernels::gemm::quantized_mpp::iron_qmm_mma_mpp;
...
/// in the legacy `tests/qmm_gpu_correctness.rs` (since removed). Replicated
...
fn run_qmm_mma_mpp(
    ...
    assert!(m.is_multiple_of(32), "iron_qmm_mma_mpp requires m %% 32 == 0 (BM=32 tile)");
    assert!(n.is_multiple_of(32), "iron_qmm_mma_mpp requires n %% 32 == 0 (BN=32 tile)");
    assert!(k.is_multiple_of(32), "iron_qmm_mma_mpp requires k %% 32 == 0 (BK=32 step)");
    ...
    let mut kernel = iron_qmm_mma_mpp::kernel_ir_for(dtype);
    kernel.mode = KernelMode::Reduction;

    let result = ctx
        .dispatch_with_grid(&kernel, &buffers, &BTreeMap::new(), [n / 32, m / 32, 1], [128, 1, 1])
        .expect("dispatch iron_qmm_mma_mpp");
...
/// Deterministic q4 weights — same per-pack pattern as `iron_qmm` /
/// `iron_qmm_mma` correctness tests so we exercise an identical bit layout.
...
#[test]
fn iron_qmm_mma_mpp_matches_cpu_reference_f32_small() {
    let m = 32usize;
    let n = 32usize;
    let k = 64usize;
...
#[test]
fn iron_qmm_mma_mpp_matches_cpu_reference_f32_multi_k() {
    let m = 32usize;
    let n = 32usize;
    let k = 512usize;
    let group_size = 64usize;
...
#[test]
fn iron_qmm_mma_mpp_matches_cpu_reference_f32_multi_tile() {
    let m = 64usize;
    let n = 64usize;
    let k = 128usize;
    let group_size = 64usize;
...
#[test]
fn iron_qmm_mma_mpp_matches_cpu_reference_f16_small() {
    let m = 32usize;
    let n = 32usize;
    let k = 64usize;
    let group_size = 64usize;
...
#[test]
fn iron_qmm_mma_mpp_matches_cpu_reference_f16_multi_tile() {
    let m = 64usize;
    let n = 64usize;
    let k = 128usize;
    let group_size = 64usize;
