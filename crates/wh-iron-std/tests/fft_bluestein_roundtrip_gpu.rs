...
fn naive_dft_direct(re: &[f32], im: &[f32], rows: usize, n: usize) -> (Vec<f32>, Vec<f32>) {
    ...
    for r in 0..rows {
        for k in 0..n {
            ...
            or[r * n + k] = acc_re as f32;
            oi[r * n + k] = acc_im as f32;
        }
    }
    (or, oi)
}
...
/// Run the full documented Bluestein pipeline (forward DFT, `inv=0`
/// throughout) for a `[rows, n_len]` real input and return the `[rows,
/// n_len]` complex output as `(re, im)` f32.
///
...
fn run_bluestein_forward(
    ...
    let x_im = vec![0.0f32; rows * n_len]; // real input
    ...
    // Unguarded store in this kernel (see fft.rs bench comment) — total
    // dispatched threads must equal m_len exactly. M=1024 is a multiple
    // of 256 for both production shapes (N=400, N=480), so tpg=256 divides
    // evenly.
    ...
    pbuf.insert("inv".into(), 0u32.to_le_bytes().to_vec());
    ...
    postbuf.insert("inv".into(), 0u32.to_le_bytes().to_vec());
...
fn bluestein_roundtrip_n400_f32() {
    ...
    let (got_re, got_im) = run_bluestein_forward(&ctx, Dt::F32, n_len, m_len, rows, &x_re, None);
    let x_im = vec![0.0f32; rows * n_len];
    let (want_re, want_im) = naive_dft_direct(&x_re, &x_im, rows, n_len);
...
fn bluestein_roundtrip_n400_filter_tap_bitflip_diverges_f32() {
    ...
    let (clean_re, clean_im) =
        run_bluestein_forward(&ctx, Dt::F32, n_len, m_len, rows, &x_re, None);
    let (corrupt_re, corrupt_im) =
        run_bluestein_forward(&ctx, Dt::F32, n_len, m_len, rows, &x_re, Some(3));
