...
//!
//! `iron_rms_norm` is generic over `N = tpg * 4`: each thread owns
...
//! |----------|----------------------|-----|-----------------------------------------|
//! | 64       | `iron_rms_norm_small`  | 32  | older 7B-class architectures                  |
//! | 128      | `iron_rms_norm`        | 32  | Qwen3-8B/14B/32B, Qwen3-class |
//! | 256      | `iron_rms_norm`        | 64  | Gemma-2/3 (E2B, 9B), Phi-3-medium       |
//!
//! `iron_rms_norm` owns 4 consecutive elements per thread (better ILP
//! at large head_dim). `iron_rms_norm_small` owns 2 elements per thread
...
use wh_iron::Context;
use wh_iron_std::kernels::norm::rms_norm::{iron_rms_norm, iron_rms_norm_small};
...
/// Run iron_rms_norm at (head_dim, rows, dtype) and compare against a
...
fn run_and_check(head_dim: usize, rows: usize, dtype: Dt, tol_abs: f32) {
    ...
    //   head_dim ≥ 128 → iron_rms_norm  (4 elems/thread, tpg=head_dim/4)
    //   head_dim < 128 → iron_rms_norm_small  (2 elems/thread,
    //                                        tpg=head_dim/2 ≥ 32)
    ...
    let (mut kernel, tpg) = if head_dim >= 128 {
        (iron_rms_norm::kernel_ir_for(dtype.to_dtype()), head_dim / 4)
    } else {
        (iron_rms_norm_small::kernel_ir_for(dtype.to_dtype()), head_dim / 2)
    };
...
// ── head_dim = 64 (older 7B-class architectures — uses iron_rms_norm_small) ─

#[test]
fn iron_rms_norm_per_head_small_head_dim_f32() {
    // older 7B-class architectures q_norm-like dispatch — head_dim=64
    // routes to `iron_rms_norm_small` (2 elems/thread, tpg=32 hits
...
#[test]
fn iron_rms_norm_per_head_small_head_dim_f16() { run_and_check(64, 512, Dt::F16, 5e-3); }

#[test]
fn iron_rms_norm_per_head_small_head_dim_bf16() { run_and_check(64, 512, Dt::Bf16, 5e-2); }

// ── head_dim = 128 (Qwen3-class) ─────────────────────────────────────

#[test]
fn iron_rms_norm_per_head_qwen3_shape_f32() {
...
#[test]
fn iron_rms_norm_per_head_qwen3_shape_f16() { run_and_check(128, 1024, Dt::F16, 5e-3); }

#[test]
fn iron_rms_norm_per_head_qwen3_shape_bf16() {
...
#[test]
fn iron_rms_norm_per_head_gemma_head_dim_f32() {
...
#[test]
fn iron_rms_norm_per_head_gemma_head_dim_f16() { run_and_check(256, 256, Dt::F16, 5e-3); }

#[test]
fn iron_rms_norm_per_head_gemma_head_dim_bf16() { run_and_check(256, 256, Dt::Bf16, 5e-2); }
