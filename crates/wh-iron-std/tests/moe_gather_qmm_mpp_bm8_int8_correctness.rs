...
//! GPU correctness for `kernels::moe::moe_mpp_bm8::iron_moe_gather_qmm_mma_int8_bm8_mpp`.
//!
...
//! Validated against the scalar `iron_moe_gather_qmm_b8` oracle (pow2-width
...
use wh_iron_std::kernels::moe::{moe_gather_qmm::iron_moe_gather_qmm_b8, moe_mpp_bm8};
...
fn run_case(case: &Case) {
    ...
    let y_mpp: Vec<f32> = {
        ...
        buffers.insert("group_size".into(), (group_size as u32).to_le_bytes().to_vec());
        let ctx = Context::new().unwrap();
        let mut k = moe_mpp_bm8::iron_moe_gather_qmm_mma_int8_bm8_mpp::kernel_ir_for(dt.to_dtype());
        k.mode = KernelMode::Reduction;
