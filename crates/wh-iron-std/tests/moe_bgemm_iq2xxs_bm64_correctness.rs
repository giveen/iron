...
fn bgemm_iq2xxs_bm64_matches_pool_kernel() {
    ...
    // 2026-08-06 on Metal (M5 Max), ~3x observed max|Δ| (filled after run).
    // Observed 0.0 (bit-identical — bm64 and the mpp pool kernel share the
    // same scalar reduction order at this shape). Floor kept nonzero so the
    // assertion tolerates legitimate ULP-level reordering on other hardware
    // without going fully scale-blind again — still ~1e4-1e5x tighter than
    // a 2% uniform-scale bug at these magnitudes (want[0..6] up to ~181).
