...
//!
//! `iron_rms_norm` is a Reduction-mode kernel that derives `n_simd = lsize / 32`
...
use wh_iron_std::kernels::norm::rms_norm::iron_rms_norm;

/// Buffers for an `n`-wide single-row `iron_rms_norm` dispatch (constexpr `n`
...
fn rms_norm_kernel() -> wh_iron::core::ir::Kernel {
    let mut k = iron_rms_norm::kernel_ir_for(DType::F32);
