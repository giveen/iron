...
//! GPU correctness for `kernels::moe::moe_mpp::iron_moe_gather_qmm_mma_int4_bm16_mpp`.
//!
//! This is the MPP (MetalPerformancePrimitives) MoE BGEMM — same algorithm
//! and output as `iron_moe_gather_qmm_mma_int4_bm16` but routes the inner
//! 16×32×16 tile through `mpp::tensor_ops::matmul2d` (Apple's NAX-tapping
//! cooperative-tensor API). Validated against the scalar `iron_moe_gather_qmm_int4`
...
use wh_iron_std::kernels::moe::{moe_gather_qmm::iron_moe_gather_qmm_int4, moe_mpp};
...
fn moe_gather_qmm_mma_int4_bm16_mpp_matches_m1_clean_tile() {
    ...
    let y_m1 = {
        ...
        let mut k = iron_moe_gather_qmm_int4::kernel_ir_for(Dt::F32.to_dtype());
    ...
    let y_mpp = {
        ...
        let ctx = Context::new().unwrap();
        let mut k =
            moe_mpp::iron_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(Dt::F32.to_dtype());
        k.mode = KernelMode::Reduction;
...
fn moe_gather_qmm_mma_int4_bm16_mpp_bf16_matches_m1_clean_tile() {
    ...
    let y_mpp = {
        ...
        let ctx = Context::new().unwrap();
        let mut k =
            moe_mpp::iron_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(Dt::Bf16.to_dtype());
        k.mode = KernelMode::Reduction;
        let r = ctx
            .dispatch_with_grid(
                &k,
