...
//! GPU correctness for `kernels::ssm::gated_delta_prep_chunk::iron_gated_delta_prep_chunk`.
//!
//! The chunked variant must match T sequential calls of
//! `iron_gated_delta_prep_step` carrying state forward — that is exactly
...
//! GPU under test: one `iron_gated_delta_prep_chunk` dispatch over `T`.
...
fn run_gpu(
    ...
    let mut prepass_kernel = iron_gated_delta_qknorm_prepass::kernel_ir_for(dt.to_dtype());
    ...
    let prepass_result = ctx
        ...
        .expect("iron_gated_delta_qknorm_prepass dispatch");
...
fn prep_chunk_f32_long_t32_small_shape() {
    ...
    // validation lives in Iron's forwardManyEquivalence over real weights.
...
fn prep_chunk_bf16_small_shape_t32() {
    ...
    // without overflow. Production-scale bf16 validation runs in Iron
