...
//! GPU correctness oracle for `iron_qmm_mma_mpp_int8` — the int8
//! quantized matmul backed by `mpp::tensor_ops::matmul2d` (MPP path).
//!
//! Dispatches `iron_qmm_mma_mpp_int8_{f32,f16,bf16}` over a small set
...
//!
//! Run:
//!   cargo test --release -p wh-iron-std --test qmm_mpp_int8_correctness -- --nocapture
...
use wh_iron_std::kernels::gemm::quantized_mpp_int8::iron_qmm_mma_mpp_int8;
...
fn run_qmm_mma_mpp_int8(
    ...
    assert!(m.is_multiple_of(32), "iron_qmm_mma_mpp_int8 requires m %% 32 == 0");
    assert!(n.is_multiple_of(32), "iron_qmm_mma_mpp_int8 requires n %% 32 == 0");
    assert!(k.is_multiple_of(32), "iron_qmm_mma_mpp_int8 requires k %% 32 == 0");
    ...
    let mut kernel = iron_qmm_mma_mpp_int8::kernel_ir_for(dtype);
    kernel.mode = KernelMode::Reduction;

    let result = ctx
        .dispatch_with_grid(&kernel, &buffers, &BTreeMap::new(), [n / 32, m / 32, 1], [128, 1, 1])
        .expect("dispatch iron_qmm_mma_mpp_int8");
...
#[test]
fn iron_qmm_mma_mpp_int8_matches_cpu_reference_f32_small() {
    let m = 32usize;
    let n = 32usize;
    let k = 64usize;
...
#[test]
fn iron_qmm_mma_mpp_int8_matches_cpu_reference_f32_multi_k() {
    let m = 32usize;
    let n = 32usize;
    let k = 512usize;
    let group_size = 32usize;
...
#[test]
fn iron_qmm_mma_mpp_int8_matches_cpu_reference_f32_multi_tile() {
    let m = 64usize;
    let n = 64usize;
    let k = 128usize;
    let group_size = 32usize;
...
#[test]
fn iron_qmm_mma_mpp_int8_matches_cpu_reference_f16_small() {
    let m = 32usize;
    let n = 32usize;
    let k = 64usize;
    let group_size = 32usize;
...
#[test]
fn iron_qmm_mma_mpp_int8_matches_cpu_reference_f16_multi_tile() {
    let m = 64usize;
    let n = 64usize;
    let k = 128usize;
    let group_size = 32usize;
...
#[test]
fn iron_qmm_mma_mpp_int8_matches_cpu_reference_bf16_small() {
    let m = 32usize;
    let n = 32usize;
    let k = 64usize;
    let group_size = 32usize;
