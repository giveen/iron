...
//! (`wh_iron_std::kernels::gemm::quantized_mma_dynamic_m`). This is the
//! bandwidth-bound prefill unlock for Iron's `Qwen35Model.forwardMany`:
...
//! `iron_qmm_mma` with grid `[N/32, m_padded/32, 1]`, then slices
...
//!
//! Run:
//!   cargo test --release -p wh-iron-std --test qmm_mma_dynamic_m_correctness -- --nocapture
...
use wh_iron_std::kernels::gemm::quantized_mma_dynamic_m as dyn_m;
...
// `iron_qmm_mma` with grid `[N/32, m_padded/32, 1]`, then slices the
...
// Iron decode step would hit — single token in, full BM tile padded.
// ═════════════════════════════════════════════════════════════════════════
