...
//! Long-context + batched coverage for `iron_sdpa_prefill_mma`.
//!
...
use wh_iron::Context;
use wh_iron_std::kernels::sdpa::steel_attn::steel_attention_mma::iron_sdpa_prefill_mma;
...
fn run_sdpa_prefill(
    ...
    // `run_sdpa_prefill` in `crates/wh-iron-std/src/run_spec.rs`).
...
#[test]
fn iron_sdpa_prefill_mma_matches_cpu_reference_t2048_f32() {
    let _g = gpu_lock();
...
#[test]
fn iron_sdpa_prefill_mma_b2_via_head_flatten_t1024_f32() {
    let _g = gpu_lock();
...
#[test]
fn iron_sdpa_prefill_mma_kernel_side_b2_t1024_f32() {
    let _g = gpu_lock();
...
#[test]
fn iron_sdpa_prefill_mma_kernel_side_b4_t512_f32() {
    let _g = gpu_lock();
...
#[test]
fn iron_sdpa_prefill_mma_matches_cpu_reference_t4096_f32() {
    let _g = gpu_lock();
