...
//! GPU correctness for `iron_moe_gather_qmm_int4_m16` and
//! `iron_moe_gather_qmm_int4_m32` — short-prefill MoE GEMV with int4
//! quantized weights and 16/32-row-per-TG batching.
//!
//! Both variants are correctness-compared against `iron_moe_gather_qmm_int4_m8`
...
//!
//! Grid:
//!   `iron_moe_gather_qmm_int4_m16`: `[m_out/16, t_rows, 1]`, TPG = 32 (1 SG).
//!   `iron_moe_gather_qmm_int4_m32`: `[m_out/32, t_rows, 1]`, TPG = 32 (1 SG).
//!
...
use wh_iron_std::kernels::moe::moe_gather_qmm::{
...
#[allow(clippy::too_many_arguments)]
fn run_kernel(
    kernel_ir: fn(wh_iron::core::dtype::DType) -> wh_iron::core::ir::Kernel,
    grid_x: usize,
    x: &[f32],
    weight_packed: &[u32],
    scales: &[f32],
...
fn run_case_m16(n_experts: usize, t_rows: usize, k_in: usize, m_out: usize, group_size: usize) {
    ...
    let _g = gpu_lock();
    let y_m8 = run_kernel(
        iron_moe_gather_qmm_int4_m8::kernel_ir_for,
        m_out / 8,
        &x,
        &weight_packed,
        &scales,
    ...
    let y_m16 = run_kernel(
        iron_moe_gather_qmm_int4_m16::kernel_ir_for,
        m_out / 16,
        &x,
        &weight_packed,
        &scales,
...
fn run_case_m32(n_experts: usize, t_rows: usize, k_in: usize, m_out: usize, group_size: usize) {
    ...
    let y_m32 = run_kernel(
        iron_moe_gather_qmm_int4_m32::kernel_ir_for,
        m_out / 32,
        &x,
        &weight_packed,
        &scales,
...
// ── iron_moe_gather_qmm_int4_m16 tests ─────────────────────────────────────────
...
// ── iron_moe_gather_qmm_int4_m32 tests ─────────────────────────────────────────
