...
//! `src/iron/sdpa_decode.rs` (`test_iron_sdpa_decode` /
//! `test_iron_sdpa_decode_swa` / `_swa_sink` / `_learned_sink`), which pins
...
//!
//!     cargo test --release -p wh-iron-std --test sdpa_decode_swa_gpu \
//!         -- --ignored --nocapture
//!
...
use wh_iron_std::kernels::sdpa::sdpa_decode::iron_sdpa_decode as sdpa_decode;
