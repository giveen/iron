...
//!
//! Run with:
//!   cargo test -p wh-iron-std --test prod_shape_gemm_isolation_bench \
//!     --release -- --ignored --nocapture
//!
//! Dense family: `iron_qmm_mma` at M in {512, 2048, 4096} x N in {512, 8192},
//! K=2048, int4-affine group_size=64 (the attention/GDN projection shapes).
//!
//! MoE-gather family: `iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan` at
...
use wh_iron_std::kernels::{
    gemm::{
        quantized::iron_qmm_mma,
        quantized_coop::iron_qmm_coop,
        quantized_mpp::iron_qmm_mma_mpp,
    },
    moe::moe_mpp_tileplan::{build_tile_plan, iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan},
};
...
fn time_qmm_mma(
    ...
    // iron_qmm_mma requires M % 32 == 0 - Grid: [N/32, M/32, 1], TG: 128
    // threads (4 SG x 32 lanes), fixed regardless of M/N/K.
    assert_eq!(m % 32, 0, "iron_qmm_mma requires M % 32 == 0");
    assert_eq!(n % 32, 0, "iron_qmm_mma requires N % 32 == 0");
    let mut kernel = iron_qmm_mma::kernel_ir_for(dt.to_dtype());
...
/// sibling `iron_qmm_mma_mpp` - isolated ahead of writing a brand new
...
fn time_qmm_mma_mpp(
    ...
    assert_eq!(m % 32, 0, "iron_qmm_mma_mpp requires M % 32 == 0");
    assert_eq!(n % 32, 0, "iron_qmm_mma_mpp requires N % 32 == 0");
    let mut kernel = iron_qmm_mma_mpp::kernel_ir_for(dt.to_dtype());
    kernel.mode = KernelMode::Reduction;
    let grid = [n / 32, m / 32, 1];
    let tg = [128usize, 1, 1];
...
/// `iron_qmm_coop` - BN=64 wide-N variant. Grid divides N by 64, not 32.
#[allow(clippy::too_many_arguments)]
fn time_qmm_coop(
    ctx: &Context,
    m: usize,
    ...
    assert_eq!(m % 32, 0, "iron_qmm_coop requires M % 32 == 0");
    assert_eq!(n % 64, 0, "iron_qmm_coop requires N % 64 == 0");
    let mut kernel = iron_qmm_coop::kernel_ir_for(dt.to_dtype());
    kernel.mode = KernelMode::Reduction;
...
fn time_moe_gather_tileplan(
    ...
    let mut kernel = iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan::kernel_ir_for(dt.to_dtype());
    kernel.mode = KernelMode::Reduction;
...
fn isolation_dense_qmm_mma_prod_shapes() {
    ...
    eprintln!("\n=== ISOLATION: iron_qmm_mma dense, K={k} GS={group_size} dtype={dt:?} ===");
...
fn isolation_dense_qmm_mma_mpp_prod_shapes() {
    ...
    eprintln!(
        "\n=== ISOLATION: iron_qmm_mma_mpp dense (tensor-unit matmul2d), K={k} GS={group_size} dtype={dt:?} ==="
    );
    for &m in &[512usize, 2048, 4096] {
        for &n in &[512usize, 8192] {
            let iters = if m * n <= 512 * 8192 { 30 } else { 15 };
...
fn isolation_dense_qmm_coop_prod_shapes() {
    ...
    eprintln!(
        "\n=== ISOLATION: iron_qmm_coop dense (BN=64 wide tensor-unit matmul2d), K={k} GS={group_size} dtype={dt:?} ==="
    );
    for &m in &[512usize, 2048, 4096] {
        for &n in &[512usize, 8192] {
            let iters = if m * n <= 512 * 8192 { 30 } else { 15 };
...
fn isolation_moe_gather_tileplan_prod_shapes() {
    ...
    eprintln!(
        "\n=== ISOLATION: iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan, {n_experts} experts top-{topk}, Zipf-skewed, dtype={dt:?} ==="
    );
