...
//! GPU correctness for `iron_gemv_q8` — Q8_0 inline-dequant gemv
...
use wh_iron_std::kernels::gemm::gemv_quantized::iron_gemv_q8;
...
fn run_case(dt: Dt, k_in: usize, m_out: usize, tol: f32) {
    ...
    let mut kernel = iron_gemv_q8::kernel_ir_for(dt.to_dtype());
...
fn grouped_gemv_q8_f32() {
    use wh_iron_std::kernels::gemm::gemv_quantized::iron_grouped_gemv_q8;
    let _g = gpu_lock();
    let k_in = 4096usize;
    ...
    let ctx = Context::new().expect("ctx");
    let mut kernel = iron_grouped_gemv_q8::kernel_ir_for(Dt::F32.to_dtype());
    kernel.mode = KernelMode::Reduction;
    let result = ctx
        .dispatch_with_grid(&kernel, &buffers, &BTreeMap::new(), [m_out, 1, 1], [32, 1, 1])
        .expect("dispatch");
