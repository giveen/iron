...
//!   cargo test -p wh-iron-std --test smallm_occupancy_microbench --release -- --ignored --nocapture
//!
...
//!
//!   1. The dense int4 MPP GEMM (`iron_qmm_mma_mpp`) at the GDN in-projection
...
//!   2. The MoE gather GEMM (`iron_moe_gather_qmm_mma_int4_bm16_mpp`, the
...
use wh_iron_std::kernels::{
    gemm::quantized_mpp::iron_qmm_mma_mpp,
    moe::{
        moe_mpp::iron_moe_gather_qmm_mma_int4_bm16_mpp,
        moe_mpp_tileplan::{build_tile_plan, iron_moe_gather_qmm_mma_int4_bm16_mpp_tileplan},
...
fn time_moe_gather_bm16_mpp(
    ...
    let mut kernel = iron_moe_gather_qmm_mma_int4_bm16_mpp::kernel_ir_for(dt.to_dtype());
...
/// Times the device-side `iron_moe_build_tile_plan` plan builder alone, and
...
fn time_build_tile_plan(
    ...
    let mut kernel = iron_moe_build_tile_plan::kernel_ir();
    kernel.mode = KernelMode::Reduction;
    let grid = [1usize, 1, 1];
...
fn bench_gdn_inproj_dense_qmm_narrow_n() {
    ...
    eprintln!("\n=== dense iron_qmm_mma_mpp: GDN in-proj shapes (K={k}, GS={group_size}) ===");
    for &t in &[512usize, 4096, 16384] {
        for (n, label) in
...
fn bench_moe_gather_bm16_mpp_skewed_vs_even_vs_degenerate() {
    ...
    eprintln!(
        "\n=== iron_moe_gather_qmm_mma_int4_bm16_mpp: skewed vs even vs degenerate routing ==="
    );
    for &t in &[512usize, 4096, 16384] {
        let m_total = t * topk;
