...
//! Microbench: `iron_moe_gather_qmm_int4` at Qwen3.6-35B-A3B shape.
//!
//! `#[ignore]`-gated — run with:
//!   cargo test -p wh-iron-std --test moe_gather_qmm_microbench --release -- --ignored --nocapture
...
fn time_gather_qmm_vd(
    ...
    let mut kernel = match variant {
        Variant::M1 => iron_moe_gather_qmm_int4::kernel_ir_for(dt.to_dtype()),
        Variant::M8 => iron_moe_gather_qmm_int4_m8::kernel_ir_for(dt.to_dtype()),
        Variant::Mma => iron_moe_gather_qmm_mma_int4::kernel_ir_for(dt.to_dtype()),
        Variant::MmaBm16 => iron_moe_gather_qmm_mma_int4_bm16::kernel_ir_for(dt.to_dtype()),
    };
