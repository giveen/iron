...
//! GPU correctness for `iron::iron_gemm_q8_mpp` and `iron_grouped_gemm_q8_mpp`
...
use wh_iron_std::kernels::gemm::gemm_q8_mpp::{iron_gemm_q8_mpp, iron_grouped_gemm_q8_mpp};
...
fn gemm_q8_mpp_matches_oracle() {
    ...
    let mut kernel = iron_gemm_q8_mpp::kernel_ir_for(Dt::F32.to_dtype());
...
fn grouped_gemm_q8_mpp_matches_oracle() {
    ...
    let ctx = Context::new().unwrap();
    let mut kernel = iron_grouped_gemm_q8_mpp::kernel_ir_for(Dt::F32.to_dtype());
    kernel.mode = KernelMode::Reduction;
    let gx = (out_dim as u32).div_ceil(64) as usize;
    let gy = (n_rows as u32).div_ceil(64) as usize;
    let r = ctx
