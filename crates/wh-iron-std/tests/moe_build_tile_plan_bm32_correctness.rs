...
//! GPU correctness for `iron_moe_build_tile_plan_bm32` (the production
//! device tile-plan builder feeding `iron_moe_gather_qmm_coop`), covering
...
//! `iron_moe_gather_qmm_coop`'s own kernel_tests, which build the tile plan
...
fn run_case(counts: &[usize], n_experts: usize) -> (Vec<u32>, Vec<u32>, Vec<u32>, usize, u32, u32) {
    ...
    let mut k = iron_moe_build_tile_plan_bm32::kernel_ir();
...
fn matches_host_reference_on_zipfish_realistic_fixture() {
    let counts = wh_iron_std::kernels::moe::moe_mpp_shared::zipfish_counts(4096, 256, 0x9E37_0001);
    assert_matches_host_reference(&counts, 256);
}
...
fn tile_count_matches_on_zipfish_at_all_three_prod_sizes() {
    for &m_total in &[4096usize, 16384, 32768] {
        let counts =
            wh_iron_std::kernels::moe::moe_mpp_shared::zipfish_counts(m_total, 256, 0x9E37_0001);
        assert_tile_count_matches_real_tiles(&counts, 256);
    }
}
