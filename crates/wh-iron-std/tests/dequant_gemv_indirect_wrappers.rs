...
//! Regression guard: `iron_dequant_gemv_int4` indirect Swift wrappers.
//!
//! Iron's GPU-router dispatches `iron_dequant_gemv_int4` indirectly (grid shape
//! from an `MTLBuffer` rather than a host `MTLSize`). This test lives in
//! `wh-iron-std` — not the CLI — because the kernel corpus lives here and
...
fn collect_kernels_with_indirect_flags() -> Vec<Kernel> {
    ...
    for (name, (b, dtypes)) in &by_name {
        for &dt in dtypes {
            ...
            k.name = mono.clone();
            if matches!(mono.as_str(), "iron_dequant_gemv_int4_f16" | "iron_dequant_gemv_int4_bf16")
            {
...
#[test]
fn iron_dequant_gemv_int4_is_registered() {
    let kernels = collect_kernels_with_indirect_flags();
    assert!(
        kernels.iter().any(|k| k.name == "iron_dequant_gemv_int4_f16"),
        "iron_dequant_gemv_int4_f16 missing from kernel set"
    );
    assert!(
        kernels.iter().any(|k| k.name == "iron_dequant_gemv_int4_bf16"),
        "iron_dequant_gemv_int4_bf16 missing from kernel set"
    );
}

#[test]
fn iron_dequant_gemv_int4_swift_wrappers_are_indirect() {
    let kernels = collect_kernels_with_indirect_flags();
    let swift = render_swift_wrappers(&kernels);
    assert!(
        swift.contains("func iron_dequant_gemv_int4_f16_indirect("),
        "indirect Swift wrapper for iron_dequant_gemv_int4_f16 dropped"
    );
    assert!(
        swift.contains("func iron_dequant_gemv_int4_bf16_indirect("),
        "indirect Swift wrapper for iron_dequant_gemv_int4_bf16 dropped"
    );
    assert!(
