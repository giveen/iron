...
use wh_iron_std::kernels::moe::{
    ...
    moe_gemv_rows_iq2xxs::iron_moe_gemv_rows_iq2xxs,
};
...
/// Replay the EXACT in-model dumped bytes (/tmp/bm64dump from Iron
/// IRON_DBG_DUMP=1). If bm64 diverges from gemv-rows here → data-triggered
...
fn bm64_replay_real_dump() {
    ...
    let meta = std::fs::read_to_string(format!("{dir}/meta.txt"))
        .expect("dump meta — run Iron with IRON_DBG_DUMP=1");
    ...
    for dt in [Dt::F32, Dt::F16] {
        ...
        // Compare wh-iron-runtime bm64 output vs the Iron-runtime bm64 output
        // on the SAME bytes. If replay-bm == in-model-gemv but != in-model-bm,
        // the Iron runtime wrongly executes the identical kernel.
...
fn run_bm64(
    ...
    let mut k = iron_moe_bgemm_iq2xxs_bm64::kernel_ir_for(dt.to_dtype());
...
fn run_gemv(
    ...
    let ctx = Context::new().expect("ctx");
    let mut k = iron_moe_gemv_rows_iq2xxs::kernel_ir_for(dt.to_dtype());
    k.mode = KernelMode::Reduction;
    let r = ctx
