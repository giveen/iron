...
//! PRODUCTION MoE coop gather GEMM (`iron_moe_gather_qmm_coop`, BM=32 tile
//! plan, wired as the prefill default in Iron's `MoELayer.swift`).
//!
//! `#[ignore]`-gated diagnostic, not a regression gate - run with:
//!   cargo test -p wh-iron-std --test moe_coop_idle_tile_ablation \
...
/// Times `iron_moe_gather_qmm_coop` at a fixed dispatch tile-axis height
...
fn time_moe_coop_at_height(
    ...
    let mut kernel = iron_moe_gather_qmm_coop::kernel_ir_for(dt.to_dtype());
...
fn ablation_moe_coop_capacity_vs_real_tile_count() {
    ...
    eprintln!(
        "\n=== F-85 decisive round: iron_moe_gather_qmm_coop idle-tile-cost ablation (capacity dispatch vs real-tile-count dispatch) ==="
    );
