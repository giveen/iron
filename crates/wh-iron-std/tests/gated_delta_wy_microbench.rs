...
//! Microbench: `iron_gated_delta_wy_chunk` vs `iron_gated_delta_step` (MLX baseline).
//!
//! Compares the chunked-WY kernel against running `iron_gated_delta_step`
//! T times (the current production GDN prefill path). Both kernels live
//! in wh-iron-std; we drive them via the same Context and time wall
//! clock.
//!
...
//! Run with: `cargo test -p wh-iron-std --test gated_delta_wy_microbench --release -- --ignored --nocapture`.
...
use wh_iron_std::kernels::ssm::{
    gated_delta::{iron_gated_delta_chunk, iron_gated_delta_step},
...
fn time_wy_chunk(
    ...
    let mut kernel = iron_gated_delta_wy_chunk::kernel_ir_for(Dt::F32.to_dtype());
...
/// Fair baseline: `iron_gated_delta_chunk` runs the full T-sequence in ONE
/// dispatch via its inner T-loop. This is the kernel `iron_gated_delta_wy_chunk`
...
fn time_chunk_kernel(
    ...
    let mut kernel = iron_gated_delta_chunk::kernel_ir_for(Dt::F32.to_dtype());
    kernel.mode = KernelMode::Reduction;
...
fn time_step_loop(
    ...
    let mut kernel = iron_gated_delta_step::kernel_ir_for(Dt::F32.to_dtype());
    kernel.mode = KernelMode::Reduction;
