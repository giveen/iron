...
//! GPU correctness for `iron::iron_moe_gather_bgemm_iq2xxs_mpp` — the
//! prefill IQ2_XXS grouped BGEMM. Oracle: per-row IQ2_XXS dequant gemv
//! (same formula as iron_gguf_dequant_iq2_xxs). Cosine ≥ 0.99 (MMA
...
use wh_iron_std::kernels::moe::moe_bgemm_iq2xxs_mpp::iron_moe_gather_bgemm_iq2xxs_mpp;
...
fn bgemm_iq2xxs_mpp_matches_gemv_oracle() {
    ...
    let mut k = iron_moe_gather_bgemm_iq2xxs_mpp::kernel_ir_for(Dt::F32.to_dtype());
    ...
    // a uniform-scale bug (dropped dequant scale, miswired group index)
    // cosine alone would miss. Calibrated 2026-08-06 on Metal (M5 Max),
    // ~3x observed max|Δ| for headroom against cross-hardware accumulation
    // order, not run-to-run noise (dispatch here is deterministic).
