...
//! GPU correctness for `kernels::ssm::gated_delta_prep::iron_gated_delta_prep_step`.
//!
...
//!   5. the recurrence (same math as `iron_gated_delta_step`)
//!
...
use wh_iron_std::kernels::ssm::gated_delta_prep::iron_gated_delta_prep_step;
...
fn run_gpu(
    ...
    assert!(dk.is_multiple_of(32), "iron_gated_delta_prep_step requires dk % 32 == 0");
    let n_total = b * hv;
    ...
    let mut kernel = iron_gated_delta_prep_step::kernel_ir_for(dt.to_dtype());
    kernel.mode = KernelMode::Reduction;

    let result = ctx
        .dispatch_with_grid(&kernel, &buffers, &BTreeMap::new(), [dv, n_total, 1], [32, 1, 1])
        .expect("iron_gated_delta_prep_step dispatch");
...
fn prep_step_f32_matches_unfused_path_when_weights_identity() {
    ...
    // RMSNorm-scaled through `iron_gated_delta_step`.
