...
//!
//! Bench (`#[ignore]`):
//!   cargo test --release -p wh-iron-std --test sdpa_decode_2pass_gpu \
//!     -- --ignored --nocapture
...
use wh_iron::{Context, DispatchSpec, ResidentBuffer, core::ir::KernelMode};
use wh_iron_std::kernels::sdpa::sdpa_decode_2pass::{
...
fn run_2pass(
    ...
    let mut p1 = iron_sdpa_decode_2pass_pass1::kernel_ir_for(dtype);
    ...
    let mut p2 = iron_sdpa_decode_2pass_pass2::kernel_ir_for(dtype);
...
// runs M1; iron bench's `sdpa_decode_2pass` f16/bf16 rows (181/147 GB/s,
...
fn run_2pass_generic(
    ...
    let (mut p1_kernel, mut p2_kernel) = match head_dim {
        64 => (
            iron_sdpa_decode_2pass_pass1_d64::kernel_ir_for(dtype),
            iron_sdpa_decode_2pass_pass2_d64::kernel_ir_for(dtype),
        ),
        96 => (
            iron_sdpa_decode_2pass_pass1_d96::kernel_ir_for(dtype),
            iron_sdpa_decode_2pass_pass2_d96::kernel_ir_for(dtype),
        ),
        256 => (
            iron_sdpa_decode_2pass_pass1_d256::kernel_ir_for(dtype),
            iron_sdpa_decode_2pass_pass2_d256::kernel_ir_for(dtype),
        ),
