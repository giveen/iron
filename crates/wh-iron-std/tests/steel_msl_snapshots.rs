...
//!   cargo insta test --accept -p wh-iron-std --test steel_msl_snapshots
//! Or interactively:
//!   cargo insta review
//!
...
use wh_iron_std::kernels::{
    gemm::{
        fp_quantized_nax,
        quantized_nax,
        steel::{steel_gemm_fused_nax, steel_gemm_gather_nax, steel_gemm_splitk_nax},
    },
    ops::hadamard_m,
};
...
/// correctly.
fn steel_msl(kernel_ir: wh_iron::core::ir::Kernel, mode: KernelMode) -> String {
...
fn steel_gemm_fused_nax_f16_msl() {
    let msl = steel_msl(
        steel_gemm_fused_nax::iron_steel_gemm_fused_nax::kernel_ir_for(DType::F16),
...
fn steel_gemm_gather_nax_f16_msl() {
    let msl = steel_msl(
        steel_gemm_gather_nax::iron_steel_gemm_gather_nax::kernel_ir_for(DType::F16),
        KernelMode::Reduction,
    );
    assert_snapshot!(msl);
}
...
fn steel_gemm_splitk_nax_f16_msl() {
    let msl = steel_msl(
        steel_gemm_splitk_nax::iron_steel_gemm_splitk_nax::kernel_ir_for(DType::F16),
        KernelMode::Reduction,
    );
    assert_snapshot!(msl);
}
...
fn steel_gemm_splitk_nax_accum_f32_msl() {
    let msl = steel_msl(
        steel_gemm_splitk_nax::iron_steel_gemm_splitk_accum_nax::kernel_ir_for(DType::F32),
        KernelMode::Grid3D,
    );
    assert_snapshot!(msl);
}
...
// ── quantized_nax ────────────────────────────────────────────────────
//
// `iron_qmm_mma_mpp` — quantized matmul over MPP cooperative tensors.
// F16 activation × int4 packed weight is the production Qwen3.6 path.

#[test]
fn quantized_nax_f16_msl() {
    let msl =
        steel_msl(quantized_nax::iron_qmm_nax::kernel_ir_for(DType::F16), KernelMode::Reduction);
    assert_snapshot!(msl);
}

// ── fp_quantized_nax ─────────────────────────────────────────────────
//
// `iron_fp_qmm_nax` — fp4 (E2M1) quantized matmul. Same MPP coop-load /
...
fn fp_quantized_nax_f16_msl() {
    let msl = steel_msl(
        fp_quantized_nax::iron_fp_qmm_nax::kernel_ir_for(DType::F16),
        KernelMode::Reduction,
    );
    assert_snapshot!(msl);
}
...
fn hadamard_m_m20_f32_msl() {
    let msl =
        steel_msl(hadamard_m::iron_hadamard_m20::kernel_ir_for(DType::F32), KernelMode::Reduction);
    assert_snapshot!(msl);
}
