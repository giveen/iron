...
//! (the commit gate) without requiring `iron test` — this harness is the
...
fn all_registered_kernel_tests_pass() {
    ...
    // Anchor the wh-iron-std kernel translation units so the linker keeps
    ...
    // alive. The `iron` runner avoids this because it dispatches kernels
    // directly; this cargo bridge has no such reference of its own.
    let n_kernels = wh_iron_std::all_kernels().count();
    assert!(
        n_kernels > 0,
        "all_kernels() is empty — the wh-iron-std kernel object files were \
    ...
    // registry::all_tests()`). Per the `wh-iron-std` lib docs, importing the
    // registry accessor through `wh_iron_std` is what pulls the std rlib into
    // this integration-test link so the `#[test_kernel]` inventory statics are
    // retained — going through `wh_iron::…` directly leaves them dead-code-
