...
//! GEMM codegen path (`IRON_VK_COOPMAT=1`).
//!
//! Drives the real `iron_gemm_q8_mpp` 64×64×32 SimdGroup CoopTile kernel
//! through `VulkanDevice::run_kernel`. The device honors `IRON_VK_COOPMAT`
...
//!
//!   # scalar:   cargo test -p wh-iron-std --features vulkan --release \
//!                  --test vulkan_coopmat_gemm -- --nocapture
//!   # coopmat:  IRON_VK_COOPMAT=1 (same line)
#![cfg(feature = "vulkan")]

use std::{collections::BTreeMap, time::Instant};

use wh_iron::{VulkanDevice, core::dtype::DType};
use wh_iron_std::kernels::gemm::gemm_q8_mpp::iron_gemm_q8_mpp;
...
fn coopmat_gemm_q8_mpp_correct_and_fast() {
    ...
    let coopmat_on = std::env::var("IRON_VK_COOPMAT").map(|v| v == "1").unwrap_or(false);
    eprintln!("=== coopmat_gemm_q8_mpp: IRON_VK_COOPMAT={} ===", coopmat_on as u8);
    ...
    let mut kernel = iron_gemm_q8_mpp::kernel_ir_for(DType::F16);
    ...
    let ref_path = std::env::temp_dir().join("iron_coopmat_scalar_ref.bin");
    if !coopmat_on {
        ...
        let ref_bytes = std::fs::read(&ref_path)
            .expect("scalar reference missing — run once WITHOUT IRON_VK_COOPMAT first");
