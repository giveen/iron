...
//! Quick perf-only timing for `iron_softmax_categorical_sample` at vocab=152K.
//!
...
use wh_iron_std::kernels::sampling::categorical_sample::iron_softmax_categorical_sample;
...
fn perf_softmax_categorical_sample_vocab_152k() {
    ...
    let mut kernel = iron_softmax_categorical_sample::kernel_ir_for(DType::F32);
    ...
    println!(
        "iron_softmax_categorical_sample vocab={n}: median={:.1}µs min={:.1}µs",
