...
use ffai_kernels_core::ir::{Kernel, KernelMode};
...
impl<'a> SingleDispatch<'a> {
    ...
    pub fn execute(&self) -> Result<DispatchResult, MetalTileError> {
        ...
        crate::dispatch::validate::validate_dispatch_geometry(
            ...
            ffai_kernels_codegen::kernel_uses_n_simd(self.kernel),
        )?;
    ...
    fn allocate_buffers(
        ...
        for (param, binding) in self.kernel.params.iter().zip(binding_plans) {
            ...
            if param.kind == ffai_kernels_core::ir::ParamKind::Strided {
...
mod perf {
    ...
    //! now). Run with:
    //!
    //!   cargo test -p ffai-kernels-runtime --release \
