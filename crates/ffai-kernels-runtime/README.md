...
```
ffai-kernels-codegen (MSL) ──► ffai-kernels-runtime (this crate) ──► host results
                                    │
                              Metal framework
...
`ffai-kernels-runtime` receives MSL source from `ffai-kernels-codegen`, compiles
...
Most users don't call `ffai-kernels-runtime` directly — they use the facade's
...
```rust,ignore
use ffai_kernels_runtime::{start_gpu_trace, stop_gpu_trace};
...
**Cache location:** `~/.cache/ffai-kernels/tuning_cache.json` (single file per machine)
...
|---|---|
| `ffai-kernels-core` | Reads kernel IR for param shapes, dtypes, and dispatch metadata |
| `ffai-kernels-codegen` | Calls `MslGenerator` to lower IR → MSL before Metal compilation |
...
- [`ffai-kernels-codegen` README](../ffai-kernels-codegen/README.md) — the MSL generator this crate calls before compiling
- [`ffai-kernels-core` README](../ffai-kernels-core/README.md) — the IR types this crate reads for dispatch metadata
- [Crate docs on docs.rs](https://docs.rs/ffai-kernels-runtime)
