...
functions parse into, that `wh-iron-codegen` optimizes and lowers,
and that `wh-iron-runtime` reads for dispatch metadata. Also provides
the `inventory`-based kernel registry and Apple GPU family probes.

## Position in the pipeline

```
wh-iron-macros ──► wh-iron-core (this crate) ──► wh-iron-codegen
...
`wh-iron-core` defines the shared types that flow through the entire
compiler pipeline. It depends on `wh-iron-macros` for the derive macros
...
```rust,ignore
use wh_iron_core::ir::{Kernel, Block, Op, Param, ParamKind, BinOpKind};
...
> `wh_iron_core::ir::ParamKind` or import from the `ir` module directly.
...
| `protocol` | The `runner ↔ CLI` wire types (`ProtocolMessage`, `ProfileInfo`, bench/test/build results) for the `__iron_runner` subprocess, plus `GpuFamily` Apple-GPU generation detection (M1–M5) |
...
| `ProtocolMessage` | JSON-line wire format for the `__iron_runner` subprocess (bench/test/build/inspect results, `ProfileInfo`) | `src/protocol.rs` | ✅ |
...
| Crate | Role |
|---|---|
| `wh-iron-macros` | Derive macros (`OpFlags`, `ValueRefs`, `VariantName`) for IR types |
...
- **New IR op variant:** `src/ir.rs` — add to the `Op` enum. Add a
  `Display` arm. Update `wh-iron-codegen` passes that exhaustively match
  `Op` (start with `type_check` and `msl::emit_block`).

- **New shape constructors:** `src/shape.rs` — add free functions or methods
  on `Shape`. If you need a macro, add it to `wh-iron-macros`.

- **New error variant:** `src/error.rs` — add to `Error` enum.

- **Tests to update:** `src/ir.rs` tests, pass tests in `wh-iron-codegen`,
  `wh-iron-macros` tests.

## Related documentation

- [Root README](../../README.md) — project overview and architecture
- [CONTRIBUTING](../../CONTRIBUTING.md) — dev setup, PR process, CI
- [`wh-iron-codegen` README](../wh-iron-codegen/README.md) — the optimization passes that operate on this IR
- [`wh-iron-macros` README](../wh-iron-macros/README.md) — how `#[kernel]` produces this IR
- [Crate docs on docs.rs](https://docs.rs/wh-iron-core)
