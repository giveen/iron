...
impl Kernel {
    ...
    /// deferred-static-alloca-size") regardless of the selected Xcode. `iron test`
...
mod tests {
    ...
    fn display_format_shows_kernel_structure() {
        ...
        let mut k = Kernel::new("iron_vadd");
        ...
        let output = format!("{k}");
        assert!(output.contains("kernel iron_vadd"), "should show kernel name: {output}");
